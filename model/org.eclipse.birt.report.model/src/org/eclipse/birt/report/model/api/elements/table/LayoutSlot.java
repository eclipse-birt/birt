...
public class LayoutSlot
{
	 ...
	 *            the layout table
	 * @param groupId
	 *            the group level if the slot is Group Header or Group Footer
	 * @param colBufferSize
	 *            the column count
	 */
	 ...
	 *            than 0, assign a new id.
	 * @return the unique cell id
	 */

	protected int addCell( int cellPos, int rowSpan, int colSpan, Cell content,
			boolean isEffectualDrop, int cellId )
	{
		assert cellPos > 0;
		...
		addOverlappedCells( checkOverlappedLayoutCells( rowId, colId, rowSpan,
				colSpan ) );
	...
	private void fillCells( int cellId, int rowId, int colId, int rowSpan,
			int colSpan, Cell content, boolean isEffectualDrop )
	{
		for ( int i = 0; i < rowSpan; i++ )
		{
			LayoutRow row = (LayoutRow) rows.get( rowId + i );
	...
	protected void addDropSpannedCells( int cellId, int colId, int colSpan,
			int rowSpanOffset, Cell content )
	{
		// in this list, must be empty.

		addOverlappedCells( checkOverlappedLayoutCells( 0, colId,
				getRowCount( ), colSpan ) );
		...
		if ( !overlappedAreas.isEmpty( ) )
		{
			...
			for ( int i = 0; i < overlappedAreas.size( ); i++ )
			{
				...
				tableContainer.addOverlappedCell( layoutCell.getContent( ),
						this, layoutRow.getRowPosn( ), layoutCell
								.getColumnPosn( ), layoutCell
								.getRowSpanOffset( ), layoutCell
								.getColumnSpanOffset( ) );
			}
	...
	/**
	 * Returns <code>LayoutRow</code>s in the row. Note that modifications on
