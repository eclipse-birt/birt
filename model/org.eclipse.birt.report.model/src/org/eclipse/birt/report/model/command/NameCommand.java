...
public class NameCommand extends AbstractElementCommand {
	...
	public static final Pattern styleNamePattern = Pattern.compile(STYLE_NAME_PATTERN, Pattern.CASE_INSENSITIVE);
	...
	public void setName(String name) throws NameException {
		...
		// if a contentElement is not local, need to make a local copy before changing
		// the name
...
//						NameException.DESIGN_EXCEPTION_NAME_FORBIDDEN );
	...
	/**
	 * Implementation of adding a symbol to a name space. Adds the name only if it
	 * is not null and it has container. This means that we can modify the element
	 * name as will if it's not attached on other elements, and the name is saved in
	 * name space only after the element is added to a slot of the container.
	 */
	...
	private void addSymbol() {
		...
		if (executor.hasNamespace()) {
			...
			getActivityStack()
					.execute(new NameSpaceRecord(executor.getNameHelper(), executor.getNameSpaceId(), element, true));
		}
	}

	/**
	 * Implementation of dropping a symbol from a name space. No need to do the drop
	 * if the current name is null.
	 */
	...
	private void renameSymbolFrom(String oldName) {
		...
		if (element.isManagedByNameSpace()) {
			RenameInNameSpaceRecord record = new RenameInNameSpaceRecord(module, element, oldName, element.getName());
