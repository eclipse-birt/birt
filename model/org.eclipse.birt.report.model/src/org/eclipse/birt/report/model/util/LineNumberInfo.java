...
public class LineNumberInfo
{
	...
	public static boolean isLineNumberSuppoerted( IStructure struct )
	{
		if ( struct instanceof IncludedCssStyleSheet
				|| struct instanceof IncludedLibrary
				|| struct instanceof EmbeddedImage
				|| struct instanceof ResultSetColumn
	...
	private void putStructure( IStructure struct, Integer lineNo )
	{
		...
		{
			includeLibStructMap.put( ( (IncludedLibrary) struct ).getNamespace( ),
					lineNo );
		}
		else if ( struct instanceof EmbeddedImage )
		{
			embeddedImageStructMap.put( ( (EmbeddedImage) struct ).getName( ),
					lineNo );
		}
		else if ( struct instanceof IncludedCssStyleSheet )
		{
			includedCssStyleSheetStructMap.put( ( (IncludedCssStyleSheet) struct ).getFileName( ),
					lineNo );
		}
		else
		{
			Structure structure = (Structure) struct;
			StructureHandle handle = new StructureHandle( structure.getElement( )
					.getHandle( module ),
	...
	private String getXPath( Object obj )
	{
		if ( obj instanceof ContainerContext )
		{
			...
			DesignElementHandle handle = context.getElement( )
					.getHandle( module );
		...
		if ( obj instanceof Structure )
		{
			...
			obj = new StructureHandle( struct.getElement( ).getHandle( module ),
					struct.getContext( ) );
		}
		if ( obj instanceof PropertyHandle
				|| obj instanceof StructureHandle
				|| obj instanceof SlotHandle )
