...
public class TabularDimensionPropSearchStrategy extends PropertySearchStrategy
{
			...
			ElementPropertyDefn prop )
	{
		...
		if ( tmpDimension == null
				|| !( tmpDimension.getContainer( ) instanceof Module ) )
			return super.getPropertyFromSelf( module, element, prop );
	...
	public static Dimension getSharedDimension( Module root,
			DesignElement element )
	{
		if ( !( element instanceof TabularDimension ) )
			return null;

		ElementRefValue tmpRef = (ElementRefValue) element.getLocalProperty(
				root, ITabularDimensionModel.INTERNAL_DIMENSION_RFF_TYPE_PROP );
