...
class CSSLengthValueHandler
{
	...
	final static String ABSOLUTE_FONT_SIZE_CONSTANTS[] = {
	...
	DimensionValue getDefaultFontSizeValue( )
	{
		...
		{
			ElementDefn styleDefn = (ElementDefn) MetaDataDictionary
	 ...
	 * 
	 * @param fontSizeValue
	 *            the absolute dimension value of font size
	 * @param relativeValue
			...
			DimensionValue relativeValue )
	{
		...
		{
			return new DimensionValue( fontSizeValue.getMeasure( )
					* relativeValue.getMeasure( ), fontSizeValue.getUnits( ) );
		}
		else if ( DesignChoiceConstants.UNITS_EX.equals( relativeValue
				.getUnits( ) ) )
		{
			return new DimensionValue( fontSizeValue.getMeasure( )
					* relativeValue.getMeasure( ) / 2, fontSizeValue.getUnits( ) );
		}
		else if ( DesignChoiceConstants.UNITS_PERCENTAGE.equals( relativeValue
				.getUnits( ) ) )
		{
			return new DimensionValue( fontSizeValue.getMeasure( )
					* relativeValue.getMeasure( ) / 100, fontSizeValue
					.getUnits( ) );
		}
	...
	{
		return DimensionUtil.isAbsoluteUnit( unit )
