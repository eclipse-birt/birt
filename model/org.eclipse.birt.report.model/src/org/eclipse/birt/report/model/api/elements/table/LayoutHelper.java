...
public final class LayoutHelper
{
			...
			Module module )
	{
		...
		for ( int i = 0, startCol = 1; i < cells.size( ); i++ )
		{
			...
			mappingSlot.addCell( startCol, cell.getRowSpan( module ), colSpan,
					cell, false );
	...
	/**
	 * Finds the space for the given cell information: the column position and
	 * the column span.
	 * 
	 * @param row
	 *            the row information.
	 * @param startPos
	 *            1-based the position where to start the search
	 * @param colSpan
	 *            the column span of the cell
	 * @return the 1-based the position for the cell
	 */

	private static int findFillInPosition( LayoutRow row, int startPos,
			int colSpan )
	{
		int startCol = 0;
	...
	protected static void applyLayoutOnSlot( LayoutSlot mappingSlot,
			ContainerSlot slot, Module module )
	{
