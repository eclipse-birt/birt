...
public class PropertyValueException extends SemanticException {
	...
	/**
	 * Error code constant indicating that the property value can not be negative or
	 * zero.
	 */
	...
	/**
	 * Error code constant indicating that the item is not type of structure list
	 * referred.
	 */
	 ...
	 * element, and the value of the property cannot be set in a derived element.
	 */
	...
	/**
	 * Error code constant indicating the choice value is not allowed for a choice
	 * type property.
	 */
	...
	/**
	 * Error codes that indicates that the report item theme type is not supported
	 * yet.
	 */
	...
	public PropertyValueException(DesignElement obj, IPropertyDefn propDefn, IPropertyDefn memberDefn, Object value,
			String errCode) {
		super(obj, errCode);
		assert propDefn != null;
	...
	public String getLocalizedMessage() {
		...
		if (sResourceKey == DESIGN_EXCEPTION_NEGATIVE_VALUE || sResourceKey == DESIGN_EXCEPTION_NON_POSITIVE_VALUE) {
			return ModelMessages.getMessage(sResourceKey, new String[] { value, this.propertyDisplayName });
		} else if (sResourceKey == DESIGN_EXCEPTION_CHOICE_NOT_FOUND || sResourceKey == DESIGN_EXCEPTION_VALUE_EXISTS
		...
		} else if (sResourceKey == DESIGN_EXCEPTION_VALUE_REQUIRED || sResourceKey == DESIGN_EXCEPTION_VALUE_LOCKED
				|| sResourceKey == DESIGN_EXCEPTION_NOT_LIST_TYPE || sResourceKey == DESIGN_EXCEPTION_ITEM_NOT_FOUND) {
			return ModelMessages.getMessage(sResourceKey, new String[] { propertyDisplayName });
		} else if (sResourceKey == DESIGN_EXCEPTION_WRONG_ITEM_TYPE) {
			PropertyDefn propDefn = element.getPropertyDefn(propertyName);

			if (memberName != null) {
				propDefn = (PropertyDefn) propDefn.getStructDefn().getMember(memberName);
			}
			...
			return ModelMessages.getMessage(sResourceKey,
					new String[] { ((IStructure) invalidValue).getStructName(), propDefn.getStructDefn().getName() });
		} else if (sResourceKey == DESIGN_EXCEPTION_UNIT_NOT_ALLOWED) {
			return ModelMessages.getMessage(sResourceKey, new String[] { value, propertyDisplayName });
		} else if (sResourceKey == DESIGN_EXCEPTION_EXTENSION_SETTING_FORBIDDEN
				|| sResourceKey == DESIGN_EXCEPTION_PROPERTY_CHANGE_FORBIDDEN) {
			return ModelMessages.getMessage(sResourceKey, new String[] { propertyDisplayName, element.getFullName() });
		} else if (sResourceKey == DESIGN_EXCEPTION_DOT_FORBIDDEN) {
			return ModelMessages.getMessage(sResourceKey, new String[] { (String) invalidValue });
		}
		return ModelMessages.getMessage(sResourceKey);
	}
}