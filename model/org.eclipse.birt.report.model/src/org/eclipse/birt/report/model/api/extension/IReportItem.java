...
public interface IReportItem
{
	...
	/**
	 * Serializes the model given the property name. The property is visible to
	 * BIRT and its type is
	 ...
	 * @return a byte array that represents the serialized extended element
	 *         model.
	 */
	 ...
	 * 
	 * @param propName
	 *            the model property name to deserialize
	 * @param data
	...
	/**
	 * Returns the value of a public property. Returns null if the property is
	 * not defined. If the <code>IReportItemFactory</code> class defines a
	 * property, then this method must accept that property name.
	 * <p>
	 * Property names can be separated with dots and can include array indexes.
	 * For example: barColors[1].pattern. This will occur if a property is
	...
	/**
	 * Checks the value of a property prior to setting it. BIRT calls this
	 * operation before setting the value. See the <code>setProperty</code>
	 ...
	 * validated the property using the <code>checkProperty</code> method, and
	 * will have created a command to set the property.
	 * <p>
	 * If this element supports styles or inheritance, then the value can be
	 * <code>null</code>, which means to clear the property value so that it
	...
	/**
	 * Performs a semantic check on the report item. The extended element can
