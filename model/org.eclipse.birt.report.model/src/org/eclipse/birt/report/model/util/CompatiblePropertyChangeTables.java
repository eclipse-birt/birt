...
public class CompatiblePropertyChangeTables
{
	...
	/**
	 * Returns the compatibility default expression type for the given property.
	 * The algorithm checks the element, the property name and the version
	 * number. If the given version < predefined version, compatibility is
	 * required.
	 * 
	 * @param elementName
	 *            the element definition name
	 * @param propName
	 *            the property name
	 * @param versionNum
	 *            the current design file version
	 * @return the default expression type
	 */

	public static String getDefaultExprType( String elementName,
			String propName, int versionNum )
	{
		for ( int i = 0; i < propToExprTable.size( ); i++ )
		{
			CompatibleProperty tmpOldProp = propToExprTable.get( i );
			if ( versionNum < tmpOldProp.compatibleVersion
					&& elementName.equalsIgnoreCase( tmpOldProp.elementName )
	...
	private static class CompatibleProperty
	{
		...
		CompatibleProperty( String elementName, String propName,
				String defaultType, int compatibleVersion )
		{
