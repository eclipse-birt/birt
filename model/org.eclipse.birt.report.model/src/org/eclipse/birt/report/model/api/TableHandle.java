...
public class TableHandle extends ListingHandle implements ITableItemModel {
	...
	/**
	 * Gets the cell at the position where the given row and column intersect within
	 * the given slot. The first row in the slot is count as 1. And so on.
	 * <p>
	...
	public ColumnHandle findColumn(int columnIndex) {
		TableColumn targetColumn = ColumnHelper.findColumn(module, getColumns().getSlot(), columnIndex);
	...
	public void setWidthToFitColumns(int dpi) throws SemanticException {
		...
		if (element.getSlot(COLUMN_SLOT).getCount() == 0) {
			throw new SemanticError(element, SemanticError.DESIGN_EXCEPTION_TABLE_NO_COLUMN_FOUND);
		}
		...
		for (int index = 0; index < columns.size(); index++) {
			...
			DimensionValue columnWidth = (DimensionValue) column.getWidth().getValue();
			if (columnWidth == null) { // column index is 1-based.
				throw new SemanticError(element, new String[] { String.valueOf(index + 1) },
			...
			if (repeat > 1) { // Process repeat
				columnWidth = new DimensionValue(columnWidth.getMeasure() * repeat, unit);
			}
		...
		if (absoluteWidths == null || relativeWidths == null) { // Only has columns with either absolute or relative
																// widths
			if (absoluteWidths == null) {
				if (!DesignChoiceConstants.UNITS_PERCENTAGE.equalsIgnoreCase(relativeWidths.getUnits())) { // Sum of
																											// percentage
																											// will not
																											// be set
		...
		} else { // Has columns with both absolute and relative widths
			if (!DesignChoiceConstants.UNITS_PERCENTAGE.equalsIgnoreCase(relativeWidths.getUnits())) { // Only
																										// percentage is
																										// acceptable
				throw new SemanticError(element, SemanticError.DESIGN_EXCEPTION_TABLE_COLUMN_INCONSISTENT_UNIT_TYPE);
			}
			double percent = relativeWidths.getMeasure();
			if (Double.compare(percent, 100) >= 0) { // Out of 100%
				throw new SemanticError(element, SemanticError.DESIGN_EXCEPTION_TABLE_COLUMN_ILLEGAL_PERCENTAGE);
			}
			percent /= 100;
