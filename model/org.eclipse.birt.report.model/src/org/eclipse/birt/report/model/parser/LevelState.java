...
abstract public class LevelState extends ReportElementState
{
	...
	public void end( ) throws SAXException
	{
		...
		{
			( (ModuleNameHelper) handler.module.getNameHelper( ) )
					.addCachedLevel( getElement( ) );
		}

		if ( handler.versionNumber < VersionUtil.VERSION_3_2_23
				&& getElement( ).getProperty( handler.module,
						ILevelModel.DATE_TIME_LEVEL_TYPE ) != null )
		{
			boolean isFound = false;
			ElementPropertyDefn attributesPropertyDefn = getElement( )
					.getPropertyDefn( ILevelModel.ATTRIBUTES_PROP );
			List attrs = (List)getElement( ).getProperty( handler.module,
			...
			if ( !isFound )
			{
				...
				if ( getElement( ) instanceof TabularLevel )
				{
					...
					attribute.setDataType( getDataType( (TabularLevel) this
							.getElement( ) ) );
					struct = attribute;
				}
	...
	{
		String columnName = level.getStringProperty( handler.module,
		...
		if ( !StringUtil.isBlank( columnName ) )
		{
			...
			if ( dataSet != null )
			{
				CachedMetaData metaData = (CachedMetaData) dataSet.getProperty(
						handler.module, IDataSetModel.CACHED_METADATA_PROP );
				if ( metaData != null )
				{
					List<ResultSetColumn> resultSet = (List<ResultSetColumn>) metaData
							.getProperty( handler.module,
					...
					if ( resultSet != null )
					{
						for ( ResultSetColumn column : resultSet )
						{
							if ( columnName.equals( column.getStringProperty(
									handler.module, ResultSetColumn.NAME_MEMBER ) ) )
							{
