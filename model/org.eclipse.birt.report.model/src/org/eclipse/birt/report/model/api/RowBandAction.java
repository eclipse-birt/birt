...
abstract class RowBandAction
{
	...
	/**
	 * Checks every cell contains row span or not. If contains any row span ,
	 * return <code>false</code>;Otherwise return <code>true</code>
	 * 
	 * @param rowHandle
	 *            handle of row.
	 * @return If contains any row span , return <code>true</code>;Otherwise
	...
	protected SlotHandle getSlotHandle( RowOperationParameters parameters )
	{
		...
		if ( reportHandle instanceof TableHandle )
		{
			if ( groupId >= 0 )
			{
				SlotHandle groups = ( (ListingHandle) reportHandle )
						.getGroups( );
				if ( groups == null || groupId < 0
						|| groupId >= groups.getCount( ) )
					return null;
			...
			{
				if ( slotId < IListingElementModel.HEADER_SLOT
