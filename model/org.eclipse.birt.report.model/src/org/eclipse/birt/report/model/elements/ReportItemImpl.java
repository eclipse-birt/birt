...
public abstract class ReportItemImpl extends ReferencableStyledElement
		implements IReportItemModel, ISupportThemeElement {
	...
	public List<SemanticException> validate(Module module) {
		...
		list.addAll(ElementReferenceValidator.getInstance().validate(module, this, DATA_SET_PROP));
	...
	public Object getProperty(Module module, ElementPropertyDefn prop) {
		...
		if (IReportItemModel.CASCADE_ACL_PROP.equals(propName) && (!getDefn().isContainer())) {
			return false;
		}
	...
	private void checkDataBindingReferring(Module lib, DesignElement element) throws ExtendsException {
		if (element instanceof ReportItem && ((ReportItem) element).isDataBindingReferring(lib)) {
			throw new ExtendsForbiddenException(null, element,
	...
	private AbstractTheme getDefaultTheme(Module module) {
		String themeType = MetaDataDictionary.getInstance().getThemeType(this.getDefn());
		if (themeType == null) {
			return null;
		}
		...
		if (reportTheme != null) {
			...
			return reportTheme.getRoot().findReportItemTheme(themeType + '-' + reportThemeName);
		}
		return null;
	}
