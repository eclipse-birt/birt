...
public final class ErrorDetail implements ErrorCodes {
	...
	/**
	 * The error description, which is used for <code>toString()</code>. Generally,
	...
	private void translate(XMLParserException e) {
		...
		if (e.getErrorCode().equalsIgnoreCase(XMLParserException.DESIGN_EXCEPTION_EXCEPTION)) {
	...
	private void translate(SemanticException e) {
		...
		if ((e instanceof CssException)) {
			...
			lineNo = module.getLineNo(((CssException) e).getIncludedStyleSheet());
		}
	...
	private void translate(ExtendedElementException e) {
		...
		subEditor = (String) e.getProperty(ExtendedElementException.SUB_EDITOR);

		String tempNumber = (String) e.getProperty(ExtendedElementException.LINE_NUMBER);
		if (tempNumber != null) {
			try {
	 ...
	 *                  </ul>
	 * @return a list containing specified semantic errors. Each element in the list
