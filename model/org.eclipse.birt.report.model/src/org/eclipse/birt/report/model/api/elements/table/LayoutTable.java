...
public class LayoutTable
{
	...
	public CellHandle getCell( int groupLevel, int slotId, int rowId, int colId )
	{
		...
		LayoutSlot slot = getComplexSlot( slotId ).getLayoutSlotWithGroupLevel(
				groupLevel );
		if ( slot == null )
			return null;
	...
	/**
	 * Return the row handle with the given row position. The
	 * <code>rowPosn</code> is regardless of the slot.
	 * 
	 * @param rowPosn
	...
	/**
	 * Return the layout row with the given row position. The
	 * <code>rowPosn</code> is regardless of the slot.
	 * 
	 * @param rowPosn
	 *            the 1-based row position
	 ...
	 *            the cell element
	 * @param slot
	 *            the layout slot where the cell resides
	 * @param rowPosn
	 *            the 1-based row position in the slost
	 * @param colPosn
	 *            the 1-based column position in the slost
	 * @param rowSpanOffset
	 *            the row span offset of the overlapped area
	 * @param colSpanOffset
	 *            the column span offset of the overlapped area
	 */

	protected void addOverlappedCell( Cell cell, LayoutSlot slot, int rowPosn,
			int colPosn, int rowSpanOffset, int colSpanOffset )
	{
	...
	protected static class OverlappedArea
	{
		 ...
		 * 
		 * @param cell
		 *            the cell that causes the overlapped area
		 * @param slot
		 *            the slot in which the overlapped occurs
		 * @param rowPosn
		 *            the row position in the slot
		 * @param columnPosn
		 *            the column position
		 * @param rowSpanOffset
		...
		private OverlappedArea( Cell cell, LayoutSlot slot, int rowPosn,
				int columnPosn, int rowSpanOffset, int colSpanOffset )
		{
			this.cell = cell;
