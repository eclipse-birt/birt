...
public class RomUpdater
{
	...
	private void logNotice( SpecElement element, String action )
			throws IOException
	{
		String msg = "Notice: " + element.getTypeName( ) + " " + element.name
				+ ": " + action;
		log.println( msg );
	...
	private void logMismatch( SpecElement element, String attrName,
			String specValue, String romValue ) throws IOException
	{
		String action = attrName + ": \"" + specValue
				+ "\" does not match rom.def value \"" + romValue + "\"";
	...
	private void logNotice( SpecElement element, SpecProperty prop,
			String action ) throws IOException
	{
		String msg = element.getTypeName( ) + " Property " + element.name + "."
				+ prop.name + ": " + action;
		log.println( "Notice: " + msg );
		System.err.println( msg );
	...
	private void logMismatch( SpecElement element, SpecProperty prop,
			String attrName, String specValue, String romValue )
			throws IOException
	{
		String action = attrName + ": \"" + specValue
				+ "\" does not match rom.def value \"" + romValue + "\"";
	...
	private void logNotice( SpecElement element, String attrib, String action )
			throws IOException
	{
	...
	private void logUpdate( SpecElement element, String attrib, String value )
			throws IOException
	{
		String msg = element.getTypeName( ) + " " + element.name + "." + attrib
				+ ": Updated to " + value;
		log.println( msg );
	}

	private void logUpdate( SpecElement element, SpecProperty prop,
			String attrName, String value ) throws IOException
	{
		String msg = element.getTypeName( ) + " Property " + element.name + "."
				+ prop.name + "." + attrName + ": Updated to " + value;
		log.println( msg );
	}
	...
	void applyObject( String attrib, String objName ) throws IOException
	{
		...
		if ( isBlank( value ) )
		{
			...
			if ( jsObj == null )
			{
				logNotice( specElement, attrib, "Object name \"" + objName
						+ "\" is not defined in rom.def" );
			}
			else
			{
	...
	private void applyAttrib( String attrib, String newValue )
			throws IOException
	{
		if ( isBlank( newValue ) )
			return;
	...
	void applySpecProperties( ) throws IOException
	{
		...
		while ( iter.hasNext( ) )
		{
			...
			applyBoolean( prop, romProp, prop.runtimeSettable,
					RUNTIME_SETTABLE, "true" );
			applyBoolean( prop, romProp, prop.required, VALUE_REQUIRED, "false" );
			checkBoolean( prop, romProp, prop.isArray, IS_LIST, "false" );
			if ( specElement.type == SpecElement.ELEMENT )
			{
				applyBoolean( prop, romProp, prop.inherited, CAN_INHERIT,
						"true" );
	...
	public void applyAttrib( SpecProperty prop, Element romProp,
			String attribValue, String attrib ) throws IOException
	{
		// Since
	...
	public void applyDefault( SpecProperty prop, Element romProp )
			throws IOException
	{
		// Default value
		...
		if ( prop.defaultValue.equalsIgnoreCase( "None" ) )
		{
			if ( !isBlank( value ) )
			{
				logNotice( specElement, prop, "rom.def has default value \""
						+ prop.defaultValue + "\" but the spec says None" );
			}
		}
	...
	public void applyBoolean( SpecProperty prop, Element romProp,
			int attribValue, String attrib, String defaultValue )
			throws IOException
	{
		// Runtime settable
	...
	public void checkBoolean( SpecProperty prop, Element romProp,
			int attribValue, String attrib, String defaultValue )
			throws IOException
	{
		if ( attribValue == SpecObject.TRI_UNKNOWN )
	...
	public void applyContext( SpecProperty prop, Element romProp )
			throws IOException
	{
		...
		if ( type == null || !type.equals( "expression" ) )
		{
			logNotice( specElement, prop,
					"Spec expression context, but ROM propety type is " + type );
			return;
		}
		...
		{
			logNotice( specElement, prop,
					"Spec has unknown value for expr context: "
							+ prop.exprContext );
			return;
		}
