...
public class LegacySpecParser extends HtmlDocReader
{
	...
	{
		return "[" + Integer.toString( parser.getLineNo( ) ) +
		...
		for ( ; ; )
		{
			...
			else if ( tag.equals( "span" ) )
			{
				...
				else
				{
					...
					else if ( value.startsWith( "font-weight: normal" )  ||
							  value.startsWith( "font-style: normal;" ) )
					{
						inIgnore = true;
					}
					else
		...
		while ( isPara( token, "MsoBodyText" ) ) //$NON-NLS-1$
		{
			...
			else if ( startsWith( line, DISPLAY_NAME ) )
			{
				...
				if ( ! tail.equals( element.displayName ) )
				{
					error( element,
							"Display name: " + tail +
							" Does not match heading " + element.displayName );
				}
			}
			else
			{
				warning( element,
						 "Unrecognized summary header: " + line );
			}
			token = getToken( );
		}
		pushToken( token );
	...
	private void parseProps( )
	{
		for ( ; ; )
		{
			...
			{
				warning( element,
						": Unrecognized h3 header: " + heading );
			}
		}
	}
	...
	private void parseProperty( String propName )
	{
		...
		for ( ; ; )
		{
			...
			else if ( heading.equalsIgnoreCase( "Runtime Scripting" ) ||
					  heading.equalsIgnoreCase( "XML Summary" ) )
			{
			...
			{
				warning( element, prop,
									"Unrecognized property header: " + heading );
				prop.description = append( prop.description, 
						"\n\n<h4>" + heading + "</h4>\n\n" +
						copyBlock( ) + "\n" );
			}
	...
	private void parsePropertySummaryBlock( SpecProperty prop )
	{
		...
		while ( isPara( token, "MsoBodyText" ) ) //$NON-NLS-1$
		{
			...
			else if ( startsWith( line, REQUIRED ) )
			{
				...
				prop.required = ( tail.indexOf( "Yes" ) != -1 )
					? SpecObject.TRI_TRUE : SpecObject.TRI_FALSE;
			}
			...
			else if ( startsWith( line, DEFAULT_VALUE ) )
			{
				...
				/* if ( tail.equals( "False" )  ||  tail.equals( "True" ) )
					prop.defaultValue = tail.toLowerCase( );
				else */ if ( ! tail.startsWith( "None" )  &&
						     ! tail.equalsIgnoreCase( "See Description." ) )
					prop.defaultValue = tail.toLowerCase( );
			}
			...
			else if ( startsWith( line, USER_VISIBLE ) )
			{
				...
				prop.runtimeSettable = ( tail.indexOf( "Yes" ) != -1 )
					? SpecObject.TRI_FALSE : SpecObject.TRI_TRUE;
			}
	...
	private void parseMethod( String methodName )
	{
		...
		for ( ; ; )
		{
			...
			{
				warning( element, method,
									"Unrecognized method header: " + heading );
	...
	private void parseMethodSummaryBlock( SpecMethod method )
	{
		...
		while ( isPara( token, "MsoBodyText" ) ) //$NON-NLS-1$
		{
			...
			{
				warning( element, method,
						"Unrecognized method summary header: " + line );
			}
			token = getToken( );
		}
		pushToken( token );
