...
public class Generator
{
	...
	private void loadModel( ) throws MetaDataParserException
	{
		try
		{
			MetaDataReader.read( "docs/rom.def" );
		}
	...
	private void loadElementDocs( )
	{
		...
		while ( iter.hasNext( ) )
		{
			...
			{
				System.err.println( "Parse of document file for element " +
									element.getName( ) + " failed." );
			}
		}
	}
	...
	private void writeDocs( ) throws FileNotFoundException
	{
		startIndex( );
		writeElements( );
		writeStructures( );
	...
	private void writeElements( ) throws FileNotFoundException
	{
	...
	public void startIndex( ) throws FileNotFoundException
	{
		String fileName = "gen/index.html";
		try
		{
	...
	public void write( DocElement element ) throws FileNotFoundException
	{
		String fileName = "gen/elements/" + element.getName( ) + ".html";  //$NON-NLS-1$//$NON-NLS-2$
	...
	public void writeSection( String title, String body )
	{
		if ( isBlank( body ) )
			return;
	...
	public void writeSectionBody( String body )
	{
		if ( isBlank( body ) )
			return;
...
//		writeln( "</table>\n<br>" );
	...
	public String makeMemberReference( String dir, String objName, String prefix, String name  )
	{
	...
	private void writeInheritedProperties( DocElement element )
	{
		...
		while ( iter.hasNext( ) )
		{
			...
			write( makeMemberReference( null, prop.definedBy( ).getName( ),
		...
		while ( iter.hasNext( ) )
		{
			...
			write( "<dt>" );
			write( makeMemberReference( null, baseElement,
					PROPERTY_NAME, prop.getName( ) ) );
			write( "</dt>\n<dd>" );
	...
	private void writeInheritedMethods( DocElement element )
	{
		...
		while ( iter.hasNext( ) )
		{
			...
			write( makeMemberReference( null, method.definedBy( ).getName( ),
	...
	public void detailRow( String label, String value )
	{
		write( "<tr><td>" );
		write( label );
		writeln( ":</td>" );
	...
	public void writeChoices( DocProperty prop )
	{
		if ( ! prop.hasChoices( ) )
			return;
	...
	private void writeMethodDetail( DocMethod method )
	{
		...
		detailRow( "Arguments", "None" ); // In R1, no element methods have arguments.
...
//		writePara( "None" ); // In R1, no element methods have arguments.
	...
	public void writeSynopsis( DocMethod method )
	{
...
//		detailRow( "Cardinality", slot.getCardinality( ) );
	...
	private void writeStructures( ) throws FileNotFoundException
	{
	...
	public void write( DocStructure struct ) throws FileNotFoundException
	{
		String fileName = "gen/structs/" + struct.getName( ) + ".html";  //$NON-NLS-1$//$NON-NLS-2$
		writer = new PrintStream( new FileOutputStream( fileName ) );
	...
	private void writeTypes( ) throws FileNotFoundException
	{
		writeIndexEntry( "Property Types", "types",
						 "The set of types used to define ROM properties." );
		
		String fileName = "gen/types.html";  //$NON-NLS-1$//$NON-NLS-2$
		writer = new PrintStream( new FileOutputStream( fileName ) );
	 ...
	 * 
	 */
	private void writePropertyIndex( ) throws FileNotFoundException
	{
		writeIndexEntry( "Property Index", "prop-index",
						 "Index of properties with a link to their definition." );
		...
		String fileName = "gen/prop-index.html";  //$NON-NLS-1$//$NON-NLS-2$
		writer = new PrintStream( new FileOutputStream( fileName ) );
		writeln( "<!DOCTYPE html PUBLIC \"-//W3C//DTD HTML 4.0 transitional//EN\">" );
		write( "<html>\n<head>\n<title>" );
		writeln( " Element (Eclipse BIRT ROM Documentation)</title>" );
...
//		 "Inheritance chart for all ROM elements." );
	...
	public void setTypeHeader( String text )
	{
		typeHeader = text;
	}

	public DocPropertyType findType( String name )
	{
		Iterator iter = propertyTypes.iterator( );
		while ( iter.hasNext( ) )
		{
