...
public class SlotDefnTest extends BaseTestCase {
	...
	public void testGetterAndSetter() throws DesignFileException {
		...
		MetadataTestUtil.setDisplayNameKey(slotDefn, "Element.ReportDesign.slot.styles"); //$NON-NLS-1$
		...
		assertEquals("Element.ReportDesign.slot.styles", slotDefn.getDisplayNameID()); //$NON-NLS-1$
	...
	public void testCanContain() {
		...
		IElementDefn elementDefn = MetaDataDictionary.getInstance().getElement(ReportDesignConstants.FREE_FORM_ITEM);
		assertNotNull(elementDefn);
		...
		assertTrue(
				slotDefn.canContain(MetaDataDictionary.getInstance().getElement(ReportDesignConstants.FREE_FORM_ITEM)));
		assertTrue(slotDefn.canContain(MetaDataDictionary.getInstance().getElement(ReportDesignConstants.LABEL_ITEM)));
		assertFalse(
				slotDefn.canContain(MetaDataDictionary.getInstance().getElement(ReportDesignConstants.CELL_ELEMENT)));
	...
	public void testBuild() throws MetaDataException {
		...
		try {
			...
			MetadataTestUtil.addType(slotDefn, ReportDesignConstants.FREE_FORM_ITEM);
		...
		MetadataTestUtil.setDisplayNameKey(slotDefn, "Element.ReportDesign.slot.body"); //$NON-NLS-1$
		MetadataTestUtil.build(slotDefn);
		assertFalse(slotDefn.canContain(MetaDataDictionary.getInstance().getElement("DataSource"))); //$NON-NLS-1$
		assertTrue(slotDefn.canContain(MetaDataDictionary.getInstance().getElement("Label"))); //$NON-NLS-1$
	...
	public void testSemanticValidator() throws MetaDataParserException {
		IElementDefn groupDefn = MetaDataDictionary.getInstance().getElement(ReportDesignConstants.TABLE_GROUP_ELEMENT);
		SlotDefn headerDefn = (SlotDefn) groupDefn.getSlot(TableGroup.HEADER_SLOT);
