...
public class PropertyCommandTest extends BaseTestCase
{
	...
	private void PropertyDefnOperate( ElementPropertyDefn prop, Object value )
	{
		...
		try
		{
			page.getHandle( design ).getPropertyHandle( prop.getName( ) )
					.setValue( value );
			assertEquals( value, page.getHandle( design ).getPropertyHandle(
			...
			throws SemanticException
	{
		...
		if ( type )
		{
			if ( value == null )
			{
				...
				o = page.getHandle( design ).getPropertyHandle( name )
						.getValue( );
				assertNull( o );
			}
			else
			{
				...
				page.getHandle( design ).getPropertyHandle( name ).setValue(
						value );
				o = page.getHandle( design ).getPropertyHandle( name )
						.getValue( );
	 ...
	 * rule on the row, ensure that the list value in table won't be affected.
	 * </li>
	 * <li>Label2 extends from label1, and label1 has defined an Action. Now
	 * add a parameter binding to label2, ensure that list value in label1 won't
	 * be affected.
	 * 
	 * </li>
	 * </ul>
	...
	public void testAddItem2( ) throws Exception
	{
		...
		table.getPrivateStyle( ).getPropertyHandle(
				StyleHandle.HIGHLIGHT_RULES_PROP ).addItem( highlightRule );
		...
		assertEquals( 1, table.getPropertyHandle(
				StyleHandle.HIGHLIGHT_RULES_PROP ).getListValue( ).size( ) );
	 ...
	 * Test Cases:
	 * <ul>
	 * <li>Table define a highlight rule list, row inherited it, change the
	 * rule property on row won't affect the value on table.</li>
	...
	public void testSetMember2( ) throws SemanticException
	{
		...
		assertEquals( "blue", table.getPropertyHandle( //$NON-NLS-1$
				StyleHandle.HIGHLIGHT_RULES_PROP ).getAt( 0 ).getMember(
				HighlightRule.COLOR_MEMBER ).getStringValue( ) );
	 ...
	 * Analysis:
	 * 
	 * In <code>removeAllItems</code>, it must confirm <code>propDefn</code>
	 * is an object of <code>StructListPropertyType</code>.
	 ...
	 * 
	 * The problem is that <code>StructListPropertyType</code> always throws
	 * an PropertyValueException when it tries to validate the value.
	...
	/**
	 * Invoke
	 * {@link org.eclipse.birt.report.model.command.PropertyCommand#replaceItem( MemberRef,
	 ...
	 * <li>Undo the command, save the file,compare with a golden file, should
	 * be</li>
	...
	public void testReplaceItemTwo( ) throws Exception
	{
		...
		MemberRef mem1 = styleHandle1.getPropertyHandle( Style.MAP_RULES_PROP )
				.getReference( );
		MemberRef mem2 = styleHandle2.getPropertyHandle( Style.MAP_RULES_PROP )
				.getReference( );
		...
		try
		{
			command1.replaceItem( mem1, new CustomColor( ), rule2
					.getStructure( ) );
			fail( );
		}
		catch ( PropertyValueException e )
