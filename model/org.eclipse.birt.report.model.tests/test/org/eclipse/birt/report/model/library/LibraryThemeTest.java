...
public class LibraryThemeTest extends BaseTestCase {
	...
	public void testCssStyleSheet() throws Exception {
		...
		CssStyleSheetHandle sheetHandle = libraryHandle.openCssStyleSheet(getResource("input/base.css").getFile());//$NON-NLS-1$
		...
		// label use it
		LabelHandle labelHandle = libraryHandle.getElementFactory().newLabel("label");//$NON-NLS-1$
		...
		CssStyleSheetHandle stylySheetHandle = themeHandle.findCssStyleSheetHandleByName("base.css"); //$NON-NLS-1$
		...
		cssStruct = (IncludedCssStyleSheet) themeHandle.getListProperty(ReportDesignHandle.CSSES_PROP).get(1);
	...
	public void testChangeTheme() throws Exception {
		...
		List rules = labelHandle.getListProperty(StyleHandle.HIGHLIGHT_RULES_PROP);
		HighlightRule rule = (HighlightRule) rules.get(0);
		ElementRefValue value = (ElementRefValue) rule.getProperty(design, HighlightRule.STYLE_MEMBER);
		assertEquals(styleHandle.getElement(), value.getElement());
		rule = (HighlightRule) rules.get(1);
		value = (ElementRefValue) rule.getProperty(design, HighlightRule.STYLE_MEMBER);
		assertEquals(styleHandle.getElement(), value.getElement());
		rule = (HighlightRule) rules.get(2);
		value = (ElementRefValue) rule.getProperty(design, HighlightRule.STYLE_MEMBER);
		assertEquals(styleHandle.getElement(), value.getElement());

		assertEquals(4, ((StyleElement) styleHandle.getElement()).getClientList().size());
	...
	public void testGetProperty() throws Exception {
		...
		assertEquals(ColorPropertyType.SILVER, label1.getProperty(Style.COLOR_PROP));

		assertEquals(ColorPropertyType.LIME, label2.getProperty(Style.COLOR_PROP));

		assertEquals(ColorPropertyType.MAROON, label3.getProperty(Style.COLOR_PROP));

		assertEquals(ColorPropertyType.GRAY, list1.getProperty(Style.COLOR_PROP));
	}
	...
	public void testSetTheme() throws Exception {
		...
		assertEquals(ColorPropertyType.AQUA, label2.getProperty(Style.COLOR_PROP));

		assertEquals(ColorPropertyType.BLUE, list1.getProperty(Style.COLOR_PROP));
		...
		ThemeHandle theme = libraryHandle.getElementFactory().newTheme("Theme3"); //$NON-NLS-1$

		try {
			designHandle.setTheme(theme);
			fail();
		} catch (ThemeException e) {
			assertEquals(ThemeException.DESIGN_EXCEPTION_NOT_FOUND, e.getErrorCode());
		...
		try {
			designHandle.setProperty(IModuleModel.THEME_PROP, theme.getElement());
			fail();
		} catch (ThemeException e) {
			assertEquals(ThemeException.DESIGN_EXCEPTION_NOT_FOUND, e.getErrorCode());
			assertEquals("Theme.Theme3", e.getTheme()); //$NON-NLS-1$
	...
	public void testAddRemoveStyleLocally() throws Exception {
		...
		StyleHandle myStyle1 = libraryHandle.getElementFactory().newStyle("libStyle2"); //$NON-NLS-1$
		...
		assertEquals(ColorPropertyType.NAVY, label1.getProperty(Style.COLOR_PROP));

		// no specified notifications for custom styles

		assertEquals(0, listener.counter);

		TextItemHandle text1 = (TextItemHandle) libraryHandle.findElement("text1"); //$NON-NLS-1$

		StyleHandle textSelector = libraryHandle.getElementFactory().newStyle("text"); //$NON-NLS-1$
		...
		assertEquals(ColorPropertyType.NAVY, text1.getProperty(Style.COLOR_PROP));
	...
	public void testSetPropertyOnSeclector() throws Exception {
		...
		assertEquals(ColorPropertyType.BLUE, text1.getProperty(Style.COLOR_PROP));
	}
	...
	public void testThemeWithAddRemoveLibrary() throws Exception {
		...
		assertEquals(ColorPropertyType.MAROON, label2.getProperty(Style.COLOR_PROP));

		assertEquals(ColorPropertyType.BLACK, list1.getProperty(Style.COLOR_PROP));
	...
	public void testCanContainStyle() throws Exception {
		...
		ThemeHandle theme = libraryHandle.getElementFactory().newTheme("testTheme"); //$NON-NLS-1$

		StyleHandle style1 = libraryHandle.getElementFactory().newStyle("sytle1"); //$NON-NLS-1$
		StyleHandle style2 = libraryHandle.getElementFactory().newStyle("sytle1"); //$NON-NLS-1$
	...
	public void testClone() throws Exception {
		...
		ThemeHandle themeCloned = (ThemeHandle) theme1.copy().getHandle(libraryHandle.getModule());

		StyleHandle style1 = libraryHandle.getElementFactory().newStyle("style1"); //$NON-NLS-1$
	...
	public void testAddStyleWithThemeInLibrary() throws Exception {
		...
		assertEquals(2, library.getNameHelper().getNameSpace(Module.THEME_NAME_SPACE).getCount());
		assertEquals(0, library.getNameHelper().getNameSpace(Module.STYLE_NAME_SPACE).getCount());
	...
	public void testCreateLibraryAndSetTheme() throws Exception {
		...
		assertEquals(ColorPropertyType.BLACK, label.getStringProperty(Style.COLOR_PROP));
	}
	...
	public void testGetDisplayLabel() throws Exception {
		...
		assertEquals("libA.theme1", ((ThemeHandle) designHandle //$NON-NLS-1$
				.getVisibleThemes(IAccessControl.DIRECTLY_INCLUDED_LEVEL).get(1))
						.getDisplayLabel(IDesignElementModel.FULL_LABEL));

		assertEquals("libA.theme1", ((ThemeHandle) designHandle //$NON-NLS-1$
				.getVisibleThemes(IAccessControl.DIRECTLY_INCLUDED_LEVEL).get(1)).getDisplayLabel());
	...
	public void testGetAllThemes() throws Exception {
		...
		List list = designHandle.getVisibleThemes(IAccessControl.DIRECTLY_INCLUDED_LEVEL);
		assertEquals(0, list.size());

		libraryHandle = designHandle.getLibrary("CompsiteLib"); //$NON-NLS-1$
		assertEquals(2, libraryHandle.getVisibleThemes(IAccessControl.DIRECTLY_INCLUDED_LEVEL).size());
		assertEquals(0, libraryHandle.getVisibleThemes(IAccessControl.NATIVE_LEVEL).size());
	}
	...
	public void testRemoveStyleInTheme() throws Exception {
		...
		LabelHandle labelHandle = (LabelHandle) designHandle.getElementByID(7l);
