...
public class ContentCommandTest extends BaseTestCase
{
	...
	public void testAdd( ) throws Exception
	{
		...
		try
		{
			SimpleMasterPageHandle pageHandle = designHandle
		...
		{
			assertEquals( ContentException.DESIGN_EXCEPTION_SLOT_IS_FULL, e
					.getErrorCode( ) );
		}
		...
		try
		{
			containerHandle.getSlot( 0 ).add(
					new TableItem( ).getHandle( design ) );
			fail( );
		}
		catch ( ContentException e )
		{
		...
		{
			assertEquals( ContentException.DESIGN_EXCEPTION_SLOT_NOT_FOUND, e
					.getErrorCode( ) );
		}
	}
	 ...
	 * 
	 */
	public void testAddElementIntoCompoundSlot( ) throws ContentException,
			NameException
	{
		...
		catch ( ContentException e )
		{
			...
			assertEquals(
					ContentException.DESIGN_EXCEPTION_CONTENT_NAME_REQUIRED, e
							.getErrorCode( ) );
		}
	...
	public void testRemove( ) throws Exception
	{
		...
		{
			assertEquals( ContentException.DESIGN_EXCEPTION_NOT_CONTAINER, e
					.getErrorCode( ) );
		}
		...
		{
			assertEquals( ContentException.DESIGN_EXCEPTION_CONTENT_NOT_FOUND,
					e.getErrorCode( ) );
		}

		// When style is removed, the clients of this style will has no style.

		SharedStyleHandle style = designHandle.getElementFactory( ).newStyle(
				"style1" ); //$NON-NLS-1$
		designHandle.getStyles( ).add( style );

		FreeFormHandle form = designHandle.getElementFactory( ).newFreeForm(
				"form2" ); //$NON-NLS-1$
		designHandle.getBody( ).add( form );
		form.setStyle( style );
		...
		OdaDataSetHandle dataSet = designHandle.getElementFactory( )
	 ...
	 * result:
	 * <li>After undo and redo both of the reference status and the element
	...
	public void testNormalCaseUndoAndRedo( ) throws Exception
	{
		...
		assertEquals( contentHandle.getElement( ), container.getSlot( 0 )
				.getContent( 2 ) );
	 ...
	 * <ul>
	 * <li>Normal case
	 * <li>Drop a content which has 'extends', 'name' property and a style.Undo
	...
	public void testUndoRedoAfterRemove( ) throws Exception
	{
		...
		catch ( SemanticException e )
		{
			...
			assertEquals( ContentException.DESIGN_EXCEPTION_NOT_CONTAINER, ex
					.getErrorCode( ) );
		}
		...
		assertEquals( style, content.getStyle( ) );
		assertEquals( content, ( (BackRef) style.getClientList( ).get( 0 ) )
				.getElement( ) );
	...
	public void testOtherMoveException( )
	{
		...
		try
		{
			containerHandle.getSlot( 0 ).move( content.getHandle( design ),
					label.getHandle( design ), 0 );
			fail( );
		}
		catch ( ContentException e1 )
		{
			assertEquals( ContentException.DESIGN_EXCEPTION_NOT_CONTAINER, e1
					.getErrorCode( ) );
		}
		...
		{
			containerHandle.getSlot( 0 ).move( content.getHandle( design ),
					toContainer.getHandle( design ), 0 );
			fail( );
		}
	...
	public void testMove( ) throws Exception
	{
		...
		sHandle.move( label1.handle( design ), toContainer.handle( design ),
				FreeForm.REPORT_ITEMS_SLOT );
		assertEquals( sHandle.getCount( ), 0 );
		assertEquals( 1, toContainer.handle( design ).getReportItems( )
				.getCount( ) );

		sHandle.add( label2.getHandle( design ) );
		assertEquals( sHandle.getCount( ), 1 );
		sHandle.move( label2.handle( design ), toContainer.handle( design ),
				FreeForm.REPORT_ITEMS_SLOT, 0 );
		assertEquals( sHandle.getCount( ), 0 );
		assertEquals( 2, toContainer.handle( design ).getReportItems( )
				.getCount( ) );
		assertEquals( label2, toContainer.handle( design ).getReportItems( )
				.get( 0 ).getElement( ) );
		...
		sHandle.move( label3.handle( design ), toContainer.handle( design ),
				FreeForm.REPORT_ITEMS_SLOT, 10 );
		assertEquals( 3, toContainer.handle( design ).getReportItems( )
				.getCount( ) );
		assertEquals( label3, toContainer.handle( design ).getReportItems( )
				.get( 2 ).getElement( ) );

		Label label4 = new Label( );
		sHandle.add( label4.getHandle( design ) );
		sHandle.move( label4.handle( design ), toContainer.handle( design ),
				FreeForm.REPORT_ITEMS_SLOT, 12 );
		assertEquals( 4, toContainer.handle( design ).getReportItems( )
				.getCount( ) );
		assertEquals( label4, toContainer.handle( design ).getReportItems( )
				.get( 3 ).getElement( ) );
	...
	public void testMoveWrongType( ) throws Exception
	{
		...
		try
		{
			designHandle.getStyles( ).move( styleHandle, designHandle,
		...
		// 2
		designHandle.getMasterPages( ).add(
				factory.newSimpleMasterPage( "page2" ) ); //$NON-NLS-1$
	...
	public void testCircularContent( ) throws Exception
	{
		...
		FreeFormHandle topHandle = (FreeFormHandle) topContainer
				.getHandle( design );
		...
		try
		{
			content.getHandle( design ).getSlot( 0 ).add(
					topContainer.getHandle( design ) );
		...
		{
			assertEquals( ContentException.DESIGN_EXCEPTION_RECURSIVE, e
					.getErrorCode( ) );
		}

		// add the container to middContainer

		middContainer.getHandle( design ).getSlot( 0 ).add(
				topContainer.getHandle( design ) );
		...
		{
			content.getHandle( design ).getSlot( 0 ).add(
					middContainer.getHandle( design ) );
		}
		catch ( ContentException e )
		{
			assertEquals( ContentException.DESIGN_EXCEPTION_RECURSIVE, e
	...
	/**
	 * Unit test to test after delete an element, whether references on the
	 * deleted contented were adjusted correctly.
	 * 
	 * Test case:
	 * 
	 * <ul>
	 ...
	 * 
	 * <p>
	 * The expected result: C extends from B. Parent reference and style
	 * reference on A are broken.
	 * 
	 * @throws Exception
	 */
	...
	public void testElementReferenceAfterDeletionAndDetachment( )
			throws Exception
	{
		// Test drop
		...
		assertEquals( dataSourceHandle.getName( ), dataSetHandle
				.getDataSourceName( ) );
	...
	private void createDataSourceAndDataSet( ) throws Exception
	{
		...
		ScriptDataSourceHandle dataSourceHandle = designHandle
				.getElementFactory( ).newScriptDataSource( "dataSource" );//$NON-NLS-1$
		ScriptDataSetHandle dataSetHandle = designHandle.getElementFactory( )
				.newScriptDataSet( "dataSet" );//$NON-NLS-1$
	...
	/**
	 * Test after delete a style, whether references on the deleted contented
	 * were adjusted correctly.
	 * 
	 * Test case:
	 * 
	...
	public void testStyleReferenceAfterDeletionAndDetachment( )
			throws Exception
	{
		// Test drop
		...
		assertEquals( styleHandle.getName( ), labelHandle
				.getStringProperty( LabelHandle.STYLE_PROP ) );
	...
	private void createStyleAndLabel( ) throws Exception
	{
		...
		SharedStyleHandle styleHandle = designHandle.getElementFactory( )
	 ...
	 * Test case:
	 * <ul>
	 * <li>Set style to label. Add label to otherCont. Add otherCont to cont.
	 * Add cont to container. Remove cont from container. The expected result
	 * is: There is no content in container. There is no content in cont. There
	 * is no content in otherCont. There is no style on label. Style's clint
	 * list is empty.
	 * </ul>
	 * 
	 * @throws Exception
	...
	public void testRemoveContainerFromContainer( ) throws Exception
	{
		...
		cont.getHandle( design ).getSlot( 0 )
				.add( otherCont.getHandle( design ) );
		otherCont.getHandle( design ).getSlot( 0 ).add(
				label.getHandle( design ) );
	...
	public void testSendNotifications( ) throws SemanticException
	{
		...
		FreeFormHandle contentHandle = designHandle.getElementFactory( )
				.newFreeForm( "Form1" ); //$NON-NLS-1$

		FreeFormHandle contentHandle2 = designHandle.getElementFactory( )
				.newFreeForm( "Form2" ); //$NON-NLS-1$
		...
		assertEquals( contentHandle.getElement( ),
				( (ContentEvent) containerListener.event ).getContent( ) );

		designHandle.getBody( ).add( contentHandle2 );
		assertEquals( MyContentListener.ADDED, containerListener.action );
		assertTrue( containerListener.event instanceof ContentEvent );
		assertEquals( contentHandle2.getElement( ),
				( (ContentEvent) containerListener.event ).getContent( ) );
	...
	public void testSendNotificationBeforeDeletion( ) throws SemanticException
	{
		...
		DataSourceHandle dataSource = factory.newOdaDataSource(
				"dataSource", null ); //$NON-NLS-1$
	 ...
	 * elements by using class in elements package.
	 * 
	 * @throws Exception
	 *             if any error occurs
	 * 
	...
	public void testAddCompoundElement( ) throws Exception
	{
		...
		CascadingParameterGroup cascadingGroup = new CascadingParameterGroup(
				"cas1" ); //$NON-NLS-1$
	 ...
	 * Test cases:
	 * 
	 * 1. with the given CascadingParameterGroup, test copy/paste twice and
	 * check changes of name spaces.
	 * 
	 * @param cacascadingGroup
	 * @throws Exception
	 */

	private void testCopyAndPasteCompoundElement(
			CascadingParameterGroup cacascadingGroup ) throws Exception
	{
		...
		CascadingParameterGroupHandle clonedCasHandle = (CascadingParameterGroupHandle) clonedCas
				.getHandle( design );
		...
		assertTrue( ns.contains( clonedCasHandle.getParameters( ).get( 0 )
				.getName( ) ) );
		assertTrue( ns.contains( clonedCasHandle.getParameters( ).get( 1 )
				.getName( ) ) );

		clonedCas = cascadingGroupHandle.copy( );
		clonedCasHandle = (CascadingParameterGroupHandle) clonedCas
				.getHandle( design );
	...
	public void testRemoveReferencableElement( ) throws Exception
	{
		...
		List conditions = cube
				.getListProperty( TabularCubeHandle.DIMENSION_CONDITIONS_PROP );
		for ( int i = 0; i < conditions.size( ); i++ )
		{
			DimensionCondition dimensionCond = (DimensionCondition) conditions
					.get( i );
			List joinConditions = (List) dimensionCond.getProperty( design,
					DimensionCondition.JOIN_CONDITIONS_MEMBER );
			for ( int j = 0; j < joinConditions.size( ); j++ )
			{
				DimensionJoinCondition dimensionJoinCond = (DimensionJoinCondition) joinConditions
						.get( j );
				dimensionJoinCond.getProperty( design,
