 ...
 * <td>Check all the properties of the image item defined in the XML file.
 * </td>
 ...
 * <td>Get a image instance from the design file, then modify the properties
 * and finally write back to an XML file</td>
 * <td>Properties are correctly set on the image item, and the final output
 * file is identical to the golden file</td>
 * </tr>
 * 
 * <tr>
...
public class ImageItemParseTest extends BaseTestCase
{
	...
	/**
	 * Return the image item defined in the design file by its name. The test
	 * cases can use this method to get different image instances for test.
	 * 
	 * @param name
	 *            The name of image item.
	...
	public void testImageContainer( ) throws Exception
	{
		...
		GraphicMasterPage page = (GraphicMasterPage) design
				.findPage( "My Page" ); //$NON-NLS-1$
		...
		assertEquals(
				"Image1", ( (ImageHandle) imageHandle ).getElement( ).getName( ) ); //$NON-NLS-1$

		imageHandle = sHandle.get( 3 );
		assertTrue( imageHandle instanceof ImageHandle );
		assertEquals(
				"Image2", ( (ImageHandle) imageHandle ).getElement( ).getName( ) ); //$NON-NLS-1$
	}

	/**
	 * Test the properties of the image. These properties are well defined in
	 * the design file.
	 * 
	 * @throws Exception
	 *             if errors occur when reads the design file
	 */
	...
	public void testReadProperties( ) throws Exception
	{
		...
		assertEquals( ReportDesign.BODY_SLOT, handle.getContainer( )
				.findContentSlot( handle ) );
	...
	/**
	 * Test the properties, which is not correctly set in the design file, of
	 * the image item, to see how the framework handle this situation.
	 * 
	 * @throws Exception
	 *             if errors occur when reads the design file
	 */
	...
	/**
	 * After getting the image item instance from the design file, set
	 * properties on this image to see the property can be correctly set on the
	 * image and also can be written into the design file.
	 * 
	 * @throws Exception
	 *             if errors occur when reads or writes the design file
	 */
	...
	public void testWriteProperties( ) throws Exception
	{
		...
		assertTrue( handle.getSize( ).compareToIgnoreCase(
				DesignChoiceConstants.IMAGE_SIZE_SIZE_TO_IMAGE ) == 0 );
	...
	/**
	 * Test the semantic error in the design file if the image file just specify
	 * the URL value.
	 * 
	 * @throws Exception
	 */
