...
public class ClassDefnTest extends AbstractMetaTest {
	...
	public void testParseRomDef() throws MetaDataParserException {
		loadMetaData(ClassDefnTest.class.getResourceAsStream("input/ClassDefnTest.def")); //$NON-NLS-1$
		...
		assertEquals("Class.MyClass.Constructor", constructor.getDisplayNameKey()); //$NON-NLS-1$
		assertEquals("Class.MyClass.Constructor.toolTip", constructor.getToolTipKey()); //$NON-NLS-1$
		...
		assertEquals("Class.MyClass.columnName.toolTip", member.getToolTipKey()); //$NON-NLS-1$
		...
		assertEquals("Class.MyClass.columnType.toolTip", member.getToolTipKey()); //$NON-NLS-1$
		...
		assertEquals("Class.MyClass.getColumnName.toolTip", method.getToolTipKey()); //$NON-NLS-1$
	...
	public void testOptionalArgument() {
		...
		for (Iterator iter1 = method.argumentListIterator(); iter1.hasNext();) {
			...
			for (Iterator iter2 = argumentList.argumentsIterator(); iter2.hasNext();) {
				ArgumentInfo argu = (ArgumentInfo) iter2.next();
				if (ArgumentInfo.OPTIONAL_ARGUMENT_NAME.equalsIgnoreCase(argu.getName()))
