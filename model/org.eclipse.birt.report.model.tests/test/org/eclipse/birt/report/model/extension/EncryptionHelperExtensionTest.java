...
public class EncryptionHelperExtensionTest extends BaseTestCase
{
	...
	public void testMetaData( )
	{
		...
		IEncryptionHelper helper = dd
				.getEncryptionHelper( "oneEncryptionHelper" ); //$NON-NLS-1$
		assertEquals(
	...
	public void testCommand( ) throws Exception
	{
		...
		{
			assertEquals(
					EncryptionException.DESIGN_EXCEPTION_INVALID_ENCRYPTION, e
							.getErrorCode( ) );
		}
		...
		assertEquals( "encryption_a", item.getElement( ) //$NON-NLS-1$
				.getLocalEncryptionID(
						(ElementPropertyDefn) item.getPropertyDefn( propName ) ) );
		assertEquals(
				"_a_New Password", ExtensionTestUtil.getLocalExtensionMapValue( (ExtendedItem) item.getElement( ), propName ) ); //$NON-NLS-1$
		...
		assertEquals( "myown", item.getStringProperty( propName ) ); //$NON-NLS-1$		
		assertEquals(
				"_a_myown", ExtensionTestUtil.getLocalExtensionMapValue( (ExtendedItem) item.getElement( ), propName ) ); //$NON-NLS-1$
	...
	public void testSetProperty( ) throws Exception
	{
		...
		DesignElementHandle item = designHandle.findElement( "item_1" ); //$NON-NLS-1$
		assertEquals(
				"New Password", item.getElement( ).getLocalProperty( design, propName ) ); //$NON-NLS-1$
		assertEquals( SimpleEncryptionHelper.ENCRYPTION_ID, item.getElement( )
				.getLocalEncryptionID(
						(ElementPropertyDefn) item.getPropertyDefn( propName ) ) );
		// set property
