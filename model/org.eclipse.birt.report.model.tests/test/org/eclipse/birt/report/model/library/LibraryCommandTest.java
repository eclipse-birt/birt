...
public class LibraryCommandTest extends BaseTestCase
{
	...
	/**
	 * Tests droping a library with different namespace but same source library
	 * file name of the already included library in report design
	 * 
	 * @throws Exception
	 *             if any exception
	 */
	...
	public void testLibraryRecursivelyIncludeLibrary( )
			throws DesignFileException
	{
		...
		try
		{
			libraryHandle.includeLibrary(
					"LibraryIncludingTwoLibraries.xml", null ); //$NON-NLS-1$
			fail( );
		}
		catch ( Exception e )
		{
	...
	/**
	 * test when the library elements is used in report, exception will throw
	 * out before the drop operation.
	 * 
	 * @throws DesignFileException
	 * @throws SemanticException
	 */

	public void testExceptionWhenDropUsedLibrary( ) throws DesignFileException,
			SemanticException
	{
		...
		DataSourceHandle designDataSource = (DataSourceHandle) designHandle
				.getElementFactory( ).newElementFrom( libDataSource,
						"designDataSource" ); //$NON-NLS-1$
		...
		ParameterHandle designParameter = (ParameterHandle) designHandle
				.getElementFactory( ).newElementFrom( parameter, "designPara" ); //$NON-NLS-1$
	...
	public void testDropLibraryWithRelativePath( ) throws Exception
	{
		...
		designHandle.includeLibrary(
				"../golden/LibraryParseTest_golden.xml", "testDrop" ); //$NON-NLS-1$//$NON-NLS-2$
	...
	public void testNameSpaceWithAddLibrary( ) throws Exception
	{
		...
		for ( int i = 1; i < 5; i++ )
			assertTrue( ( (DesignElement) elements.get( i ) )
					.isContentOf( designTable ) );
		...
		assertTrue( ( (DesignElement) elements.get( 6 ) )
				.isContentOf( designGrid ) );
	}
}
