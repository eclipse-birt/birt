...
public class PropertyLocalizationTest extends BaseTestCase
{
     ...
     * internal data structure for a line in the input test file.
     */
    static class Case
    {
     ...
     * <p>
     * Element represents an DesignElement.<p>
     * Property name represents property name of the element you what to set.<p>
     * ULocale represents for a locale that you want the input string to be parsed in.<p>
     * Input string is what you want the user to input.<p>
     * isOK = "F" means that the setProperty() call should fail because the input string is not in the format of the locale.
     * isOK = "P" means that the setProperty() call should success.
     * 
     * @param fileName
     * @throws Exception
     * 
    ...
    private void prepareCases( String fileName ) throws Exception
    {
        ...
        while ( ( line = br.readLine( ) ) != null )
        {
            ...
            if ( st.countTokens( ) != 5 )
                throw new Exception(
                        "Invalid input, lineNo = " + lineNo + " tokens = " + st.countTokens( ) ); //$NON-NLS-1$ //$NON-NLS-2$
            ...
            try
            {
                designElement = (DesignElement) Class
                        .forName(
                                "org.eclipse.birt.report.model.elements." + element ).newInstance( ); //$NON-NLS-1$
            }
            catch ( ClassNotFoundException e )
            {
                throw new Exception(
                        "Invalid element name, lineNo = " + lineNo, e ); //$NON-NLS-1$
            }
            ...
            {
                throw new Exception(
                        "Only P/F allowed for 'isOK' column, lineNo = " + lineNo ); //$NON-NLS-1$
            }
     ...
     * 
     * @param input
     *            string to convert, which represents one java character.
     * @return java character
     * @throws Exception
     *             if the input string is not a hex string.
     */
    ...
    /**
     * 
     * Run through the input test file, find the required DesignElement, and set
     * its property using an input string. Check that the setProperty operation
     * should throw an Exception if isOK = "F"(fail), and that now Exception if
     * isOK = "P"(pass)
     * <p>
     * The test file should follow the following format:
     * <p>
     ...
     * <p>
     * ULocale represents for a locale that you want the input string to be
     * parsed in.
     * <p>
     * Input string is what you want the user to input.
     * <p>
     * isOK = "F" means that the setProperty() call should fail because the
     * input string is not in the format of the locale. isOK = "P" means that
     * the setProperty() call should success.
     * 
    ...
    public void testSetStringProperty( ) throws DesignFileException
    {
        ...
        for ( int i = 0; i < cases.size( ); i++ )
        {
            ...
            DesignElementHandle elementHandle = new MockupElementHandle(
                    design, aCase.element );

            if ( aCase.isOK ) // No exception thrown.
            {
                try
                {
                    elementHandle.setProperty( aCase.propName,
                            aCase.inputString );
                }
     ...
     * @throws DesignFileException
     */
    public void testValidateInputStringInEn( ) throws PropertyValueException,
            DesignFileException
    {
        openDesign( "PropertyLocalizationTest.xml", ULocale.ENGLISH ); //$NON-NLS-1$
        ...
        assertEquals( IColorConstants.RED,
                propertyType.validateInputString( null, null, propDefn, "red" ) ); //$NON-NLS-1$
        try
        {
        ...
        assertEquals( new Boolean( true ),
                propertyType.validateInputString( null, null, null, "true" ) ); //$NON-NLS-1$
    ...
    public void testValidateInputStringInZH_CN( ) throws PropertyValueException
    {
        ...
        propertyType = MetaDataDictionary.getInstance( ).getPropertyType(
                PropertyType.BOOLEAN_TYPE );
        assertEquals( new Boolean( true ),
                propertyType.validateInputString( null, null, null, "\u771f" ) ); //$NON-NLS-1$
        assertEquals( new Boolean( false ),
                propertyType.validateInputString( null, null, null, "\u5047" ) ); //$NON-NLS-1$
