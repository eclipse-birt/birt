...
public class PropertyDefnTest extends AbstractMetaTest
{
	...
	{
		/*
		 * <Property name="TestProperty" type="String" displayname=""
		 * mask="Change" default="" group="" checks=""/>
		 */
		...
		assertEquals(
				"Element.ReportDesign.author", propertyDefn.getDisplayNameID( ) ); //$NON-NLS-1$
	...
	public void testGetAllowedUnits( ) throws MetaDataParserException
	{
		...
		PropertyDefn propDefn = (ElementPropertyDefn) styleDefn
				.getProperty( Style.FONT_STYLE_PROP );
	...
	/**
	 * Test per-property validation. 1. A design file has an invalid property
	 * value.
	 * <p>
	...
	public void testValidator( ) throws DesignFileException, SemanticException
	{
		...
		catch ( SemanticException e1 )
		{
			...
			assertEquals(
					PropertyValueException.DESIGN_EXCEPTION_NEGATIVE_VALUE, e1
							.getErrorCode( ) );
		}
	...
	public void testParseRom( ) throws MetaDataParserException
	{
		...
		SystemPropertyDefn defn = (SystemPropertyDefn) ed
				.getProperty( SimpleDataSet.AFTER_CLOSE_METHOD );

		// the element reference type no choices.

		assertTrue( defn.isEditable( ) );

		ed = MetaDataDictionary.getInstance( ).getElement(
				ReportDesignConstants.STYLE_ELEMENT );
	...
	public void testSemanticValidator( ) throws MetaDataParserException
	{
		...
		PropertyDefn typeDefn = (PropertyDefn) masterPageDefn
				.getProperty( MasterPage.TYPE_PROP );
