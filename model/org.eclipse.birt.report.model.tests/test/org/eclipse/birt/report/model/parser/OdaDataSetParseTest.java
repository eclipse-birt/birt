...
public class OdaDataSetParseTest extends BaseTestCase
{
	...
	private void parse( ) throws Exception
	{
		...
		OdaDataSourceHandle dataSource = (OdaDataSourceHandle) designHandle
				.findDataSource( "extendedDataSource" ); //$NON-NLS-1$
		...
		List parameters = (List) dataSet
				.getProperty( OdaDataSet.PARAMETERS_PROP );
		assertEquals( 5, parameters.size( ) );

		int i = 0;
		OdaDataSetParameter parameter = (OdaDataSetParameter) parameters
				.get( i++ );
		...
		assertEquals( DesignChoiceConstants.COLUMN_DATA_TYPE_INTEGER, parameter
				.getDataType( ) );
		assertTrue( parameter.isOptional( ) );
		...
		List columnHints = (List) dataSet
				.getProperty( SimpleDataSet.COLUMN_HINTS_PROP );
		assertEquals( 1, columnHints.size( ) );

		i = 0;
		ColumnHint columnHint = (ColumnHint) columnHints.get( i++ );
		PropertyDefn member = (PropertyDefn) columnHint.getDefn( ).getMember(
		...
		assertEquals(
				"message.column-hint.username", columnHint.getProperty( design, member ) ); //$NON-NLS-1$
		member = (PropertyDefn) columnHint.getDefn( ).getMember(
		...
		assertEquals(
				"message.column-hint.help", columnHint.getProperty( design, member ) ); //$NON-NLS-1$
		member = (PropertyDefn) columnHint.getDefn( ).getMember(
		...
		ArrayList filters = (ArrayList) dataSet
				.getProperty( SimpleDataSet.FILTER_PROP );

		assertEquals( 1, filters.size( ) );
		assertEquals(
				"lt", ( (FilterCondition) filters.get( 0 ) ).getOperator( ) ); //$NON-NLS-1$
		assertEquals(
				"filter expression", ( (FilterCondition) filters.get( 0 ) ).getExpr( ) ); //$NON-NLS-1$
		assertEquals(
				"value1 expression", ( (FilterCondition) filters.get( 0 ) ).getValue1( ) ); //$NON-NLS-1$
		assertEquals(
				"value2 expression", ( (FilterCondition) filters.get( 0 ) ).getValue2( ) ); //$NON-NLS-1$
	...
	private void write( ) throws Exception
	{
		...
		PropertyDefn posDefn = (PropertyDefn) dataSet.getElement( )
				.getPropertyDefn( SimpleDataSet.PARAMETERS_PROP )
				.getStructDefn( ).getMember( DataSetParameter.POSITION_MEMBER );
		PropertyDefn nameDefn = (PropertyDefn) dataSet.getElement( )
				.getPropertyDefn( SimpleDataSet.PARAMETERS_PROP )
				.getStructDefn( ).getMember( DataSetParameter.NAME_MEMBER );
		PropertyDefn dataTypeDefn = (PropertyDefn) dataSet.getElement( )
				.getPropertyDefn( SimpleDataSet.PARAMETERS_PROP )
		...
		nameDefn = (PropertyDefn) dataSet.getElement( ).getPropertyDefn(
				SimpleDataSet.PARAM_BINDINGS_PROP ).getStructDefn( ).getMember(
				ParamBinding.PARAM_NAME_MEMBER );
		PropertyDefn valueDefn = (PropertyDefn) dataSet.getElement( )
				.getPropertyDefn( SimpleDataSet.PARAM_BINDINGS_PROP )
		...
		posDefn = (PropertyDefn) dataSet.getElement( ).getPropertyDefn(
				SimpleDataSet.RESULT_SET_PROP ).getStructDefn( ).getMember(
				ResultSetColumn.POSITION_MEMBER );
		nameDefn = (PropertyDefn) dataSet.getElement( ).getPropertyDefn(
				SimpleDataSet.RESULT_SET_PROP ).getStructDefn( ).getMember(
				ResultSetColumn.NAME_MEMBER );
		dataTypeDefn = (PropertyDefn) dataSet.getElement( ).getPropertyDefn(
				SimpleDataSet.RESULT_SET_PROP ).getStructDefn( ).getMember(
		...
		nameDefn = (PropertyDefn) dataSet.getElement( ).getPropertyDefn(
				SimpleDataSet.COMPUTED_COLUMNS_PROP ).getStructDefn( )
				.getMember( ComputedColumn.NAME_MEMBER );
		PropertyDefn expressionDefn = (PropertyDefn) dataSet.getElement( )
				.getPropertyDefn( SimpleDataSet.COMPUTED_COLUMNS_PROP )
				.getStructDefn( ).getMember( ComputedColumn.EXPRESSION_MEMBER );

		List columns = (List) dataSet.getElement( ).getProperty( design,
				SimpleDataSet.COMPUTED_COLUMNS_PROP );
		assertEquals( 3, columns.size( ) );
		...
		computedColumn
				.setDataType( DesignChoiceConstants.COLUMN_DATA_TYPE_DECIMAL );
		...
		StructureDefn structureDefn = (StructureDefn) dataSet.getElement( )
				.getPropertyDefn( SimpleDataSet.COLUMN_HINTS_PROP )
				.getStructDefn( );
		columnHint.setProperty( (PropertyDefn) structureDefn
				.getMember( ColumnHint.COLUMN_NAME_MEMBER ), "new username" ); //$NON-NLS-1$
		columnHint.setProperty( (PropertyDefn) structureDefn
				.getMember( ColumnHint.ALIAS_MEMBER ), "new userid" ); //$NON-NLS-1$
		columnHint.setProperty( (PropertyDefn) structureDefn
				.getMember( ColumnHint.SEARCHING_MEMBER ),
				DesignChoiceConstants.SEARCH_TYPE_INDEXED );
		columnHint.setProperty( (PropertyDefn) structureDefn
				.getMember( ColumnHint.EXPORT_MEMBER ),
				DesignChoiceConstants.EXPORT_TYPE_IF_REALIZED );
		columnHint.setProperty( (PropertyDefn) structureDefn
				.getMember( ColumnHint.ANALYSIS_MEMBER ),
				DesignChoiceConstants.ANALYSIS_TYPE_DETAIL );
		columnHint.setProperty( (PropertyDefn) structureDefn
				.getMember( ColumnHint.PARENT_LEVEL_MEMBER ), "new level" ); //$NON-NLS-1$
		columnHint.setProperty( (PropertyDefn) structureDefn
				.getMember( ColumnHint.FORMAT_MEMBER ), "new format" ); //$NON-NLS-1$
		columnHint.setProperty( (PropertyDefn) structureDefn
				.getMember( ColumnHint.DISPLAY_NAME_ID_MEMBER ),
				"new display name id" ); //$NON-NLS-1$
		columnHint.setProperty( (PropertyDefn) structureDefn
				.getMember( ColumnHint.DISPLAY_NAME_MEMBER ),
				"new display name" ); //$NON-NLS-1$
		columnHint.setProperty( (PropertyDefn) structureDefn
				.getMember( ColumnHint.HELP_TEXT_ID_MEMBER ),
				"new help text id" ); //$NON-NLS-1$
		columnHint.setProperty( (PropertyDefn) structureDefn
				.getMember( ColumnHint.HELP_TEXT_MEMBER ), "new help text" ); //$NON-NLS-1$
	}
	...
	public void testWriteEmptyQueryText( ) throws Exception
	{
		...
		OdaDataSetHandle odaHandle = (OdaDataSetHandle) designHandle
				.findDataSet( "myDataSet2" ); //$NON-NLS-1$
	...
	public void testProperties( ) throws Exception
	{
		...
		OdaDataSetHandle emptyDsHandle = (OdaDataSetHandle) designHandle
		...
		List resultSetProps = dataSet
				.getListProperty( IDataSetModel.RESULT_SET_PROP );
		...
		ComputedColumnHandle computedColumn = (ComputedColumnHandle) columns
				.next( );
		assertEquals( "column1", computedColumn.getName( ) ); //$NON-NLS-1$
		assertEquals( "expression1", computedColumn.getExpression( ) ); //$NON-NLS-1$
		assertEquals( DesignChoiceConstants.COLUMN_DATA_TYPE_INTEGER,
		...
		assertEquals( 2, dataSet.getListProperty( OdaDataSetHandle.FILTER_PROP )
				.size( ) );

		dataSet.removeFilter( fc );
		assertEquals( 1, dataSet.getListProperty( OdaDataSetHandle.FILTER_PROP )
				.size( ) );

		// writes properties to get a new file.

		OdaDataSource dataSource2 = (OdaDataSource) design
				.findDataSource( "myDataSource2" ); //$NON-NLS-1$
	...
	public void testDataSetParameterSynchronization( ) throws Exception
	{
		...
		DataSetHandle dataSetHandle = designHandle.getElementFactory( )
				.newOdaDataSet( "DataSet" ); //$NON-NLS-1$
		...
		PropertyHandle parameterHandle = dataSetHandle
		...
		PropertyHandle dataSetBindingHandle = dataSetHandle
				.getPropertyHandle( DataSetHandle.PARAM_BINDINGS_PROP );
		dataSetBindingHandle.addItem( dataSetBinding );

		// Add one table to design

		TableHandle tableHandle = designHandle.getElementFactory( )
				.newTableItem( "Table" ); //$NON-NLS-1$
		...
		PropertyHandle tableBindingHandle = tableHandle
				.getPropertyHandle( TableHandle.PARAM_BINDINGS_PROP );
		tableBindingHandle.addItem( tableBinding );

		// Add one image to design

		ImageHandle imageHandle = designHandle.getElementFactory( ).newImage(
				"Image" ); //$NON-NLS-1$
		...
		parameterHandle.getAt( 0 ).setProperty( DataSetParameter.NAME_MEMBER,
				"param2" ); //$NON-NLS-1$
