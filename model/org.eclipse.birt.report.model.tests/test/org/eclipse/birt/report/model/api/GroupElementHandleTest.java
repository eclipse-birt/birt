...
public class GroupElementHandleTest extends BaseTestCase
{
	 ...
	 * definitions(P1,P2,P3,P4,P5) and 2 user property definitions(UP1,UP2).
	 * Apply property on elEments as:
	 * <p>
	 * 1.
	 * <ul>
	...
	public void testGetCommonProperties( ) throws MetaDataException
	{
		...
		DesignElementHandle handle1 = new FakeElementHandle( design,
				new MockupDesignElement( elemDefn1 ) );
		DesignElementHandle handle2 = new FakeElementHandle( design,
				new MockupDesignElement( elemDefn2 ) );
		...
		assertEquals( elements.size( ), groupElementHandle.getElements( )
				.size( ) );

		List commProperties = groupElementHandle.getCommonProperties( );
		assertEquals( 1, commProperties.size( ) );
		assertEquals(
				"prop3", ( (PropertyDefn) commProperties.get( 0 ) ).getName( ) ); //$NON-NLS-1$
		...
		handle1 = new FakeElementHandle( design, new MockupDesignElement(
				elemDefn1 ) );
	...
	public void testGetCommonPropertiesWithSlotHandle( )
			throws MetaDataException
	{
		...
		elements1
				.add( new SlotHandle( formHandle, FreeForm.REPORT_ITEMS_SLOT ) );

		GroupElementHandle groupElementHandle1 = new SimpleGroupElementHandle(
				designHandle, elements1 );
	...
	public void testIsSameType( )
	{
		...
		GroupElementHandle groupElementHandle2 = new SimpleGroupElementHandle(
				designHandle, elements2 );
		...
		GroupElementHandle groupElementHandle3 = new SimpleGroupElementHandle(
				designHandle, elements3 );
	...
	public void testSetValue( ) throws SemanticException,
			UserPropertyException, PropertyValueException, MetaDataException
	{
		// 1. set a property defined in rom.def.

		// Two kind of master page share property "Height"
		DesignElementHandle handle1 = elemFactory
		...
		groupElementHandle.setStringProperty( MasterPage.TYPE_PROP,
				DesignChoiceConstants.PAGE_SIZE_CUSTOM );
		groupElementHandle.setStringProperty( MasterPage.HEIGHT_PROP, "12pt" ); //$NON-NLS-1$

		assertEquals(
				"12pt", handle1.getStringProperty( MasterPage.HEIGHT_PROP ) ); //$NON-NLS-1$
		assertEquals(
				"12pt", handle2.getStringProperty( MasterPage.HEIGHT_PROP ) ); //$NON-NLS-1$
		...
		UserPropertyDefn up1 = new UserPropertyDefn( );
		up1.setName( "UP1" ); //$NON-NLS-1$
		up1.setType( MetaDataDictionary.getInstance( ).getPropertyType(
		...
		// "height" is shared
		GroupPropertyHandle propHandle = groupElementHandle
	...
	public void testIsExtendedElements( ) throws ContentException,
			NameException, ExtendsException
	{
		ArrayList elements = new ArrayList( );
	...
	public void testShareSameValue( ) throws SemanticException
	{
		...
		assertEquals(
				"bookmark1", groupElementHandle.getStringProperty( Label.BOOKMARK_PROP ) ); //$NON-NLS-1$

		label1.setBookmark( null );
		label2.setBookmark( null );

		assertTrue( groupElementHandle.shareSameValue( Label.BOOKMARK_PROP ) );
		assertEquals( null, groupElementHandle
				.getStringProperty( Label.BOOKMARK_PROP ) );
	...
	public void testGetStringValue( ) throws SemanticException
	{
		...
		assertEquals(
				"Hi, Rock", groupElementHandle.getStringProperty( DataSource.COMMENTS_PROP ) ); //$NON-NLS-1$

		// different value.

		element2.setProperty( DataSource.COMMENTS_PROP, "Hi, Ivy" ); //$NON-NLS-1$
		assertEquals( null, groupElementHandle
				.getStringProperty( DataSource.COMMENTS_PROP ) );
		...
		groupElementHandle = new SimpleGroupElementHandle( designHandle,
				elements );
		assertEquals(
				"123.123mm", groupElementHandle.getStringProperty( Label.X_PROP ) ); //$NON-NLS-1$
