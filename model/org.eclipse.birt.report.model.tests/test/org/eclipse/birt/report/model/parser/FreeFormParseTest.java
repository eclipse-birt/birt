...
public class FreeFormParseTest extends ParserTestCase
{
	...
	/*
	 * <free-form> <comments>Sample Section </comments> <display-name>The
	 * Section </display-name> <toc>"This Section" </toc> <style name="My
	 ...
	 * <free-form name="freeForm2"/> <data distinct="repeat"
	 * distinct-reset="foo"> <value-expr>[price] * [quan] </value-expr>
	 * <help-text>This is a data item. </help-text> </data> <text>
	 * <value-expr>[foo.bar] </value-expr> </text> <grid name="grid"/>
	 * 
	...
	public void testParser( ) throws Exception
	{
		...
		assertEquals (
				"Sample Section", freeFormHandle.getStringProperty ( FreeForm.COMMENTS_PROP ) ); //$NON-NLS-1$
		assertEquals (
				"The Section", freeFormHandle.getStringProperty ( FreeForm.DISPLAY_NAME_PROP ) ); //$NON-NLS-1$
		...
		assertEquals (
				"Inner freeform", ( ( FreeFormHandle ) freeForm1 ).getStringProperty ( FreeForm.COMMENTS_PROP ) ); //$NON-NLS-1$
		assertEquals ( 2, ( ( FreeFormHandle ) freeForm1 ).getSlot ( 0 )
				.getCount ( ) );

		//retrieve the 3d leve freeform---level3FreeForm
		
		FreeFormHandle level3FreeForm = ( FreeFormHandle ) ( ( FreeFormHandle ) freeForm1 )
				.getSlot ( 0 ).get ( 1 );
	...
	public void testWritter( ) throws Exception
	{
		...
		freeFormHandle.setStringProperty ( FreeForm.COMMENTS_PROP,
				"new comment" ); //$NON-NLS-1$
		assertEquals (
	...
	public void testSemanticCheck( ) 
	{
		...
		catch ( DesignFileException e )
		{
			...
			assertEquals( NameException.DESIGN_EXCEPTION_DUPLICATE, ((ErrorDetail) e.getErrorList( ).get( i ++ )).getErrorCode( ) );
			assertEquals( XMLParserException.DESIGN_EXCEPTION_UNKNOWN_TAG, ((ErrorDetail) e.getErrorList( ).get( i ++ )).getErrorCode( ) );
			}
