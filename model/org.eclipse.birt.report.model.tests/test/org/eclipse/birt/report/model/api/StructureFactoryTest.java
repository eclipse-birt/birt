...
public class StructureFactoryTest extends BaseTestCase
{
	...
	public void testLiteralString( ) throws SemanticException
	{
		...
		OdaDataSetParameter setParameter = StructureFactory
		...
		OdaDataSetParameterHandle paramHandle = (OdaDataSetParameterHandle) dataSet
				.getPropertyHandle( OdaDataSetHandle.PARAMETERS_PROP ).addItem(
						setParameter );
	...
	public void testNewStructure( )
	{
		...
		ResultSetColumn resultSetColumn = StructureFactory
		...
		SelectionChoice selectionChoice = StructureFactory
	...
	{
		TextItemHandle textHandle = designHandle.getElementFactory( )
				.newTextItem( "text1" ); //$NON-NLS-1$

		designHandle.getBody( ).add( textHandle );

		ComputedColumn column1 = StructureFactory.newComputedColumn(
				textHandle, "new_column" ); //$NON-NLS-1$
