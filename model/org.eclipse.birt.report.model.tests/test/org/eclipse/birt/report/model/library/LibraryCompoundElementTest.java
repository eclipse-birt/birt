...
public class LibraryCompoundElementTest extends BaseTestCase
{
	...
	public void testParserForElementProperty( ) throws Exception
	{
		...
		TabularCubeHandle cube = (TabularCubeHandle) designHandle
				.findCube( "designCube" ); //$NON-NLS-1$
		assertEquals(
				lib.findDataSet( "firstDataSet" ), cube.getDataSet( ).getElement( ) ); //$NON-NLS-1$
		// filter
		...
		assertEquals(
				lib.findDataSet( "secondDataSet" ), hierarchy.getDataSet( ).getElement( ) ); //$NON-NLS-1$
