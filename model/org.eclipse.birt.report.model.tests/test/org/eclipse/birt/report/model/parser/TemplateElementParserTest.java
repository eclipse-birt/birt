...
public class TemplateElementParserTest extends BaseTestCase
{
	 ...
	 * <p>
	 * <ul>
	 * <li>case 1 -- get the template parameter definition by name from the
	 * module handle.
	 ...
	 * <li>case 4 -- the default elements in the template definitions are not
	 * put into the namespace
	 * <li>case 5 -- get the template data set, template report item from the
	 * module and the data set namespace or element namespace
	 * <li>case 6 -- all the properties of the template report items and
	 * template data sets are right
	 * <li>case 7 -- check the reference between template definitions and
	 * template elements
	 * </ul>
	 * 
	 * 
	...
	public void testParser( ) throws Exception
	{
		...
		assertEquals( templateDataSetParam.getElement( ), ns
				.getElement( "templateDataSetParam" ) ); //$NON-NLS-1$

		// get the properties for the template definitions

		assertEquals( "templateLabelParam", templateLabelParam.getName( ) );//$NON-NLS-1$

		assertEquals( DesignChoiceConstants.TEMPLATE_ELEMENT_TYPE_LABEL,
				templateLabelParam.getAllowedType( ) );
		...
		LabelHandle defaultLabel = (LabelHandle) templateLabelParam
				.getDefaultElement( );
		...
		assertEquals( DesignChoiceConstants.TEMPLATE_ELEMENT_TYPE_DATA_SET,
				templateDataSetParam.getAllowedType( ) );
		...
		ScriptDataSetHandle defaultDataSet = (ScriptDataSetHandle) templateDataSetParam
				.getDefaultElement( );
		...
		assertFalse( defaultLabel == designHandle.findElement( defaultLabel
				.getName( ) ) );
		assertNotNull( designHandle.findElement( defaultLabel.getName( ) ) );
		assertFalse( defaultDataSet == designHandle.findDataSet( defaultDataSet
				.getName( ) ) );
		...
		TemplateDataSetHandle templateDataSet = designHandle
				.findTemplateDataSet( "templateDataSet" ); //$NON-NLS-1$
		...
		ElementRefValue refTemplateParam = (ElementRefValue) templateDataSet
				.getElement( ).getProperty( design,
						TemplateDataSetHandle.REF_TEMPLATE_PARAMETER_PROP );
		assertNotNull( refTemplateParam );
		assertEquals( templateDataSetParam.getElement( ), refTemplateParam
				.getElement( ) );
		assertEquals(
				templateDataSetParam.getName( ),
				templateDataSet
						.getProperty( TemplateDataSetHandle.REF_TEMPLATE_PARAMETER_PROP ) );
		assertEquals( DesignChoiceConstants.TEMPLATE_ELEMENT_TYPE_DATA_SET,
				templateDataSet.getAllowedType( ) );
		...
		assertEquals( "all", ( (StructureHandle) templateLabel //$NON-NLS-1$
				.visibilityRulesIterator( ).next( ) ).getMember(
				HideRule.FORMAT_MEMBER ).getStringValue( ) );
		assertEquals( "1+1=3", ( (StructureHandle) templateLabel //$NON-NLS-1$
				.visibilityRulesIterator( ).next( ) ).getMember(
				HideRule.VALUE_EXPR_MEMBER ).getStringValue( ) );
		...
		refTemplateParam = (ElementRefValue) templateLabel.getElement( )
				.getProperty( design,
						TemplateDataSetHandle.REF_TEMPLATE_PARAMETER_PROP );
		assertNotNull( refTemplateParam );
		assertEquals( templateLabelParam.getElement( ), refTemplateParam
				.getElement( ) );
		assertEquals(
				templateLabelParam.getName( ),
				templateLabel
						.getProperty( TemplateDataSetHandle.REF_TEMPLATE_PARAMETER_PROP ) );
		assertEquals( DesignChoiceConstants.TEMPLATE_ELEMENT_TYPE_LABEL,
				templateLabel.getAllowedType( ) );
		...
		TemplateParameterDefinition templateParam = (TemplateParameterDefinition) templateDataSetParam
				.getElement( );
		assertEquals( 1, templateParam.getClientList( ).size( ) );
		assertEquals( templateDataSet.getElement( ), ( (BackRef) templateParam
				.getClientList( ).get( 0 ) ).getElement( ) );

		templateParam = (TemplateParameterDefinition) templateLabelParam
				.getElement( );
		assertEquals( 1, templateParam.getClientList( ).size( ) );
		assertEquals( templateLabel.getElement( ), ( (BackRef) templateParam
				.getClientList( ).get( 0 ) ).getElement( ) );
	}
	...
	/**
	 * Tests the design file parser exception if the template elements refer a
	 * wrong type template definition. Test the semantic errors.
	 * 
	 * @throws Exception
	 */
	...
	public void testErrors( ) throws Exception
	{
		...
		catch ( DesignFileException e )
		{
			...
			XMLParserException exception = (XMLParserException) e
					.getExceptionList( ).get( 0 );
			assertTrue( exception.getException( ) instanceof DesignParserException );
			DesignParserException cause = (DesignParserException) exception
					.getException( );
			assertEquals(
	...
	public void testCreateTemplate( ) throws Exception
	{
		...
		stack.undo( );
		templateLabel = (TemplateReportItemHandle) designHandle
				.findElement( name );
		...
		templateLabel = (TemplateReportItemHandle) labelHandle
				.createTemplateElement( labelHandle.getName( ) );
		assertEquals( templateLabel, designHandle.findElement( labelHandle
				.getName( ) ) );
		assertNotNull( templateLabel );
		assertEquals( NotificationEvent.TEMPLATE_TRANSFORM_EVENT,
				listener.event.getEventType( ) );
		...
		DataSetHandle dataSetHandle = designHandle.getElementFactory( )
		...
		try
		{
			templateDataSet = (TemplateDataSetHandle) dataSetHandle
					.createTemplateElement( name );
			fail( );
		}
		...
		templateDataSet = (TemplateDataSetHandle) dataSetHandle
				.createTemplateElement( name );
		assertNotNull( templateDataSet );
		assertEquals( templateDataSet.getElement( ), designHandle
				.findTemplateDataSet( "templateDataSet1" ) ); //$NON-NLS-1$
	...
	public void testTransformTemplate( ) throws Exception
	{
		...
		assertEquals( dataSet.getElement( ), designHandle.findDataSet( dataSet
				.getName( ) ) );
		assertNull( designHandle
				.findTemplateDataSet( templateDataSet.getName( ) ) );
		...
		assertEquals( templateDataSet, designHandle
				.findTemplateDataSet( templateDataSet.getName( ) ) );
		...
		try
		{
			templateDataSet.setProperty(
					TemplateDataSetHandle.REF_TEMPLATE_PARAMETER_PROP, null );
			templateDataSet.transformToDataSet( dataSet );
			fail( );
		}
		catch ( SemanticException e )
		...
		assertEquals( label.getElement( ), designHandle.findElement( label
				.getName( ) ) );
	...
	/**
	 * Tests a report item or data set based on template definition to be
	 * reverted to a template element.
	 * 
	 * @throws Exception
	 */
	...
	public void testRevertToTemplate( ) throws Exception
	{
		...
		assertEquals(
				"templateDataSetParam", dataSet.getElement( ).getTemplateParameterElement( design ).getName( ) ); //$NON-NLS-1$
	...
	{
		designHandle = ( new DesignEngine( new DesignConfig( ) ) )
				.newSessionHandle( (ULocale) null ).createDesign( );
		LabelHandle labelHandle = designHandle.getElementFactory( ).newLabel(
				"label" ); //$NON-NLS-1$
		assertNotNull( labelHandle );

		ContainerSlot templateParams = designHandle.getElement( ).getSlot(
		...
		TemplateElementHandle templateLabel = labelHandle
	...
	public void testCheckAdd( ) throws Exception
	{
		...
		label.setProperty( IDesignElementModel.REF_TEMPLATE_PARAMETER_PROP,
				"templateLabelParam" ); //$NON-NLS-1$
		designHandle.getBody( ).add( label );
		assertEquals( designHandle, label.getContainer( ) );
		assertEquals(
				"templateLabelParam", label.getProperty( IDesignElementModel.REF_TEMPLATE_PARAMETER_PROP ) ); //$NON-NLS-1$
		...
		TableHandle table = designHandle.getElementFactory( ).newTableItem(
				null );
		table.setProperty( IDesignElementModel.REF_TEMPLATE_PARAMETER_PROP,
				"templateLabelParam" ); //$NON-NLS-1$
		designHandle.getBody( ).add( table );
		assertEquals( designHandle, table.getContainer( ) );
		assertNull( table
				.getProperty( IDesignElementModel.REF_TEMPLATE_PARAMETER_PROP ) );

		// add a label which defines a template param can not be found

		label = designHandle.getElementFactory( ).newLabel( null );
		label.setProperty( IDesignElementModel.REF_TEMPLATE_PARAMETER_PROP,
				"NonexisttemplateLabelParam" ); //$NON-NLS-1$
		designHandle.getBody( ).add( label );
		assertEquals( designHandle, label.getContainer( ) );
		assertNull( label
				.getProperty( IDesignElementModel.REF_TEMPLATE_PARAMETER_PROP ) );
	...
	/**
	 * Tests canTransformToTemplate(). Now support template for data set and
	 * part of report items. Auto text does not support template.
	 * 
	 * @throws Exception
	 */
	...
	public void testCanTransformToTemplateForReportItem( ) throws Exception
	{
		...
		AutoTextHandle autoText = designHandle.getElementFactory( )
				.newAutoText( "test" ); //$NON-NLS-1$
