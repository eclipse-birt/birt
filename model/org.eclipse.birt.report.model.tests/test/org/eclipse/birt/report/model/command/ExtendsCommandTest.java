...
public class ExtendsCommandTest extends BaseTestCase {
	...
	protected void setUp() throws Exception {
		...
		NameSpace ns = design.getNameHelper().getNameSpace(((ElementDefn) element.getDefn()).getNameSpaceID());
		ns.insert(element);
	...
	public void testSetExtendsName() throws ExtendsException {
		...
		/*
		 * tests the normal case from DesignElementHandle.setExtendsName with undo/redo.
		 */
	...
	public void testSetExtendsExceptions() {
		...
		try {
			...
			assertEquals(e.getErrorCode(), InvalidParentException.DESIGN_EXCEPTION_PARENT_NOT_FOUND);
		}
		...
		try {
			...
			assertEquals(e1.getErrorCode(), WrongTypeException.DESIGN_EXCEPTION_WRONG_TYPE);
		}
		...
		try {
			...
			assertEquals(e2.getErrorCode(), CircularExtendsException.DESIGN_EXCEPTION_SELF_EXTEND);
		}
		...
		try {
			...
			assertEquals(e.getErrorCode(), ExtendsForbiddenException.DESIGN_EXCEPTION_CANT_EXTEND);
		}
	...
	public void testCirclarExtends() {
		...
		try {
			...
			assertEquals(e1.getErrorCode(), CircularExtendsException.DESIGN_EXCEPTION_CIRCULAR);
		}
	...
	public void testSetExtendsElement() throws ExtendsException {
		...
		try {
			...
			assertEquals(e.getErrorCode(), ExtendsException.DESIGN_EXCEPTION_UNNAMED_PARENT);
		}
		...
		LabelHandle label = (LabelHandle) ((CellHandle) ((RowHandle) grid1.getRows().get(0)).getCells().get(0))
				.getContent().get(0);
		try {
			assertTrue(label.getElement().isVirtualElement());
			label.setExtends(parent.getHandle(design));
		} catch (ExtendsException e) {
			assertEquals(ExtendsForbiddenException.DESIGN_EXCEPTION_EXTENDS_FORBIDDEN, e.getErrorCode());
		}
	...
	public void testLocalizeElement() throws DesignFileException, IOException, SemanticException {
		...
		LabelHandle label1Handle = (LabelHandle) designHandle.findElement("Label1"); //$NON-NLS-1$
		...
		assertEquals("8pt", label1.getLocalProperty(module, Label.WIDTH_PROP).toString()); //$NON-NLS-1$
		...
		assertEquals("left", label1.getStringProperty(module, Style.TEXT_ALIGN_PROP)); //$NON-NLS-1$
		...
		// Copied from parent.
		assertEquals("First Page", label1.getLocalProperty(module, Label.TEXT_PROP)); //$NON-NLS-1$

		// Copied from style3. "BackGroudColor" and "BorderLeftColor" should also
		// extends from parent.

		assertEquals("x-small", label1.getLocalProperty(module, Style.FONT_SIZE_PROP).toString()); //$NON-NLS-1$
		assertEquals("black", label1.getLocalProperty(module, Style.BORDER_LEFT_COLOR_PROP));
		assertEquals("red", label1.getLocalProperty(module, Style.BACKGROUND_COLOR_PROP));

		// From selector is not copied.

		assertEquals("left", label1.getLocalProperty(module, Style.TEXT_ALIGN_PROP)); //$NON-NLS-1$
		...
		assertEquals("24pt", grid1.getLocalProperty(module, GridItem.HEIGHT_PROP).toString()); //$NON-NLS-1$
		assertEquals("40mm", grid1.getProperty(module, GridItem.WIDTH_PROP).toString()); //$NON-NLS-1$
		...
		assertEquals("New Address", innerLabel1.getElement().getLocalProperty(module, Label.TEXT_PROP).toString()); //$NON-NLS-1$
		...
		assertEquals("24pt", grid1.getLocalProperty(module, GridItem.HEIGHT_PROP).toString()); //$NON-NLS-1$
		assertEquals("40mm", grid1.getLocalProperty(module, GridItem.WIDTH_PROP).toString()); //$NON-NLS-1$
