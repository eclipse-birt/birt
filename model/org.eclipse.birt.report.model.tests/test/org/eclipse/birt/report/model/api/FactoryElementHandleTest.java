...
public class FactoryElementHandleTest extends BaseTestCase
{
	...
	public void testGetAllFactoryStyles( ) throws Exception
	{
		...
		List<StyleHandle> factoryStyles = null;
		StyleHandle styleHandle = null;

		// simple free-form, only default selector returned
		DesignElementHandle freeForm = designHandle
		...
		styleHandle = factoryStyles.get( 0 );
		assertEquals( IColorConstants.RED, styleHandle.getColor( )
				.getStringValue( ) );
		...
		DesignElementHandle extendedItem = designHandle
				.findElement( "testMatrix_1" ); //$NON-NLS-1$
		factoryStyles = extendedItem.getFactoryElementHandle( )
				.getAllFactoryStyles( );
		assertEquals( 1, factoryStyles.size( ) );
		styleHandle = factoryStyles.get( 0 );
		assertEquals( ( (ElementDefn) extendedItem.getDefn( ) ).getSelector( ),
				styleHandle.getName( ) );
		...
		assertTrue( cell.getElement( ).getElementSelectors( ).contains(
				styleHandle.getName( ) ) );
		// the second is the related container style, that is column style
		styleHandle = factoryStyles.get( 1 );
		DesignElementHandle column = designHandle.getElementByID( 20 );
		assertEquals( column.getStringProperty( IStyleModel.FONT_WEIGHT_PROP ),
				styleHandle.getFontWeight( ) );
		assertEquals( column.getStringProperty( IStyleModel.FONT_STYLE_PROP ),
				styleHandle.getFontStyle( ) );
		assertEquals(
				column.getStringProperty( IStyleModel.FONT_VARIANT_PROP ),
