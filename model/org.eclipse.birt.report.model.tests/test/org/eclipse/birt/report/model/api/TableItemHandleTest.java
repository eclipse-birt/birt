...
public class TableItemHandleTest extends BaseTestCase {
	...
	public void testRowCopyPasteAction() throws Exception {
		...
		TableHandle table1 = (TableHandle) designHandle.findElement("My table1"); //$NON-NLS-1$
		TableHandle table2 = (TableHandle) designHandle.findElement("My table2");//$NON-NLS-1$
		TableHandle table3 = (TableHandle) designHandle.findElement("My table3");//$NON-NLS-1$
		...
		assertEquals("PHONE", ((DataItem) obj).getLocalProperty(null, "resultSetColumn")); //$NON-NLS-1$//$NON-NLS-2$
	...
	public void testRowCopyPasteInNestedTable() throws Exception {
		...
		RowOperationParameters parameters1 = new RowOperationParameters(TableItem.HEADER_SLOT, -1, 0);
		RowOperationParameters parameters2 = new RowOperationParameters(TableItem.DETAIL_SLOT, -1, 0);
	...
	public void testSetPageBreakOnRow() throws SemanticException {
		...
		assertNotNull(tableRow.getDefn().getProperty(IStyleModel.PAGE_BREAK_AFTER_PROP));
		assertNotNull(tableRow.getDefn().getProperty(IStyleModel.PAGE_BREAK_BEFORE_PROP));
		assertNotNull(tableRow.getDefn().getProperty(IStyleModel.PAGE_BREAK_INSIDE_PROP));
	}
	...
	public void testSummaryTable() throws Exception {
		...
		TableHandle table = designHandle.getElementFactory().newTableItem("testTable"); //$NON-NLS-1$
		...
		assertFalse(table.canContain(IListingElementModel.DETAIL_SLOT, table.getElementFactory().newTableRow()));
		assertFalse(table.canContain(IListingElementModel.DETAIL_SLOT, ReportDesignConstants.ROW_ELEMENT));
	}
	...
	public void testTableWidthCalculation() throws Exception {
		...
		checkCalculationException("testTable7", //$NON-NLS-1$
				SemanticError.DESIGN_EXCEPTION_TABLE_COLUMN_INCONSISTENT_RELATIVE_UNIT);
		checkCalculationException("testTable8", //$NON-NLS-1$
