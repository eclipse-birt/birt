...
public class UserPropertyCommandTest extends BaseTestCase
{
	 ...
	 * <ul>
	 * <li>create a user property.Set name, Type, Default value, choice for it.
	 * Add it into masterPage. Then set property value for it.
	 * </ul>
	 * 
	 * @throws Exception
	 */
	...
	public void testAddUserProperty( ) throws Exception
	{
		...
		UserPropertyDefnHandle newPropDefnHandle = dataHandle
				.getUserPropertyDefnHandle( "myProp4" ); //$NON-NLS-1$
		...
		UserPropertyDefn upd = (UserPropertyDefn) dataHandle.getPropertyHandle(
				"myProp1" ) //$NON-NLS-1$
				.getDefn( );
	...
	public void testDropUserProperty( ) throws Exception
	{
		...
		UserPropertyDefnHandle userPropHandle = dataHandle
				.getUserPropertyDefnHandle( "myProp2" ); //$NON-NLS-1$
		...
		{
			assertEquals( UserPropertyException.DESIGN_EXCEPTION_NOT_FOUND, e
					.getErrorCode( ) );
		}
		...
		DataItemHandle parentDataHandle = (DataItemHandle) designHandle
				.findElement( "base data" ); //$NON-NLS-1$
		DataItemHandle childDataHandle = (DataItemHandle) designHandle
				.findElement( "child data" ); //$NON-NLS-1$
		assertNotNull( parentDataHandle
				.getUserPropertyDefnHandle( "stringProp" ) ); //$NON-NLS-1$
	...
	public void testSetUserPropertyDefn( ) throws Exception
	{
		...
		assertNotNull( dataHandle );
		UserPropertyDefn old = (UserPropertyDefn) dataHandle.getPropertyHandle(
				"myProp1" ).getDefn( ); //$NON-NLS-1$

		UserPropertyCommand updCommand = new UserPropertyCommand( design,
				dataHandle.getElement( ) );
		...
		old = (UserPropertyDefn) dataHandle
				.getPropertyHandle( "myProp2" ).getDefn( ); //$NON-NLS-1$
		...
		assertEquals( new Double( value ).toString( ), new Double( dataHandle
				.getFloatProperty( "updNew" ) ).toString( ) ); //$NON-NLS-1$
		...
		{
			assertEquals( UserPropertyException.DESIGN_EXCEPTION_NAME_REQUIRED,
					e.getErrorCode( ) );
		}
	...
	public void testNotification( ) throws DesignFileException,
			SemanticException
	{
	...
	class MyUserPropertyListener implements Listener
	{
				...
				NotificationEvent ev )
		{
			if ( ev.getEventType( ) == NotificationEvent.USER_PROP_EVENT )
			{
				...
				if ( action == ADDED
						&& event.getAction( ) == UserPropertyEvent.DROP )
					action = CHANGED;
				else
				{
