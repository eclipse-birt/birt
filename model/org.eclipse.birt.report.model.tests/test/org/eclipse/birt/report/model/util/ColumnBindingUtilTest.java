...
public class ColumnBindingUtilTest extends BaseTestCase
{
	...
	public void testAddColumnBinding( ) throws Exception
	{
		...
		ComputedColumnHandle boundColumn = ColumnBindingUtil.addColumnBinding( table,
				column );
		...
		ComputedColumnHandle newBoundColumn = ColumnBindingUtil.addColumnBinding( table,
				copiedColumn );
		assertEquals( 1, propHandle.getItems( ).size( ) );
		...
		try
		{
			ColumnBindingUtil.addColumnBinding( table,
