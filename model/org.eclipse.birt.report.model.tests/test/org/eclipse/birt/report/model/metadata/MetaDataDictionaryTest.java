...
public class MetaDataDictionaryTest extends AbstractMetaTest
{
	...
	public void testChoiceSet( )
	{
		...
		assertNotNull( dict
				.getChoiceSet( DesignChoiceConstants.CHOICE_FONT_WEIGHT ) );
		assertNotNull( dict
		...
		assertEquals( extendedChoice.getDisplayNameKey( ),
				"Choices.fontSize.xx-small" ); //$NON-NLS-1$
		assertEquals( extendedChoice.getName( ),
				DesignChoiceConstants.FONT_SIZE_XX_SMALL );

		extendedChoice = (Choice) choices[1];
		assertEquals( extendedChoice.getDisplayNameKey( ),
				"Choices.fontSize.x-small" ); //$NON-NLS-1$
		assertEquals( extendedChoice.getName( ),
				DesignChoiceConstants.FONT_SIZE_X_SMALL );

		// tests extendedChoiceType about font.size in the element Style

		IElementDefn defn = dict
				.getElement( ReportDesignConstants.STYLE_ELEMENT );
		assertNotNull( defn );

		SystemPropertyDefn fontSizeDefn = (SystemPropertyDefn) defn
	...
	{
		IValueValidator positiveValidator = dict
				.getValueValidator( "PositiveValueValidator" ); //$NON-NLS-1$
