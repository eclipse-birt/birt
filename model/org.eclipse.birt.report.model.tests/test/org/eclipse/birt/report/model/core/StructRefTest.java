...
public class StructRefTest extends BaseTestCase
{
	...
	public void testStructReferenceBySet( ) throws Exception
	{
		...
		assertEquals(
				"Image1", ( (BackRef) clients.get( 0 ) ).getElement( ).getName( ) ); //$NON-NLS-1$
		assertEquals( IImageItemModel.IMAGE_NAME_PROP, ( (BackRef) clients
				.get( 0 ) ).getPropertyName( ) );
		assertEquals(
				"Image2", ( (BackRef) clients.get( 1 ) ).getElement( ).getName( ) ); //$NON-NLS-1$
		assertEquals( IImageItemModel.IMAGE_NAME_PROP, ( (BackRef) clients
				.get( 1 ) ).getPropertyName( ) );
		clients = emImageTwo.getClientList( );
		assertEquals( 1, clients.size( ) );
		assertEquals(
				"Image3", ( (BackRef) clients.get( 0 ) ).getElement( ).getName( ) ); //$NON-NLS-1$
		assertEquals( IImageItemModel.IMAGE_NAME_PROP, ( (BackRef) clients
				.get( 0 ) ).getPropertyName( ) );
		...
		StructRefValue value = (StructRefValue) imageHandle.getElement( )
				.getProperty( design, IImageItemModel.IMAGE_NAME_PROP );
	...
	public void testDeleteAndReplace( ) throws Exception
	{
		...
		assertEquals( imageHandle.getName( ) + MyListener.STATUS, listener
				.getNotification( ) );
		imageHandle.getElement( ).validateWithContents( design );
		assertTrue( imageHandle.getElement( ).getErrors( ).size( ) > 0 );
	}
	...
	public void testSetProperty( ) throws Exception
	{
		...
		emImageTwoHandle.getMember( EmbeddedImage.NAME_MEMBER ).setValue(
				"image new" ); //$NON-NLS-1$
	...
	{
		openDesign( fileName );
		FreeFormHandle element = designHandle.getElementFactory( ).newFreeForm(
				"test" ); //$NON-NLS-1$
		designHandle.getBody( ).add( element );
		PropertyHandle mapRules = element
		...
		assertEquals( DesignChoiceConstants.MAP_OPERATOR_EQ, ruleHandle
				.getOperator( ) );
		MyListener listener = new MyListener( );
		...
		assertEquals( element.getName( ) + MyListener.STATUS, listener
				.getNotification( ) );
	}
