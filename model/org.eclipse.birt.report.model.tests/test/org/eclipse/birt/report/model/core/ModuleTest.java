...
public class ModuleTest extends BaseTestCase
{
	...
	/**
	 * Tests the id of all the elements after parsing are right. Test the id map
	 * in the design is right.
	 * 
	 * @throws Exception
	 */
	...
	public void testIDWithParser( ) throws Exception
	{
		...
		FreeFormHandle parent = (FreeFormHandle) designHandle
		...
		FreeFormHandle child = (FreeFormHandle) designHandle
				.findElement( "child" ); //$NON-NLS-1$
		assertEquals( 5, child.getID( ) );
		LabelHandle labelTwo = (LabelHandle) designHandle
				.findElement( "labelTwo" ); //$NON-NLS-1$
	...
	public void testIDWithCommand( ) throws Exception
	{
		...
		table = designHandle.getElementFactory( ).newTableItem(
				"table", 0, 0, 1, 0 ); //$NON-NLS-1$
		assertNotNull( table );
