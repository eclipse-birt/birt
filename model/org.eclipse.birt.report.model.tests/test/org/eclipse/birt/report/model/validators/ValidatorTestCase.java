...
public abstract class ValidatorTestCase extends BaseTestCase
{
	class MyListener implements IValidationListener
	{
		...
		public void elementValidated( DesignElementHandle focus,
				ValidationEvent ev )
		{
		...
		/**
		 * Returns whether the error exists in the error pool. This method is
		 * used to check the errors caught by element validator.
		 * 
		 * @param element
		...
		protected boolean hasError( DesignElementHandle element,
				String validatorName, String errorCode )
		{
			return pool.hasErrorFromValidation( element.getElement( ),
					validatorName, errorCode );
		}

		/**
		 * Returns whether the error exists in the error pool. This method is
		 * used to check the errors caught by property validator.
		 * 
		 * @param element
		 *            the element to check
		 * @param validatorName
		 *            the validator name
		 * @param propName
		 *            the name of the property on which the validator is applied
		 * @param errorCode
		 *            the error code to check
		 * @return return true if the given error code is in pool.
		 */
		...
		protected boolean hasError( DesignElementHandle element,
				String validatorName, String propName, String errorCode )
		{
	...
	class ErrorPool
	{
		...
		void updateErrorStatus( ValidationEvent event )
		{
			for ( int i = errors.size( ) - 1; i >= 0; i-- )
			{
				...
				if ( event.getTarget( ) == detail.getElement( )
						&& detail.getValidationID( ).equalsIgnoreCase(
		...
		private boolean hasErrorFromValidation( DesignElement element,
				String validationID, String errorCode )
		{
