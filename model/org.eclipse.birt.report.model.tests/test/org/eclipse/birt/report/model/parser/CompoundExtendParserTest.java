...
public class CompoundExtendParserTest extends BaseTestCase
{
	...
	public void testParser( ) throws Exception
	{
		...
		TableHandle baseTable = (TableHandle) designHandle
				.findElement( "baseTable" ); //$NON-NLS-1$
		assertEquals( ColorPropertyType.GREEN, baseTable.getHeader( ).get( 0 )
				.getElement( ).getLocalProperty( design, Style.COLOR_PROP ) );

		TableHandle interTable = (TableHandle) designHandle
				.findElement( "innerTable" ); //$NON-NLS-1$
		assertNotNull( interTable );

		assertEquals( ColorPropertyType.YELLOW, interTable.getHeader( ).get( 0 )
				.getElement( ).getSlot( TableRow.CONTENT_SLOT ).getContent( 0 )
		...
		assertEquals( ColorPropertyType.BLUE, row.getLocalProperty( design,
				Style.COLOR_PROP ) );
	...
	public void testPropertySearch( ) throws DesignFileException
	{
		...
		LabelHandle labelHandle = (LabelHandle) cellHandle1.getContent( ).get(
				0 );
		LabelHandle labelHandle1 = (LabelHandle) cellHandle1.getContent( ).get(
				1 );
		...
		assertEquals(
				"red", labelHandle.getStringProperty( StyleHandle.COLOR_PROP ) ); //$NON-NLS-1$
		assertEquals(
				"12mm", labelHandle.getStringProperty( StyleHandle.FONT_SIZE_PROP ) ); //$NON-NLS-1$

		DataItemHandle dataHandle = (DataItemHandle) cellHandle1.getContent( )
				.get( 2 );
	 ...
	 * Test cases:
	 * <p>
	 * 1. Ensure that if an element has parent, its contents(slot) will be
	 * ignored during parsing, a semantic warning is logged.
	 * <p>
	 ...
	 * structure is not allowed to be overridden. Properties include:
	 * cell.colSpan; cell.rowSpan; cell.drop; cell.column
	 */
	...
	public void testWriter( ) throws Exception
	{
		...
		LabelHandle label1 = (LabelHandle) ( (CellHandle) ( (RowHandle) grid1
