...
public class OdaElementExtensionTest extends BaseTestCase
{
	...
	public void testParser( ) throws DesignFileException
	{
		...
		assertEquals(
				"org.eclipse.birt.report.data.oda.jdbc", dataSourceHandle.getExtensionID( ) );//$NON-NLS-1$
		assertEquals( "User", dataSourceHandle.getStringProperty( "odaUser" ) );//$NON-NLS-1$ //$NON-NLS-2$ 
		assertEquals(
				"DoNotKnow", dataSourceHandle.getStringProperty( "odaPassword" ) );//$NON-NLS-1$ //$NON-NLS-2$
		assertEquals( "URL", dataSourceHandle.getStringProperty( "odaURL" ) );//$NON-NLS-1$ //$NON-NLS-2$
		assertEquals(
				"DriverClass", dataSourceHandle.getStringProperty( "odaDriverClass" ) );//$NON-NLS-1$ //$NON-NLS-2$
	 ...
	 * should not be null.
	 * <li>to create an oda data source from an existed oda data source.
	...
	public void testNewElementFrom( ) throws Exception
	{
		...
		OdaDataSourceHandle extendsSource = (OdaDataSourceHandle) designHandle
				.getElementFactory( )
		...
		OdaDataSetHandle extendsSet = (OdaDataSetHandle) designHandle
				.getElementFactory( ).newElementFrom( dataSet, "dataSet1" ); //$NON-NLS-1$
	 ...
	 * 
	 * <ul>
	 * <li>dataSource1 has extendsion id, dataSource1 hasn't. Exception is
	 * thrown.
	...
	public void testSetExtends( ) throws Exception
	{
		...
		OdaDataSourceHandle dataSource1 = designHandle.getElementFactory( )
				.newOdaDataSource( "ds1", DATASOURCE_EXTENSION_ID ); //$NON-NLS-1$
		designHandle.getDataSources( ).add( dataSource1 );

		OdaDataSourceHandle dataSource2 = designHandle.getElementFactory( )
				.newOdaDataSource( "ds2", null ); //$NON-NLS-1$
		...
		dataSource2.getElement( ).setProperty(
