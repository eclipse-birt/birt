...
public class ReportDesignUserDefinedMessagesTest extends BaseTestCase
{
	 ...
	 * test getMessage().
	 * <p>
	 * 1. locale is ENGLISH, translated text is defined in translations, keyed
	 * by "en".
	 * <p>
	...
	public void testGetMessage( ) throws DesignFileException
	{
		...
		String titleID = design.getStringProperty( design,
				ReportDesign.TITLE_ID_PROP );
		...
		String descriptionID = design.getStringProperty( design,
				ReportDesign.DESCRIPTION_ID_PROP );
	...
	{
		openDesign( "ReportDesignUserDefinedMessagesTest.xml", ULocale.ENGLISH ); //$NON-NLS-1$
		String text = design.getMessage(
				"ResourceKey.testKey2", new ULocale( "en", "AU" ) ); //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
		assertEquals( "text en_AU", text ); //$NON-NLS-1$

		text = design.getMessage(
				"ResourceKey.testKey2", new ULocale( "en", "US" ) ); //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
		assertEquals( "text en_US", text ); //$NON-NLS-1$

		text = design.getMessage(
				"ResourceKey.testKey2", new ULocale( "en", "GB" ) ); //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
	 ...
	 * <p>
	 * 2. design file defined no translations, return should be null.
	 * <p>
	 * <p>
	 * 
	 * @throws DesignFileException
	 */
	public void testGetTranslations( ) throws DesignFileException
	{
		openDesign( "ReportDesignUserDefinedMessagesTest.xml", ULocale.ENGLISH ); //$NON-NLS-1$
		List translations = design.getTranslations( );
		assertEquals( 12, translations.size( ) );

		openDesign( "ReportDesignUserDefinedMessagesTest1.xml", ULocale.ENGLISH ); //$NON-NLS-1$
		assertNull( design.getTranslations( ) );

		openDesign( "ReportDesignUserDefinedMessagesTest2.xml", ULocale.ENGLISH ); //$NON-NLS-1$      
		translations = design.getTranslations( );
		assertEquals( 6, translations.size( ) );

	}

	/**
	 * test to find a translation from the design. The translation is identified
	 * by its resourcekey and locale.
	 * <p>
	 * 1. Find a translation, locale is null, return should be the translation
	 * without a locale.
	 * <p>
	 * 2. Find a translation, locale is "zh_CN", return should be the
	 * translation with the loclae "zh_CN".
	 * <p>
	 * 3. Find a translation, the resource key is not defined return should be
	 * null.
	 * <p>
	 * 
	 * @throws DesignFileException
	...
	public void testFindTranslation( ) throws DesignFileException
	{
		...
		Translation translation = design.findTranslation(
				"ResourceKey.ReportDesign.Description", null ); //$NON-NLS-1$
		assertEquals( "sample report(default)", translation.getText( ) ); //$NON-NLS-1$
	...
	/**
	 * Get an string array containing all the resource keys defined.
	 * <p>
	 * 
	 * @throws DesignFileException
	 */
	public void testGetTranslationResourceKeys( ) throws DesignFileException
	{
		openDesign( "ReportDesignUserDefinedMessagesTest.xml", ULocale.ENGLISH ); //$NON-NLS-1$
		String[] resourceKeys = design.getTranslationResourceKeys( );
		assertEquals( 4, resourceKeys.length );

		openDesign( "ReportDesignUserDefinedMessagesTest2.xml", ULocale.ENGLISH ); //$NON-NLS-1$
		resourceKeys = design.getTranslationResourceKeys( );
		assertEquals( 2, resourceKeys.length );
