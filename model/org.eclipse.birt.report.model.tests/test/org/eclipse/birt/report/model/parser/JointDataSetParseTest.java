 ...
 * blank field.</td>
 * <td>After the design file is parsed, six sematic errors should be returned.</td>
 * </tr>
 * 
 * <tr>
...
public class JointDataSetParseTest extends BaseTestCase
{
	 ...
	 *             design file can't be parsed correctly.
	 */
	...
	public void testParser( ) throws DesignFileException
	{
		...
		assertEquals( DesignChoiceConstants.JOIN_TYPE_INNER,
				joinConditionHandle.getJoinType( ) );
		assertEquals( DesignChoiceConstants.JOIN_OPERATOR_EQALS,
				joinConditionHandle.getOperator( ) );
		...
		assertEquals( DesignChoiceConstants.COLUMN_DATA_TYPE_STRING, column
				.getDataType( ) );
		...
		List columnHints = (List) dataSet
				.getProperty( DataSet.COLUMN_HINTS_PROP );
		...
		ArrayList filters = (ArrayList) dataSet
				.getProperty( DataSet.FILTER_PROP );
	...
	/**
	 * If there is any blank field in the join condition, a semantic error
	...
	public void testSemanticCheck( ) throws DesignFileException
	{
		...
		assertEquals( SemanticError.DESIGN_EXCEPTION_INVALID_ELEMENT_REF, error
				.getErrorCode( ) );
		assertEquals( "JointDataSet", error.getElement( ).getName( ) ); //$NON-NLS-1$
	}

	/**
	 * Checks there is a semantic error. It is the value required exception with
	 * the given data set.
	 * 
	 * @param errors
	 *            the error list.
	...
	{
		assertTrue( PropertyValueException.DESIGN_EXCEPTION_VALUE_REQUIRED
				.equals( errorDetail.getErrorCode( ) )
