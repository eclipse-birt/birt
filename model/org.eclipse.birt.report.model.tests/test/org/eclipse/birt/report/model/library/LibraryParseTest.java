...
public class LibraryParseTest extends BaseTestCase
{
	...
	public void testResolveStyle( ) throws Exception
	{
		...
		// 'codename'
		LabelHandle labelHandle3 = (LabelHandle) designHandle
				.findElement( "label3" );//$NON-NLS-1$
		...
		assertTrue( ( (Style) labelHandle3.getStyle( ).getElement( ) )
				.getContainer( ) instanceof Theme );
		assertTrue( ( (Style) labelHandle4.getStyle( ).getElement( ) )
				.getContainer( ) instanceof Theme );
	...
	public void testParser( ) throws Exception
	{
		...
		assertEquals(
				"W.C. Fields", libraryHandle.getStringProperty( Library.AUTHOR_PROP ) ); //$NON-NLS-1$
		assertEquals(
				"http://company.com/reportHelp.html", libraryHandle.getStringProperty( Library.HELP_GUIDE_PROP ) ); //$NON-NLS-1$
		assertEquals(
				"Whiz-Bang Plus", libraryHandle.getStringProperty( Library.CREATED_BY_PROP ) ); //$NON-NLS-1$

		// title

		assertEquals(
				"TITLE_ID", libraryHandle.getStringProperty( Library.TITLE_ID_PROP ) ); //$NON-NLS-1$
		assertEquals(
				"Sample Report", libraryHandle.getStringProperty( Library.TITLE_PROP ) ); //$NON-NLS-1$

		// comments

		assertEquals(
				"First sample report.", libraryHandle.getStringProperty( Library.COMMENTS_PROP ) ); //$NON-NLS-1$

		// description

		assertEquals(
				"DESCRIP_ID", libraryHandle.getStringProperty( Library.DESCRIPTION_ID_PROP ) ); //$NON-NLS-1$
		assertEquals(
				"This is a first sample report.", libraryHandle.getStringProperty( Library.DESCRIPTION_PROP ) ); //$NON-NLS-1$

		// color-palette

		PropertyHandle colorPalette = libraryHandle
		...
		// config-vars
		PropertyHandle configVarHandle = libraryHandle
		...
		// images
		PropertyHandle imageHandle = libraryHandle
		...
		assertEquals( "imagetesAAA", //$NON-NLS-1$
				new String( Base64.encodeBase64( image.getData( libraryHandle
						.getModule( ) ) ) ).substring( 0, 11 ) );
		...
		IncludedCssStyleSheetHandle css = (IncludedCssStyleSheetHandle) theme
	...
	public void testWriter( ) throws Exception
	{
		...
		libraryHandle.setProperty( Library.INCLUDE_RESOURCE_PROP,
				"new include resource" ); //$NON-NLS-1$
		...
		libraryHandle.setProperty( Library.DESCRIPTION_ID_PROP,
