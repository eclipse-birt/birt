...
public class StyleNameTest extends BaseTestCase
{
	...
	public void testNameSpace( ) throws Exception
	{
		...
		List<DesignElement> styles = design.getSlot( ReportDesign.STYLE_SLOT )
				.getContents( );
		DesignElement style = styles.get( 0 );
		assertEquals( style, styleNs
				.getElement( style.getName( ).toLowerCase( ) ) );
	...
	/**
	 * Tests the validation about the style name. Validation is done when
	 * inserting a named style to design tree and when renaming a style.
	 * 
	 * @throws Exception
	 */
	public void testNameValidator( ) throws Exception
	{
		...
		DesignElementHandle styleHandle = designHandle.getElementFactory( )
				.newStyle( invalidName );
		...
		{
			assertEquals( NameException.DESIGN_EXCEPTION_INVALID_STYLE_NAME, e
					.getErrorCode( ) );
		}
	...
	/**
	 * Tests the renaming an old design file style name. At the same time, we
	 * will update the style reference in all styled elements.
	 * 
	 * @throws Exception
	 */
	...
	/**
	 * Tests the renaming logic that the styles in design with duplicate name
	 * with the library theme style.
	 * 
	 * @throws Exception
	 */
	...
	/**
	 * Tests the failed case: design element refers theme styles that will have
	 * to be renamed for its name is invalid or duplicate with different cases.
	 * 
	 * @throws Exception
	 */
