...
public class GroupPropertyHandleTest extends BaseTestCase
{
	...
	public void testSetValue( ) throws SemanticException
	{
		...
		assertEquals(
				"Rock likes soccer", handle1.getStringProperty( MasterPage.COMMENTS_PROP ) ); //$NON-NLS-1$ 
		assertEquals(
				"Rock likes soccer", handle2.getStringProperty( MasterPage.COMMENTS_PROP ) ); //$NON-NLS-1$ 
		assertEquals(
				"Rock likes soccer", handle3.getStringProperty( MasterPage.COMMENTS_PROP ) ); //$NON-NLS-1$ 
	...
	public void testgetReferenceElementValueList( ) throws DesignFileException
	{
		...
		assertEquals( 2, groupPropertyHandle.getReferenceableElementList( )
				.size( ) );
	}
	...
	public void testGetLocalValue( ) throws Exception
	{
		LabelHandle label = designHandle.getElementFactory( )
				.newLabel( "local" ); //$NON-NLS-1$
		LabelHandle parentLabel = designHandle.getElementFactory( ).newLabel(
				"parent" ); //$NON-NLS-1$
		LabelHandle childLabel = designHandle.getElementFactory( ).newLabel(
				"child" ); //$NON-NLS-1$
		LabelHandle styledLabel = designHandle.getElementFactory( ).newLabel(
				"styled" ); //$NON-NLS-1$
		...
		parentLabel.setProperty( IStyleModel.BORDER_LEFT_STYLE_PROP,
				DesignChoiceConstants.LINE_STYLE_DOUBLE );
		styledLabel.setStyle( style );
		label.setProperty( IStyleModel.BORDER_LEFT_STYLE_PROP,
				DesignChoiceConstants.LINE_STYLE_DOUBLE );
		...
		groupElementHandle = new SimpleGroupElementHandle( designHandle,
				elements );
		groupPropertyHandle1 = groupElementHandle
				.getPropertyHandle( IStyleModel.BORDER_LEFT_STYLE_PROP );
		...
		assertEquals( DesignChoiceConstants.LINE_STYLE_DOUBLE,
				groupPropertyHandle1.getStringValue( ) );
		assertEquals( DesignChoiceConstants.LINE_STYLE_DOUBLE,
				groupPropertyHandle1.getLocalStringValue( ) );
	}
