...
public class ElementDefnTest extends AbstractMetaTest {
	...
	public void testGetProperty() throws MetaDataException {
		...
		ElementDefn label = (ElementDefn) MetaDataDictionary.getInstance().getElement(ReportDesignConstants.LABEL_ITEM);

		SystemPropertyDefn p = (SystemPropertyDefn) reportItem.getProperty(ReportItem.BOOKMARK_PROP);
		assertNotNull(p);
	...
	public void testGetLocalPropertiesAndGetProperties() {
		...
		ElementDefn reportElement = (ElementDefn) MetaDataDictionary.getInstance().getElement("ReportElement"); //$NON-NLS-1$
		ElementDefn reportItem = (ElementDefn) MetaDataDictionary.getInstance()
				.getElement(ReportDesignConstants.REPORT_ITEM);
		ElementDefn label = (ElementDefn) MetaDataDictionary.getInstance().getElement(ReportDesignConstants.LABEL_ITEM);
		...
		SystemPropertyDefn fontColorProperty = (SystemPropertyDefn) label.getProperty(Style.COLOR_PROP);
		SystemPropertyDefn fontFamilyProperty = (SystemPropertyDefn) label.getProperty(Style.FONT_FAMILY_PROP);
		...
		assertNotNull(reportElement.getProperty(DesignElement.DISPLAY_NAME_PROP));
		...
		assertTrue(reportElement.isKindOf(MetaDataDictionary.getInstance().getElement("ReportElement"))); //$NON-NLS-1$
		assertTrue(reportItem.isKindOf(MetaDataDictionary.getInstance().getElement("ReportElement"))); //$NON-NLS-1$
		assertTrue(label.isKindOf(MetaDataDictionary.getInstance().getElement("ReportElement"))); //$NON-NLS-1$

		// ReportItem and Label are kind of ReportItem

		assertFalse(
				reportElement.isKindOf(MetaDataDictionary.getInstance().getElement(ReportDesignConstants.REPORT_ITEM)));
		assertTrue(reportItem.isKindOf(MetaDataDictionary.getInstance().getElement(ReportDesignConstants.REPORT_ITEM)));
		assertTrue(label.isKindOf(MetaDataDictionary.getInstance().getElement(ReportDesignConstants.REPORT_ITEM)));

		// Only Label is kind of Label

		assertFalse(
				reportElement.isKindOf(MetaDataDictionary.getInstance().getElement(ReportDesignConstants.LABEL_ITEM)));
		assertFalse(reportItem.isKindOf(MetaDataDictionary.getInstance().getElement(ReportDesignConstants.LABEL_ITEM)));
		assertTrue(label.isKindOf(MetaDataDictionary.getInstance().getElement(ReportDesignConstants.LABEL_ITEM)));
	}
	...
	public void testGetLocalMethodsAndGetMethods() {
		...
		IElementDefn freeFormDefn = MetaDataDictionary.getInstance().getElement(ReportDesignConstants.FREE_FORM_ITEM);

		int i = 0;

		List list = simpleDataSetDefn.getMethods();
		assertEquals(5, list.size());
		assertEquals(SimpleDataSet.BEFORE_OPEN_METHOD, ((PropertyDefn) list.get(i++)).getName());
		assertEquals(SimpleDataSet.BEFORE_CLOSE_METHOD, ((PropertyDefn) list.get(i++)).getName());
		assertEquals(SimpleDataSet.ON_FETCH_METHOD, ((PropertyDefn) list.get(i++)).getName());
		assertEquals(SimpleDataSet.AFTER_OPEN_METHOD, ((PropertyDefn) list.get(i++)).getName());
		assertEquals(SimpleDataSet.AFTER_CLOSE_METHOD, ((PropertyDefn) list.get(i++)).getName());
	...
	public void testGetLocalExpressionsAndGetExpression() {
		...
		assertEquals(ReportItem.BOOKMARK_PROP, ((PropertyDefn) list.get(0)).getName());
	...
	public void testElementWithOneSlot() {
		...
		assertEquals("Element.FreeForm.slot.reportItems", ((SlotDefn) container.getSlot(0)) //$NON-NLS-1$
				.getDisplayNameID());
		assertEquals(true, container.getSlot(0).isMultipleCardinality());
		assertTrue(container.getSlot(0)
				.canContain(MetaDataDictionary.getInstance().getElement(ReportDesignConstants.FREE_FORM_ITEM)));
		assertTrue(container.getSlot(0)
				.canContain(MetaDataDictionary.getInstance().getElement(ReportDesignConstants.LABEL_ITEM)));

		assertFalse(container.canContain(-1,
				MetaDataDictionary.getInstance().getElement(ReportDesignConstants.LABEL_ITEM)));
		assertTrue(
				container.canContain(0, MetaDataDictionary.getInstance().getElement(ReportDesignConstants.LABEL_ITEM)));
		assertFalse(container.canContain(0,
	...
	public void testElementWithNoSlot() {
		...
		IElementDefn label = MetaDataDictionary.getInstance().getElement(ReportDesignConstants.LABEL_ITEM);
	...
	public void testAddSlotToElement() {
		...
		MetadataTestUtil.addType(slot, ReportDesignConstants.DATA_SOURCE_ELEMENT);
	...
	public void testMethod() throws MetaDataParserException {
		...
		loadMetaData(ElementDefnTest.class.getResourceAsStream("input/ElementDefnTest.def")); //$NON-NLS-1$
	...
	/**
	 * Test the style property can not be retrieved from the container which can not
	 * have style.
	 */
	 ...
	 * <code>ExtendedDataSet.PRIVATE_DRIVER_PROPERTIES_PROP</code> are invisible to
	 * users.
	 */

	public void testPropertyVisiblity() {
		IElementDefn dataSetDefn = MetaDataDictionary.getInstance().getElement(ReportDesignConstants.ODA_DATA_SET);

		Iterator iter = dataSetDefn.getLocalProperties().iterator();
		while (iter.hasNext()) {
			ElementPropertyDefn prop = (ElementPropertyDefn) iter.next();

			if (OdaDataSet.RESULT_SET_NAME_PROP.equalsIgnoreCase(prop.getName())
					|| OdaDataSet.PRIVATE_DRIVER_PROPERTIES_PROP.equalsIgnoreCase(prop.getName())
		...
		assertFalse(reportItemDefn.isPropertyVisible(IReportItemModel.CUBE_PROP));

		IElementDefn dataDefn = MetaDataDictionary.getInstance().getElement(ReportDesignConstants.DATA_ITEM);
