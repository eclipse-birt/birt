...
public class LabelHandleTest extends BaseTestCase
{
	...
	public void testSetGetProperties( ) throws SemanticException
	{
		...
		assertEquals(
				"100000000000000000000000000000000pt", label.getHeight( ).getStringValue( ) ); //$NON-NLS-1$
		...
		assertEquals(
				"normal", label5.getProperty( StyleHandle.FONT_STYLE_PROP ) ); //$NON-NLS-1$
		...
		assertEquals( DesignChoiceConstants.FONT_STYLE_ITALIC, label5
				.getProperty( StyleHandle.FONT_STYLE_PROP ) );
	}
	...
	public void testGetLocalizationMessage( )
	{
		...
		LabelHandle handle11 = (LabelHandle) designHandle
				.findElement( "label11" );//$NON-NLS-1$
		assertEquals( "label in i18n", handle11.getDisplayText( ) );//$NON-NLS-1$
		assertEquals(
				"label in i18n", handle11.getRoot( ).getMessage( handle11.getTextKey( ) ) );//$NON-NLS-1$

		LabelHandle handle12 = (LabelHandle) designHandle
				.findElement( "label12" );//$NON-NLS-1$
		assertEquals(
				"", handle12.getRoot( ).getMessage( handle12.getTextKey( ) ) );//$NON-NLS-1$

		LabelHandle handle15 = (LabelHandle) designHandle
				.findElement( "label15" );//$NON-NLS-1$
		...
		LabelHandle handle16 = (LabelHandle) designHandle
				.findElement( "label16" );//$NON-NLS-1$
		assertEquals( "actuate", handle16.getDisplayText( ) );//$NON-NLS-1$
		assertEquals(
				"actuate", handle16.getRoot( ).getMessage( handle16.getTextKey( ) ) );//$NON-NLS-1$
	}
