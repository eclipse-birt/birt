...
public class DesignElementHandleTest extends BaseTestCase
{
	...
	/**
	 * Test findContentSlot method on DesignElementHandle; and the move
	 * operation of SlotHandle.
	 * 
	 * @throws Exception
	 */
	public void testFindContentSlot( ) throws Exception
	{
		GraphicMasterPageHandle mHandle = (GraphicMasterPageHandle) designHandle
		...
		assertTrue( textHandle.getElement( ).getContainer( ) == mHandle
				.getElement( ) );
		...
		SlotHandle detailSlotOfTable = tableHandle
				.getSlot( TableItem.DETAIL_SLOT );
		...
		assertTrue( textHandle.getElement( ).getContainer( ) == cellHandle
				.getElement( ) );
		cellHandle.getContent( ).move( textHandle, designHandle,
	...
	public void testGetContainerSlot( ) throws Exception
	{
		...
		assertEquals( slotHandle.getElement( ), designHandle
				.findMasterPage( "My Page" ) ); //$NON-NLS-1$
	 ...
	 * <ul>
	 * <li>
	 * {@link org.eclipse.birt.report.model.api.DesignElementHandle#getStyle()}
	 * <li>
	 ...
	 * <li>
	 * {@link org.eclipse.birt.report.model.api.DesignElementHandle#getName()}
	 * <li>
	...
	public void testStyle( ) throws Exception
	{
		...
		PrivateStyleHandle privateStyleHandle = (PrivateStyleHandle) handle
				.getPrivateStyle( );
	...
	{
		DesignElementHandle label4Handle = designHandle
				.findElement( "bodyLabel4" ); //$NON-NLS-1$
	...
	/**
	 * Tests other related methods of DesignElementHandle. Following methods
	 * have been tested:
	 * <ul>
	 * <li>
	...
	public void testOtherMethods( ) throws Exception
	{
		...
		handle.moveTo( slotHandle.getElementHandle( ),
		...
		assertEquals(
				"bodyLabel5", handle.getDisplayLabel( DesignElement.USER_LABEL ) ); //$NON-NLS-1$

		assertEquals(
				"bodyLabel5", handle.getDisplayLabel( DesignElement.SHORT_LABEL ) ); //$NON-NLS-1$

		assertEquals(
				"bodyLabel5(\"Fifth Label in body.\")", handle.getDisplayLabel( DesignElement.FULL_LABEL ) ); //$NON-NLS-1$
	...
	public void testContainment( ) throws SemanticException
	{
		...
		MasterPageHandle masterPageHandle = factory
		...
		try
		{
			...
			masterPageHandle.getSlot( SimpleMasterPage.PAGE_HEADER_SLOT ).add(
					freeForm1 );
			masterPageHandle.getSlot( SimpleMasterPage.PAGE_HEADER_SLOT ).add(
					freeForm2 );
			fail( );
	...
	public void testPropertyMethods( ) throws Exception
	{
		...
		DesignElementHandle elementHandle = dataSethandle
				.getElementProperty( SimpleDataSet.DATA_SOURCE_PROP );
		assertEquals( "myDataSource", elementHandle.getName( ) ); //$NON-NLS-1$

		// CAN_SHRINK_PROP has not any value.

		FactoryPropertyHandle factoryHandle = styleHandle
				.getFactoryPropertyHandle( Style.CAN_SHRINK_PROP );
		assertNull( factoryHandle );

		factoryHandle = styleHandle
				.getFactoryPropertyHandle( Style.MASTER_PAGE_PROP );
		assertNull( factoryHandle );

		factoryHandle = styleHandle
				.getFactoryPropertyHandle( Label.BOOKMARK_PROP );
		assertNull( factoryHandle );

		// uses width and height to test getFloatProperty.

		double floatValue = ( (DimensionValue) handle
				.getProperty( ReportItem.WIDTH_PROP ) ).getMeasure( );
		assertTrue( 2 == floatValue );

		floatValue = ( (DimensionValue) handle
				.getProperty( ReportItem.HEIGHT_PROP ) ).getMeasure( );
		...
		BigDecimal bigDecimalValue = styleHandle
				.getNumberProperty( Style.WIDOWS_PROP );
		assertTrue( 2 == bigDecimalValue.intValue( ) );

		// no this property, will be null.

		bigDecimalValue = handle
				.getNumberProperty( SimpleDataSet.DATA_SOURCE_PROP );
		...
		assertTrue( 55 == styleHandle.getNumberProperty( Style.WIDOWS_PROP )
				.intValue( ) );
		...
		handle.setProperty( ReportItem.HEIGHT_PROP, new DimensionValue( 2.34,
		...
		assertEquals( DesignChoiceConstants.FONT_FAMILY_SERIF, styleHandle
	...
	public void testSetProperties( ) throws SemanticException
	{
		...
		assertEquals(
				"red", labelHandle.getStringProperty( Style.BACKGROUND_COLOR_PROP ) ); //$NON-NLS-1$
		assertEquals(
				"solid", labelHandle.getStringProperty( Style.BORDER_LEFT_STYLE_PROP ) ); //$NON-NLS-1$
	...
	public void testClearElements( ) throws SemanticException
	{
		...
		assertEquals( 4, pageHandle.getSlot( GraphicMasterPage.CONTENT_SLOT )
				.getCount( ) );
		pageHandle.clearContents( GraphicMasterPage.CONTENT_SLOT );
		assertEquals( 0, pageHandle.getSlot( GraphicMasterPage.CONTENT_SLOT )
				.getCount( ) );
	 ...
	 * </li>
	 * </ul>
	 * Add a new Label to the end of the contents slot of a GraphicMasterPage.
	 * The label should be added and it should in the end position.
	 * 
	...
	public void testAddElement( ) throws ContentException, NameException
	{
		...
		assertEquals( 4, pageHandle.getSlot( GraphicMasterPage.CONTENT_SLOT )
				.getCount( ) );
		LabelHandle newLabel = new ElementFactory( design )
				.newLabel( "American Navy 1921" ); //$NON-NLS-1$
		pageHandle.addElement( newLabel, GraphicMasterPage.CONTENT_SLOT );
		assertEquals( 5, pageHandle.getSlot( GraphicMasterPage.CONTENT_SLOT )
				.getCount( ) );
		assertEquals( 4, pageHandle.getSlot( GraphicMasterPage.CONTENT_SLOT )
				.findPosn( newLabel ) );
		...
		ImageHandle newImage = new ElementFactory( design )
				.newImage( "Rock in deep blue sea" ); //$NON-NLS-1$

		// add an image to position 3.
		pageHandle.addElement( newImage, GraphicMasterPage.CONTENT_SLOT, 3 );
		assertEquals( 6, pageHandle.getSlot( GraphicMasterPage.CONTENT_SLOT )
				.getCount( ) );
		assertEquals( 3, pageHandle.getSlot( GraphicMasterPage.CONTENT_SLOT )
				.findPosn( newImage ) );
	...
	public void testSemanticCheck( ) throws DesignFileException
	{
		...
		assertEquals( SemanticError.DESIGN_EXCEPTION_MISSING_DATA_SET,
				( (ErrorDetail) list.get( 3 ) ).getErrorCode( ) );
	...
	public void testCanContain( ) throws SemanticException
	{
		...
		MasterPageHandle page = designHandle.getElementFactory( )
				.newGraphicMasterPage( null );
		...
		assertTrue( designHandle.getComponents( ).canContain(
				ReportDesignConstants.GRID_ITEM ) );

		assertTrue( designHandle.canContain( ReportDesign.COMPONENT_SLOT,
				ReportDesignConstants.GRID_ITEM ) );

		designHandle.getComponents( ).add( label );

		assertTrue( designHandle
				.canContain( ReportDesign.COMPONENT_SLOT, label ) );
		designHandle.getComponents( ).canContain(
				ReportDesignConstants.GRID_ITEM );
	...
	/**
	 * Tests the function to retrieve the path of an element. The path begins
	 * from the root of the design tree.
	 * 
	 * @throws SemanticException
	 * 
	 */
	...
	/**
	 * The listener does not modify the <code>listeners</code> of a design
	 * element. Used to test <code>drop</code> method in
