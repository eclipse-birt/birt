...
public class LabelItemParserTest extends ParserTestCase {
	...
	public void testParser() throws Exception {
		...
		assertEquals("Today's Date", labelHandle.getStringProperty(Label.TEXT_PROP)); //$NON-NLS-1$
		...
		assertEquals("\"labelFace\"", labelHandle.getStringProperty(Style.FONT_FAMILY_PROP)); //$NON-NLS-1$
		assertEquals("4mm", labelHandle.getStringProperty(Style.FONT_SIZE_PROP)); //$NON-NLS-1$
		assertEquals("bold", labelHandle.getStringProperty(Style.FONT_WEIGHT_PROP)); //$NON-NLS-1$

		// test the style property whcih was not defined on the label itself
		assertEquals("none", labelHandle.getStringProperty(Style.TEXT_UNDERLINE_PROP)); //$NON-NLS-1$
		...
		assertEquals("yellow", labelHandle.getStringProperty(Style.COLOR_PROP)); //$NON-NLS-1$
		...
		labelHandle = (LabelHandle) designHandle.findElement("listHeaderLabel"); //$NON-NLS-1$
		...
		labelHandle = (LabelHandle) designHandle.findElement("listDetailLabel"); //$NON-NLS-1$
		...
		labelHandle = (LabelHandle) designHandle.findElement("listFooterLabel"); //$NON-NLS-1$
		...
		labelHandle = (LabelHandle) designHandle.findElement("scratchpadLabel"); //$NON-NLS-1$
	...
	public void testSemanticCheck() {
		try {
			...
			assertEquals(detail.getErrorCode(), NameException.DESIGN_EXCEPTION_DUPLICATE);
		}
	...
	private LabelHandle getLabel() throws Exception {
		openDesign(fileName);
		MasterPageHandle masterPageHandle = (MasterPageHandle) designHandle.getMasterPages().get(0);
