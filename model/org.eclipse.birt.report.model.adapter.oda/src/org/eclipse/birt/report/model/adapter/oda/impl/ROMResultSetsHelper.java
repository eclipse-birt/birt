 ...
 * change case ) 3. All the rest new column definitions will be regarded as
 * newly added case. 4. All the rest old column will be regarded as deleted
 * case. Find old column hint: Use matched old column name to find old column
 * hint. Generate new column: 1. For all no change case, keep the old name,
 * because there may be some bindings in the report using the old name. 2. After
 * processing no change cases, assign new name to the rest new columns. Generate
 * new name: Use jdbc alias as name, if there is any duplicate, add "_1", "_2",
 * etc.
 */
...
public class ROMResultSetsHelper {
	...
	public ROMResultSetsHelper(List<ColumnDefinition> oldColumnDefns, List<OdaResultSetColumnHandle> oldColumns,
			List<ColumnHintHandle> oldColumnHints, List<ColumnDefinition> newColumnDefns, String dataSourceId,
			String dataSetId) {
