...
class DesignerStateAdapter
{
	...
	static void updateROMDesignerState( DesignerState designerState,
			ReportElementHandle reportElement ) throws SemanticException
	{
		if ( designerState == null || reportElement == null )
			return;

		CommandStack cmdStack = reportElement.getModuleHandle( )
				.getCommandStack( );
		...
		{
			romDesignerState = ( (OdaDataSourceHandle) reportElement )
					.getDesignerState( );
		}
		...
		{
			OdaDesignerState tmpDesignerState = StructureFactory
	...
	static DesignerState createOdaDesignState(
			OdaDesignerStateHandle designerState )
	{
		...
		DesignerStateContent stateContent = ODADesignFactory.getFactory( )
