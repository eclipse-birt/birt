...
public class MeasureViewHandleTest extends BaseTestCase
{
	...
	public void testFilters( ) throws Exception
	{
		...
		ExtendedItemHandle extendedItem = (ExtendedItemHandle) designHandle
				.getElementByID( 72l );
		MeasureViewHandle measure = (MeasureViewHandle) extendedItem
				.getReportItem( );
		Iterator iterator = measure.filtersIterator( );
		FilterConditionElementHandle filter = (FilterConditionElementHandle) iterator
				.next( );

		assertEquals( "data[\"EMPLOYEENUMBER\"]", filter.getExpr( ).trim( ) );//$NON-NLS-1$

		FilterConditionElementHandle newFilter = designHandle
		...
		extendedItem.getPropertyHandle( MeasureViewHandle.FILTER_PROP ).add(
				newFilter );
		iterator = measure.filtersIterator( );

		FilterConditionElementHandle temp = (FilterConditionElementHandle) iterator
				.next( );
