...
class ColumnWalker2 implements ICrosstabConstants
{
	...
	private int tmpStartDimensionIndex, tmpStartLevelIndex,
			tmpEndDimensionIndex, tmpEndLevelIndex;
	...
	ColumnWalker2( CrosstabReportItemHandle item, CubeCursor cursor )
	{
		...
		lastGroupIndex = GroupUtil.getGroupIndex( crosstabItem,
				COLUMN_AXIS_TYPE,
				cdCount - 1,
				-1 );
		...
		switch ( currentState )
		{
			...
			case STATE_MEASURE_HEADER :
				...
				if ( cdCount > 0 )
				{
					...
					columnEdgeCursor = (EdgeCursor) cubeCursor.getOrdinateEdge( )
							.get( 0 );
				...
				if ( cdCount > 0 )
				{
					while ( hasNext )
					{
						...
						for ( int i = tmpStartDimensionIndex; i < cdCount; i++ )
						{
							...
							for ( int j = start; j < dv.getLevelCount( ); j++ )
							{
								int positionIndex = GroupUtil.getGroupIndex( crosstabItem,
										COLUMN_AXIS_TYPE,
										i,
										j );
								...
								else if ( p != columnDimensionPositions[positionIndex] )
								{
									...
									if ( currentState != STATE_COLUMN_TOTAL_BEFORE )
									{
										...
										{
											DimensionViewHandle sdv = crosstabItem.getDimension( COLUMN_AXIS_TYPE,
													t );
											...
											for ( int u = subEnd; u >= subStart; u-- )
											{
												...
												int cuPx = GroupUtil.getGroupIndex( crosstabItem,
														COLUMN_AXIS_TYPE,
														t,
														u );
												...
												// column if available
												if ( cuPx != lastGroupIndex
														&& slv.getAggregationHeader( ) != null
									...
									for ( int t = tmpEndDimensionIndex; t < cdCount; t++ )
									{
										...
										for ( int u = subStart; u < sdv.getLevelCount( ); u++ )
										{
											...
											if ( cuPx != lastGroupIndex
													&& slv.getAggregationHeader( ) != null
													&& AGGREGATION_HEADER_LOCATION_BEFORE.equals( slv.getAggregationHeaderLocation( ) ) )
											{
												if ( mCount > 0
														&& !isVerticalMeasure )
												{
