			...
			DNDLocation location )
	{
				...
				&& target instanceof CrosstabCellEditPart )
		{
			...
			if ( ICrosstabCellAdapterFactory.CELL_MEASURE_AGGREGATION.equals( posType )
					...
					|| ( ICrosstabCellAdapterFactory.CELL_MEASURE_VIRTUAL.equals( posType ) && transfer.equals( DesignerPaletteFactory.TIMEPERIOD_TEMPLATE ) ) )
			{
				if ( transfer.equals( DesignerPaletteFactory.TIMEPERIOD_TEMPLATE ) )
				{
					CrosstabReportItemHandle reportHandle = (CrosstabReportItemHandle) CrosstabUtil.getReportItem( (DesignElementHandle) ( (CrosstabCellEditPart) target ).getParent( )
								.getModel( ) );
			...
			try
			{
				...
				if ( dialog.open( ) == Window.OK )
				{
					if ( target instanceof VirtualCellEditPart )
					{
						...
						ComputedMeasureViewHandle computedMeasure = itemHandle.insertComputedMeasure( bindingHandle.getName( ),
								itemHandle.getComputedMeasures( ).size( ) );
						...
						if ( ICrosstabConstants.MEASURE_DIRECTION_HORIZONTAL.equals( itemHandle.getMeasureDirection( ) ) )
						{
							...
							String defaultUnit = itemHandle.getModelHandle( )
									.getModuleHandle( )
									.getDefaultUnits( );
						}
