...
public class CrosstabHandleAdapter extends BaseCrosstabAdapter {
			...
			if (adjustGrandColumn > 0) {
				...
				if (cellColumnNumber > first.getColumnNumber() + first.getColumnSpan() - 1) {
					...
					if (cellColumnNumber <= beforeColumnNumber) {
						...
						cellAdapter.setColumnNumber(cellColumnNumber - beforeColumnNumber + first.getColumnNumber()
								+ first.getColumnSpan() - 1);
					}
				}
			}
			if (adjustGrandRow > 0) {
				...
				if (cellRowNumber > first.getRowNumber() + first.getRowSpan() - 1) {
					if (cellRowNumber <= beforeRowNumber) {
						...
						cellAdapter.setRowNumber(
								cellRowNumber - beforeRowNumber + first.getRowNumber() + first.getRowSpan() - 1);
					}
				}
			}
		}
	...
	private void buildLeftConner(List list) {
		...
		if (handle.getHeader() == null) {
			first = factory.createCrosstabCellAdapter(LEFT_CONNER, null, 1, rowBase, 1, columnBase, false);
		} else {
			...
			if (handle.getHeaderCount() > 1) {
				...
				// if ((columnBase*temp == handle.getHeaderCount( )&& needSpan != 2) ||
				// (needSpan == 2 && rowBase * temp == handle.getHeaderCount( )))
				if (rowCount * columnCount == handle.getHeaderCount()) {
					for (int i = 0; i < rowCount; i++) {
						for (int j = 0; j < columnCount; j++) {
							...
							CrosstabCellAdapter cellAdapter = factory.createCrosstabCellAdapter(
									ICrosstabCellAdapterFactory.CROSSTAB_HEADER, handle.getHeader(i * columnCount + j),
									i + 1, rowSpan, j + 1, columnSpan, false);
							list.add(cellAdapter);
						}
					}
				} else {
					first = factory.createCrosstabCellAdapter(ICrosstabCellAdapterFactory.CROSSTAB_HEADER,
							handle.getHeader(), 1, rowBase, 1, columnBase, false);
				}
			} else {
				first = factory.createCrosstabCellAdapter(ICrosstabCellAdapterFactory.CROSSTAB_HEADER,
						handle.getHeader(), 1, rowBase, 1, columnBase, false);
	...
	private List buildMeasures() {
		...
		for (int i = 0; i < count; i++) {
			...
			for (int j = 0; j < addregationCellCount; j++) {
				...
				CrosstabCellAdapter aggregationCell = factory.createCrosstabCellAdapter(
						ICrosstabCellAdapterFactory.CELL_MEASURE_AGGREGATION, cell, row, 1, column, 1, isVertical());
		...
		if (retValue.isEmpty()) {
			CrosstabCellAdapter first = factory.createCrosstabCellAdapter(
					ICrosstabCellAdapterFactory.CELL_MEASURE_VIRTUAL, null, 1, -1, 1, -1, false);
			retValue.add(first);
		}
	...
	private List buildRowArea() {
		...
		for (int i = count - 1; i >= 0; i--) {
			...
			for (int j = levelCount - 1; j >= 0; j--) {
				...
				CrosstabCellAdapter cellAdapter = factory.createCrosstabCellAdapter(type, cellHandle, 1, rowNumber,
				...
				if (preLevelHandle != null && preLevelHandle.getAggregationHeader() != null) {
					...
					if (list.size() != 0) {
						...
						CrosstabCellAdapter preCellAdapter = factory.createCrosstabCellAdapter(
								ICrosstabCellAdapterFactory.CELL_SUB_TOTAL, preCellHandle, rowNumber + 1,
								// rowNumber,
								isVertical() ? preMeasureCount : 1, columnNumber,
								columnNumber - (isVertical() && !isHideHeader() ? 1 : 0), false);
		...
		if (grandCell != null && !retValue.isEmpty()) {
			...
			int size = list.size();
			if (size > 0) {
				CrosstabCellAdapter grandCellAdapter = factory.createCrosstabCellAdapter(
						ICrosstabCellAdapterFactory.CELL_GRAND_TOTAL, grandCell, rowNumber + 1, isVertical() ? size : 1,
						columnNumber, columnNumber - (isVertical() && !isHideHeader() ? 1 : 0), false);
		...
		if (retValue.isEmpty()) {
			CrosstabCellAdapter columnVirtual = factory
					.createCrosstabCellAdapter(ICrosstabCellAdapterFactory.CELL_ROW_VIRTUAL, null, 1, -1, 1, -1, false);
	...
	private List buildColumnArea() {
		...
		for (int i = count - 1; i >= 0; i--) {
			...
			for (int j = levelCount - 1; j >= 0; j--) {
				...
				String type = j == 0 ? ICrosstabCellAdapterFactory.CELL_FIRST_LEVEL_HANDLE
						: ICrosstabCellAdapterFactory.CELL_LEVEL_HANDLE;
				// add the levelHandle cell
				CrosstabCellAdapter cellAdapter = factory.createCrosstabCellAdapter(type, cellHandle, rowNumber, 1, 1,
						columnNumber, false);
				...
				if (preLevelHandle != null && preLevelHandle.getAggregationHeader() != null) {
					...
					if (list.size() != 0) {
						...
						CrosstabCellAdapter preCellAdapter = factory.createCrosstabCellAdapter(
								ICrosstabCellAdapterFactory.CELL_SUB_TOTAL, preCellHandle, rowNumber,
								rowNumber - (isVertical() || isHideHeader() ? 0 : 1), columnNumber + 1,
								isVertical() ? 1 : preMeasureCount, false);

						retValue.add(preCellAdapter);
						// add subTotal measure handle
						// if ( measureCount != 0 )
						// {
						int addCount = addMesureHeader(retValue, columnNumber, ICrosstabConstants.COLUMN_AXIS_TYPE,
								list, preLevelHandle);
		...
		if (grandCell != null && !retValue.isEmpty()) {
			...
			if (size > 0) {
				CrosstabCellAdapter grandCellAdapter = factory.createCrosstabCellAdapter(
						ICrosstabCellAdapterFactory.CELL_GRAND_TOTAL, grandCell, rowNumber,
						rowNumber - (isVertical() || isHideHeader() ? 0 : 1), columnNumber + 1, isVertical() ? 1 : size,
						false);
		...
		if (retValue.isEmpty()) {
			CrosstabCellAdapter columnVirtual = factory.createCrosstabCellAdapter(
					ICrosstabCellAdapterFactory.CELL_COLUMN_VIRTUAL, null, 1, -1, 1, -1, false);
	...
	private CrosstabCellAdapter getCell(int rowNumber, int columnNumber, List list) {
		for (int i = 0; i < list.size(); i++) {
			...
			if (rowNumber >= adapter.getRowNumber() && rowNumber < adapter.getRowNumber() + adapter.getRowSpan()
					&& columnNumber >= adapter.getColumnNumber()
					&& columnNumber < adapter.getColumnNumber() + adapter.getColumnSpan()) {
				return adapter;
			}
		}
		return null;
	...
	private int addMesureHeaderToVirtual(List list, int rowNumber, int rowSpan, int columnNumber, int columnSpan,
		...
		for (int k = 0; k < measureCount; k++) {
			...
			CrosstabCellAdapter measureCellAdapt = factory.createCrosstabCellAdapter(
					ICrosstabCellAdapterFactory.CELL_MEASURE_HEADER, preMeasureCellHandle, rowNumber, rowSpan,
					columnNumber, columnSpan, false);
			if (isVertical()) {
				rowNumber++;
			} else {
				columnNumber++;
	...
	private int addMesureHeader(List list, int baseColumn, int area, List measures, LevelViewHandle levelHandle) {
		...
		for (int k = 0; k < measureCount; k++) {
			...
			CrosstabCellAdapter measureCellAdapt = factory.createCrosstabCellAdapter(
					ICrosstabCellAdapterFactory.CELL_MEASURE_HEADER, preMeasureCellHandle, 1, -1, baseColumn + k + 1,
					-1, isVertical());
	...
	// code
	private LevelViewHandle getPreviousLevelViewHandle(DimensionViewHandle dimensionHandle,
	...
	protected class DefaultCrocsstabCellAdapterFactory implements ICrosstabCellAdapterFactory {
		...
		public CrosstabCellAdapter createCrosstabCellAdapter(String type, CrosstabCellHandle handle, int rowNumber,
			...
			if (CELL_LEVEL_HANDLE.equals(type) || CELL_FIRST_LEVEL_HANDLE.equals(type)) {
				...
				// ( (TotalCrosstabCellHandleAdapter) retValue ).setType(
				// CrosstabHeaderHandleAdapter.CROSSTAB_HEADER );
			...
			} else if (CELL_MEASURE_AGGREGATION.equals(type) || CELL_MEASURE.equals(type)) {
	...
	public void setRowHeight(int number, int value) {
		...
		try {
			CrosstabCellHandle cell = CrosstabModelUtil.locateRowHeightCell(getCrosstabReportItemHandle(), handle);
	...
	public CrosstabCellHandle getRowOprationCell(int rowNumber) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (cellAdapter.getRowNumber() == rowNumber && cellAdapter.getRowSpan() == 1
					&& cellAdapter.getCrosstabCellHandle() != null && (!isVertical() || !cellAdapter.getPositionType()
	...
	public CrosstabCellHandle getColumnOprationCell(int columnNumber) {
		...
		for (int i = 0; i < size; i++) {
			CrosstabCellAdapter cellAdapter = (CrosstabCellAdapter) oldModelList.get(i);
			if (cellAdapter.getColumnNumber() == columnNumber && cellAdapter.getColumnSpan() == 1
					&& cellAdapter.getCrosstabCellHandle() != null && (isVertical() || !cellAdapter.getPositionType()
							.equals(ICrosstabCellAdapterFactory.CELL_MEASURE_HEADER))) {
				return cellAdapter.getCrosstabCellHandle();
			}
...
//		}
		...
		while (parent != null) {
			if (parent instanceof LevelViewHandle) {
				return ICrosstabConstants.AGGREGATION_HEADER_LOCATION_BEFORE
						.equals(((LevelViewHandle) parent).getAggregationHeaderLocation());
			}
	...
	private boolean isGrandBefore(int type) {
		CrosstabReportItemHandle handle = getCrosstabReportItemHandle();
		return ICrosstabConstants.GRAND_TOTAL_LOCATION_BEFORE
				.equals(handle.getCrosstabView(type).getGrandTotalLocation());
	}
