...
public class DataSetPropertiesWizardPage extends DataSetWizardPage
{
	...
	private URL[] getPojoDataSetClassPath( )
	{
		...
		String classPath = Utils.getPublicProperty( dataSource,
	...
	protected DataSetDesign collectDataSetDesign( DataSetDesign design )
	{
		...
		if ( pq == null )
		{
			pq = new PojoQuery( Constants.DEFAULT_VERSION, 
		...
		if ( query != null && query.length( ) > 0 )
		{
			...
			catch ( OdaException e )
			{
				ExceptionHandler.showException( getControl( ).getShell( ), 
						Messages.getString( "FailedParseQueryTitle" ),  //$NON-NLS-1$
	...
	public DataSetPropertiesWizardPage( String pageName, String title,
	...
	private void initPageInfos( )
	{
		...
		classLoader = new URLClassLoader( allClassPaths,
	...
	private String[] getFilteredClassNames( String[] allClassNames )
	{
		...
		for ( int i = 0; i < allClassNames.length; i++ )
		{
			try
			{
				if ( org.eclipse.birt.report.data.oda.pojo.util.Utils.isPojoDataSetClass( classLoader.loadClass( allClassNames[i] ) ) )
				{
	...
	public IWizardPage getNextPage( )
	{
		...
		if ( page instanceof ColumnMappingWizardPage )
		{
			...
			if ( cmw.getPojoDataSetClass( ) == null
					|| !cmw.getPojoDataSetClass( )
							.trim( )
							.equals( pojoDataSetClass ) )
			{
