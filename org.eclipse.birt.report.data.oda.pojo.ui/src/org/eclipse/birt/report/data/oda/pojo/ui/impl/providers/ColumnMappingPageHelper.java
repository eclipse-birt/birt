...
package org.eclipse.birt.report.data.oda.pojo.ui.impl.providers;
...
public class ColumnMappingPageHelper
{
	...
	public ColumnDefinition[] addColumnDefinitions(
	...
	public boolean isValidParamName( VariableParameter vp, String name, String type )
	{
		...
		for ( int i = 0; i < definitions.size( ); i++ )
		{
			List<VariableParameter> paramList = definitions.get( i )
					.getVariableParameters( );
			for ( int k = 0; k < paramList.size( ); k++ )
			{
				if ( !vp.equals( paramList.get( k ) )
						&& vp.getName( ).equals( paramList.get( k ).getName( ) ) )
				{
					return vp.getDataType( )
							.equalsIgnoreCase( paramList.get( k ).getDataType( ) );
				}
			}
			...
			for ( int k = 0; k < allParams.size( ); k++ )
			{
				...
				{
					if ( !vp.equals( item )
							&& name.equals( ( (VariableParameter) item ).getName( ) ) )
		...
		for ( int i = 0; i < result.length; i++ )
		{
			...
			else if ( m instanceof Method )
			{
				...
				for ( Class c : ( (Method) m ).getParameterTypes( ) )
				{
					...
					VariableParameter vp = new VariableParameter( DEFAULT_PARAM_NAME_PREFIX
							+ index,
							pt.getName( ) );
					index++;
					params.add( vp );
				...
				result[i] = new MethodSource( m.getName( ),
						params.toArray( new IMethodParameter[0] ) );
			}
			assert false;
		}
		return result;
