...
import org.eclipse.birt.report.data.oda.pojo.ui.impl.dialogs.MethodParameterDialog.IModifyValidator;
...
import org.eclipse.birt.report.data.oda.pojo.ui.util.Constants;
...
public class ColumnMappingDialog extends StatusDialog
{
	 ...
	 * @param shell
	 */
	public ColumnMappingDialog( Shell shell, ColumnDefinition cd,
			ColumnMappingPageHelper helper, boolean isEditMode, boolean containsParameter )
	{
		super( shell );
		this.input = cd;
		this.helper = helper;
	...
	protected Control createDialogArea( Composite parent )
	{
		...
		{
			this.name = isEditMode ? input.getName( )
					: helper.getDistinctName( input.getName( ) );
		...
		getShell( ).setText( isEditMode
				? Messages.getString( "DataSet.EditColumnMapping" ) : Messages.getString( "DataSet.AddColumnMapping" ) ); //$NON-NLS-1$ //$NON-NLS-2$

		HelpUtil.setSystemHelp( composite,
				HelpUtil.CONEXT_ID_DATASET_POJO_COLUMN_MAPPING_DIALOG );
	...
	private void createRightArea( Composite paramArea )
	{
		...
		final Table table = new Table( right, SWT.SINGLE
		...
		checkBoxViewer.addSelectionChangedListener( new ISelectionChangedListener( ) {
			...
			{
				editBtn.setEnabled( checkBoxViewer.getTable( )
						.getSelectionCount( ) == 1 );
			}
		} );
	...
	private void createTreeViewer( Composite paramArea )
	{
		...
		final TreeViewer treeViewer = new TreeViewer( left, SWT.BORDER
				| SWT.SINGLE );
			...
			if ( element instanceof IMethodParameter )
			{
				...
				dialog.setValidator( new IModifyValidator( ) {
					...
					public boolean validateInputValue( Object value, Object[] args )
					{
						...
						{
							return helper.isValidParamName( (VariableParameter) value, (String)args[0], (String)args[1] );
						}
						return true;
					}
	...
	private void validateSyntax( )
	{
		...
		if ( !( isEditMode && this.input.getName( ).equals( this.name ) )
				&& !helper.isUniqueColumnName( this.name ) )
		{
			status = getMiscStatus( IStatus.ERROR,
					Messages.getString( "ColumnMappingDialog.error.DuplicatedColumnName" ) ); //$NON-NLS-1$
		}
		else if ( Utils.isEmptyString( this.name ) )
		{
			status = getMiscStatus( IStatus.ERROR,
					Messages.getString( "ColumnMappingDialog.error.EmptyColumnName" ) ); //$NON-NLS-1$
		}
		else if ( !containsParam
				&& txtMappingPath != null
				&& Utils.isEmptyString( txtMappingPath.getText( ) ) )
		{
			status = getMiscStatus( IStatus.ERROR,
					Messages.getString( "ColumnMappingDialog.error.EmptyMappingPath" ) ); //$NON-NLS-1$
		}
		else
		{
			status = getOKStatus( );
				...
				IStructuredContentProvider
	{
		...
		public String getColumnText( Object element, int columnIndex )
		{
			if ( element instanceof IMethodParameter )
			{
				...
				if ( columnIndex == 1 )
				{
					return ( param instanceof VariableParameter )
							? ( (VariableParameter) param ).getName( )
							: Constants.DISPLAY_NONE_VALUE;
				}
				else if ( columnIndex == 2 )
				{
					return param.getStringValue( ) == null ? "" : param.getStringValue( ) //$NON-NLS-1$
							.toString( );
				}
				else
				{
