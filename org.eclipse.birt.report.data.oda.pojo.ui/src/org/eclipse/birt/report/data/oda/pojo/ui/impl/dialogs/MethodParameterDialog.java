...
public class MethodParameterDialog extends StatusDialog
{
	...
	protected MethodParameterDialog( IMethodParameter param )
	{
		...
		this.value = param.getStringValue( ) == null ? "" : param.getStringValue( ) //$NON-NLS-1$
				.toString( );
	}
	...
	protected Control createDialogArea( Composite parent )
	{
		...
		HelpUtil.setSystemHelp( composite,
				HelpUtil.CONEXT_ID_DATASET_POJO_METHOD_PARAMETER );
	...
	private void createDialogContent( Composite composite )
	{
		...
		checkBtn.addSelectionListener( new SelectionAdapter( ) {
			...
			public void widgetSelected( SelectionEvent e )
			{
				...
				nameText.setText( toBeMapped ? name
	...
	private void validateSyntax( )
	{
		...
		else if ( this.toBeMapped
				&& ( this.name == null || this.name.trim( ).length( ) == 0 ) )
		{
			status = getMiscStatus( IStatus.ERROR,
					Messages.getString( "MethodParameterDialog.error.missingName" ) ); //$NON-NLS-1$
		}
		else if ( !this.toBeMapped
				&& ( this.value == null || this.value.trim( ).length( ) == 0 ) )
		{
			status = getMiscStatus( IStatus.ERROR,
					Messages.getString( "MethodParameterDialog.error.missingValue" ) ); //$NON-NLS-1$
		}
		else if ( validator != null )
		{
			...
			if ( ( this.param instanceof VariableParameter )
					&& !validator.validateInputValue( (VariableParameter) this.param,
							args ) )
			{
				status = getMiscStatus( IStatus.ERROR,
						Messages.getFormattedString( "MethodParameterDialog.error.invalidParamName", new Object[]{this.name} ) ); //$NON-NLS-1$
			}
			else if ( !validator.validateInputValue( new VariableParameter( name,
					type ),
					args ) )
			{
				status = getMiscStatus( IStatus.ERROR,
