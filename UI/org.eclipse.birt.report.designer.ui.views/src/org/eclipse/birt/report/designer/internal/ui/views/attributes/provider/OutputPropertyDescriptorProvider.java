...
public class OutputPropertyDescriptorProvider extends
		...
		if (checked) {
			...
			while (visibilities.hasNext()) {
				...
				if (format.equalsIgnoreCase(handle.getFormat())) {
					try {
						getVisibilityPropertyHandle(element).removeItem(
								handle.getStructure());
			...
			if (visibilities == null || !visibilities.hasNext()) {
				element.setProperty(ColumnHandle.VISIBILITY_PROP,
						new ArrayList());
			}
		}
		return true;
	...
	public void clearProperty() throws SemanticException {
	...
	public Iterator visibilityRulesIterator(DesignElementHandle handle) {
		...
		if (handle instanceof ReportItemHandle) {
			visibilities = ((ReportItemHandle) handle)
	...
	public void saveAllOutput(Expression value) throws Exception {
		...
		for (Iterator iter = DEUtil.getInputElements(input).iterator(); iter
			...
			updateHideRule(element, DesignChoiceConstants.FORMAT_TYPE_ALL,
					true, value);
		}
		stack.commit();
	}

	public void saveSpecialOutput(boolean[] selections, Expression[] expressions)
			throws Exception {
		CommandStack stack = getActionStack();
		stack.startTrans(Messages.getString("VisibilityPage.menu.SaveHides")); //$NON-NLS-1$
		...
		for (Iterator iter = DEUtil.getInputElements(input).iterator(); iter
			...
			for (int i = 0; i < getTypeInfo().length; i++) {
				...
				if (!updateHideRule(element, getTypeInfo()[i], selections[i],
						expressions[i])) {
					stack.rollback();
					return;
				}
			...
			if (hideForAll) {
				if (!updateHideRule(element,
						DesignChoiceConstants.FORMAT_TYPE_ALL, false, null)) {
					stack.rollback();
					return;
				}
			}
	...
	public Expression getExpression(Object obj) {
		HideRuleHandle ruleHandle = (HideRuleHandle) obj;
		return (Expression) ruleHandle.getExpressionProperty(
				HideRule.VALUE_EXPR_MEMBER).getValue();
	}
	...
	public void emptyProperty(DesignElementHandle element)
			throws SemanticException {
		clearProperty(element);
		element.setProperty(ColumnHandle.VISIBILITY_PROP, new ArrayList());
	}

	public void emptyProperty() throws SemanticException {
		CommandStack stack = getActionStack();
		stack.startTrans(Messages.getString("VisibilityPage.menu.SaveHides")); //$NON-NLS-1$
