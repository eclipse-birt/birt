	...
	private String[] getModifiedStyles() {
		...
		if (module instanceof ReportDesignHandle) {
			...
			for (int i = 0; i < cssList.size(); i++) {
				...
				IncludedCssStyleSheetHandle inCss = reportDesign.findIncludedCssStyleSheetHandleByProperties(
						css.getFileName(), css.getExternalCssURI(), css.isUseExternalCss());
			...
			if (theme != null) {
				...
				for (int i = 0; i < cssList.size(); i++) {
					CssStyleSheetHandle css = cssList.get(i);
					IncludedCssStyleSheetHandle inCss = theme.findIncludedCssStyleSheetHandleByProperties(
							css.getFileName(), css.getExternalCssURI(), css.isUseExternalCss());
					if (css.isUseExternalCss() || css.getExternalCssURI() != null) {
						isEditable = true;
						break;
