...
public class SampleReportsEntry implements ResourceEntry
{
	...
	public SampleReportsEntry( String[] filePattern, String entryname,
			String fragmentPath, SampleReportsEntry parentEntry, boolean isFile )
	{
				...
				for ( int i = 0; i < filePattern.length; i++ )
				{
					...
					for ( int j = 0; j < patterns.length; j++ )
					{
								...
								&& enumeration.hasMoreElements( ) )
						{
							...
							{
								if ( pathtoken[m].equals( "" ) || pathtoken[m].equals( fragmentPath.substring( fragmentPath.indexOf( '/' ) + 1 ) ) ) //$NON-NLS-1$
									continue;
								SampleReportsEntry child = parent.getChild( pathtoken[m] );
								if ( child == null )
								{
									child = new SampleReportsEntry( pathtoken[m],
	...
	private SampleReportsEntry( String entryname, String fragmentPath,
	...
	public Object getAdapter( Class adapter )
	{
		if ( adapter == ReportDesignHandle.class )
		{
			if ( getChildren( ).length == 0 && this.sampleReport == null )
			{
				try
				{
					this.sampleReport = SessionHandleAdapter.getInstance( )
							.getSessionHandle( )
