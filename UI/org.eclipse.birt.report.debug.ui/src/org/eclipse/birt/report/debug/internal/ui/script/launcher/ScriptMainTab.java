 ...
 * ScriptMainTab
 */
public class ScriptMainTab extends AbstractLaunchConfigurationTab implements
		IReportLaunchConstants
{
	...
	private static final String[] FILETYPE = new String[]{
	...
	{
		Composite parentG = ScriptSWTFactory.createComposite( parent,
				parent.getFont( ),
				1,
				1,
				GridData.FILL_BOTH );
	...
	private void buildTypeArea( Composite parentG )
	{
		...
		Composite pane = ScriptSWTFactory.createComposite( parentG,
				font,
				3,
				1,
	...
	{
		Font font = parentG.getFont( );
		Group outputGroup = ScriptSWTFactory.createGroup( parentG,
				Messages.getString( "ScriptMainTab.msg.output" ), //$NON-NLS-1$
				2,
				1,
				GridData.FILL_BOTH );

		ScriptSWTFactory.createLabel( outputGroup,
				Messages.getString( "ScriptMainTab.msg.format" ), font, 1 ); //$NON-NLS-1$
	...
	{
		Group debugTypeGroup = ScriptSWTFactory.createGroup( parentG,
				Messages.getString( "ScriptMainTab.msg.debug.type" ), //$NON-NLS-1$
				1,
				1,
				GridData.FILL_BOTH );
	...
	{
		Group taskTypeGroup = ScriptSWTFactory.createGroup( parentG,
				Messages.getString( "ScriptMainTab.msg.task.type" ), //$NON-NLS-1$
				1,
				1,
				GridData.FILL_BOTH );
		...
		bRun = ScriptSWTFactory.createRadioButton( taskTypeGroup,
				Messages.getString( "ScriptMainTab.msg.task.type.run" ) ); //$NON-NLS-1$
	...
	private void buildEngineHome( Composite parentG )
	{
		...
		Composite enginrHomeFolder = ScriptSWTFactory.createComposite( engineHomeGroup,
				font,
				3,
				2,
				GridData.FILL_BOTH,
	...
	private void buildFileName( Composite parentG )
	{
		...
		Group mainGroup = ScriptSWTFactory.createGroup( parentG,
				...
				1,
				GridData.FILL_HORIZONTAL );
		Composite comp = ScriptSWTFactory.createComposite( mainGroup,
				font,
				3,
				2,
				GridData.FILL_BOTH,
		...
		Button fSearchButton = createPushButton( comp,
				Messages.getString( "ScriptMainTab.button.text.search" ), //$NON-NLS-1$
				null );
	...
	{
		return DebugUI.getDefault( )
				.getImageRegistry( )
				.get( DebugUI.IMAGE_DEBUGGER_ICON_NAME );
	}
	...
	public void initializeFrom( ILaunchConfiguration configuration )
	{
		try
		{
			...
			fMainText.setText( configuration.getAttribute( ATTR_REPORT_FILE_NAME,
					getDefaultFileName( ) ) );
			fEngineHome.setText( configuration.getAttribute( ATTR_ENGINE_HOME,
					"" ) ); //$NON-NLS-1$
			fTempFolder.setText( configuration.getAttribute( ATTR_TEMP_FOLDER,
					getDefaultTempFolder( ) ) );

			bUseDefaultEngineHome.setSelection( configuration.getAttribute( ATTR_USE_DEFULT_ENGINE_HOME,
					true ) );
			fEngineHome.setEnabled( !bUseDefaultEngineHome.getSelection( ) );
			engineHomeSearchButton.setEnabled( !bUseDefaultEngineHome.getSelection( ) );
			engineHomeVariableButton.setEnabled( !bUseDefaultEngineHome.getSelection( ) );
			...
			bOpenTarget.setSelection( configuration.getAttribute( ATTR_OPEN_TARGET,
					false ) );
	...
	public void performApply( ILaunchConfigurationWorkingCopy configuration )
	{
		...
		if ( cmbOutputFormat.getItemCount( ) > 0
				&& cmbOutputFormat.getSelectionIndex( ) != -1 )
		{
			configuration.setAttribute( ATTR_TARGET_FORMAT,
					cmbOutputFormat.getItem( cmbOutputFormat.getSelectionIndex( ) ) );
		}

		configuration.setAttribute( ATTR_OPEN_TARGET,
				bOpenTarget.getSelection( ) );
		...
		{
			configuration.setAttribute( IReportLaunchConstants.ATTR_CLASSPATH,
					(List) null );
		}
		else
		{
