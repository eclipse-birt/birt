...
public class TableBorderLayer extends FreeformLayer
{
	 ...
	 * @param colIndex
	 * @param data
	 *            [style][width][color][rowIndex][colIndex].
	 */
	private void drawBottom( Graphics g, int rowIndex, int colIndex, int x,
			int y, int w, int h, int[] data )
	{
		...
		if ( colIndex > 0 )
		{
			int[] nLeft = borderData[( rowIndex + 1 )
					* ( 2 * colCount + 1 )
					+ colIndex
					- 1];
			nLeftWidth = nLeft[1];
		...
		{
			int[] nRight = borderData[( rowIndex + 1 )
					* ( 2 * colCount + 1 )
					+ colIndex
					+ 1];
		...
		{
			int[] nLeftTop = borderData[( rowIndex )
					* ( 2 * colCount + 1 )
					+ colCount
					+ colIndex];
		...
		{
			int[] nLeftBottom = borderData[( rowIndex + 1 )
					* ( 2 * colCount + 1 )
					+ colCount
					+ colIndex];
		...
		{
			int[] nRightTop = borderData[( rowIndex )
					* ( 2 * colCount + 1 )
					+ colCount
					+ colIndex
					+ 1];
		...
		{
			int[] nRightBottom = borderData[( rowIndex + 1 )
					* ( 2 * colCount + 1 )
					+ colCount
					+ colIndex
					+ 1];
		...
		if ( ( ( TableBorderCollisionArbiter.canExtend( data,
				nLeftWidth,
				nLeftStyle,
				nLeftBottomWidth,
				nLeftBottomStyle,
				...
				nLeftTopY,
				true,
				false ) || TableBorderCollisionArbiter.isBrotherWin( data,
				nLeftWidth,
				nLeftStyle,
				nLeftX,
				nLeftY,
				...
				nLeftTopY,
				true,
				false ) ) && ( nLeftTopX != -2 && nLeftBottomX != -2 ) )
				|| ( data[3] == -2 ) )
		{
			rx = ( colIndex == 0 ) ? ( x )
					: ( x - nlexWidth / 2 - nlexWidth % 2 );
			rw = ( colIndex == 0 ) ? ( w )
					: ( w + nlexWidth / 2 + nlexWidth % 2 );
		...
		if ( ( ( TableBorderCollisionArbiter.canExtend( data,
				...
				nRightBottomY,
				false,
				false ) ) && ( nRightTopX != -2 && nRightBottomX != -2 ) )
				|| ( data[3] == -2 ) )
		{
		...
		{
			rw -= ( ( colIndex == colCount - 1 ) ? ( nrexWidth )
					: ( nrexWidth / 2 + nrexWidth % 2 ) );
		}
		...
		else if ( data[3] == nLeftBottomX
				&& data[4] == nLeftBottomY
				&& data[3] == nRightBottomX
	 ...
	 *            [style][width][color][rowIndex][colIndex].
	 */
	private void drawRight( Graphics g, int rowIndex, int colIndex, int x,
			int y, int w, int h, int[] data )
	{
		if ( data[0] == 0 && data[1] == 0 )
		{
		...
		if ( rowIndex > 0 )
		{
			int[] nTop = borderData[( rowIndex - 1 )
					* ( 2 * colCount + 1 )
					+ colCount
					+ colIndex
					+ 1];
		...
		if ( rowIndex < rowCount - 1 )
		{
			int[] nBottom = borderData[( rowIndex + 1 )
					* ( 2 * colCount + 1 )
					+ colCount
					+ colIndex
					+ 1];
		...
		{
			int[] nTopLeft = borderData[rowIndex
					* ( 2 * colCount + 1 )
					+ colIndex];
		...
		if ( colIndex < colCount - 1 && rowIndex >= 0 )
		{
			int[] nTopRight = borderData[rowIndex
					* ( 2 * colCount + 1 )
					+ colIndex
					+ 1];
		...
		{
			int[] nBottomLeft = borderData[( rowIndex + 1 )
					* ( 2 * colCount + 1 )
					+ colIndex];
		...
		if ( colIndex < colCount - 1 && rowIndex <= rowCount - 1 )
		{
			int[] nBottomRight = borderData[( rowIndex + 1 )
					* ( 2 * colCount + 1 )
					+ colIndex
					+ 1];
		...
		if ( ( ( TableBorderCollisionArbiter.canExtend( data,
				...
				nTopRightY,
				true,
				true ) || TableBorderCollisionArbiter.isBrotherWin( data,
				nTopWidth,
				nTopStyle,
				nTopX,
				nTopY,
				...
				nTopRightY,
				true,
				true ) ) && ( nTopLeftX != -2 && nTopRightX != -2 ) )
				|| ( data[3] == -2 ) )
		{
			ry = ( rowIndex == 0 ) ? ( y )
					: ( y - ntexWidth / 2 - ntexWidth % 2 );
			rh = ( rowIndex == 0 ) ? ( h )
					: ( h + ntexWidth / 2 + ntexWidth % 2 );
		...
		if ( ( ( TableBorderCollisionArbiter.canExtend( data,
				...
				nBottomLeftY,
				false,
				true ) ) && ( nBottomLeftX != -2 && nBottomRightX != -2 ) )
				|| ( data[3] == -2 ) )
		{
		...
		{
			rh -= ( ( rowIndex == rowCount - 1 ) ? ( nbexWidth )
					: ( nbexWidth / 2 + nbexWidth % 2 ) );
		}
		...
		else if ( data[3] == nTopRightX
				&& data[4] == nTopRightY
				&& data[3] == nBottomRightX
		...
		Rectangle r = new Rectangle( x + w - data[1] / 2 - data[1] % 2,
				ry,
				data[1],
				rh );
	 ...
	 *            [style][width][color][rowIndex][colIndex].
	 */
	private void drawTop( Graphics g, int rowIndex, int colIndex, int x, int y,
			int w, int h, int[] data )
	{
		...
		if ( colIndex < colCount - 1 )
		{
			int[] nRight = borderData[( rowIndex )
					* ( 2 * colCount + 1 )
					+ colIndex
					+ 1];
			nRightWidth = nRight[1];
		...
		if ( rowIndex > 0 && colIndex >= 0 )
		{
			int[] nLeftTop = borderData[( rowIndex - 1 )
					* ( 2 * colCount + 1 )
					+ colCount
					+ colIndex];
			nLeftTopWidth = nLeftTop[1];
		...
		if ( rowIndex <= rowCount - 1 && colIndex >= 0 )
		{
			int[] nLeftBottom = borderData[( rowIndex )
					* ( 2 * colCount + 1 )
					+ colCount
					+ colIndex];
			nLeftBottomWidth = nLeftBottom[1];
		...
		if ( rowIndex > 0 && colIndex <= colCount - 1 )
		{
			int[] nRightTop = borderData[( rowIndex - 1 )
					* ( 2 * colCount + 1 )
					+ colCount
					+ colIndex
					+ 1];
		...
		if ( rowIndex <= rowCount - 1 && colIndex <= colCount - 1 )
		{
			int[] nRightBottom = borderData[( rowIndex )
					* ( 2 * colCount + 1 )
					+ colCount
					+ colIndex
					+ 1];
		...
		if ( TableBorderCollisionArbiter.canExtend( data,
				...
				true,
				false )
				|| TableBorderCollisionArbiter.isBrotherWin( data,
						nLeftWidth,
						nLeftStyle,
						nLeftX,
						nLeftY,
	 ...
	 *            [style][width][color][rowIndex][colIndex].
	 */
	private void drawLeft( Graphics g, int rowIndex, int colIndex, int x,
			int y, int w, int h, int[] data )
	{
		if ( data[0] == 0 && data[1] == 0 )
		{
