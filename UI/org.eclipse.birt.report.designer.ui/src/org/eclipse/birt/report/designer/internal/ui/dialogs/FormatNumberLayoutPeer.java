...
public class FormatNumberLayoutPeer extends FormatLayoutPeer
{
	...
	public FormatNumberLayoutPeer( int pageAlignment, boolean isFormStyle,
			boolean showLocale )
	{
	...
	@Override
	protected void fireFormatChanged( String newCategory, String newPattern,
			String newLocale )
	{
		fireFormatChanged( StyleHandle.NUMBER_FORMAT_PROP,
				newCategory,
				newPattern,
				newLocale );
	}
	...
	@Override
	protected void initiatePageLayout( String categoryStr, String patternStr,
			String localeStr )
	{
		initiatePageLayout( DesignChoiceConstants.NUMBER_FORMAT_TYPE_CUSTOM,
				categoryStr,
				patternStr,
				localeStr );
	...
	private void setFmtPatternFromControls( )
	{
		...
		else if ( category.equals( DesignChoiceConstants.NUMBER_FORMAT_TYPE_CUSTOM ) )
		{
			...
			pattern.setPattern( customFormatCodeTextBox.getText( ).length( ) == 0 ? null
					: customFormatCodeTextBox.getText( ) );
		}
	}
	...
	protected void createTable( Composite parent )
	{
		...
		new TableItem( customFormatTable, SWT.NONE ).setText( new String[]{
				...
				new NumberFormatter( FormatNumberPattern.getPatternForCategory( DesignChoiceConstants.NUMBER_FORMAT_TYPE_CURRENCY,
						locale ) ).format( DEFAULT_PREVIEW_NUMBER )
		} );
		new TableItem( customFormatTable, SWT.NONE ).setText( new String[]{
				formatAdapter.getDisplayName4Category( DesignChoiceConstants.NUMBER_FORMAT_TYPE_FIXED ),
				new NumberFormatter( FormatNumberPattern.getPatternForCategory( DesignChoiceConstants.NUMBER_FORMAT_TYPE_FIXED,
						locale ) ).format( DEFAULT_PREVIEW_NUMBER )
		} );
		new TableItem( customFormatTable, SWT.NONE ).setText( new String[]{
				formatAdapter.getDisplayName4Category( DesignChoiceConstants.NUMBER_FORMAT_TYPE_PERCENT ),
				new NumberFormatter( FormatNumberPattern.getPatternForCategory( DesignChoiceConstants.NUMBER_FORMAT_TYPE_PERCENT,
						locale ) ).format( DEFAULT_PREVIEW_NUMBER )
		} );
		new TableItem( customFormatTable, SWT.NONE ).setText( new String[]{
				formatAdapter.getDisplayName4Category( DesignChoiceConstants.NUMBER_FORMAT_TYPE_SCIENTIFIC ),
				new NumberFormatter( FormatNumberPattern.getPatternForCategory( DesignChoiceConstants.NUMBER_FORMAT_TYPE_SCIENTIFIC,
						locale ) ).format( DEFAULT_PREVIEW_NUMBER )
		} );
			...
			String localeName )
	{
		...
		else if ( category.equals( DesignChoiceConstants.NUMBER_FORMAT_TYPE_CUSTOM ) )
		{
			if ( StringUtil.isBlank( customPreviewTextBox.getText( ) )
					|| isValidNumber( customPreviewTextBox.getText( ) ) )
			{
				fmtStr = new NumberFormatter( patternStr, locale ).format( num );
			}
			else
