...
public class ExpressionProvider implements ISortableExpressionProvider,
	...
	protected static Operator[] OPERATORS_COMPARISON = new Operator[] {
			...
			new Operator(
					"<=", Messages.getString("ExpressionProvider.Operator.LessEqual")), //$NON-NLS-1$ //$NON-NLS-2$ 
			new Operator(
					"!=", Messages.getString("ExpressionProvider.Operator.NotEqual")), //$NON-NLS-1$ //$NON-NLS-2$ 
			new Operator(
	...
	protected static final Operator[] OPERATORS_COMPUTATIONAL = new Operator[] {
			...
			new Operator(
					"/", Messages.getString("ExpressionProvider.Operator.Divides")), //$NON-NLS-1$ //$NON-NLS-2$ 
			new Operator(
					"++ ", Messages.getString("ExpressionProvider.Operator.Inc")), //$NON-NLS-1$ //$NON-NLS-2$ 
			new Operator(
					"-- ", Messages.getString("ExpressionProvider.Operator.Dec")), //$NON-NLS-1$ //$NON-NLS-2$ 
	};

	/** Logical operators and their descriptions */
	protected static final Operator[] OPERATORS_LOGICAL = new Operator[] {
			new Operator(
					"&&", Messages.getString("ExpressionProvider.Operator.And")), //$NON-NLS-1$ //$NON-NLS-2$ 
			new Operator(
					"||", Messages.getString("ExpressionProvider.Operator.Or")), //$NON-NLS-1$ //$NON-NLS-2$ 
	...
	protected static final Operator[] OPERATORS_ON_BAR = new Operator[] {
			...
			OPERATOR_SEPARATOR,
			new Operator(
					"!", Messages.getString("ExpressionProvider.Operator.Not")),//$NON-NLS-1$ //$NON-NLS-2$
			new Operator(
					"=", Messages.getString("ExpressionProvider.Operator.Equals")),//$NON-NLS-1$ //$NON-NLS-2$
			...
			OPERATOR_SEPARATOR,
			new Operator(
					"&", Messages.getString("ExpressionProvider.Operator.BitAnd")),//$NON-NLS-1$ //$NON-NLS-2$
			new Operator(
					"|", Messages.getString("ExpressionProvider.Operator.BitOr")),//$NON-NLS-1$ //$NON-NLS-2$
			new Operator(
					"(", Messages.getString("ExpressionProvider.Operator.LeftBracket")),//$NON-NLS-1$ //$NON-NLS-2$
			new Operator(
					")", Messages.getString("ExpressionProvider.Operator.RightBracket")),//$NON-NLS-1$ //$NON-NLS-2$
	};
	...
	protected static final String DISPLAY_TEXT_LOGICAL = Messages
			.getString("ExpressionProvider.Operators.Logical"); //$NON-NLS-1$
	...
	public static final String DATASETS = Messages
			.getString("ExpressionProvider.Category.DataSets"); //$NON-NLS-1$
	public static final String OPERATORS = Messages
			.getString("ExpressionProvider.Category.Operators"); //$NON-NLS-1$
	public static final String COLUMN_BINDINGS = Messages
			.getString("ExpressionProvider.Category.ColumnBinding"); //$NON-NLS-1$
	public static final String CURRENT_CUBE = Messages
			.getString("ExpressionProvider.Category.DataCubes"); //$NON-NLS-1$
	...
	public static final String PARAMETERS = Messages
			.getString("ExpressionProvider.Category.Parameters"); //$NON-NLS-1$
	public static final String VARIABLES = Messages
			.getString("ExpressionProvider.Variables"); //$NON-NLS-1$
	public static final String VARIABLES_REPORT = Messages
			.getString("ExpressionProvider.Variables.Report"); //$NON-NLS-1$
	public static final String VARIABLES_PAGE = Messages
			.getString("ExpressionProvider.Variables.Page"); //$NON-NLS-1$
	public static final String NATIVE_OBJECTS = Messages
			.getString("ExpressionProvider.Category.NativeObjects");//$NON-NLS-1$
	public static final String BIRT_OBJECTS = Messages
			.getString("ExpressionProvider.Category.BirtObjects");//$NON-NLS-1$

	protected static final String ALL = Messages
			.getString("ExpressionProvider.Label.All"); //$NON-NLS-1$

	private static final String TOOLTIP_BINDING_PREFIX = Messages
			.getString("ExpressionProvider.Tooltip.ColumnBinding"); //$NON-NLS-1$
	...
	public Operator[] getOperators() {
		if (adapterProvider != null) {
			...
			if (cats != null) {
				...
				System.arraycopy(OPERATORS_ON_BAR, 0, rt, 0,
						OPERATORS_ON_BAR.length);
				System.arraycopy(cats, 0, rt, OPERATORS_ON_BAR.length,
						cats.length);
	...
	public Object[] getCategory() {
		...
		if (filterList != null && !filterList.isEmpty()) {
			for (Iterator<ExpressionFilter> iter = filterList.iterator(); iter
					.hasNext();) {
	...
	protected List<Object> getCategoryList() {
		...
		if (elementHandle.getModuleHandle() instanceof ReportDesignHandle
				&& ((ReportDesignHandle) elementHandle.getModuleHandle())
						.getPageVariables().size() > 0) {
			for (VariableElementHandle veh : ((ReportDesignHandle) elementHandle
					.getModuleHandle()).getPageVariables()) {
				if (DesignChoiceConstants.VARIABLE_TYPE_REPORT.equals(veh
						.getType())) {
		...
		if (parent instanceof String) {
			...
			Arrays.sort(result, new Comparator<Object>() {
				...
				public int compare(Object o1, Object o2) {
					if (o1 instanceof ILocalizableInfo[]
							&& o2 instanceof ILocalizableInfo[]) {
	...
	protected List<Object> getChildrenList(Object parent) {
		...
		if (parent instanceof Object[]) {
			...
			if (VARIABLES_REPORT.equals(parent)) {
				for (VariableElementHandle variable : ((ReportDesignHandle) elementHandle
						.getModuleHandle()).getPageVariables()) {
					if (DesignChoiceConstants.VARIABLE_TYPE_REPORT
			...
			// }
				...
				if (COLUMN_BINDINGS.equals(parent)) {
					...
					if (elementHandle instanceof TabularMeasureHandle) {
						cube = (CubeHandle) ((TabularMeasureHandle) elementHandle)
								.getContainer().getContainer();
					} else {
						cube = ((ReportItemHandle) elementHandle).getCube();
					}
					Object nodeProviderAdapter = ElementAdapterManager
							.getAdapter(cube, INodeProvider.class);
					if (nodeProviderAdapter != null) {
						return Arrays
								.asList(((INodeProvider) nodeProviderAdapter)
										.getChildren(cube));
					}
				} else if (BIRT_OBJECTS.equals(parent)) {
					childrenList.addAll(getClassList(false));
					try {
						IScriptFunctionCategory[] categorys = FunctionProvider
								.getCategories();
			...
			for (Iterator iter = DEUtil.getMembers(classInfo).iterator(); iter
					.hasNext();) {
				childrenList.add(new ILocalizableInfo[] { classInfo,
						(IMemberInfo) iter.next() });
			}
			for (Iterator iter = DEUtil.getMethods(classInfo, true).iterator(); iter
					.hasNext();) {
			...
			try {
				IScriptFunction[] functions = ((IScriptFunctionCategory) parent)
						.getFunctions();
			...
			if (parent instanceof ReportItemHandle) {
				...
				if (parent == elementHandle) {
					ReportItemHandle root = DEUtil
							.getBindingRoot(elementHandle);
					DesignElementHandle container = elementHandle;
					while (root != null && container != null
							&& container != root) {
						...
						while (iter1.hasNext()) {
							...
							if (!nameList.contains(column.getName())) {
								childrenList
										.add(new InheritedComputedColumnHandle(
												column));
								nameList.add(column.getName());
							}
						}
			...
			Object nodeProviderAdapter = ElementAdapterManager.getAdapter(
					parent, INodeProvider.class);
			if (nodeProviderAdapter != null) {
	...
	public String getDisplayText(Object element) {
		Object nodeProviderAdapter = ElementAdapterManager.getAdapter(element,
				INodeProvider.class);
		if (nodeProviderAdapter != null) {
			return ((INodeProvider) nodeProviderAdapter)
		...
		if (element instanceof Object[]) {
			if (element instanceof Operator[]) {
				...
				if (info instanceof IMethodInfo) {
					...
					int argIndex = (((ILocalizableInfo[]) element).length > 2) ? (((IIndexInfo) ((ILocalizableInfo[]) element)[2])
							.getIndex()) : 0;
					int idx = -1;

					Iterator argumentListIter = method.argumentListIterator();
					while (argumentListIter.hasNext()) {
						IArgumentInfoList arguments = (IArgumentInfoList) argumentListIter
								.next();
						...
						for (Iterator iter = arguments.argumentsIterator(); iter
								.hasNext();) {
			...
			String categoreName = ((IScriptFunctionCategory) element).getName() == null ? Messages
					.getString("ExpressionTreeSupport.Category.Global") //$NON-NLS-1$
	...
	public Image getImage(Object element) {
		...
		if (element instanceof Operator) {
			...
			return ReportPlatformUIImages
					.getImage(IReportGraphicConstants.ICON_INHERIT_COLUMN);
		} else if (element instanceof DesignElementHandle) {
