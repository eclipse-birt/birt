			...
			if ( div % divsPerMajorMark == 0 )
			{
				...
				if ( isHorizontal( ) )
				{
					Dimension numSize = FigureUtilities.getStringExtents( num,
							getFont( ) );
					/*
					 * FigureUtilities is increasing the width reported by GC by
					 * 1, so we remove it here. Also, if the actual width (the
					 * one reported by the GC) is even, we want to increase it
					 * by 1. This will ensure that when marks marks are erased
					 * because they are too close to the number, they are erased
