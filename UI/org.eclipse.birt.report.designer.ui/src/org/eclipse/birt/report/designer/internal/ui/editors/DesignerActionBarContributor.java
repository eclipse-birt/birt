...
public class DesignerActionBarContributor extends org.eclipse.gef.ui.actions.ActionBarContributor {
	...
	private static final RegisterAction[] elementActions = new RegisterAction[] {
			...
			null, new RegisterAction(MergeAction.ID, Messages.getString("DesignerActionBarContributor.element.merge")), //$NON-NLS-1$
			new RegisterAction(SplitAction.ID, Messages.getString("DesignerActionBarContributor.element.split")), //$NON-NLS-1$
			new RegisterAction(CopyCellContentsContextAction.ID,
					Messages.getString("CopyCellContentsContextAction.actionText")), //$NON-NLS-1$
			null,
			new RegisterAction(CreatePlaceHolderPartAction.ID, Messages.getString("CreatePlaceHolderAction.text")), //$NON-NLS-1$
			new RegisterAction(RevertToReportItemPartAction.ID, Messages.getString("RevertToReportItemAction.text")), //$NON-NLS-1$
			null, new RegisterAction(CreateChartAction.ID, Messages.getString("CreateChartAction.text")), //$NON-NLS-1$
			null,
			new RegisterAction(AddGroupAction.ID, Messages.getString("DesignerActionBarContributor.element.group")), //$NON-NLS-1$ ,
	};
	...
	protected void buildActions() {
		...
		addRetargetAction(
				new RetargetAction(AddStyleAction.ID, Messages.getString("DesignerActionBarContributor.style.new"))); //$NON-NLS-1$
		...
		registerActions(new RegisterAction[] { new RegisterAction(GEFActionConstants.TOGGLE_RULER_VISIBILITY,
				Messages.getString("DesignerActionBarContributor.menu.element-showRuler"), //$NON-NLS-1$
				IAction.AS_CHECK_BOX) });
		registerActions(new RegisterAction[] { new RegisterAction(ToggleMarginVisibilityAction.ID,
				ToggleMarginVisibilityAction.LABEL, IAction.AS_CHECK_BOX) });
		...
		addRetargetAction(new RetargetAction(InsertRelativeTimePeriodAction.ID, InsertRelativeTimePeriodAction.TEXT));
	}
	...
	private RegisterAction[] getInsertElementActions() {
		if (insertElementActions == null) {
			...
			if (!extensionPoints.isEmpty()) {
				for (int k = 0; k < extensionPoints.size(); k++) {
					...
					RegisterAction extAction = new RegisterAction(extension.getName(),
							menuLabel == null ? extension.getDisplayName() : menuLabel);

					elementSorter.addElement(
							(String) point.getAttribute(IExtensionConstants.ATTRIBUTE_PALETTE_CATEGORY), extAction);
				}
			}
			...
			insertElementActions = new RegisterAction[insertActions.length + actions.size()];
	...
	public void contributeToToolBar(IToolBarManager tbm) {
		...
		if (getPage() != null && getPage().getActivePart() != null) {
			zoomComboContributionItem
					.setZoomManager((ZoomManager) getPage().getActivePart().getAdapter(ZoomManager.class));
		}
	...
	public void contributeToMenu(IMenuManager menubar) {
		...
		// Insert Menu
		MenuManager insertMenu = new MenuManager(Messages.getString("DesignerActionBarContributor.menu.insert"), //$NON-NLS-1$
				M_INSERT);
		...
		// Element Menu
		MenuManager elementMenu = new MenuManager(Messages.getString("DesignerActionBarContributor.menu.element"), //$NON-NLS-1$
				M_ELEMENT);
	...
	private void contributeElementMenu(MenuManager elementMenu) {
		...
		MenuManager insertGroupMenu = new MenuManager(Messages.getString("DesignerActionBarContributor.element.group"), //$NON-NLS-1$
				InsertGroupMenuAction.ID);
	...
	private void contributeActionsToMenu(IMenuManager menu, RegisterAction[] actions) {
		for (int i = 0; i < actions.length; i++) {
			if (actions[i] != null) {
	...
	private void contributeStyleMenu(MenuManager newMenu) {
		...
		// edit
		MenuManager editStyleMenu = new MenuManager(
				Messages.getString("DesignerActionBarContributor.menu.style-rule-edit")); //$NON-NLS-1$
		...
		// apply
		MenuManager applyStyleMenu = new MenuManager(
				Messages.getString("DesignerActionBarContributor.menu.style-rule-apply")); //$NON-NLS-1$
		...
		// delete
		MenuManager delStyleMenu = new MenuManager(
				Messages.getString("DesignerActionBarContributor.menu.style-rule-remove")); //$NON-NLS-1$
