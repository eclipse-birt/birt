...
public class EditBindingAction extends InsertRowAction
{
    ...
    public void run()
    {
        //Get the first item in the list and pass the model object to the dialog
        TableEditPart editPart = getTableEditPart();
        if(editPart != null)
        {
            CommandStack stack = SessionHandleAdapter.getInstance( ).getReportDesign( ).getActivityStack( );
            ...
            DataBindingDialog dialog = new DataBindingDialog(PlatformUI.getWorkbench( )
                    .getDisplay( )
                    .getActiveShell( ), (DesignElementHandle) editPart.getModel() );

            boolean commit =  (dialog.open() == Dialog.OK);
            String newDataSetName = processor.getStringValue(items);
            if(currentDataSetName != null && !currentDataSetName.equals(newDataSetName))
            {
                commit = MessageDialog.openQuestion(null, Messages.getString("dataBinding.title.changeDataSet"), Messages.getString("dataBinding.message.changeDataSet")); //$NON-NLS-1$
            }
            if(commit)
            {
                stack.commit();
