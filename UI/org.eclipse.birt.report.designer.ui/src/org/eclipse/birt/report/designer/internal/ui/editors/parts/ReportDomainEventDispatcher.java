...
public class ReportDomainEventDispatcher extends DomainEventDispatcher
{
	...
	public void dispatchKeyTraversed( TraverseEvent e )
	{
		...
		if ( e.detail == SWT.TRAVERSE_TAB_NEXT )
		{
			...
			{
				nextFigure = focusManager.getNextFocusableFigure(
						getRoot( ), focusOwner );
			}
			else
			{
				nextFigure = focusManager.getNextFocusableFigureInSameOrder(
						getRoot( ), focusOwner );
			}
		}
		else if ( e.detail == SWT.TRAVERSE_TAB_PREVIOUS )
		{
			...
			if ((e.stateMask & SWT.CTRL) > 0)
			{
				nextFigure = focusManager
				.getPreviousFocusableFigure( getRoot( ), focusOwner );
			}
			else
			{
				nextFigure = focusManager.getPreviousFocusableFigureInSameOrder(
						getRoot( ), focusOwner );
			}
