...
public class CascadingParametersDialog extends BaseDialog {
	...
	private static final String CHOICE_NONE = Messages
	...
	private static final String CHOICE_NULL_VALUE = Messages
			.getString("CascadingParametersDialog.Choice.NullValue"); //$NON-NLS-1$

	private static final String CHOICE_BLANK_VALUE = Messages
			.getString("CascadingParametersDialog.Choice.BlankValue"); //$NON-NLS-1$
	...
	private static final String LABEL_VALUES = Messages
			.getString("CascadingParametersDialog.Label.values"); //$NON-NLS-1$
	...
	private static final String LABEL_PARAMETERS = Messages
			.getString("CascadingParametersDialog.Label.parameters"); //$NON-NLS-1$
	...
	private static final String LABEL_PARAM_NAME = Messages
			.getString("CascadingParametersDialog.Label.param.name"); //$NON-NLS-1$

	private static final String LABEL_LIST_LIMIT = Messages
			.getString("CascadingParametersDialog.Label.listLimit"); //$NON-NLS-1$

	private static final String LABEL_DATA_TYPE = Messages
			.getString("CascadingParametersDialog.Label.dataType"); //$NON-NLS-1$

	private static final String LABEL_DISPLAY_TYPE = Messages
			.getString("CascadingParametersDialog.Label.displayType"); //$NON-NLS-1$
	...
	private static final String LABEL_HELP_TEXT = Messages
			.getString("CascadingParametersDialog.Label.helpText"); //$NON-NLS-1$

	private static final String LABEL_FORMAT_AS = Messages
			.getString("CascadingParametersDialog.Label.formatAs"); //$NON-NLS-1$
	...
	private static final String BUTTON_IS_REQUIRED = Messages
			.getString("CascadingParametersDialog.Button.isRequired"); //$NON-NLS-1$
	...
	private static final String RADIO_SINGLE = Messages
			.getString("CascadingParametersDialog.Radio.Single"); //$NON-NLS-1$

	private static final String RADIO_MULTIPLE = Messages
			.getString("CascadingParametersDialog.Radio.Mutli"); //$NON-NLS-1$

	private static final String COLUMN_NAME = Messages
			.getString("CascadingParametersDialog.Label.column.name"); //$NON-NLS-1$

	private static final String COLUMN_DATA_SET = Messages
			.getString("CascadingParametersDialog.Label.column.dataSet"); //$NON-NLS-1$

	private static final String COLUMN_VALUE = Messages
			.getString("CascadingParametersDialog.Label.column.value"); //$NON-NLS-1$
	...
	private static final double DEFAULT_PREVIEW_NUMBER = Double
			.parseDouble("1234.56"); //$NON-NLS-1$
	...
	private String PROPERTY_LABEL_STRING[] = { LABEL_PARAM_NAME,
			LABEL_GROUP_PROMPT_TEXT, LABEL_PARAMTER_PROMPT_TEXT,
			LABEL_DATA_TYPE, LABEL_DISPLAY_TYPE, LABEL_DEFAULT_VALUE };

	CLabel errorMessageLine;

	private String OPTION_LABEL_STRING[] = { LABEL_HELP_TEXT, LABEL_FORMAT_AS,
			LABEL_LIST_LIMIT };
	...
	protected Control createDialogArea(Composite parent) {
		...
		ScrolledComposite sc = new ScrolledComposite((Composite) super
				.createDialogArea(parent), SWT.H_SCROLL | SWT.V_SCROLL);
		...
		maxStrLengthProperty = getMaxStrLength(PROPERTY_LABEL_STRING, composite);
	...
	private String validateDefaultValue() {
		...
		String tempType = DATA_TYPE_CHOICE_SET.findChoiceByDisplayName(
				dataTypeChooser.getText()).getName();
			...
			try {
				...
				if (!((DesignChoiceConstants.PARAM_TYPE_STRING
					...
					if (DesignChoiceConstants.PARAM_TYPE_DATETIME
						...
						tempDefaultValue = convertToStandardFormat(DataTypeUtil
					...
					ParameterValidationUtil.validate(getSelectedDataType(),
							STANDARD_DATE_TIME_PATTERN, tempDefaultValue,
							ULocale.getDefault());
	...
	private String convertToStandardFormat(Date date) {
		...
		return new DateFormatter(STANDARD_DATE_TIME_PATTERN, ULocale
	...
	private void updateDataSets() {
		ArrayList elementsList = new ArrayList(inputParameterGroup
	...
	private void createDynamicParamsPart(Composite parent) {
		...
		table = new Table(comp, SWT.FULL_SELECTION | SWT.HIDE_SELECTION
				| SWT.BORDER);
		...
		int[] columnWidths = new int[] { 140, 110, 130, 130, };
		String[] columns = new String[] { COLUMN_NAME, COLUMN_DATA_SET,
				COLUMN_VALUE, COLUMN_DISPLAY_TEXT };
		...
		valueTable.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				Object param = ((StructuredSelection) selection)
						.getFirstElement();
				if (param != selectedParameter) {
					if (param instanceof ScalarParameterHandle) {
						try {
							...
							valueTable.setSelection(new StructuredSelection(
									selectedParameter));
						}
		...
		addBtn.setText(Messages
				.getString("CascadingParametersDialog.Button.Add")); //$NON-NLS-1$
		addBtn.addSelectionListener(new SelectionAdapter() {

			public void widgetSelected(SelectionEvent e) {
				AddEditCascadingParameterDialog dialog = new AddEditCascadingParameterDialog(
						Messages
								.getString("CascadingParametersDialog.Title.AddCascadingParameter")); //$NON-NLS-1$
				if (dialog.open() != Dialog.OK) {
					return;
				}
				try {
					inputParameterGroup.getParameters().add(
							dialog.getParameter());
				...
				refreshValueTable();
				valueTable.setSelection(new StructuredSelection(dialog
						.getParameter()));
				updateButtons();
			}
		});
		...
		editBtn.setText(Messages
				.getString("CascadingParametersDialog.Button.Edit")); //$NON-NLS-1$
		...
		delBtn.setText(Messages
				.getString("CascadingParametersDialog.Button.Delete")); //$NON-NLS-1$
	...
	protected void editParameter(ScalarParameterHandle param) {
		...
		CommandStack cmdStack = SessionHandleAdapter.getInstance()
				.getReportDesignHandle().getCommandStack();
		cmdStack
				.startTrans(Messages
						.getString("CascadingParametersDialog.Title.EditCascadingParameter")); //$NON-NLS-1$
	...
	private void createPropertiesPart(Composite parent) {
		...
		createLabel(propertiesGroup, LABEL_PARAMTER_PROMPT_TEXT,
		...
		dataTypeChooser.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (selectedParameter != null) {
					changeDataType(DATA_TYPE_CHOICE_SET
							.findChoiceByDisplayName(dataTypeChooser.getText())
							.getName());
					try {
						selectedParameter.setDataType(DATA_TYPE_CHOICE_SET
		...
		displayTypeChooser
				.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		displayTypeChooser.setItems(new String[] { DISPLAY_NAME_CONTROL_LIST,
				DISPLAY_NAME_CONTROL_COMBO });
				...
				if (selectedParameter != null) {
					try {
						...
						if (PARAM_CONTROL_COMBO.equals(newControlType)) {
							...
							selectedParameter
									.setProperty(
											ScalarParameterHandle.MUCH_MATCH_PROP,
											null);
						}
	...
	private void createOptionsPart(Composite parent) {
		...
		formatField = new Text(formatArea, SWT.BORDER | SWT.SINGLE
				| SWT.READ_ONLY);
		...
		previewLable = new Label(preview, SWT.CENTER | SWT.HORIZONTAL
				| SWT.VIRTUAL);
		...
		isRequired.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (selectedParameter != null) {
					try {
						selectedParameter.setIsRequired(isRequired
								.getSelection());
					} catch (SemanticException e1) {
						ExceptionHandler.handle(e1);
					}
	...
	protected boolean initDialog() {
		...
		promptTextEditor.setText(UIUtil.convertToGUIString(inputParameterGroup
				.getPromptText()));

		if (DesignChoiceConstants.DATA_SET_MODE_MULTIPLE
				.equals(inputParameterGroup.getDataSetMode())) {
	...
	private void validateDefaultValues() throws BirtException {
		...
		for (Iterator iter = elementsList.iterator(); iter.hasNext();) {
			...
			if (!((DesignChoiceConstants.PARAM_TYPE_STRING.endsWith(tempType)) || (DesignChoiceConstants.PARAM_TYPE_BOOLEAN
					.endsWith(tempType)))) {
	...
	protected void okPressed() {
		try {
			...
			inputParameterGroup.setName(UIUtil.convertToModelString(
					cascadingNameEditor.getText(), true));
			inputParameterGroup.setPromptText(promptTextEditor.getText());

			if (isSingle()) {
				inputParameterGroup
						.setDataSetMode(DesignChoiceConstants.DATA_SET_MODE_SINGLE);
			} else {
				inputParameterGroup
						.setDataSetMode(DesignChoiceConstants.DATA_SET_MODE_MULTIPLE);
			}
		} catch (BirtException e) {
			ExceptionHandler.handle(e);
			refreshParameterProperties();
	...
	private void deleteRow() {
		...
		ScalarParameterHandle choice = (ScalarParameterHandle) ((IStructuredSelection) valueTable
		...
		if (index != -1) {
			selection = new StructuredSelection(valueTable.getTable().getItem(
					index).getData());
		} else {
	...
	private String[] getDataSetColumns(ScalarParameterHandle handle,
		...
		for (Iterator iter = metaHandle.getResultSet().iterator(); iter
				.hasNext();) {
			ResultSetColumnHandle columnHandle = (ResultSetColumnHandle) iter
					.next();
	...
	private IStructuredContentProvider contentProvider = new IStructuredContentProvider() {
		...
		public Object[] getElements(Object inputElement) {
			ArrayList elementsList = new ArrayList(inputParameterGroup
					.getParameters().getContents());
			for (Iterator iter = elementsList.iterator(); iter.hasNext();) {
				ScalarParameterHandle handle = (ScalarParameterHandle) iter
						.next();

				String[] columns = getDataSetColumns(handle, false);
				boolean found = false;
				for (int i = 0; i < columns.length; i++) {
					if (DEUtil.getColumnExpression(columns[i]).equals(
							handle.getValueExpr())) {
						found = true;
						break;
					}
	...
	protected int getTableIndex(Object element) {
		Object[] input = ((IStructuredContentProvider) valueTable
				.getContentProvider()).getElements(valueTable.getInput());
	...
	private void refreshParameterProperties() {
		...
		dataTypeChooser.setText(DATA_TYPE_CHOICE_SET.findChoice(
				selectedParameter.getDataType()).getDisplayName());
		...
		helpTextEditor.setText(UIUtil.convertToGUIString(selectedParameter
				.getHelpText()));

		if (selectedParameter.getPropertyHandle(
	...
	private void clearDefaultValueText() {
		...
		if (textValue != null
				&& (textValue.equals(CHOICE_NULL_VALUE) || textValue
	...
	private void initFormatField(String selectedDataType) {
		...
		if (choiceSet == null) {
			...
			if ((formatCategroy != null && choiceSet.findChoice(formatCategroy) == null)
					|| (selectedParameter.getCategory() == null && selectedParameter
							.getPattern() == null)) {
				if (DesignChoiceConstants.PARAM_TYPE_STRING
						.equals(selectedDataType)) {
	...
	private String getInputDisplayName() {
		...
		if (DesignChoiceConstants.PARAM_CONTROL_LIST_BOX
				.equals(selectedParameter.getControlType())) {
	...
	private void doPreview(String pattern) {
		...
		if (DesignChoiceConstants.PARAM_TYPE_STRING.equals(type)) {
			formatStr = new StringFormatter(pattern, ULocale.getDefault())
					.format(DEFAULT_PREVIEW_STRING);
		} else if (DesignChoiceConstants.PARAM_TYPE_DATETIME.equals(type)) {
			pattern = pattern
			...
			formatStr = new DateFormatter(pattern).format(new Date());
				...
				|| DesignChoiceConstants.PARAM_TYPE_FLOAT.equals(type)) {
			formatStr = new NumberFormatter(pattern)
					.format(DEFAULT_PREVIEW_NUMBER);
		} else if (DesignChoiceConstants.PARAM_TYPE_INTEGER.equals(type)) {
			formatStr = new NumberFormatter(pattern)
					.format(DEFAULT_PREVIEW_INTEGER_NUMBER);
		}
	...
	private String getColumnName(ScalarParameterHandle handle, String column) {
		...
		if (cmdh != null) {
			for (Iterator iter = cmdh.getResultSet().iterator(); iter.hasNext();) {
				ResultSetColumnHandle element = (ResultSetColumnHandle) iter
						.next();
	...
	private boolean matchDataType(ScalarParameterHandle handle,
		...
		if (DesignChoiceConstants.PARAM_TYPE_STRING.equals(type)
				|| DesignChoiceConstants.COLUMN_DATA_TYPE_ANY
						.equals(columnHandle.getDataType())) {
			return true;
			...
			return DesignChoiceConstants.COLUMN_DATA_TYPE_INTEGER
					.equals(columnHandle.getDataType());
		} else if (DesignChoiceConstants.PARAM_TYPE_DATE.equals(columnHandle
				.getDataType())) {
			return DesignChoiceConstants.COLUMN_DATA_TYPE_DATE.equals(type);
		} else if (DesignChoiceConstants.PARAM_TYPE_TIME.equals(columnHandle
				.getDataType())) {
	...
	private void saveParameterProperties() throws SemanticException {
		if (selectedParameter != null) {
			selectedParameter.setPromptText(UIUtil.convertToModelString(
					promptText.getText(), false));
			selectedParameter.setHelpText(UIUtil.convertToModelString(
					helpTextEditor.getText(), true));
			if (selectedParameter.getDataType().equals(
					DesignChoiceConstants.PARAM_TYPE_STRING)) {
				...
				} else if (defaultValueChooser.getText().equals(
						CHOICE_BLANK_VALUE)) {
					selectedParameter.setDefaultValue(""); //$NON-NLS-1$
				} else {
					selectedParameter.setDefaultValue(defaultValueChooser
							.getText());
				}
			} else {
				selectedParameter.setDefaultValue(UIUtil.convertToModelString(
						defaultValueChooser.getText(), true));
			}

			if (StringUtil.isBlank(listLimit.getText())) {
				selectedParameter.setProperty(
						ScalarParameterHandle.LIST_LIMIT_PROP, null);
			} else {
				selectedParameter.setListlimit(Integer.parseInt(listLimit
						.getText()));
			}
			// selectedParameter.setAllowNull( allowNull.getSelection( ) );
			selectedParameter.setIsRequired(isRequired.getSelection());
			selectedParameter.setName(UIUtil.convertToModelString(
					paramNameEditor.getText(), true));
	...
	class AddEditCascadingParameterDialog extends BaseDialog {
		...
		public final String DATASET_NONE = Messages
				.getString("CascadingParametersDialog.items.None"); //$NON-NLS-1$

		public final String DISPLAY_TEXT_NONE = Messages
				.getString("CascadingParametersDialog.items.None"); //$NON-NLS-1$
		...
		protected void updateButtons() {
			...
			if ((dataset.getItemCount() == 1 && dataset.getItem(0).equals(
					DATASET_NONE))
		...
		protected Control createDialogArea(Composite parent) {
			...
			labelName.setText(Messages
					.getString("AddEditCascadingParameterDialog.label.name")); //$NON-NLS-1$
			...
			labelDataset
					.setText(Messages
							.getString("AddEditCascadingParameterDialog.label.dataset")); //$NON-NLS-1$
			...
			dataset.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					if (dataset.getSelectionIndex() == -1
							|| (dataset.getItemCount() == 1 && dataset
									.getText().equals(DATASET_NONE))) {
						...
						try {
							...
							if (parameter.getDataSet() != null) {
								if (getFirstParameter() == null
									...
									inputParameterGroup.setDataSet(parameter
											.getDataSet());
								}
			...
			labelValue.setText(Messages
					.getString("AddEditCascadingParameterDialog.label.value")); //$NON-NLS-1$
			...
			value.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					try {
						parameter.setValueExpr(DEUtil.getColumnExpression(value
								.getText()));
						if (dataTypes.length == value.getItemCount()
								&& value.getSelectionIndex() > -1) {
							parameter.setDataType(dataTypes[value
									.getSelectionIndex()]);
						}
					} catch (SemanticException e1) {
					}
			...
			labelDisplayText
					.setText(Messages
							.getString("AddEditCascadingParameterDialog.label.displaytext")); //$NON-NLS-1$
				...
				public void widgetSelected(SelectionEvent e) {
					try {
						...
						// DISPLAY_TEXT_NONE
						{
							...
							parameter
									.setLabelExpr(DEUtil
											.getColumnExpression(displayText
													.getText()));
						}
		...
		protected boolean initDialog() {
			...
			if (isSingle()
				...
				if (inputParameterGroup.getDataSet() != null) {
					dataset.select(dataset.indexOf(inputParameterGroup
							.getDataSet().getName()));
		...
		private String[] getDataSetColumns(ScalarParameterHandle handle,
			...
			for (Iterator iter = metaHandle.getResultSet().iterator(); iter
				...
				ResultSetColumnHandle columnHandle = (ResultSetColumnHandle) iter
						.next();
				valueList.add(columnHandle.getColumnName());
				dataTypeList.add(ModuleUtil
						.convertColumnTypeToParamType(columnHandle
								.getDataType()));
			}
		...
		private void updateComboFromDataSet() {
			...
			// ( newParameter == null )
			{
				...
				try {
					parameter
							.setControlType(DesignChoiceConstants.PARAM_CONTROL_LIST_BOX);
					parameter
				...
				// name.setText( parameter.getName( ) );
				if (getFirstParameter() != null
						&& !(multiDataSet.isEnabled() && multiDataSet
								.getSelection())) {
					try {
						parameter.setDataSetName(inputParameterGroup
								.getDataSet().getName());
						value.setItems(getDataSetColumns(parameter, true));
						displayText
								.setItems(getDataSetColumns(parameter, false));
						value.setEnabled(true);
						displayText.setEnabled(true);
					} catch (SemanticException e) {
