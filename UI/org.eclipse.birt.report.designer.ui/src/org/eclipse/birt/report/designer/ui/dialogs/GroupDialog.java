	...
	private void createFieldArea(Composite parent) {
		...
		intervalRange.addVerifyListener(new VerifyListener() {
			...
			public void verifyText(VerifyEvent event) {
				...
				StringBuilder newString = new StringBuilder().append(inputtedText.substring(0, beginIndex));

				newString.append(event.text);
				newString.append(inputtedText.substring(endIndex));

				event.doit = false;

				try {
					double value = format.parse(newString.toString()).doubleValue();
					if (value >= 0) {
						event.doit = true;
					}
				} catch (Exception e) {
