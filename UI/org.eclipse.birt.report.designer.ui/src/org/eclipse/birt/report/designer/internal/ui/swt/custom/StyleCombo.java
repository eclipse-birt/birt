...
public final class StyleCombo extends Composite
{
	...
	public Point computeSize( int wHint, int hHint, boolean changed )
	{
		...
		Point labelSize = label.computeSize( labelExtent.x,
				labelExtent.y,
				changed );
		...
		height = Math.max( hHint, Math.max( labelSize.y, arrowSize.y )
				+ 2
				* borderWidth );
		...
		width = Math.max( computeSizeWidth, labelSize.x
				+ arrowSize.x
				+ 2
				* borderWidth );
	...
	private void initImages( )
	{
		...
		imageWidth = size.x
				- 10
				- UIUtil.getMaxStringWidth( (String[]) provider.getDisplayItems( ),
						this );
		...
		for ( int i = 0; i < provider.getItems( ).length; i++ )
		{
			...
			item.setImage( 0, provider.getImage( provider.getItems( )[i],
					imageWidth,
					imageHeight,
					table,
					this ) );
	...
	private void disposeImages( )
	{
		...
		{
			if ( treeItems[i].getImage( ) != null
					&& !treeItems[i].getImage( ).isDisposed( ) )
