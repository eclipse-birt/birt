...
public class RowDragTracker extends TableDragGuideTracker {
	...
	private void resizeFixRow(int value, int start, int end) {
		...
		try {
			MetricUtility.updateDimension(((RowHandle) row).getHeight(), rowHeight + getTrueValue(value));
		} catch (SemanticException e) {
			ExceptionHandler.handle(e);
		}
	}
	...
	protected Dimension getDragWidth(int startNumber, int endNumber) {
		...
		return new Dimension(part.getMinHeight(startNumber) - getRowHeight(start), Integer.MAX_VALUE);
	...
	protected void fitResize() {
		...
		try {
			if (part.getTableAdapter().isSupportHeight()) {
				...
				DimensionValue dimensionValue = new DimensionValue(tbWidth, getDefaultUnits());
