...
public class InputParameterDialog extends BaseDialog {
	...
	private Composite createParamSection(ScalarParameter param, Composite parent) {
		...
		helper.createControl(container, param, paramValues.get(param.getHandle().getName()));
...
abstract class AbstractParameterControlHelper implements IParameterControlHelper {
	...
	private boolean validateRequiredItem() {
		if (isRequired) {
			...
			if (paramterValue == null || (paramterValue instanceof String && StringUtil.isBlank((String) paramterValue))
					|| (paramterValue instanceof Object[] && ((Object[]) paramterValue).length == 0)) {
				MessageDialog.openError(parent.getShell(), "Error", Messages.getFormattedString(
						"InputParameterDialog.err.requiredParam", new String[] { paramterHandleName })); // $NON-NLS-1$
																											// //$NON-NLS-2$
				return false;
			}
		}
			...
			try {
				...
				MessageDialog.openError(parent.getShell(),
						...
						Messages.getFormattedString("InputParameterDialog.err.invalidValue", //$NON-NLS-1$
								new String[] { paramValue.toString(), parameter.getHandle().getDataType() }));
				return false;
			}
		}
		return true;
	...
	private void init(Object para, Object lastValue) {
		...
		isStringType = parameter.getHandle().getDataType().equals(DesignChoiceConstants.PARAM_TYPE_STRING);
	...
	private void createControlLabel() {
		...
		controlLabel.setText(parameter.getHandle().getDisplayLabel() + (isRequired ? ": *" : ":")); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	protected String formatString(String str, ScalarParameter para) {
		...
		try {
			if (DesignChoiceConstants.PARAM_TYPE_STRING.equals(type)) {
				...
				formatPattern = formatPattern.equals(DesignChoiceConstants.DATE_FORMAT_TYPE_UNFORMATTED)
...
class StaticTextParameterControlHelper extends AbstractParameterControlHelper {
	...
	private void createText() {
		input = new Text(parent, parameter.getHandle().isConcealValue() ? SWT.BORDER | SWT.PASSWORD : SWT.BORDER);
...
class CheckBoxParameterControlHelper extends AbstractParameterControlHelper {
	...
	private void createCheckBox() {
		...
		controlLabel.setLayoutData(GridDataFactory.fillDefaults().span(2, 1).create());
...
class RadioParameterControlHelper extends AbstractParameterControlHelper {
	...
	protected void prepare() {
		...
		for (IParameterSelectionChoice choice : list) {
			if (InputParameterSelectionChoice.BLANKVALUECHOICE.getValue().equals(choice.getValue())) {
				isContainNull = true;
			}
		}
	...
	private void initRadioButton(Button button, IParameterSelectionChoice choice, String choiceLabel) {
		if ((choice.getValue() == defaultValue)
				|| (choice.getValue() != null && choice.getValue().equals(defaultValue))) {
			button.setSelection(true);
			putConfigValue(paramterHandleName, button.getData());
			clearSelectRadio(radioItems);
		} else if (defaultValue == null && choiceLabel.equals(InputParameterSelectionChoice.NULLVALUECHOICE)) {
			button.setSelection(true);
	...
	private String getChoiceLabel(IParameterSelectionChoice choice) {
		...
		if (choiceLabel == null) {
			choiceLabel = choice.getValue() == null ? NULL_VALUE_STR : getFormatLabelString(choice, parameter);
		}
		return choiceLabel;
	}
...
abstract class SelectionParameterControlHelper extends AbstractParameterControlHelper {
	 ...
	 * @param comboData
	 */
	private void dealWithValueNotInComboList(Object value, Combo combo, ScalarParameter listParam, boolean isCascade,
			Object comboData) {
		if (!isCascade || (isCascade && isContainValue(listParam.getDefaultObject(), comboData))) {
	...
	protected void setSelectValueAfterInitCombo(int selectIndex, Combo combo) {
		...
		if (!found) {
			dealWithValueNotInComboList(defaultValue, combo, parameter, isCascade, valueList);
		}
	}
	...
	protected void initListValue(Object value, org.eclipse.swt.widgets.List list, ScalarParameter listParam) {
		...
		putConfigValue(listParam.getHandle().getName(), newValueList.toArray(new Object[newValueList.size()]));
	}
	 ...
	 * group : then do nothing; else then if default value not in value list,then
	 * add it to value list.
	 */
	private void dealCascade() {
		isCascade = isCascadeParameter(parameter);
		if (!isCascade) {
			addDefaultToValueList(formatString(parameter.getDefaultValue(), parameter), parameter.getDefaultObject(),
					valueList);
		}
	}
	 ...
	 * @param list
	 */
	private void addDefaultToValueList(final String defaultValueLabel, final Object defaultValue, List list) {
		if (performed) {
			return;
		}
		...
		if (!contains && defaultValue != null) {
			IParameterSelectionChoice choice = new InternalParameterSelectionChoice(defaultValueLabel, defaultValue);
			list.add(choice);
		}
		performed = true;
	}
	...
	protected void addCascadeParamterRelation() {
		if (isCascadeGroup) {
			if (cascadeGroup.getPreParameter(parameter) != null) {
				parameterDialog.addPostParamter(cascadeGroup.getPreParameter(parameter), this);
			}
		}
	}
	...
	private void emptyControlItem() {
		if (getControl() instanceof Combo) {
			...
			((org.eclipse.swt.widgets.List) getControl())
					.setItems(controlResetItems.toArray(new String[controlResetItems.size()]));
		}
	}
}
		...
		combo.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent e) {
				...
				if (combo.indexOf(combo.getText()) < 0) {
					try {
						putConfigValue(paramterHandleName, parameter.converToDataType(combo.getText()));
					} catch (BirtException e1) {
						MessageDialog.openError(combo.getShell(),
								Messages.getString("InputParameterDialog.err.invalidValueTitle"), //$NON-NLS-1$
								Messages.getFormattedString("InputParameterDialog.err.invalidValue", //$NON-NLS-1$
										new String[] { combo.getText(), parameter.getHandle().getDataType() }));
						return;
					}
				} else {
					putConfigValue(paramterHandleName, combo.getData(String.valueOf(combo.indexOf(combo.getText()))));
				}
		...
		combo.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (combo.getSelectionIndex() != -1) {
					putConfigValue(paramterHandleName, combo.getData(String.valueOf(combo.getSelectionIndex())));
				}
				doWithCascadeGroup();
			}
		});
	...
	private void initCombo() {
		...
		for (IParameterSelectionChoice choice : valueList) {
			...
			if (choice.getValue() == null && choice.getLabel() == null) {
				...
				if (choice.getValue() != null && choice.getValue().equals(defaultValue) && choice.getLabel() != null
						&& !choice.getLabel().equals(InputParameterSelectionChoice.BLANKVALUECHOICE.getValue())) {
