...
public class MenuButton extends Composite {
	...
	protected void paintControl(PaintEvent e) {
		...
		Color fg = isEnabled() ? getForeground()
				: new Color(e.gc.getDevice(), UIUtil.blend(getBackground().getRGB(), getForeground().getRGB(), 70));
		try {
			...
			if (menu != null) {
				...
				e.gc.drawLine(size.width - left, (size.height - height) / 2, size.width - left,
			...
			if (text != null && text.trim().length() > 0) {
				...
				e.gc.drawText(text, (size.width - left) / 2 + MARGIN_GAP, (size.height - fontHeight) / 2,
			...
			if (image != null) {
				...
				if (!isEnabled()) {
					imageTemp = new Image(e.gc.getDevice(), image, SWT.IMAGE_DISABLE);
				}
				...
				if (!isEnabled()) {
					e.gc.drawImage(imageTemp, 0, 0, imageTemp.getImageData().width, imageTemp.getImageData().height,
							(size.width - left) / 2 + MARGIN_GAP, Math.round(((float) (size.height - imageHeight) / 2)),
