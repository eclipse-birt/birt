...
public class FormatDateTimeLayoutPeer extends FormatLayoutPeer
{
	...
	private static final String LABEL_CUSTOM_PREVIEW_DATETIME = Messages.getString( "FormatDateTimePage.label.preview.dateTime" ); //$NON-NLS-1$
	...
	private String defaultPreviewText = new DateFormatter( ENTER_DATE_TIME_GUIDE_FORMAT,
			ULocale.getDefault( ) ).format( defaultDate );

	public FormatDateTimeLayoutPeer( int dateTimeType, int pageAlignment,
	...
	protected void fireFormatChanged( String newCategory, String newPattern,
			String newLocale )
	{
		fireFormatChanged( StyleHandle.DATE_TIME_FORMAT_PROP,
				newCategory,
				newPattern,
				newLocale );
	}
	...
	protected void initiatePageLayout( String categoryStr, String patternStr,
			String localeStr )
	{
		initiatePageLayout( ( (FormatDateTimeAdapter) formatAdapter ).getCustomCategoryName( ),
				categoryStr,
				patternStr,
				localeStr );
	}
	...
	protected void updatePreview( )
	{
		...
		if ( getPreviewText( ) != null
				&& !getPreviewText( ).equals( defaultPreviewText ) )
		{
			try
			{
		...
		if ( ( (FormatDateTimeAdapter) formatAdapter ).getCustomCategoryName( )
				.equals( category ) )
		{
		...
		{
			String pattern = null;
			if ( !( (FormatDateTimeAdapter) formatAdapter ).getUnformattedCategoryDisplayName( )
					.equals( category ) )
			{
