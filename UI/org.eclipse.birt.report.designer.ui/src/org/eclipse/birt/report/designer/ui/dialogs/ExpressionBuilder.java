...
public class ExpressionBuilder extends BaseTitleAreaDialog {
	...
	private class TableContentProvider implements IStructuredContentProvider {
		...
		public Object[] getElements(Object inputElement) {
			...
			if (inputElement instanceof PropertyHandle || inputElement instanceof TabularMeasureGroupHandle
					|| inputElement instanceof DimensionHandle) {
				return EMPTY;
			}
			// ignore items that cannot be inserted as (part of) expression.
			else if (inputElement instanceof DesignElementHandle && getAdapter() != null
				...
				return EMPTY;
			} else if (inputElement instanceof ReportElementHandle && getAdapter() != null
			...
			if (event.getSource() == functionTable) {
				...
				if (table.getSelectionCount() == 1) {
					...
					messageLine.setText("<form><p> <b>" //$NON-NLS-1$
							...
							+ "</b>: " //$NON-NLS-1$
							+ message + "</p></form>", true, false); //$NON-NLS-1$
	...
	private void handleDefaultSelection() {
		...
		if (defaultSelection.length > 0 && defaultSelection[0] != null) {
			categoryTable.setSelection(new StructuredSelection(defaultSelection[0]), true);

			if (defaultSelection.length > 1 && defaultSelection[1] != null) {
				subCategoryTable.setSelection(new StructuredSelection(defaultSelection[1]), true);

				if (defaultSelection.length > 2 && defaultSelection[2] != null) {
					functionTable.setSelection(new StructuredSelection(defaultSelection[2]), true);
				}
			}
		}
	}
	...
	protected SourceViewer createSourceViewer(Composite parent) {
		...
		JSEditorInput editorInput = new JSEditorInput(expression, getEncoding());
	...
	private Color getForegroundColor(IPreferenceStore preferenceStore) {
		Color color = preferenceStore.getBoolean(AbstractTextEditor.PREFERENCE_COLOR_FOREGROUND_SYSTEM_DEFAULT) ? null
				: createColor(preferenceStore, AbstractTextEditor.PREFERENCE_COLOR_FOREGROUND, Display.getCurrent());
	...
	private Color getBackgroundColor(IPreferenceStore preferenceStore) {
		Color color = preferenceStore.getBoolean(AbstractTextEditor.PREFERENCE_COLOR_BACKGROUND_SYSTEM_DEFAULT) ? null
				: createColor(preferenceStore, AbstractTextEditor.PREFERENCE_COLOR_BACKGROUND, Display.getCurrent());
	...
	protected boolean validateScript() {
		...
		try {
			...
			errorMessage = Messages.getFormattedString("ExpressionBuilder.Script.Error", //$NON-NLS-1$
					new Object[] { Integer.toString(row), Integer.toString(column), e.getLocalizedMessage() });
			return false;
		} finally {
	...
	private void generateDate(final ToolBar toolBar, final ToolItem calendar) {
		...
		final DateTime colorDialog = new DateTime(shell, SWT.CALENDAR | SWT.FLAT);
