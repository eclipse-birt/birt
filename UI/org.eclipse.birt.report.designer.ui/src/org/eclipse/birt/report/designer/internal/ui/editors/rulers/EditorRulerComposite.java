...
public class EditorRulerComposite extends Composite {
	...
	private GraphicalViewer createRulerContainer(int orientation) {
		...
		if (isHorizontal) {
			canvas.getViewport().setHorizontalRangeModel(
					new RulerDefaultRangeModel(editor.getViewport().getHorizontalRangeModel()));
		} else {
			canvas.getViewport()
					.setVerticalRangeModel(new RulerDefaultRangeModel(editor.getViewport().getVerticalRangeModel()));
		}
	...
	private void doLayout() {
		...
		leftWidth = left == null ? 0 : left.getControl().computeSize(SWT.DEFAULT, SWT.DEFAULT).x;
		rightWidth = 0;
		topHeight = top == null ? 0 : top.getControl().computeSize(SWT.DEFAULT, SWT.DEFAULT).y;
		bottomHeight = 0;
	...
	protected org.eclipse.draw2d.geometry.Rectangle getScaleValue(org.eclipse.draw2d.geometry.Rectangle value) {
	...
	public void setMargin(int direction) {
		...
		case TOP_RIGHT: {
			Object obj = ((RulerProvider) diagramViewer.getProperty(RulerProvider.PROPERTY_HORIZONTAL_RULER))
					.getRuler();
			if (obj instanceof EditorRuler) {
				((EditorRuler) obj).setRightMargin(getLayoutSize().right() - getRightMargin());
			}
			break;
		}
		case LEFT_TOP: {
		...
		case LEFT_BOTTOM: {
			Object obj = ((RulerProvider) diagramViewer.getProperty(RulerProvider.PROPERTY_VERTICAL_RULER)).getRuler();
			if (obj instanceof EditorRuler) {
				((EditorRuler) obj).setRightMargin(getLayoutSize().bottom() - getBottomMargin());
			}
			break;
		}
		default:
	...
	private void changeGuide(DragGuideInfo info) {
		if (info.horizontal) {
			Object obj = ((EditorRulerProvider) diagramViewer.getProperty(RulerProvider.PROPERTY_HORIZONTAL_RULER))
					.getRuler();
			if (obj instanceof EditorRuler) {
			...
			Object obj = ((EditorRulerProvider) diagramViewer.getProperty(RulerProvider.PROPERTY_VERTICAL_RULER))
					.getRuler();
			if (obj instanceof EditorRuler) {
				EditorRuler ruler = (EditorRuler) obj;
	...
	public org.eclipse.draw2d.geometry.Rectangle getLayoutSize() {
		...
		return new org.eclipse.draw2d.geometry.Rectangle(0, 0, dim.width, dim.height);
	}
	...
	private MasterPageHandle getMasterPageHandle() {
		return SessionHandleAdapter.getInstance().getFirstMasterPageHandle(getReportDesignHandle());
	}
