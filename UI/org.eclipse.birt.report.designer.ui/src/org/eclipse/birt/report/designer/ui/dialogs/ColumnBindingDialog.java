...
public class ColumnBindingDialog extends BaseDialog {
	...
	public ColumnBindingDialog(ReportItemHandle input, Shell parent, String title, boolean canSelect,
	 ...
	 * @param canAggregate
	 * @param canSelect
	 * @param filterSelf   don't show input item handle's binding
	 */
	public ColumnBindingDialog(ReportItemHandle input, String title, boolean canAggregate, boolean canSelect,
			boolean filterSelf) {
		super(title);
		setInput(input);
		this.canSelect = canSelect;
	...
	protected void addBinding(ComputedColumn column) {
		try {
			DEUtil.addColumn(DEUtil.getBindingHolder(inputElement), column, false);
	...
	private String getColumnName(String expression) {
		...
		for (Iterator iter = columnList.iterator(); iter.hasNext();) {
			...
			if (LinkedDataSetUtil.bindToLinkedDataSet(inputElement)
					? DEUtil.getDataExpression(columnName).equals(expression)
	...
	protected void okPressed() {
		...
		if (getSelectColumnHandle() != null && ExtendedDataModelUIAdapterHelper.isBoundToExtendedData(inputElement)) {
			DataModelAdapterStatus status = DataModelAdapterUtil.validateRelativeTimePeriod(inputElement,
					getSelectColumnHandle());
	...
	private void resetDataSetReference(Object value, boolean clearHistory) {
		try {
			...
			if (clearHistory) {
				...
				inputElement.getPropertyHandle(ReportItemHandle.PARAM_BINDINGS_PROP).clearValue();
			}
	...
	private void resetReference(Object value) {
		if (value == null && inputElement.getDataBindingType() == ReportItemHandle.DATABINDING_TYPE_DATA) {
	...
	public void save(Object saveValue) throws SemanticException {
		if (saveValue instanceof BindingInfo) {
			...
			case ReportItemHandle.DATABINDING_TYPE_DATA:
				...
				int ret = 0;
				if (!NullDatasetChoice.equals(((BindingInfo) loadValue()).getBindingValue().toString())
						|| inputElement.getColumnBindings().iterator().hasNext()) {
					MessageDialog prefDialog = new MessageDialog(UIUtil.getDefaultShell(),
	...
	private void setInput(ReportItemHandle input) {
		...
		isDataSetVisible = DEUtil.getBindingHolder(inputElement).getElement().getDefn()
		...
		canAggregate = dialogHelper == null ? false : dialogHelper.canProcessAggregation();
	}
	...
	private void updateSelection() {
		if (canSelect) {
			...
			if (getSelectColumnHandle() != null) {
				((CheckboxTableViewer) bindingTable).setChecked(getSelectColumnHandle(), true);
			}
		}
	}
