...
public class FontPreferencePage extends BaseStylePreferencePage {
	...
	protected void createFieldEditors() {
		...
		name = new EditableComboFieldEditor(StyleHandle.FONT_FAMILY_PROP,
				Messages.getString(((StyleHandle) model).getPropertyHandle(
						StyleHandle.FONT_FAMILY_PROP).getDefn()
						.getDisplayNameID()), getFontChoiceArray(),
				getFieldEditorParent());

		color = new ColorFieldEditor(StyleHandle.COLOR_PROP, Messages
				.getString(((StyleHandle) model).getPropertyHandle(
						StyleHandle.COLOR_PROP).getDefn().getDisplayNameID()),
				getFieldEditorParent());

		size = new ComboBoxMeasureFieldEditor(StyleHandle.FONT_SIZE_PROP,
				Messages.getString(((StyleHandle) model).getPropertyHandle(
						StyleHandle.FONT_SIZE_PROP).getDefn()
						.getDisplayNameID()),
				getChoiceArray(DesignChoiceConstants.CHOICE_FONT_SIZE),
				getChoiceArray(DesignChoiceConstants.CHOICE_UNITS),
				getFieldEditorParent());
		size.setDefaultUnit(((StyleHandle) model).getPropertyHandle(
				StyleHandle.FONT_SIZE_PROP).getDefaultUnit());

		style = new ComboBoxFieldEditor(StyleHandle.FONT_STYLE_PROP, Messages
				.getString(((StyleHandle) model).getPropertyHandle(
						StyleHandle.FONT_STYLE_PROP).getDefn()
						.getDisplayNameID()),
				getChoiceArray(DesignChoiceConstants.CHOICE_FONT_STYLE),
				getFieldEditorParent());

		weight = new ComboBoxFieldEditor(StyleHandle.FONT_WEIGHT_PROP, Messages
				.getString(((StyleHandle) model).getPropertyHandle(
						StyleHandle.FONT_WEIGHT_PROP).getDefn()
						.getDisplayNameID()),
		...
		docoration = new DecorationFieldEditor(StyleHandle.TEXT_UNDERLINE_PROP,
				Messages.getString(((StyleHandle) model).getPropertyHandle(
						StyleHandle.TEXT_UNDERLINE_PROP).getDefn()
						.getDisplayNameID()), StyleHandle.TEXT_OVERLINE_PROP,
				Messages.getString(((StyleHandle) model).getPropertyHandle(
						StyleHandle.TEXT_OVERLINE_PROP).getDefn()
						.getDisplayNameID()),
				StyleHandle.TEXT_LINE_THROUGH_PROP, Messages
						.getString(((StyleHandle) model).getPropertyHandle(
								StyleHandle.TEXT_LINE_THROUGH_PROP).getDefn()
								.getDisplayNameID()), Messages
						.getString("FontPreferencePage.label.fontDecoration"), //$NON-NLS-1$
				getFieldEditorParent());
	...
	private void updatePreview() {
		if (sample != null) {
			String fontFamily = name.getValueForName(name.getComboBoxControl(
					null).getText());
			...
			String familyValue = (String) DesignerConstants.familyMap
					.get(fontFamily);
			...
			int sizeValue = Integer.valueOf(
					(String) DesignerConstants.fontMap.get(fontSize))
					.intValue();

			if (size.InComboNamesList(size.getComboBoxControl(
					getFieldEditorParent()).getText())) {
				fontSize = size.getBoxValueForName(size.getComboBoxControl(
						getFieldEditorParent()).getText());
				if (DesignChoiceConstants.FONT_SIZE_LARGER.equals(fontSize)) {
					fontSize = DesignChoiceConstants.FONT_SIZE_LARGE;
				} else if (DesignChoiceConstants.FONT_SIZE_SMALLER
						.equals(fontSize)) {
					fontSize = DesignChoiceConstants.FONT_SIZE_SMALL;
				}
				sizeValue = Integer.valueOf(
						(String) DesignerConstants.fontMap.get(fontSize))
						.intValue();
			} else {
				String text = size.getComboBoxControl(getFieldEditorParent())
						.getText();
				String pre = size.getMeasureValueForName(size
						.getMeasureControl(getFieldEditorParent()).getText());

				if (DEUtil.isValidNumber(text)) {
					sizeValue = (int) CSSUtil
							.convertToPoint(new DimensionValue(Double
									.parseDouble(text), pre));
				}
			}

			boolean italic = false;
			String fontStyle = style.getValueForName(style.getComboBoxControl(
					getFieldEditorParent()).getText());
			if (DesignChoiceConstants.FONT_STYLE_ITALIC.equals(fontStyle)) {
				italic = true;
			}

			String fontWeight = weight.getValueForName(weight
					.getComboBoxControl(null).getText());
			boolean bold = false;
			int fw = 400;
			if (DesignChoiceConstants.FONT_WEIGHT_NORMAL.equals(fontWeight)) {
				// no change.
			} else if (DesignChoiceConstants.FONT_WEIGHT_BOLD
					.equals(fontWeight)) {
				bold = true;
				fw = 700;
			} else if (DesignChoiceConstants.FONT_WEIGHT_BOLDER
					.equals(fontWeight)) {
				bold = true;
				fw = 1000;
			} else if (DesignChoiceConstants.FONT_WEIGHT_LIGHTER
					.equals(fontWeight)) {
				fw = 100;
			} else {
				try {
	...
	private String[][] getChoiceArray(String key, Comparator comparator) {
		IChoice[] choices = DesignEngine.getMetaDataDictionary().getChoiceSet(
				key).getChoices(comparator);
