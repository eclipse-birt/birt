...
public class BasePaletteFactory {
	...
	private static final String AUTOTEXT_LABEL_PAGE_COUNT = Messages
			.getString("BasePaletteFactory.AutoTextLabel.PageCount"); //$NON-NLS-1$
						...
						try {
							...
							if (datasetInfo[1] instanceof Object[]) {
								...
								if (bindingCount > 0) {
									...
									InsertInLayoutUtil.insertToCell(dataSet, table, table.getHeader(), columns, true);

									if (!isSummaryTable) {
										InsertInLayoutUtil.insertToCell(dataSet, table, table.getDetail(), columns,
												false);
									}
								}
							}
	...
	public static class AutoTextToolExtends extends AbstractToolHandleExtends {
		...
		public boolean preHandleMouseUp() {
			...
			try {
				...
				} else if (IReportElementConstants.AUTOTEXT_TOTAL_PAGE_COUNT
		...
		public boolean preHandleMouseUp() {
			...
			if (IReportElementConstants.AUTOTEXT_VARIABLE.equalsIgnoreCase((String) request.getNewObjectType())) {
				...
				if (reportHandle instanceof ReportDesignHandle) {
					SelectVariableDialog dialog = new SelectVariableDialog(
	...
	public static class TextToolExtends extends AbstractToolHandleExtends {
		...
		public boolean preHandleMouseUp() {
			...
			// .getElementFactory( )
			// .newTextItem( null );
			TextItemHandle textItemHandle = DesignElementFactory.getInstance().newTextItem(null);
			try {
				...
				if (IReportElementConstants.AUTOTEXT_DATE.equalsIgnoreCase(type)) {
					...
					text = AUTOTEXT_LABEL_CREATE_ON + "<value-of>new Date()</value-of>"; //$NON-NLS-1$
					textItemHandle.setContentType(DesignChoiceConstants.TEXT_CONTENT_TYPE_HTML);
					...
					if (author != null) {
						...
						String pluginVersion = ReportPlugin.getDefault().getBundle().getHeaders()
					...
					text = AUTOTEXT_LABEL_LAST_PRINTED + "<value-of>new Date()</value-of>"; //$NON-NLS-1$
					textItemHandle.setContentType(DesignChoiceConstants.TEXT_CONTENT_TYPE_HTML);
				}
				// else if (
		...
		public boolean preHandleMouseUp() {
			...
			if (IReportElementConstants.AUTOTEXT_PAGEXOFY.equals(type)) {
				...
				try {
					...
					((CellHandle) cellList.get(0)).getContent().add(autoTextHandle);
				...
				try {
					...
					((CellHandle) cellList.get(0)).getContent().add(textHandle);

					AutoTextHandle autoTextHandle = factory.newAutoText(null);
					autoTextHandle.setAutoTextType(DesignChoiceConstants.AUTO_TEXT_PAGE_NUMBER);
					((CellHandle) cellList.get(1)).getContent().add(autoTextHandle);
					...
					((CellHandle) cellList.get(2)).getContent().add(textHandle);
				} catch (SemanticException e) {
					ExceptionHandler.handle(e);
				}
				...
				if (dlg.open() == Window.OK && dlg.getResult() instanceof Object[]) {
					Object[] data = (Object[]) dlg.getResult();
					grid = factory.newGridItem(null, ((Integer) data[1]).intValue(), ((Integer) data[0]).intValue());
				} else {
					return false;
				}
			} else {
		...
		public boolean preHandleMouseUp() {
			if (getRequest().getNewObjectType() instanceof DataSetHandle) {
				...
				try {
					...
					Object newHandle = InsertInLayoutUtil.performInsert(elementHandle, getTargetEditPart());
					if (newHandle == null)
						return false;
					setModel(newHandle);
