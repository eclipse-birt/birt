...
public class CGridData {
	...
	/**
	 * maximumWidth specifies the maximum width in pixels. This value applies only
	 * if isHorizontalAlignFill is true. A value of SWT.DEFAULT means that the
	 * maximum width will be the result of Control.computeSize(int, int, boolean)
	...
	/**
	 * minimumHeight specifies the minimum height in pixels. This value applies only
	 * if grabExcessVerticalSpace is true. A value of SWT.DEFAULT means that the
	 * minimum height will be the result of Control.computeSize(int, int, boolean)
	 * where hHint is determined by GridData.heightHint.
	 *
	...
	/**
	 * verticalAlignment specifies how controls will be positioned vertically within
	 * a cell.
	 *
	 ...
	 * cell</li>
	 * <li>SWT.CENTER: Position the control in the horizontal center of the
	 * cell</li>
	 * <li>SWT.END (or SWT.RIGHT): Position the control at the right of the
	 * cell</li>
	 ...
	 * passed into Control.computeSize(int, int, boolean) to determine the preferred
	 * size of the control.
	 *
	 * The default value is SWT.DEFAULT.
	 * 
	...
	/**
	 * heightHint specifies the preferred height in pixels. This value is the hHint
	 * passed into Control.computeSize(int, int, boolean) to determine the preferred
	 * size of the control.
	 *
	 * The default value is SWT.DEFAULT.
	 ...
	 * is <code>true</code>, the following rules apply to the width of the cell:
	 * </p>
	 * <ul>
	 * <li>If extra horizontal space is available in the parent, the cell will grow
	 * to be wider than its preferred width. The new width will be "preferred width
	 ...
	 * controls in any of the spanned columns, the last column in the span will grab
	 * the extra space. If there is at least one other grabbing control in the span,
	 * the grabbing will be spread over the columns already marked as
	 ...
	 * depending on the size of the parent Composite. If grabExcessVerticalSpace is
	 * <code>true</code>, the following rules apply to the height of the cell:
	 * </p>
	 * <ul>
	 * <li>If extra vertical space is available in the parent, the cell will grow to
	 * be taller than its preferred height. The new height will be "preferred height
	 * + delta" where delta is the extra vertical space divided by the number of
	 * grabbing rows.</li>
	 * <li>If there is not enough vertical space available in the parent, the cell
	 * will shrink until it reaches its minimum height as specified by
	 * GridData.minimumHeight. The new height will be the maximum of "minimumHeight"
	 * and "preferred height - delta", where delta is the amount of space missing
	 ...
	 * extra space. If there is at least one other grabbing control in the span, the
	 * grabbing will be spread over the rows already marked as
	...
	/**
	 * exclude informs the layout to ignore this control when sizing and positioning
	 * controls. If this value is <code>true</code>, the size and position of the
	 * control will not be managed by the layout. If this value is
	 * <code>false</code>, the size and position of the control will be computed and
	 * assigned.
	 * 
	...
	/**
	 * Style bit for <code>new GridData(int)</code>. Position the control at the top
	...
	/**
	 * Style bit for <code>new GridData(int)</code> to position the control in the
	...
	/**
	 * Style bit for <code>new GridData(int)</code> to position the control at the
	...
	/**
	 * Style bit for <code>new GridData(int)</code> to resize the control to fill
	...
	/**
	 * Style bit for <code>new GridData(int)</code> to resize the control to fit the
	...
	/**
	 * Style bit for <code>new GridData(int)</code> to resize the control to fill
	 * the cell vertically and to fit the remaining vertical space. FILL_VERTICAL =
	...
	/**
	 * Style bit for <code>new GridData(int)</code> to resize the control to fill
	 * the cell horizontally and vertically and to fit the remaining horizontal and
	 ...
	 *                                  within a cell
	 * @param verticalAlignment         how control will be positioned vertically
	 *                                  within a cell
	 ...
	 * @since 3.0
	 */
	public CGridData(int horizontalAlignment, int verticalAlignment, boolean grabExcessHorizontalSpace,
