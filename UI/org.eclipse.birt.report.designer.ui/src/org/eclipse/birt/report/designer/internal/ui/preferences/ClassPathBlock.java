...
public class ClassPathBlock extends OptionsConfigurationBlock
{
	...
	private static final String[] ALL_ARCHIVES_FILTER_EXTENSIONS = new String[]{
			"*.jar;*.zip", "*.*"}; //$NON-NLS-1$ //$NON-NLS-2$
	public static final String[] JAR_ZIP_FILTER_EXTENSIONS = new String[]{
		"*.jar;*.zip"}; //$NON-NLS-1$
	...
	public ClassPathBlock( IStatusChangeListener context, IProject project )
	{
		...
		String[] buttonLabels = new String[]{
				Messages.getString( "ClassPathBlock_button.addExtJars" ),//$NON-NLS-1$ 
				Messages.getString( "ClassPathBlock_button.addExtFolder" ), //$NON-NLS-1$ 

				/* */null,
				Messages.getString( "ClassPathBlock_button.up" ),//$NON-NLS-1$ 
	...
	private CPListElement[] openExternalClassFolderDialog(
			CPListElement existing )
	{
		...
		{
			IPath configured = configureExternalClassFolderEntries( getShell( ),
					existing.getPath( ) );
			if ( configured != null )
			{
				return new CPListElement[]{
	...
	public static List<String> getEntries( String value )
	{
		...
		{
			retValue.add( ( (CPListElement) elements.get( i ) ).getPath( )
					.toFile( )
					.getAbsolutePath( ) );
		}
