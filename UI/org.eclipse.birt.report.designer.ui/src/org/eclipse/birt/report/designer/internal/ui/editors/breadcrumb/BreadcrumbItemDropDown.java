...
class BreadcrumbItemDropDown
{
	...
	public void showMenu( )
	{
		...
		fShell = new Shell( fToolBar.getShell( ), SWT.RESIZE
				| SWT.TOOL
				| SWT.ON_TOP );
		...
		fDropDownViewer = new TreeViewer( composite, SWT.SINGLE
				| SWT.H_SCROLL
				| SWT.V_SCROLL );
				...
				if ( tree.equals( e.getSource( ) ) )
				{
					...
					if ( o instanceof TreeItem )
					{
						...
						else if ( clientArea.y + clientArea.height - e.y < tree.getItemHeight( ) / 4 )
						{
							...
							{
								Point p = tree.toDisplay( e.x, e.y );
								Item item = fDropDownViewer.scrollDown( p.x,
										p.y );
								if ( item instanceof TreeItem )
								{
									fLastItem = (TreeItem) item;
		...
		if ( index < fParent.getViewer( ).getItemCount( ) - 1 )
		{
			...
			fDropDownViewer.setSelection( new StructuredSelection( child ),
					true );
