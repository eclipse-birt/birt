...
public class DefaultNodeProvider implements INodeProvider {
	...
	public static final String WARNING_DIALOG_MESSAGE_EMPTY_LIST = Messages
			.getString("DefaultNodeProvider.WarningDialog.EmptyList"); //$NON-NLS-1$
	...
	public void createContextMenu(TreeViewer sourceViewer, Object object, IMenuManager menu) {
		...
		if (sourceViewer != null) {
			...
			if (renameAction.isEnabled()) {// if can rename,add to menu
				menu.add(renameAction);
			}
		}
		// Delete action
		DeleteAction deleteAction = new DeleteAction(object);
		if (deleteAction.isEnabled()) {// if can delete,add to menu
		...
		menu.add(new Separator(IWorkbenchActionConstants.MB_ADDITIONS + "-refresh"));//$NON-NLS-1$

		menu.add(new Separator(IWorkbenchActionConstants.MB_ADDITIONS + "-export"));//$NON-NLS-1$
	...
	public Object getParent(Object model) {
		...
		if (model instanceof SlotHandle) {
			...
			if (handle instanceof CellHandle || handle.getContainer() instanceof CellHandle
	...
	public Image getNodeIcon(Object model) {
		...
		if (iconName != null) {// if the getIconName is defined
	 ...
	 *         available for the given model
	 */
	public String getIconName(Object model) {// Do nothing
		return null;
	}
	...
	public boolean performRequest(Object model, Request request) throws Exception {
		if (request.getType().equals(IRequestConstants.REQUEST_TYPE_INSERT)) {
			...
			if (slotHandle != null) {
				...
				return performInsert(model, slotHandle, type, position, extendsData);
			}
	...
	private boolean performRevertToTemplateItem(DesignElementHandle handle) {
		try {
			handle.revertToTemplate(
					ReportPlugin.getDefault().getCustomName(ReportDesignConstants.TEMPLATE_REPORT_ITEM));
	...
	private boolean performCreatePlaceHolder(ReportElementHandle handle) {
		...
		String name = ReportPlugin.getDefault().getCustomName(ReportDesignConstants.TEMPLATE_REPORT_ITEM);
		if (name == null) {
			name = ""; //$NON-NLS-1$
		}
		...
		try {
			...
			do {
				TemplateReportItemPropertiesDialog dialog = new TemplateReportItemPropertiesDialog(
						handle.getDefn().getDisplayName(), name, desc);
				if (dialog.open() == Window.OK) {
					...
					if (!bIsNameExist) {
						...
						ExceptionHandler.openErrorMessageBox(
								Messages.getString("performCreatePlaceHolder.errorMessage.title"), //$NON-NLS-1$
	...
	protected DesignElementHandle createElement(ElementDetailHandle slotHandle, String type) throws Exception {
		if (type == null) {
			...
			if (supportList.size() == 0) {
				ExceptionHandler.openMessageBox(WARNING_DIALOG_TITLE, WARNING_DIALOG_MESSAGE_EMPTY_LIST,
						SWT.ICON_WARNING);
				return null;
	...
	protected DesignElementHandle createElement(ElementDetailHandle slotHandle, String type, Map extendData)
			throws Exception {
	...
	protected boolean performInsert(Object model, ElementDetailHandle slotHandle, String type, String position,
		...
		DesignElementHandle elementHandle = createElement(slotHandle, type, extendData);
