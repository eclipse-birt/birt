...
public class CGridLayout extends Layout
{
	...
	CGridData getData( Control[][] grid, int row, int column, int rowCount,
		...
		for ( int i = 0; i < count; i++ )
		{
			...
			if ( data.grabExcessHorizontalSpace && data.minimumWidth > 0 )
			{
				if ( data.cacheWidth < data.minimumWidth )
				{
					...
					data.computeSize( child, Math.max( 0, data.minimumWidth
							- trim ), data.heightHint, false );
				}
			}else
			if((data.horizontalAlignment == SWT.FILL) && data.maximumWidth > 0)
			{
				if ( data.cacheWidth > data.maximumWidth )
				{
					...
					data.cacheWidth = data.cacheHeight = SWT.DEFAULT;
					int tmpWidth = Math.max( 0, data.maximumWidth
							- trim );
			...
			{
				data.cacheHeight = Math.min( data.cacheHeight,
						data.maximumHeight );
			}
		}
			...
			for ( int i = 0; i < rowCount; i++ )
			{
				...
				for ( int j = 0; j < columnCount; j++ )
				{
					...
					if ( data != null )
					{
						...
						int childHeight = Math.min( data.cacheHeight,
								cellHeight );
