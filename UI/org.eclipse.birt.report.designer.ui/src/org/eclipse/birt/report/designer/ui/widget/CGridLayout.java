...
public class CGridLayout extends Layout {
	...
	protected boolean flushCache(Control control) {
		...
		if (data != null) {
			((CGridData) data).flushCache();
		}

		return true;
	}

	CGridData getData(Control[][] grid, int row, int column, int rowCount, int columnCount, boolean first) {
		Control control = grid[row][column];
		if (control != null) {
			CGridData data = (CGridData) control.getLayoutData();
	...
	Point layout(Composite composite, boolean move, int x, int y, int width, int height, boolean flushCache) {
		...
		for (int i = 0; i < count; i++) {
			...
			} else if ((data.horizontalAlignment == SWT.FILL) && data.maximumWidth > 0) {
				if (data.cacheWidth > data.maximumWidth) {
					int trim = 0;
					// TEMPORARY CODE
					if (child instanceof Scrollable) {
						Rectangle rect = ((Scrollable) child).computeTrim(0, 0, 0, 0);
					...
					data.cacheWidth = data.cacheHeight = SWT.DEFAULT;
					int tmpWidth = Math.max(0, data.maximumWidth - trim);
					data.computeSize(child, tmpWidth, data.heightHint, false);
				}
			}

			if (data.grabExcessVerticalSpace && data.minimumHeight > 0) {
				data.cacheHeight = Math.max(data.cacheHeight, data.minimumHeight);
			} else if ((data.verticalAlignment == SWT.FILL) && data.maximumHeight > 0) {
				data.cacheHeight = Math.min(data.cacheHeight, data.maximumHeight);
			}
		}
		...
		for (int j = 0; j < columnCount; j++) {
			for (int i = 0; i < rowCount; i++) {
				CGridData data = getData(grid, i, j, rowCount, columnCount, true);
				if (data != null) {
					int hSpan = Math.max(1, Math.min(data.horizontalSpan, columnCount));
					if (hSpan == 1) {
						...
						if (!data.grabExcessHorizontalSpace || data.minimumWidth != 0) {
							w = !data.grabExcessHorizontalSpace || data.minimumWidth == SWT.DEFAULT ? data.cacheWidth
									: data.minimumWidth;
			...
			for (int j = 0; j < columnCount; j++) {
				...
				if (data != null) {
					...
					if (vSpan == 1) {
						...
						if (!data.grabExcessVerticalSpace || data.minimumHeight != 0) {
							h = !data.grabExcessVerticalSpace || data.minimumHeight == SWT.DEFAULT ? data.cacheHeight
									: data.minimumHeight;
