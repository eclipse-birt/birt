...
public class ReportItemBuilderUITest implements IReportItemBuilderUI
{
		...
		public int open( ExtendedItemHandle handle )
		{
			...
			if ( handle.getProperty( TestingMatrixUI.TEST_PROPERTY[1] ) == null )
			{
				...
				try
				{
					handle.setProperty( TestingMatrixUI.TEST_PROPERTY[1],
					        TestingMatrixUI.TEST_ELEMENT_CONTENT[1] );
				}
			...
			try
			{
				...
				handle.setProperty( TestingMatrixUI.TEST_PROPERTY[0],
				        TestingMatrixUI.TEST_ELEMENT_CONTENT_EDITED[0] );
