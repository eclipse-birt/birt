...
public class DNDUtilTest extends BaseTestCase
{
	...
	private void createSource( )
	{
		...
		{
			dataItems[i] = getElementFactory( ).newDataItem( "DataItem"
					+ ( i + 1 ) );
		}
	...
	{
		return (CellHandle) ( (RowHandle) ( table.getHeader( ).get( 0 ) ) ).getCells( )
				.get( 0 );
	}
	...
	private CellHandle getCell( int type, int row, int column )
	{
		...
		if ( type == TableItem.HEADER_SLOT )
			...
			return null;
		return (CellHandle) ( (RowHandle) ( rows.get( row ) ) ).getCells( )
				.get( column );
	}
	...
	{
		assertTrue( "Clone duplicated elements",
				getLength( DNDUtil.cloneSource( getSelection( ) ) ) == 1 );

		SlotHandle listDetail = getElementFactory( ).newList( "list" )
				.getDetail( );
		try
		{
		...
		assertTrue( "Clone list detail",
				getLength( DNDUtil.cloneSource( listDetail ) ) == 2 );
	...
	public void testHandleValidateTargetCanContain( )
	{
		assertFalse( "Validate null",
				DNDUtil.handleValidateTargetCanContain( null, null ) );

		assertTrue( "Validate container",
				DNDUtil.handleValidateTargetCanContain( getCell( ),
						dataItems[3] ) );

		assertTrue( "Validate brothers",
				DNDUtil.handleValidateTargetCanContain( dataItems[0],
						dataItems[3] ) );
		...
		assertTrue( "Validate parameter group",
				DNDUtil.handleValidateTargetCanContain( groupChild, groupParent ) );

		SlotHandle listDetailSlot = getElementFactory( ).newList( "list" )
				.getDetail( );
		DataItemHandle dataItem = getElementFactory( ).newDataItem( "data" );
		assertTrue( "Validate list detail",
				DNDUtil.handleValidateTargetCanContain( listDetailSlot,
						dataItem ) );
		...
		try
		{
			...
			assertTrue( "Validate list detail brothers",
					DNDUtil.handleValidateTargetCanContain( label,
							dataItem,
							true ) == DNDUtil.CONTAIN_PARENT );
		}
		catch ( Exception e )
		{
			e.printStackTrace( );
	...
	{
		assertFalse( "Validate null",
				DNDUtil.handleValidateTargetCanContainMore( null, 0 ) );

		assertTrue( "Validate contain 1",
				DNDUtil.handleValidateTargetCanContainMore( getCell( ), 1 ) );
		assertTrue( "Validate contain 2",
				DNDUtil.handleValidateTargetCanContainMore( getCell( ), 2 ) );

		SimpleMasterPageHandle masterPage = (SimpleMasterPageHandle) getReportDesignHandle( ).getMasterPages( )
				.get( 0 );

		assertTrue( "Validate master page 0",
				DNDUtil.handleValidateTargetCanContainMore( masterPage.getPageHeader( ),
						0 ) );
		assertTrue( "Validate master page 1",
				DNDUtil.handleValidateTargetCanContainMore( masterPage.getPageHeader( ),
						1 ) );
	...
	public void testHandleValidateTargetCanContainType( )
	{
		SimpleMasterPageHandle masterPage = (SimpleMasterPageHandle) getReportDesignHandle( ).getMasterPages( )
				.get( 0 );
		assertFalse( "Validate null",
				DNDUtil.handleValidateTargetCanContainType( null,
						ReportDesignConstants.TEXT_ITEM ) );
		assertTrue( "Validate text",
				DNDUtil.handleValidateTargetCanContainType( masterPage.getPageHeader( ),
						ReportDesignConstants.TEXT_ITEM ) );
		assertTrue( "Validate grid",
				DNDUtil.handleValidateTargetCanContainType( masterPage.getPageHeader( ),
						ReportDesignConstants.GRID_ITEM ) );
		assertTrue( "Validate label",
				DNDUtil.handleValidateTargetCanContainType( masterPage.getPageHeader( ),
						ReportDesignConstants.LABEL_ITEM ) );
		assertTrue( "Validate data",
				DNDUtil.handleValidateTargetCanContainType( masterPage.getPageHeader( ),
						ReportDesignConstants.DATA_ITEM ) );
		assertTrue( "Validate image",
				DNDUtil.handleValidateTargetCanContainType( masterPage.getPageHeader( ),
						ReportDesignConstants.IMAGE_ITEM ) );
		assertFalse( "Validate table",
				DNDUtil.handleValidateTargetCanContainType( masterPage.getPageHeader( ),
	...
	public void testHandleValidateDragInOutline( )
	{
		...
		assertFalse( "can't copy empty list band",
				DNDUtil.handleValidateDragInOutline( listHandle.getHeader( ) ) );

		try
		{
			listHandle.getHeader( )
					.add( getElementFactory( ).newLabel( "listband" ) );
		}
		catch ( Exception e2 )
		{
			e2.printStackTrace( );
		}
		assertTrue( "can't copy content list band",
				DNDUtil.handleValidateDragInOutline( listHandle.getHeader( ) ) );

		list.clear( );
		list.add( listHandle.getDetail( ) );
		assertFalse( "can't drag empty list detail",
		...
		try
		{
			listHandle.getDetail( )
					.add( getElementFactory( ).newDataItem( "data" ) );
		}
		...
		list.add( listHandle.getDetail( ) );
		assertTrue( "can drag list detail",
				DNDUtil.handleValidateDragInOutline( new StructuredSelection( list ) ) );

		list.clear( );
		list.add( getElementFactory( ).newTableItem( "" ).getDetail( ) );
		assertFalse( "can't drag table detail",
				DNDUtil.handleValidateDragInOutline( new StructuredSelection( list ) ) );

		list.clear( );
		list.add( getElementFactory( ).newCell( ) );
		assertFalse( "can't drag cell",
				DNDUtil.handleValidateDragInOutline( new StructuredSelection( list ) ) );
	}
	...
	public void testCopyHandles( )
	{
		...
		assertTrue( "Cell is blank before",
				getCell( ).getContent( ).getCount( ) == 0 );
		DNDUtil.copyHandles( dataItems[0], getCell( ), 0 );
		assertTrue( "Cell has one after",
				getCell( ).getContent( ).getCount( ) == 1 );
		assertTrue( "copy item's container has same contents",
	...
	public void testMoveHandles( )
	{
		...
		assertTrue( "Cell has one after",
				getCell( ).getContent( ).getCount( ) == 1 );
		assertTrue( "copy item's container has less contents than before",
				container.getCount( ) == count - 1 );
	}
	...
	public void testEditPart2Model( )
	{
		...
		{
			Object obj = i.next( );
			assertTrue( obj.toString( ), obj instanceof DesignElementHandle
					|| obj instanceof SlotHandle );
		}
	}
	...
	public void testMoveHandlesPosition( )
	{
		...
		DataItemHandle newHandle = (DataItemHandle) getCellForMultiple( ).getContent( )
				.get( pos );
		assertTrue( "validate move position--from outside",
				newHandle.getResultSetColumn( ).equals( value ) );
		assertTrue( "check count", getCellForMultiple( ).getContent( )
				.getCount( ) == 4 );

		pos = 0;
		DNDUtil.moveHandles( newHandle, getCellForMultiple( ), pos );
		newHandle = (DataItemHandle) getCellForMultiple( ).getContent( )
				.get( pos );
		assertTrue( "validate move position--move forward",
				newHandle.getResultSetColumn( ).equals( value ) );
		assertTrue( "check count", getCellForMultiple( ).getContent( )
				.getCount( ) == 4 );
		...
		newHandle = (DataItemHandle) getCellForMultiple( ).getContent( )
				.get( pos - 1 );
		assertTrue( "validate move position--move backward",
				newHandle.getResultSetColumn( ).equals( value ) );
		assertTrue( "check count", getCellForMultiple( ).getContent( )
				.getCount( ) == 4 );
	}
...
//	}
	...
	public void testDropSource( )
	{
		...
		try
		{
			...
			lg.addElement( getElementFactory( ).newLabel( "lg" ),
					GroupHandle.HEADER_SLOT );
		}
		catch ( ContentException e1 )
		{
			e1.printStackTrace( );
	...
	public void testCopyHandlesSibling( )
	{
		...
		try
		{
			...
			int position = DNDUtil.calculateNextPosition( label,
					DNDUtil.CONTAIN_PARENT );
			DNDUtil.copyHandles( text, label );
			assertTrue( "length is right", getLength( detail ) == 3 );
			assertTrue( "position is right",
					detail.get( position ) instanceof TextItemHandle );
		}
		catch ( Exception e )
		{
			e.printStackTrace( );
		...
		try
		{
			...
			int position = DNDUtil.calculateNextPosition( dataItems[2],
					DNDUtil.CONTAIN_PARENT );
			DNDUtil.copyHandles( text, dataItems[2] );
			assertTrue( "length is right",
					getLength( getCellForMultiple( ) ) == 4 );
			assertTrue( "position is right", getCellForMultiple( ).getContent( )
					.get( position ) instanceof TextItemHandle );
		}
		catch ( Exception e )
		{
			e.printStackTrace( );
		...
		DNDUtil.copyHandles( cloneElements, dataItems[0] );
		assertTrue( "multiple order",
				getCellForSingle( ).getContent( ).get( 0 ) instanceof DataItemHandle );
		assertTrue( "multiple order",
				getCellForSingle( ).getContent( ).get( 1 ) instanceof LabelHandle );
		assertTrue( "multiple order",
				getCellForSingle( ).getContent( ).get( 2 ) instanceof TextItemHandle );
		assertTrue( "multiple order",
				getCellForSingle( ).getContent( ).get( 1 ) instanceof LabelHandle );
		assertTrue( "multiple order",
				getCellForSingle( ).getContent( ).get( 2 ) instanceof TextItemHandle );
	...
	{
		{
			...
			assertTrue( "test cell row in",
					DNDUtil.checkContainerExists( cell00, new Object[]{
							cell00, row0
					} ) );
			assertTrue( "test cell column in",
					DNDUtil.checkContainerExists( cell00, new Object[]{
							cell00, column0
					} ) );
			assertTrue( "test cell table in",
					DNDUtil.checkContainerExists( cell00, new Object[]{
							cell00, table
					} ) );
			assertFalse( "test cell row out",
					DNDUtil.checkContainerExists( cell00, new Object[]{
							cell00, row1
					} ) );
			assertFalse( "test cell column out",
					DNDUtil.checkContainerExists( cell00, new Object[]{
							cell00, column1
					} ) );
		}

		{
			...
			assertTrue( "test table group in",
					DNDUtil.checkContainerExists( tg, new Object[]{
							tg, table
					} ) );
			assertTrue( "test table group row in",
					DNDUtil.checkContainerExists( tgRow, new Object[]{
							tgRow, table
					} ) );
			assertTrue( "test table group row in",
					DNDUtil.checkContainerExists( tgRow, new Object[]{
...

		{
			...
			assertTrue( "test list group in", DNDUtil.checkContainerExists( lg,
					new Object[]{
							lg, list
					} ) );
			assertTrue( "test list group content in",
					DNDUtil.checkContainerExists( datal, new Object[]{
							datal, lg
					} ) );
			assertTrue( "test list group content in",
					DNDUtil.checkContainerExists( datal, new Object[]{
	...
	public void testIsInSameColumn( )
	{
		...
		DesignElementHandle column0 = table.getColumns( ).get( 0 );
		DesignElementHandle column1 = table.getColumns( ).get( 1 );
		DesignElementHandle column3 = getElementFactory( ).newTableItem( "2", 3 )
				.getColumns( )
				.get( 0 );
		assertTrue( "test cell column in",
				DNDUtil.isInSameColumn( new Object[]{
						cell00, column0
				} ) );
		assertFalse( "test cell column out",
				DNDUtil.isInSameColumn( new Object[]{
		...
		assertFalse( "test cell other out",
				DNDUtil.isInSameColumn( new Object[]{
						cell00, row0
				} ) );
		assertFalse( "test cell 3 out", DNDUtil.isInSameColumn( new Object[]{
				cell00, column0, column1
		} ) );
		assertTrue( "test cell 3 in", DNDUtil.isInSameColumn( new Object[]{
				cell00, column0, column0
		} ) );
		...
		assertTrue( "test grid cell column in",
				DNDUtil.isInSameColumn( new Object[]{
						gridCell00, gc0
				} ) );
		assertFalse( "test grid cell column out",
				DNDUtil.isInSameColumn( new Object[]{
						gridCell00, gc1
				} ) );
	}
