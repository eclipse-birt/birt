...
public class GroupDialog extends TitleAreaDialog {
	...
	protected void initDialog() {
		if (hierarchy != null) {
			...
			DimensionHandle dimension = DesignElementFactory.getInstance().newTabularDimension(null);
	...
	protected void saveResult() {
		...
		if (isNew) {
			...
			if (dataField != null) {
				...
				if (hierarchy.getDataSet() == null && hierarchy.getLevelCount() == 0 && cube != null
		...
		if (regularButton.getSelection()) {
			try {
				...
				if (isNew) {
					TabularLevelHandle level = DesignElementFactory.getInstance().newTabularLevel(
							(DimensionHandle) hierarchy.getContainer(), OlapUtil.getDataFieldDisplayName(dataField));
			...
			if (levelList.size() > 0) {
				for (int i = 0; i < OlapUtil.getDateTimeLevelTypeChoices().length; i++) {
					...
					if (levelList.contains(dateType) && !dateTypeSelectedList.contains(dateType)) {
				...
				for (int i = 0; i < dateTypeSelectedList.size(); i++) {
					...
					if (!levelList.contains(dateType)) {
						...
						for (; j < levelList.size(); j++) {
							if (getDateTypeNames(getLevelTypesByDateType()).indexOf(
									dateType) < getDateTypeNames(getLevelTypesByDateType()).indexOf(levelList.get(j))) {
								TabularLevelHandle level = DesignElementFactory.getInstance().newTabularLevel(
										(DimensionHandle) hierarchy.getContainer(), getDateTypeDisplayName(dateType));
								try {
									hierarchy.add(HierarchyHandle.LEVELS_PROP, level, j);
									level.setColumnName(dataField.getColumnName());
									level.setDataType(DesignChoiceConstants.COLUMN_DATA_TYPE_INTEGER);
									level.setDateTimeLevelType(dateType);
	...
	protected void createLevelViewerPart(Composite content) {
		...
		/**
		 * The two listener behaviors are so special behaviors, because they are used to
		 * fix the bug 205934
		 */
