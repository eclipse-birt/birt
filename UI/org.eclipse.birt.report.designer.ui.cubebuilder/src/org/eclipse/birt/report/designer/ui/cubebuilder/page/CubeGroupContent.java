								...
								else if ( element instanceof LevelHandle )
								{
									...
									( (LevelHandle) element ).getContainer( )
											.add( IHierarchyModel.LEVELS_PROP,
													...
													index );
									/*
									 * LevelPropertyDialog dialog = new
									 * LevelPropertyDialog( true );
									 * dialog.setInput( level ); if (
									 * dialog.open( ) == Window.CANCEL ) {
									 * SessionHandleAdapter.getInstance( )
									 * .getCommandStack( ) .rollback( ); } else
									 */
									SessionHandleAdapter.getInstance( )
	...
	protected void updateButtons( )
	{
		...
		if ( selections.size( ) == 1 )
		{
					...
					|| obj instanceof VirtualField )
			{
				...
				if ( dimenTemp != null )
				{
					DataSetHandle table = ( (TabularHierarchyHandle) dimenTemp.getDefaultHierarchy( ) ).getDataSet( );
		...
		while ( iterator.hasNext( ) )
		{
			...
			while ( iter.hasNext( ) )
			{
				...
				if ( obj instanceof TabularLevelHandle )
				{
					 ...
					 * true ); dialog.setInput( level ); if ( dialog.open( ) ==
								...
								.equals( VirtualField.TYPE_DIMENSION ) ) )
				{
					...
					try
					{
						...
						else
						{
							...
							/*
							 * LevelPropertyDialog dialog = new
							 * LevelPropertyDialog( true ); dialog.setInput(
							 * level ); if ( dialog.open( ) == Window.CANCEL ) {
							 * SessionHandleAdapter.getInstance( )
							 * .getCommandStack( ) .rollback( ); } else
							 */
							SessionHandleAdapter.getInstance( )
