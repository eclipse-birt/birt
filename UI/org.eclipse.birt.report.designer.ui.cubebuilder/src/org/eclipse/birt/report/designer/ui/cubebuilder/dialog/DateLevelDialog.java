...
public class DateLevelDialog extends BaseTitleAreaDialog {
	...
	static {
		...
		formatMap.put(DesignChoiceConstants.DATE_TIME_LEVEL_TYPE_YEAR,
				new String[][] { { new DateFormatter("yyyy").format(defaultDate), //$NON-NLS-1$
						"yyyy" //$NON-NLS-1$
				}, { new DateFormatter("yy").format(defaultDate), //$NON-NLS-1$
						"yy" //$NON-NLS-1$
				} });
		formatMap.put(DesignChoiceConstants.DATE_TIME_LEVEL_TYPE_MONTH,
				new String[][] { { new DateFormatter("MM").format(defaultDate), //$NON-NLS-1$
						"MM" //$NON-NLS-1$
		...
		formatMap.put(DesignChoiceConstants.DATE_TIME_LEVEL_TYPE_DAY_OF_YEAR,
				new String[][] { { new DateFormatter("MMMM dd, yyyy").format(defaultDate), //$NON-NLS-1$
		...
		formatMap.put(DesignChoiceConstants.DATE_TIME_LEVEL_TYPE_DAY_OF_MONTH,
				new String[][] { { new DateFormatter("dd").format(defaultDate), //$NON-NLS-1$
						"dd" //$NON-NLS-1$
				} });
		formatMap.put(DesignChoiceConstants.DATE_TIME_LEVEL_TYPE_DAY_OF_WEEK,
				new String[][] { { new DateFormatter("EEEE").format(defaultDate), //$NON-NLS-1$
						"EEEE" //$NON-NLS-1$
				} });
		formatMap.put(DesignChoiceConstants.DATE_TIME_LEVEL_TYPE_HOUR,
				new String[][] { { new DateFormatter("HH:mm:ss aaa").format(defaultDate), //$NON-NLS-1$
	...
	private String[] getAvailableDateTypeDisplayNames() {
		...
		for (int i = 0; i < dateTypeList.size(); i++) {
			dateTypeDisplayList.add(getDateTypeDisplayName(dateTypeList.get(i).toString()));
		}
	...
	private void initLevelDialog() {
		...
		if (typeCombo.getSelectionIndex() > -1) {
			formatCombo.setItems(
					getFormatDisplayItems(getAvailableDateTypeNames().get(typeCombo.getSelectionIndex()).toString()));
		}
		formatCombo.add(NONE, 0);
		formatCombo.setText(getDateFormatDisplayName(input.getDateTimeLevelType(), input.getDateTimeFormat()));
	}
	...
	private void createContentArea(Composite parent) {
		...
		typeCombo.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				formatCombo.setItems(getFormatDisplayItems(
						getAvailableDateTypeNames().get(typeCombo.getSelectionIndex()).toString()));
				formatCombo.add(NONE, 0);
			...
			for (int i = 0; i < helperProviders.length; i++) {
				...
				if (helperProvider != null && helper == null) {
					...
					if (helper != null) {
						...
						helper.setProperty(BuilderConstants.SECURITY_EXPRESSION_CONTEXT, input);
						helper.setProperty(BuilderConstants.SECURITY_EXPRESSION_PROVIDER,
								new CubeACLExpressionProvider(input));
						helper.setProperty(BuilderConstants.SECURITY_EXPRESSION_PROPERTY, input.getACLExpression());
