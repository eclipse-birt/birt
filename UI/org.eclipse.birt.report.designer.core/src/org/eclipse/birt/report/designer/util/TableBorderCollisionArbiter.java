...
public class TableBorderCollisionArbiter
{
	 ...
	 * equal, compare the style, less win; or keep the original. This is the
	 ...
	 * during actual drawing according to the CSS2.0 specification. This is the
	 * part 2 of the algorithm.
	 */
	public static boolean canExtend( int[] data, int extNBWidth,
			int extNBStyle, int leftHandWidth, int leftHandStyle,
			int leftHandX, int leftHandY, int rightHandWidth,
			int rightHandStyle, int rightHandX, int rightHandY, boolean isHead,
			boolean isVertical )
	{
		...
		/**
		 * In this case, current width is greater than any of other three edges,
		 * should extend.
		 */
		...
		/**
		 * In this case, current width is equal to one neighbour edge and
		 * greater than two neighbour edge.
		 */
		 ...
		 * greater than two neighbour edge.
		 */
		if ( data[1] > extNBWidth
				&& data[1] > leftHandWidth
				&& data[1] == rightHandWidth )
		{
		...
		/**
		 * In this case, current width is equal to two neighbour edge and
		 * greater than one neighbour edge.
		 */
		if ( data[1] > extNBWidth && data[1] == maxHandWidth )
		{
			...
			else if ( leftHandStyle == 0 && rightHandStyle != 0 )
			{
				...
				return ( data[0] < rightHandStyle )
						|| ( data[0] == rightHandStyle && ( isHead || isRightBrother ) );
			}
			else if ( data[0] == 0 )
			{
				return false;
				...
				&& data[1] == extNBWidth )
		{
			...
			else if ( extNBStyle != 0 && rightHandStyle == 0 )
			{
				...
				return ( data[0] < extNBStyle )
						|| ( data[0] == extNBStyle && isHead );
			}
			else if ( extNBStyle == 0 && rightHandStyle != 0 )
			{
				if ( data[0] == 0 || data[0] > rightHandStyle )
	...
	/**
	 * This is the part 3 of the border collision algorithm according to the
	 * CSS2.0 specification. This part is used to fix some side-situation.
	 */
	public static boolean isBrotherWin( int[] data, int extNBWidth,
			int extNBStyle, int extNBX, int extNBY, int leftHandWidth,
			int leftHandStyle, int leftHandX, int leftHandY,
			int rightHandWidth, int rightHandStyle, int rightHandX,
			int rightHandY, boolean isHead, boolean isVertical )
	{
		// if left hand is brother.
		if ( data[3] == leftHandX && data[4] == leftHandY )
		{
			return canExtend( new int[]{
					leftHandStyle, leftHandWidth, 0, 0, 0
			},
					rightHandWidth,
					rightHandStyle,
					extNBWidth,
		...
		{
			return canExtend( new int[]{
					rightHandStyle, rightHandWidth, 0, 0, 0
			},
					leftHandWidth,
					leftHandStyle,
					data[1],
