...
public class DEUtil {
	...
	public static String getRowNumExpression(DesignElementHandle baseElement, DesignElementHandle elementHandle,
			boolean checkOutLevel) {
		StringBuilder exp = new StringBuilder(IReportElementConstants.BINDING_COLUMN_PREFIX);
		if (checkOutLevel) {
			for (int i = 0; i < getBindingLevel(elementHandle, baseElement); i++) {
				exp.append(IReportElementConstants.OUTER_BINDING_COLUMN_PREFIX);
			}
		}
		exp.append(".__rownum"); // $NON-NLS-2$
		return exp.toString();
	}
