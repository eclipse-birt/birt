...
public class LibraryImportTest extends BaseTestCase
{
	...
	public void testImportLibrary( ) throws Exception
	{
		...
		TextItemHandle textLibHandle = (TextItemHandle) libHandle
		...
		TableHandle tableLibHandle = (TableHandle) libHandle
				.findElement( "myTable" ); //$NON-NLS-1$
		assertNotNull( "Table should not be null", tableLibHandle ); //$NON-NLS-1$
		StyleHandle styleLibHandle = libHandle
				.findStyle( "myStyle" ); //$NON-NLS-1$
		assertNotNull( "Style should not be null", styleLibHandle ); //$NON-NLS-1$
		ParameterHandle parameterLibHandle = libHandle
				.findParameter( "Parameter1" ); //$NON-NLS-1$
		assertNotNull( "Parameter should not be null", parameterLibHandle ); //$NON-NLS-1$
		LabelHandle labelLibHandle = (LabelHandle) libHandle
				.findElement( "myLabel" ); //$NON-NLS-1$
		assertNotNull( "Label should not be null", labelLibHandle ); //$NON-NLS-1$
		ImageHandle imageLibHandle = (ImageHandle) libHandle
				.findElement( "myImage" ); //$NON-NLS-1$
		assertNotNull( "Image should not be null", imageLibHandle ); //$NON-NLS-1$
		DataItemHandle dataLibHandle = (DataItemHandle) libHandle
				.findElement( "myData" ); //$NON-NLS-1$
		assertNotNull( "Data should not be null", dataLibHandle ); //$NON-NLS-1$
		DataSourceHandle dataSourceLibHandle = libHandle
				.findDataSource( "db2d" ); //$NON-NLS-1$
		assertNotNull( "Datasource should not be null", dataSourceLibHandle ); //$NON-NLS-1$
		DataSetHandle dataSetLibHandle = libHandle
				.findDataSet( "db2ds" ); //$NON-NLS-1$
		assertNotNull( "Dataset should not be null", dataSetLibHandle ); //$NON-NLS-1$

		TextItemHandle textHandle = (TextItemHandle) designHandle
				.getElementFactory( ).newElementFrom( textLibHandle, "myText" ); //$NON-NLS-1$
		TableHandle tableHandle = (TableHandle) designHandle
				.getElementFactory( )
				.newElementFrom( tableLibHandle, "myTable" ); //$NON-NLS-1$
		StyleHandle styleHandle = designHandle
				.getElementFactory( ).newStyle( "myStyle" ); //$NON-NLS-1$
		ParameterHandle parameterHandle = (ParameterHandle) designHandle
				.getElementFactory( ).newElementFrom( parameterLibHandle,
						"Parameter1" ); //$NON-NLS-1$
		LabelHandle labelHandle = (LabelHandle) designHandle
				.getElementFactory( )
				.newElementFrom( labelLibHandle, "myLabel" ); //$NON-NLS-1$
		ImageHandle imageHandle = (ImageHandle) designHandle
				.getElementFactory( )
				.newElementFrom( imageLibHandle, "myImage" ); //$NON-NLS-1$
		DataItemHandle dataHandle = (DataItemHandle) designHandle
				.getElementFactory( ).newElementFrom( dataLibHandle, "myData" ); //$NON-NLS-1$
		DataSourceHandle dataSourceHandle = designHandle
				.getElementFactory( ).newOdaDataSource( "db2d" ); //$NON-NLS-1$
		DataSetHandle dataSetHandle = designHandle
				.getElementFactory( ).newOdaDataSet( "db2ds" ); //$NON-NLS-1$
