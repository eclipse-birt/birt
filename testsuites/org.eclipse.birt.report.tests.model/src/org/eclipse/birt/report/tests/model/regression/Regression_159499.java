...
public class Regression_159499 extends BaseTestCase
{
	 ...
	 * <li>3, New a label, go to property editor, select "Table of Contents",
	 * set "Label", set Style for TOC.
	 * <p>
	 * <br>
	 * 
	...
	{
		/**
		 * openLibrary( LIB ); // find label LabelHandle lib_label =
		 * (LabelHandle) libraryHandle.findElement( "label1" ); assertNotNull(
		 * lib_label ); // find style StyleHandle style =
		 * libraryHandle.findStyle( "Style" ); assertNotNull( style ); // set a
		 * toc TOC toc = StructureFactory.createTOC( ); TOCHandle tocHandle =
		 * (TOCHandle) lib_label.addTOC( toc ); tocHandle.setStyleName(
		 * style.getName( ) ); assertEquals( "Style", tocHandle.getStyleName( ) );
		 * assertEquals( "#FF0000", tocHandle.getColor( ).getStringValue( ) );
		 * assertEquals( "\"Arial\"", tocHandle.getFontFamily( ).getValue( ) );
		 * assertEquals( "#FFFF00", tocHandle.getBackgroundColor(
		 * ).getStringValue( ) ); saveAs( libraryHandle,
		 * "Regression_159499_lib_temp.xml" );
		 */
	 ...
	 * <li>1, New a report.
	 * <li>2, New a custom style "Style1", set Font=Arial, Font Color= Red,
	 * Background color=Yellow. New a custom style "Style2", set Font=Cursive,
	 * Font Color= Fuchsia, Background color=Blue.
	 * <li>3, New a label, go to property editor,Select "Table of Contents",
	 * set "Label", set Style1 for TOC.
