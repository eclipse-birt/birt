...
public class ReportDocumentTest extends EngineCase
{
	...
	public void testReportDocument( )
	{
		...
		try
		{
			...
			assertEquals( "table toc expression isn't TableSection",
					"TableSection", tableNode.getDisplayString( ) );
			assertNotNull( "table toc doesn't contain nodes", tableNode
					.getChildren( ) );

			assertEquals( "table toc doesn't contain 6 nodes", 6, tableNode
					.getChildren( ).size( ) );
			headerNode = (TOCNode) tableNode.getChildren( ).get( 0 );
			groupNode = (TOCNode) ( (TOCNode) tableNode.getChildren( ).get( 1 ) )
					.getChildren( ).get( 0 );
			...
			assertEquals( "header toc expression isn't HeaderSection",
					"HeaderSection", headerNode.getDisplayString( ) );
			assertEquals( "group toc expression isn't GroupSection",
					"GroupSection", groupNode.getDisplayString( ) );
			assertEquals( "footer toc expression isn't FooterSection",
					"FooterSection", footerNode.getDisplayString( ) );

			assertNotNull( "table group toc doesn't contain nodes", groupNode
					.getChildren( ) );
			detailNode = (TOCNode) ( (TOCNode) tableNode.getChildren( ).get( 1 ) )
					.getChildren( ).get( 1 );
			assertEquals( "detail toc expression isn't DetailSection",
					"DetailSection", detailNode.getDisplayString( ) );

			// check Archive(done)
			IDocArchiveReader arch = reportDoc.getArchive( );
			assertNotNull( "get null document archive", arch );
			assertEquals( "get incorrect document archive", archive.getName( ),
					arch.getName( ) );
			...
			assertTrue( "bookmarks doesn't contain " + hMark, bookmarks
					.contains( hMark ) );
			assertTrue( "bookmarks doesn't contain " + gMark, bookmarks
					.contains( gMark ) );
			assertTrue( "bookmarks doesn't contain " + dMark, bookmarks
					.contains( dMark ) );
			assertTrue( "bookmarks doesn't contain " + fMark, bookmarks
					.contains( fMark ) );
			assertTrue( "bookmarks doesn't contain " + pMark, bookmarks
					.contains( pMark ) );
			...
			List table_children = reportDoc
					.getChildren( table_toc.getNodeID( ) );
			assertNotNull( "table toc's children is null", table_children );
			assertEquals( "table toc contains 6 children", 6, table_children
					.size( ) );
			...
			assertNotNull( "p1 parameter returned null", params
					.get( new String( "p1" ) ) );
			assertNotNull( "p2 parameter returned null", params
					.get( new String( "p2" ) ) );
			assertEquals( "p1 parameter returned wrong value", "p1string",
					params.get( new String( "p1" ) ) );
			assertEquals( "p1 parameter returned wrong value",
			...
			assertEquals( "return wrong reportRunnable", reportRunnable
					.getReportEngine( ), report.getReportEngine( ) );
		}
		catch ( Exception e )
		{
			e.printStackTrace( );
	...
	public void testReportDocument_abnormal( )
	{
		...
		/*
		 * String
		 * report_design="D:/TEMP/workspace3.1/org.eclipse.birt.report.tests.engine/input/report_document.rptdesign";
		 * String
		 * report_document="D:/TEMP/workspace3.1/org.eclipse.birt.report.tests.engine/output/report_document";
		 */
		...
		try
		{
			...
			assertEquals( "findToc should return null", null, tocNode
					.getDisplayString( ) );
			...
			assertEquals( "getChildren should return root node's children", 1,
					children.size( ) );
