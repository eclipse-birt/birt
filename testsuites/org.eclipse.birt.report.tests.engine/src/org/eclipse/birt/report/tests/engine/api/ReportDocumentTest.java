...
public class ReportDocumentTest extends EngineCase
{
	...
	public void testReportDocument( )
	{
		...
		try
		{
			...
			assertEquals(
					"table toc expression isn't TableSection",
					"TableSection",
					tableNode.getDisplayString( ) );
			...
			assertEquals(
					"header toc expression isn't HeaderSection",
					"HeaderSection",
					headerNode.getDisplayString( ) );
			assertEquals(
					"group toc expression isn't GroupSection",
					"GroupSection",
					groupNode.getDisplayString( ) );
			assertEquals(
					"footer toc expression isn't FooterSection",
					"FooterSection",
					footerNode.getDisplayString( ) );
			...
			detailNode = (TOCNode) ( (TOCNode) tableNode.getChildren( ).get( 1 ) )
					...
					.get( 1 );
			assertEquals(
					"detail toc expression isn't DetailSection",
					"DetailSection",
					detailNode.getDisplayString( ) );

			// check Archive(done)
			IDocArchiveReader arch = reportDoc.getArchive( );
			assertNotNull( "get null document archive", arch );
			assertEquals(
					"get incorrect document archive",
					archive.getName( ),
					arch.getName( ) );
			...
			assertEquals(
					"return wrong page number which contains bookmark",
					2,
					reportDoc.getPageNumber( pMark ) );
			...
			assertNotNull( "p2 parameter returned null", params
					.get( new String( "p2" ) ) );
			assertEquals(
					"p1 parameter returned wrong value",
					"p1string",
					params.get( new String( "p1" ) ) );
			assertEquals(
					"p1 parameter returned wrong value",
					new Integer( 2 ),
					params.get( new String( "p2" ) ) );

			// check report document name
			String name = report_document.substring( report_document
					.indexOf( "org" ), report_document.length( ) );
			name = name.replace( '/', '\\' );
			assertEquals( "return wrong report document name", name.replace(
					'\\',
					'/' ), reportDoc.getName( ).substring(
					reportDoc.getName( ).indexOf( "org" ),
					reportDoc.getName( ).length( ) ).replace( '\\', '/' ) );
	...
	public void testReportDocument_abnormal( )
	{
		...
		try
		{
			...
			assertEquals(
					"getChildren should return root node's children",
					1,
					children.size( ) );
