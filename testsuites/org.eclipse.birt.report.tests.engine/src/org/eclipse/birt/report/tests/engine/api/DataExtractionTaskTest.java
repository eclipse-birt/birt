...
public class DataExtractionTaskTest extends EngineCase {
	...
	public void testDataExtractionWithFilter() throws Exception {
		...
		try {
			...
			filterExpression[0] = new FilterDefinition(
					new ConditionalExpression("row[\"territory\"]", ConditionalExpression.OP_EQ, "\"EMEA\"", null));
			...
			if (result != null) {
				...
				if (data != null) {
					...
					assertEquals("Fail to extract filtered data1", 4, officecode);
					if (data.next()) {
						officecode = Integer.parseInt(data.getValue("code").toString());
						assertEquals("Fail to extract filtered data2", 7, officecode);
					}
				}
				data.close();
			} else {
	...
	public void testDataExtractionFromIID_subquery() throws Exception {
		...
		try {
			...
			IFilterDefinition[] filterExpression = new IFilterDefinition[1];
			filterExpression[0] = new FilterDefinition(new ConditionalExpression("row[\"name\"]",
					ConditionalExpression.OP_EQ, "\"Signal Gift Stores\"", null));
			extractTask.setFilters(filterExpression);
			...
			filterExpression[0] = new FilterDefinition(
					new ConditionalExpression("row[\"number\"]", ConditionalExpression.OP_EQ, "112", null));
			extractTask.setFilters(filterExpression);
			result = extractTask.extract();
