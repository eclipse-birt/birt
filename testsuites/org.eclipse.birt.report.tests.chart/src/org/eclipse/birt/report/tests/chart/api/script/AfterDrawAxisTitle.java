 ...
 * Description:
 * </p>
 * function afterDrawAxisTitle()
 * </p>
 */

public class AfterDrawAxisTitle extends ChartTestCase {

	private static String OUTPUT = "AfterDrawAxisTitle.jpg"; //$NON-NLS-1$
	...
	public static void main(String[] args) {
		new AfterDrawAxisTitle();
	}

	/**
	 * Constructor
	 */
	public AfterDrawAxisTitle() {
	...
	public static final Chart createLineChart() {
		...
		cwaLine.setScript("function afterDrawAxisTitle( axis, label, context )" //$NON-NLS-1$
				+ "{importPackage(Packages.java.lang); " //$NON-NLS-1$
				+ "System.out.println(label.getCaption().getValue()); " //$NON-NLS-1$
				+ "if (label.getCaption().getValue() == \"X-Axis Title\") " //$NON-NLS-1$
				+ "{System.out.println(\"X\");} " //$NON-NLS-1$
				+ "else" //$NON-NLS-1$
				+ "{System.out.println(\"Y\");} }" //$NON-NLS-1$
		);
