	...
	protected void creatImageCompare( String folderName ) throws Exception
	{
		...
		for ( int i = 0; i < diffreports.length; i++ )
		{
			try
			{
				...
				LabelHandle labelGoldenHandle = (LabelHandle) designHandle
						.getElementFactory( ).newLabel( "goldenLabel" );
				LabelHandle labelOutputHandle = (LabelHandle) designHandle
						.getElementFactory( ).newLabel( "outputLabel" );
				ImageHandle imageGoldenHandle = (ImageHandle) designHandle
