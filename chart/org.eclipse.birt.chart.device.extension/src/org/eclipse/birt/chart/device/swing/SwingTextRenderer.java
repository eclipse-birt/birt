...
final class SwingTextRenderer implements IConstants
{
     ...
     * @return
     */
    public synchronized static final SwingTextRenderer instance(SwingDisplayServer sxs)
    {
        if (_tr == null)
        {
     ...
     *            The position of the label w.r.t. the location specified by 'lo'
     * @param lo
     *            The location (specified as a 2d point) where the text is to be rendered
     * @param la
     *            The chart model structure containing the encapsulated text (and attributes) to be rendered
     */
        ...
        throws RenderingException
    {
        ...
        showTopValue(idr, LocationImpl.create(bb.getLeft(), bb.getTop() + bb.getHeight()), la, false);
    }
    ...
    private final void showLeftValue(IPrimitiveRenderer ipr, Location lo, Label la, boolean bShadow)
    {
        ...
        if (dAngleInDegrees == 0)
        {
            ...
            if (bShadow) // RENDER THE SHADOW
            {
                g2d.setPaint(
                	new GradientPaint(
                	    new Point2D.Double(dX + 3, (dY - dYHalfOffset) + 3),
                	    (Color) _sxs.getColor(la.getShadowColor()), 
                	    new Point2D.Double(dX + 3 + dFW, (dY - dYHalfOffset) + 3 + dFH),
        ...
        else if (dAngleInDegrees > 0 && dAngleInDegrees < 90)
        {
            ...
            if (bShadow)
            {
                ...
                g2d.setPaint(
                	new GradientPaint(
                	    new Point2D.Double(dX + 3, (dY - dFH) + 3),
                	    (Color) _sxs.getColor(la.getShadowColor()), 
                	    new Point2D.Double(dX + 3 + dFW, (dY - dFH) + 3 + dFH),
                	    (Color) _sxs.getColor(la.getShadowColor().translucent()) 
            	    )
                );
        ...
        else if (dAngleInDegrees < 0 && dAngleInDegrees > -90)
        {
            ...
            if (bShadow)
            {
                ...
                g2d.setPaint(
                    new GradientPaint(
	            	    new Point2D.Double(dX + 3, (dY - dH) + 3),
	            	    (Color) _sxs.getColor(la.getShadowColor()), 
        ...
        else if (dAngleInDegrees < 0 && dAngleInDegrees > -90)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < iLC; i++)
                {
                    ...
                    tl.draw(g2d, (float) (dX + dXOffset + ins.getLeft()), (float) ((dY - dD - dH * (iLC - i - 1)) - ins
                        .getBottom()));
                }
    ...
    private final void showTopValue(IPrimitiveRenderer ipr, Location lo, Label la, boolean bShadow)
    {
        ...
        else if (dAngleInDegrees < 0 && dAngleInDegrees > -90)
        {
            ...
            g2d.rotate(-dAngleInRadians, dX, dY);
            /*
             * final RotatedRectangle rr = computePolygon(IConstants.ABOVE, la, lo.getX(), lo.getY());
             * g2d.setColor(Color.blue); g2d.draw(rr); final BoundingBox bb = computeBox(IConstants.ABOVE, la,
        ...
        else if (dAngleInDegrees == 90)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < itm.getLineCount(); i++)
                {
                    ...
                    tl.draw(g2d, (float) (dX + dXOffset + ins.getLeft()),
                        (float) (((dY - dD) - ((itm.getLineCount() - i) * dH - (iLC + 1) * dH / 2)) + ins.getTop()));
                }
