...
/**
 * Representation of an indexed palette of arbitrary size. This can be used by image encoders which require limited
 * palettes (eg 256 colour PNGs). The palette is autogenerated from a number of images and potentially some extra
 * colours which must be included. Optional transparency is supported, but only in a binary way - any colour which is
 * not totally opaque is deemed to be totally transparent. (This means we can get away with a single index for all
 * transparent colours.) The generation method is pretty basic, removing a single colour at a time. This is fine for
 * images with a smallish number of colours to begin with (eg tiled collections of 256-colour images) but bad for
 * full-colour photos.
 */
final class Palette
{
    ...
    Palette(BufferedImage[] images, Color[] extraColours, int size, boolean wantTransparent)
    {
        ...
        /*
         * Reduction method: Find the colour with the minimum population Find the colour closest to it Merge the two
         */
        while (colours.size() > size)
        {
            ...
            closeCe
                .setGreen((closeCe.getGreen() * closeCe.population + minCe.getGreen() * minCe.population) / totalPop);
    ...
    private static class ColourEntry
    {
        ...
        /*
         * Compare two colours. We ignore the alpha value as transparency is just the first colour, which is ignored in
         * the algorithm.
         */
