 ...
 */
public class MeterSeriesUIProvider implements ISeriesUIProvider
{
	...
	{
		return new MeterSeriesAttributeComposite( parent, SWT.NONE, series );
	}
			...
			Object oContext )
	{
		...
		{
			sPrefix = Messages.getString( "MeterSeriesUIProvider.Lbl.BasePrefix" ); //$NON-NLS-1$
		}
		else
		{
			sPrefix = Messages.getString( "MeterSeriesUIProvider.Lbl.OrthogonalPrefix" ); //$NON-NLS-1$
		}
		...
		{
			sTitle = sPrefix
					+ Messages.getString( "MeterSeriesUIProvider.Lbl.SeriesDefinition" ); //$NON-NLS-1$
		}
		else
		{
