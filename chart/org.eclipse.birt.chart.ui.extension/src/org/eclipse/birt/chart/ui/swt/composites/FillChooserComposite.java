...
public class FillChooserComposite extends Composite implements
		SelectionListener, MouseListener, DisposeListener, KeyListener {
	...
	private final String[] saImageTypes = new String[] { "*.gif", "*.jpg",
			"*.png" };
	...
	private Color[] createColorMap(Display display) {
		return new Color[] { new Color(display, 0, 0, 0),
				new Color(display, 154, 50, 0), new Color(display, 51, 51, 0),
				new Color(display, 0, 50, 0), new Color(display, 0, 50, 100),
				new Color(display, 0, 0, 128), new Color(display, 51, 51, 153),
				new Color(display, 51, 51, 51),

				new Color(display, 128, 0, 0), new Color(display, 254, 102, 0),
				new Color(display, 124, 124, 0), new Color(display, 0, 128, 0),
				new Color(display, 0, 128, 128), new Color(display, 0, 0, 254),
				new Color(display, 102, 102, 153),
				new Color(display, 128, 128, 128),

				new Color(display, 254, 0, 0), new Color(display, 254, 153, 0),
				...
				new Color(display, 254, 254, 0), new Color(display, 0, 254, 0),
	...
	private void toggleDropDown() {
		if (cmpDropDown == null || cmpDropDown.isDisposed()
			...
			createDropDownComponent(pLoc.x, pLoc.y + cnvSelection.getSize().y
					+ 1);
		} else {
	...
	public void widgetSelected(SelectionEvent e) {
		...
		if (oSource.equals(btnDown)) {
			...
			if (fCurrent instanceof Image) {
				...
				sImageFile = sFullPath
						.substring(sFullPath.lastIndexOf("/") + 1);
				sStartFolder = sFullPath.substring(0, sFullPath
						.lastIndexOf("/"));
			}
			...
			try {
				...
				sImgPath = "file:///" + fDlg.getFilterPath() + File.separator
						+ fDlg.getFileName();
			}
			...
			RGB rgb = cDlg.getRGB();
			if (rgb != null) {
				ColorDefinition cdNew = AttributeFactory.eINSTANCE
	...
	private void fireHandleEvent(int iType) {
		for (int iL = 0; iL < vListeners.size(); iL++) {
			Event se = new Event();
			se.widget = this;
			se.data = (fCurrent != null) ? fCurrent : ColorDefinitionImpl
					.TRANSPARENT();
			se.type = iType;
	...
	public void mouseDown(MouseEvent e) {
		...
		if (e.getSource().equals(cnvSelection)) {
			...
			ColorDefinition cTmp = AttributeFactory.eINSTANCE
					.createColorDefinition();
			Color clrTmp = ((ColorSelectionCanvas) e.getSource()).getColorAt(
					e.x, e.y);
...
class ColorSelectionCanvas extends Canvas implements PaintListener {
	...
	public void paintControl(PaintEvent pe) {
		...
		for (int iR = 0; iR < 5; iR++) {
			for (int iC = 0; iC < 8; iC++) {
				...
				gc.fillRectangle(iC * iCellWidth, iR * iCellHeight, iCellWidth,
						iCellHeight);
				// Hilight currently selected color if it exists in this list
				if (colorSelection != null
						&& colorSelection.equals(colorMap[(iR * 8) + iC])) {
					gc.drawRectangle(iC * iCellWidth, iR * iCellHeight,
							iCellWidth - 2, iCellHeight - 2);
					gc.setForeground(cWhite);
					gc.drawRectangle(iC * iCellWidth + 1, iR * iCellHeight + 1,
							iCellWidth - 3, iCellHeight - 3);
					gc.setForeground(cBlack);
				}
			}
		}
	...
	/**
	 * This method assumes a color array of 40 colors arranged with equal sizes
	 * in a 8x5 grid.
	 * 
	 * @param x
	 * @param y
