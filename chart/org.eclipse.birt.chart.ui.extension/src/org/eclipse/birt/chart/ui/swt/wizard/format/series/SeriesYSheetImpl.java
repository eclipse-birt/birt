...
public class SeriesYSheetImpl extends SubtaskSheetImpl implements Listener, SelectionListener {
	...
	public void createControl(Composite parent) {
		...
		btnLabelVisible = getContext().getUIFactory().createChartCheckbox(cmpBottom, SWT.NONE, false);
		{
			org.eclipse.birt.chart.model.component.Label l = null;
			org.eclipse.birt.chart.model.component.Label defLabel = null;
			if (isMeterSeries()) {
				l = ((DialSeries) getCurrentDesignTimeSeries()).getDial().getLabel();
			...
			btnLabelVisible.setSelectionState(
					l.isSetVisible() ? (l.isVisible() ? ChartCheckbox.STATE_SELECTED : ChartCheckbox.STATE_UNSELECTED)
		...
		if (isShowTaskLabelsAvailable()) {
			...
			{
				org.eclipse.birt.chart.model.component.Label l = ((GanttSeries) getCurrentDesignTimeSeries())
						.getDecorationLabel();
				btnDecoVisible.setDefaultSelection(((GanttSeries) defSeries).getDecorationLabel().isVisible());
				btnDecoVisible.setSelectionState(l.isSetVisible()
						? (l.isVisible() ? ChartCheckbox.STATE_SELECTED : ChartCheckbox.STATE_UNSELECTED)
						: ChartCheckbox.STATE_GRAYED);
	...
	protected void createButtonGroup(Composite parent) {
		...
		if (isMeterSeries()) {
			...
			if (needDialNeedle()) {
				...
				popup = new NeedleSheet(Messages.getString("SeriesYSheetImpl.Label.Needles"), //$NON-NLS-1$
						getContext(), getCurrentDesignTimeSeries());
				Button btnNeedles = createToggleButton(cmp, BUTTON_NEEDLES,
			...
			popup = new SeriesRegionSheet(Messages.getString("SeriesYSheetImpl.Label.Region"), //$NON-NLS-1$
					getContext(), getCurrentDesignTimeSeries());
			Button btnDialRegion = createToggleButton(cmp, BUTTON_REGIONS,
			...
			popup = new DialTickSheet(Messages.getString("DialTicksDialog.Title.DialTicks"), //$NON-NLS-1$
					getContext(), ((DialSeries) getCurrentDesignTimeSeries()));
			Button btnDialTicks = createToggleButton(cmp, BUTTON_TICKS,
			...
			popup = new DialScaleSheet(Messages.getString("DialScaleDialog.Title.DialScale"), //$NON-NLS-1$
					getContext(), ((DialSeries) getCurrentDesignTimeSeries()));
			Button btnDialScale = createToggleButton(cmp, BUTTON_SCALE,
		...
		if (isMarkerAvailable()) {
			popup = new LineSeriesMarkerSheet(Messages.getString("SeriesYSheetImpl.Label.Markers"), //$NON-NLS-1$
					getContext(), (LineSeries) getCurrentDesignTimeSeries());
			Button btnLineMarker = createToggleButton(cmp, BUTTON_MARKERS,
		...
		if (isTrendlineAvailable()) {
			popup = new SeriesTrendlineSheet(Messages.getString("SeriesYSheetImpl.Label.Trendline"), //$NON-NLS-1$
					getContext(), getCurrentDesignTimeSeries());
			Button btnTrendline = createToggleButton(cmp, BUTTON_CURVE,
					Messages.getString("SeriesYSheetImpl.Label.Trendline&"), //$NON-NLS-1$
					popup, btnShowCurveLine.getSelection());
		...
		if (getContext().isInteractivityEnabled()
			...
			popup = new InteractivitySheet(Messages.getString("SeriesYSheetImpl.Label.Interactivity"), //$NON-NLS-1$
					getContext(), getCurrentDesignTimeSeries().getTriggers(), getCurrentDesignTimeSeries(),
		...
		for (ISeriesButtonEntry buttonEntry : getSeriesUIProvider(series).getCustomButtons(getContext(), sd)) {
			Button button = createToggleButton(cmp, buttonEntry.getButtonId(), buttonEntry.getPopupName(),
					buttonEntry.getPopupSheet(), buttonEntry.isEnabled());
	...
	protected void createPieSeriesButton(Composite cmp) {
		...
		popup = new PieTitleSheet(Messages.getString("SeriesYSheetImpl.Label.Titles"), //$NON-NLS-1$
				getContext(), getCurrentDesignTimeSeries());
		Button btnPieTitle = createToggleButton(cmp, BUTTON_TITLE, Messages.getString("SeriesYSheetImpl.Label.Titles&"), //$NON-NLS-1$
				popup);
	...
	protected void createDifferenceSeriesButton(Composite cmp) {
		...
		popup = new LineSeriesMarkerSheet(Messages.getString("SeriesYSheetImpl.Label.PositiveMarkers"), //$NON-NLS-1$
				getContext(), (DifferenceSeries) getCurrentDesignTimeSeries(), true);
		Button btnPLineMarker = createToggleButton(cmp, BUTTON_POSITIVE_MARKERS,
		...
		popup = new LineSeriesMarkerSheet(Messages.getString("SeriesYSheetImpl.Label.NegativeMarkers"), //$NON-NLS-1$
				getContext(), (DifferenceSeries) getCurrentDesignTimeSeries(), false);
		Button btnNLineMarker = createToggleButton(cmp, BUTTON_NEGATIVE_MARKERS,
	...
	protected void createGanttSeriesButton(Composite cmp) {
		...
		popup = new DecorationSheet(Messages.getString("SeriesYSheetImpl.Label.Decoration"), //$NON-NLS-1$
				getContext(), (GanttSeries) getCurrentDesignTimeSeries());
		Button btnDecoration = createToggleButton(cmp, BUTTON_DECORATION,
				Messages.getString("SeriesYSheetImpl.Label.Decoration&"), //$NON-NLS-1$
				popup, getContext().getUIFactory().canEnableUI(btnDecoVisible));
	...
	protected boolean needDialNeedle() {
		return getChart() instanceof DialChart && !((DialChart) getChart()).isDialSuperimposition();
	}
	...
	protected void createSeriesLabelBtnUI(Composite cmp) {
		...
		popup = new SeriesLabelSheet(Messages.getString("SeriesYSheetImpl.Label.Labels"), //$NON-NLS-1$
				getContext(), getCurrentDesignTimeSeries());
		Button btnLabel = createToggleButton(cmp, BUTTON_LABEL, Messages.getString("SeriesYSheetImpl.Label.Labels&"), //$NON-NLS-1$
	...
	protected void createDialLabelBtnUI(Composite cmp) {
		...
		popup = new DialLabelSheet(Messages.getString("SeriesYSheetImpl.Label.DialLabels"), //$NON-NLS-1$
				getContext(), getCurrentDesignTimeSeries());
		Button btnDialLabel = createToggleButton(cmp, BUTTON_DIAL_LABELS,
	...
	protected void getSeriesAttributeUI(Series series, Composite parent) {
		getSeriesUIProvider(series).getSeriesAttributeSheet(parent, series, getContext());
	}
	...
	public void widgetSelected(SelectionEvent e) {
		...
		if (e.widget.equals(btnShowCurveLine)) {
			...
			if (btnShowCurveLine.getSelection()) {
				...
				cf.eAdapters().addAll(getCurrentDesignTimeSeries().eAdapters());
			...
			if (isMeterSeries()) {
				setToggleButtonEnabled(BUTTON_DIAL_LABELS, getContext().getUIFactory().canEnableUI(btnLabelVisible));
				if (isAuto) {
					((DialSeries) getCurrentDesignTimeSeries()).getDial().getLabel().unsetVisible();
				} else {
					((DialSeries) getCurrentDesignTimeSeries()).getDial().getLabel()
				...
				setToggleButtonEnabled(BUTTON_LABEL, getContext().getUIFactory().canEnableUI(btnLabelVisible));
				if (isAuto) {
			...
			setToggleButtonEnabled(BUTTON_DECORATION, getContext().getUIFactory().canEnableUI(btnDecoVisible));
			if (btnDecoVisible.getSelectionState() == ChartCheckbox.STATE_GRAYED) {
				((GanttSeries) getCurrentDesignTimeSeries()).getDecorationLabel().unsetVisible();
			} else {
			...
			if (btnDecoVisible.getSelectionState() != ChartCheckbox.STATE_SELECTED && btnDecoration.getSelection()) {
	...
	protected boolean isTrendlineAvailable() {
		return (getChart() instanceof ChartWithAxes) && (!isGanttSeries()) && (!isDifferenceSeries())
				&& (getChart().getDimension() != ChartDimension.THREE_DIMENSIONAL_LITERAL)
				&& getContext().isEnabled(ChartUIConstants.SUBTASK_SERIES_Y + ChartUIConstants.BUTTON_CURVE);
	}
	...
	protected boolean isMarkerAvailable() {
		return getCurrentDesignTimeSeries() instanceof LineSeries && !isDifferenceSeries();
	}
}
