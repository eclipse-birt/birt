...
public class Dial extends BaseRenderer
{
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.birt.chart.render.ISeriesRenderer#compute(org.eclipse.birt.chart.model.attribute.Bounds,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.birt.chart.render.ISeriesRenderer#renderSeries(org.eclipse.birt.chart.device.IPrimitiveRenderer,
	 *      org.eclipse.birt.chart.model.layout.Plot,
	 *      org.eclipse.birt.chart.render.ISeriesRenderingHints)
	 */
			...
			ISeriesRenderingHints isrh ) throws ChartException
	{
		...
		final Bounds boCB = isDialSuperimposed( ) ? getPlotBounds( )
				: getCellBounds( );
		...
		if ( isDialSuperimposed( ) )
		{
			...
			for ( int i = 1; i < iSeriesCount; i++ )
			{
				...
				if ( first )
				{
					...
					startAngle = ( (Dial) br ).getActualRenderer( )
							.getDialStartAngle( );
					stopAngle = ( (Dial) br ).getActualRenderer( )
							.getDialStopAngle( );
					sc = ( (Dial) br ).getActualRenderer( ).getDialScale( );
					inverseScale = ( (Dial) br ).getActualRenderer( )
							.isInverseScale( );
				...
				else
				{
					radius = Math.max( radius,
							( (Dial) br ).getActualRenderer( ).getDialRadius( ) );
				}
			}
			...
			for ( int i = 1; i < iSeriesCount; i++ )
			{
				...
				if ( sc.getMin( ) == null || sc.getMax( ) == null )
				{
					...
					for ( int j = 1; j < iSeriesCount; j++ )
					{
						...
						min = Math.min( min,
								( (Dial) innerbr ).getActualRenderer( )
										.getValue( ) );
						max = Math.max( max,
								( (Dial) innerbr ).getActualRenderer( )
										.getValue( ) );
					...
					if ( sc.getMin( ) == null
							&& Methods.getLocation( asc, min ) < asc.getStart( ) )
					{
						nsc = sc.copyInstance( );
					...
					if ( sc.getMax( ) == null
							&& Methods.getLocation( asc, max ) > asc.getEnd( ) )
					{
						if ( nsc == null )
						{
							nsc = sc.copyInstance( );
					...
					if ( needChange )
					{
						sc = nsc;
						asc = ( (Dial) br ).getActualRenderer( )
								.getAutoScale( startAngle, stopAngle, nsc, boCB );
					}
				}

				// Compute the spacing using the final scale.
				extraSpacing = ( (Dial) br ).getActualRenderer( )
						.getDialExtraSpacing( asc );
			...
			for ( int i = 1; i < iSeriesCount; i++ )
			{
				...
				( (Dial) br ).getActualRenderer( ).updateRadius( radius );
				( (Dial) br ).getActualRenderer( )
						.updateStartAngle( startAngle );
				...
				( (Dial) br ).getActualRenderer( )
						.updateExtraSpacing( extraSpacing );
				( (Dial) br ).getActualRenderer( )
						.updateInverseScale( inverseScale );
			}
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.birt.chart.render.ISeriesRenderer#renderLegendGraphic(org.eclipse.birt.chart.device.IPrimitiveRenderer,
			...
			Fill fPaletteEntry, Bounds bo ) throws ChartException
	{
		...
		final DialSeries ds = (DialSeries) getSeries( );
		final ColorDefinition cd = ds.getDial( )
				.getLineAttributes( )
				.getColor( );
