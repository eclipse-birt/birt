...
public final class Gantt extends AxesRenderer implements IAxesDecorator
{
			...
			ISeriesRenderingHints isrh ) throws ChartException
	{
		...
		for ( int i = 0; i < dpha.length; i++ )
		{
			...
			if ( cwa.isTransposed( ) )
			{
				...
				loaLine[0] = goFactory.createLocation( dStart, dY
						+ iLineWidth
						/ 2.0 );
				loaLine[1] = goFactory.createLocation( dStart, dY
						- iLineWidth
						/ 2.0 );
				loaLine[2] = goFactory.createLocation( dEnd, dY
						- iLineWidth
						/ 2.0 );
				loaLine[3] = goFactory.createLocation( dEnd, dY
						+ iLineWidth
						/ 2.0 );
			}
			else
			...
			// SYMMETRIC CODE)
			{
				...
				loaLine = new Location[4];
				loaLine[0] = goFactory.createLocation( dX + iLineWidth / 2.0,
						dStart );
				loaLine[1] = goFactory.createLocation( dX - iLineWidth / 2.0,
						dStart );
				loaLine[2] = goFactory.createLocation( dX - iLineWidth / 2.0,
						dEnd );
				loaLine[3] = goFactory.createLocation( dX + iLineWidth / 2.0,
						dEnd );
			}
			...
			if ( lia.isVisible( ) && geStart != null && geEnd != null )
			{
				...
				if ( ( iLineWidth == 1 )
						|| ( lia.getStyle( ) != LineStyle.SOLID_LITERAL ) )
				{
					lre = ( (EventObjectCache) ipr ).getEventObject( WrappedStructureSource.createSeriesDataPoint( gs,
							dpha[i] ),
				...
				{
					pre = ( (EventObjectCache) ipr ).getEventObject( WrappedStructureSource.createSeriesDataPoint( gs,
							dpha[i] ),
			...
			if ( geStart != null && mStart.isVisible( ) || dpha[i].isOutside( ) )
			{
				...
				switch ( mStartPosition.getValue( ) )
				{
					case Position.ABOVE :
						if ( bTransposed )
						{
							loStartMarker.set( loStart.getX( ), loStart.getY( )
									- mStart.getSize( ) );
						}
						break;
					case Position.BELOW :
				...
				if ( fPaletteEntry instanceof MultipleFill
						&& ( (MultipleFill) fPaletteEntry ).getFills( ).size( ) > 0 )
				{
					startFill = ( (MultipleFill) fPaletteEntry ).getFills( )
							.get( 0 );
				}
			...
			if ( geEnd != null && mEnd.isVisible( ) || dpha[i].isOutside( ) )
			{
				...
				switch ( mEndPosition.getValue( ) )
				{
					case Position.ABOVE :
						if ( bTransposed )
						{
							loEndMarker.set( loEnd.getX( ), loEnd.getY( )
									- mEnd.getSize( ) );
						}
						break;
					case Position.BELOW :
				...
				if ( fPaletteEntry instanceof MultipleFill
						&& ( (MultipleFill) fPaletteEntry ).getFills( ).size( ) > 1 )
				{
					endFill = ( (MultipleFill) fPaletteEntry ).getFills( )
							.get( 1 );
				}
		...
		if ( laDataPoint.isVisible( ) )
		{
			...
			final double dLabelGap = Math.max( Math.max( mStart.isVisible( ) ? mStart.getSize( )
					: 0,
					mEnd.isVisible( ) ? mEnd.getSize( ) : 0 ),
					( lia.isVisible( ) ? iLineWidth / 2 : 0 )
							+ ( outlinelia.isVisible( ) ? outlinelia.getThickness( )
									: 0 ) );
			...
			for ( int i = 0; i < dpha.length; i++ )
			{
				...
				{
					laDataPoint.getCaption( )
							.setValue( dpha[i].getDisplayValue( ) );
				}
				...
				switch ( pDataPoint.getValue( ) )
				{
					case Position.ABOVE :
						if ( !cwa.isTransposed( ) )
						{
							loDataPoint.set( faX[i], faY[i][1]
									- mEnd.getSize( )
									- p.getVerticalSpacing( ) );
						}
						else
						{
							loDataPoint.set( ( faY[i][0] + faY[i][1] ) / 2,
									faX[i] - dLabelGap - p.getVerticalSpacing( ) );
						}
						break;
					case Position.BELOW :
						if ( !cwa.isTransposed( ) )
						{
							loDataPoint.set( faX[i], faY[i][0]
									+ mStart.getSize( )
									+ p.getVerticalSpacing( ) );
						}
						else
					...
					case Position.LEFT :
						...
						{
							loDataPoint.set( faY[i][0]
									- dLabelGap
									- p.getHorizontalSpacing( ), faX[i] );
						}
						break;
					case Position.RIGHT :
						if ( !cwa.isTransposed( ) )
				...
				if ( laDataPoint.isVisible( ) )
				{
					renderLabel( WrappedStructureSource.createSeriesDataPoint( gs,
							dpha[i] ),
			...
			Fill fPaletteEntry, Bounds bo ) throws ChartException
	{
		...
		Location[] points = new Location[]{
				goFactory.createLocation( bo.getLeft( ) + 1, bo.getTop( )
						+
						bo.getHeight( ) /
						2d +
						liaMarker.getThickness( ) /
						2d ),
		...
		{
			Location loStartMarker = goFactory.createLocation( bo.getLeft( ) + 1,
			...
			switch ( mStartPosition.getValue( ) )
			{
				case Position.ABOVE :
					if ( isTransposed( ) )
					{
						loStartMarker.set( loStartMarker.getX( ),
								loStartMarker.getY( ) - markerSize );
					}
					break;
				case Position.BELOW :
					if ( isTransposed( ) )
		...
		{
			Location loEndMarker = goFactory.createLocation( bo.getLeft( )
					+ bo.getWidth( )
					- 1, bo.getTop( ) + bo.getHeight( ) / 2 );
			Position mEndPosition = transposePosition( gs.getEndMarkerPosition( ) );
			...
			switch ( mEndPosition.getValue( ) )
			{
				case Position.ABOVE :
					if ( isTransposed( ) )
					{
						loEndMarker.set( loEndMarker.getX( ),
								loEndMarker.getY( ) - mEnd.getSize( ) );
					}
					break;
				case Position.BELOW :
					if ( isTransposed( ) )
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.birt.chart.render.IAxesDecorator#computeDecorationThickness(org.eclipse.birt.chart.device.IDisplayServer,
	 *      org.eclipse.birt.chart.computation.withaxes.OneAxis)
	 */
	public double[] computeDecorationThickness( IDisplayServer xs, OneAxis ax )
			throws ChartException
	{
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.birt.chart.render.AxesRenderer#getAxesDecorator(org.eclipse.birt.chart.computation.withaxes.OneAxis)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.birt.chart.render.IAxesDecorator#decorateAxes(org.eclipse.birt.chart.device.IPrimitiveRenderer,
	 *      org.eclipse.birt.chart.render.ISeriesRenderingHints,
	 *      org.eclipse.birt.chart.computation.withaxes.OneAxis)
	 */
	public void decorateAxes( IPrimitiveRenderer ipr,
			ISeriesRenderingHints isrh, OneAxis ax ) throws ChartException
	{
		GanttSeries gs = (GanttSeries) getSeries( );
		Label la = gs.getDecorationLabel( );
		if ( la == null || !la.isVisible( ) )
		...
		double dXTick1 = ( ( iMajorTickStyle & IConstants.TICK_LEFT ) == IConstants.TICK_LEFT ) ? ( axisCoord
				- IConstants.TICK_SIZE - 1 )
				: ( axisCoord - 1 );
		double dXTick2 = ( ( iMajorTickStyle & IConstants.TICK_RIGHT ) == IConstants.TICK_RIGHT ) ? ( axisCoord
				+ IConstants.TICK_SIZE + 1 )
				: ( axisCoord + 1 );
