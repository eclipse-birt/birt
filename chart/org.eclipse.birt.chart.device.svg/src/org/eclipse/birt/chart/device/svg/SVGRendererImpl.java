...
/**
 * Provides a reference implementation of a SVG device renderer. It translates chart primitives into
 * standard SVG v1.1 rendering primitives.
 */
public class SVGRendererImpl extends SwingRendererImpl {
    ...
    public final void after() throws RenderingException
    {
        ...
        {
            throw new RenderingException("Unable to write chart image to GIF output handle defined by " + oOutputIdentifier, null);
        }
    }
	/**
	...
	protected Document createSvgDocument(double width, double height) throws Exception {
		...
		svgDocument.getDocumentElement().setAttribute("width",Double.toString(width));
		svgDocument.getDocumentElement().setAttribute("height",Double.toString(height));
		return svgDocument;
	}   
	...
	public void enableInteraction(InteractionEvent ie)
	   ...
	   for (int x = 0; x < triggers.length; x++){
		      ...
		      {		      	
		        svggc.getCurrentParent().setAttribute("onmousedown","parent.location='"+((URLValue) tg.getAction().getValue()).getBaseUrl()+"'");
		      }                	
        }    
	}
}
