...
final class SwtTextRenderer extends TextRendererAdapter
{
	 ...
	 * @param b
	 */
	private void showRightValue( IPrimitiveRenderer ipr, Location lo, Label la,
			boolean b )
	{
	 ...
	 * @param b
	 */
	private void showLeftValue( IPrimitiveRenderer ipr, Location lo, Label la,
			boolean b )
	{
		// TODO not used temporarily
	}
	 ...
	 * @param b
	 */
	private void showBottomValue( IPrimitiveRenderer ipr, Location lo,
			Label la, boolean b )
	{
		// TODO not used temporarily
	}
			...
			double dY )
	{
		...
		if ( R31Enhance.isR31Available( ) )
		{
			...
			if ( la.getCaption( ).getFont( ).getRotation( ) != 0 )
			{
				...
				float rotate = (float) la.getCaption( )
						.getFont( )
						.getRotation( );
	...
	final Image rotatedTextAsImage(IChartComputation cComp, Label la )
	{
		...
		final double dFWt = Math.abs( dFW
				* dCosTheta
				+ dFH
				* Math.abs( dSineTheta ) );
		final double dFHt = Math.abs( dFH
				* dCosTheta
				+ dFW
				* Math.abs( dSineTheta ) );
		...
		final Rectangle r = new Rectangle( (int) dX,
				(int) ( dY - dFH ),
				(int) dFW,
				(int) dFH );
		...
		for ( int i = 0; i < itm.getLineCount( ); i++ )
		{
			gc.drawText( itm.getLine( iLC - i - 1 ),
					(int) ( dX + ins.getLeft( ) ),
		...
		final ImageData imgdtaT = new ImageData( clipWt,
				clipHt,
				32,
				PALETTE_DATA );
		...
		final int neg = ( dAngleInDegrees < 0 ) ? (int) ( clipHs * dSineTheta )
				: 0;
		final int pos = ( dAngleInDegrees > 0 ) ? (int) ( clipWs * Math.abs( dSineTheta ) )
				: 0;
