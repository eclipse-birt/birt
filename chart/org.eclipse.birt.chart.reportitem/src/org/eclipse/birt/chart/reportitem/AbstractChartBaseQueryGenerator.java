...
public abstract class AbstractChartBaseQueryGenerator {
	 ...
	 *                                    expression or subquery. If current query
	 *                                    definition is subquery, true then always
	 *                                    create a new binding. If not subquery and
	 *                                    the expression is simply a binding name,
	 *                                    always do not add the new binding.
	 */
	public AbstractChartBaseQueryGenerator(ReportItemHandle handle, Chart cm, boolean bCreateBindingForExpression,
	 ...
	 */
	public abstract IDataQueryDefinition createBaseQuery(IDataQueryDefinition parent) throws ChartException;
	 ...
	 * @throws ChartException
	 */
	public abstract IDataQueryDefinition createBaseQuery(List<String> columns) throws ChartException;
			...
			for (Query qry : series.getDataDefinition()) {
				...
				if (qlist.contains(qry)) {
					try {
						...
						if (aggName != null) {
							...
							colBinding
									.setExpression(ChartReportItemUtil.adaptExpression(exprCodec, modelAdapter, false));
						}
				...
				try {
					if (!query.getBindings().containsKey(colBinding.getBindingName())) {
						...
						if (ChartUtil.compareVersion(fChartModel.getVersion(), "2.6.1") < 0) //$NON-NLS-1$
						{
	...
	protected String generateUniqueBindingName(String expr) {
		String name = StructureFactory.newComputedColumn(fReportItemHandle, ChartUtil.escapeSpecialCharacters(expr))
				.getName();
		if (fNameSet.contains(name)) {
	...
	protected void generateExtraBindings(BaseQueryDefinition query) throws ChartException {
		...
		GroupDefinition optionalYGroupDefinition = createYGrouping(query, orthSD);

		if (optionalYGroupDefinition != null && orthSD.isSetSorting()) {
			bindSortOnOptionalY(optionalYGroupDefinition, orthSD, orthAxisArray, categorySD, query);
		}

		// 2.2 Add base grouping.
		GroupDefinition categoryGroupDefinition = createCategoryGrouping(query, categorySD);
	...
	protected void addExtraBinding(BaseQueryDefinition query, ExpressionCodec exprCodec) throws ChartException {
	 ...
	 * @param valueExprMap
	 */
	protected void bindSortOnCategorySeries(BaseQueryDefinition query, SeriesDefinition categorySD,
			GroupDefinition categoryGroupDefinition, Map<String, String[]> valueExprMap, Axis[] orthAxisArray)
			throws ChartException {
		...
		if (ChartReportItemUtil.isBaseGroupingDefined(categorySD) && categoryGroupDefinition != null) {
				...
				if (nameNewExprArray != null && nameNewExprArray.length == 2) {
					...
					if (isYSeriesExpression(sortExpr)) {
						...
						if (aggFunc != null) {
							...
							if (aFunc.getParametersCount() > 0) {
								String[] parameters = categorySD.getGrouping().getAggregateParameters()
	 ...
	 * @throws ChartException
	 */
	private Map<String, String[]> addAggregateBindings(BaseQueryDefinition query, SeriesDefinition categorySD,
		...
		if (fChartModel instanceof ChartWithAxes) {
			for (int i = 0; i < orthAxisArray.length; i++) {
				addValueSeriesAggregateBindingForGrouping(query, ((Axis) orthAxisArray[i]).getSeriesDefinitions(),
						innerMostGroupDef, valueExprMap, categorySD);
			}
		} else if (fChartModel instanceof ChartWithoutAxes) {
			addValueSeriesAggregateBindingForGrouping(query, categorySD.getSeriesDefinitions(), innerMostGroupDef,
					valueExprMap, categorySD);
		}
		return valueExprMap;
	}

	protected void bindSortOnOptionalY(GroupDefinition yGroupingDefinition, SeriesDefinition orthSD,
			Axis[] orthAxisArray, SeriesDefinition categorySD, BaseQueryDefinition query) throws ChartException {
		// Create a new sort
		...
		if (sortKey == null || ((orthSD.getQuery() != null && sortKey.equals(orthSD.getQuery().getDefinition()))
	...
	protected GroupDefinition createYGrouping(BaseQueryDefinition query, SeriesDefinition orthSD) {
		...
		if (ChartReportItemUtil.isYGroupingDefined(orthSD)) {
			...
			yGroupDefinition.setKeyExpression(ChartReportItemUtil.adaptExpression(exprCodec, modelAdapter, false));

			yGroupDefinition
					.setInterval(ChartReportItemUtil.convertToDtEGroupUnit(dataType, groupUnit, groupIntervalRange));
			yGroupDefinition.setIntervalRange(
					ChartReportItemUtil.convertToDtEIntervalRange(dataType, groupUnit, groupIntervalRange));
	...
	protected GroupDefinition createCategoryGrouping(BaseQueryDefinition query, SeriesDefinition baseSD) {
		...
		if (ChartReportItemUtil.isBaseGroupingDefined(baseSD)) {
			...
			String baseExpr = baseSD.getDesignTimeSeries().getDataDefinition().get(0).getDefinition();
			...
			baseGroupDefinition.setKeyExpression(ChartReportItemUtil.adaptExpression(exprCodec, modelAdapter, false));
			baseGroupDefinition
					.setInterval(ChartReportItemUtil.convertToDtEGroupUnit(dataType, groupUnit, groupIntervalRange));
			baseGroupDefinition.setIntervalRange(
					ChartReportItemUtil.convertToDtEIntervalRange(dataType, groupUnit, groupIntervalRange));
	 ...
	 * @throws ChartException
	 */
	protected String getAggFunExpr(String sortKey, SeriesDefinition baseSD, Axis[] orthAxisArray)
			throws ChartException {
				...
				for (SeriesDefinition sd : sds) {
					if (sd.getDesignTimeSeries().getDataDefinition() != null
						...
						if (sortKey.equals(q.getDefinition())) {
							aggFunction = ChartUtil.getAggregateFunctionExpr(sd, baseAggFunExpr, q);
							break;
						}
					}
				}
	...
	protected String getValidSortExpr(SeriesDefinition sd) {
		...
		if (sd.getSortKey() != null && sd.getSortKey().getDefinition() != null) {
			sortExpr = sd.getSortKey().getDefinition();
		} else {
			sortExpr = sd.getDesignTimeSeries().getDataDefinition().get(0).getDefinition();
		}
	 ...
	 */
	protected void setBindingExpressionDueToAggregation(Binding binding, String expression, String chartAggFunName)
