...
public class ChartCubeUtil extends ChartItemUtil {
	...
	public final static DimensionValue DEFAULT_COLUMN_WIDTH = new DimensionValue(80, DesignChoiceConstants.UNITS_PT);
	public final static DimensionValue DEFAULT_ROW_HEIGHT = new DimensionValue(30, DesignChoiceConstants.UNITS_PT);
	...
	public static List<LevelHandle> getAllLevels(CubeHandle cube) {
		if (cube.getContentCount(ICubeModel.DIMENSIONS_PROP) > 0) {
			...
			while (dimensions.hasNext()) {
				...
				HierarchyHandle hierarchy = (HierarchyHandle) (dimensionHandle)
	...
	public static List<String> getAllLevelsBindingName(CrosstabReportItemHandle xtab) {
		...
		// Add row levels
		content = getFirstContent(getInnermostLevelCell(xtab, ICrosstabConstants.ROW_AXIS_TYPE));
		if (content instanceof DataItemHandle) {
			DataItemHandle dataItemHandle = (DataItemHandle) content;
			levels.add(dataItemHandle.getResultSetColumn());
		}
	 ...
	 * @since 2.3
	 */
	public static CrosstabCellHandle getXtabContainerCell(DesignElementHandle chartHandle, boolean bOnlyAggrCell)
			throws BirtException {
		DesignElementHandle container = chartHandle.getContainer();
		if (container instanceof ExtendedItemHandle) {
			ExtendedItemHandle xtabHandle = (ExtendedItemHandle) container;
	...
	public static String createDimensionExpression(LevelHandle level) {
		...
		if (level instanceof TabularLevelHandle && ((TabularLevelHandle) level).getDisplayColumnName() != null
				&& ((TabularLevelHandle) level).getDisplayColumnName().trim().length() > 0) {
			return ExpressionUtil.createJSDimensionExpression(level.getContainer().getContainer().getName(),
					level.getName(), ICubeQueryUtil.DISPLAY_NAME_ATTR);
		}
		return ExpressionUtil.createJSDimensionExpression(level.getContainer().getContainer().getName(),
				level.getName());
	}
	...
	public static String createLevelBindingName(LevelHandle level) {
		...
		return level.getContainer().getContainer().getName() + EXPRESSION_SPLITTOR + level.getName();
	}
	...
	public static String createMeasureBindingName(MeasureHandle measure) {
		...
		return measure.getContainer().getName() + EXPRESSION_SPLITTOR + measure.getName();
	}
	...
	public static ComputedColumnHandle findLevelBinding(ReportItemHandle handle, String dimensionName,
			String levelName) {
		if (dimensionName != null && levelName != null) {
			...
			for (Iterator<ComputedColumnHandle> bindings = getAllColumnBindingsIterator(handle); bindings.hasNext();) {
				...
				if (levelNames != null && dimensionName.equals(levelNames[0]) && levelName.equals(levelNames[1])) {
					return cch;
				}
			}
		}
	...
	public static ComputedColumnHandle findLevelAttrBinding(ReportItemHandle handle, String dimensionName,
			String levelName, String laName) {
	...
	public static String generateComputedColumnName(AggregationCellHandle cell, String expressionIndicator) {
		...
		if (cell.getContents().size() > 0 && cell.getContents().get(0) instanceof DataItemHandle) {
	 ...
	 * @return binding name list
	 */
	public static ComputedColumnHandle findDimensionBinding(ExpressionCodec exprCodec, String dimName, String levelName,
			Collection<ComputedColumnHandle> bindings) {
		for (ComputedColumnHandle cch : bindings) {
	...
	public static boolean isDetailCell(AggregationCellHandle cell) {
		return IMeasureViewConstants.DETAIL_PROP
				.equals(cell.getModelHandle().getContainerPropertyHandle().getPropertyDefn().getName());
	}

	public static boolean isAggregationCell(AggregationCellHandle cell) {
		return IMeasureViewConstants.AGGREGATIONS_PROP
				.equals(cell.getModelHandle().getContainerPropertyHandle().getPropertyDefn().getName());
	}

	public static int getXTabAxisType(boolean bTransposed) {
		return bTransposed ? ICrosstabConstants.ROW_AXIS_TYPE : ICrosstabConstants.COLUMN_AXIS_TYPE;
	}
	 ...
	 * @throws BirtException
	 */
	public static void updateXTabForAxis(AggregationCellHandle cell, ExtendedItemHandle hostChartHandle,
			boolean bTransOld, ChartWithAxes cmNew) throws BirtException {
		if (cell == null) {
			return;
		}
		...
		if (bTransOld != bTransNew) {
			...
			List<ExtendedItemHandle> otherPlotCharts = findChartInOtherMeasures(cell, true);
			for (int i = 0; i < otherPlotCharts.size(); i++) {
				...
				if (chartInOtherMeasure != null) {
					...
					ChartWithAxes cwa = updateChartModelWhenTransposing(chartInOtherMeasure, cmNew);
					AggregationCellHandle cellAgg = getXtabContainerCell(chartInOtherMeasure);
					if (removeAxisChartInXTab(cellAgg, bTransOld, true)) {
						bNewTotalJustAdded = addAxisChartInXTab(cellAgg, cwa, chartInOtherMeasure, bNewTotalJustAdded);
					} else {
			...
			if (removeAxisChartInXTab(cell, bTransOld, true)) {
				addAxisChartInXTab(cell, cmNew, hostChartHandle, bNewTotalJustAdded);
			} else {
				// If axis chart is not deleted, just update cells
				updateAxisChartCells(cell, cmNew);
			}
	...
	private static ChartWithAxes updateChartModelWhenTransposing(ExtendedItemHandle eih, ChartWithAxes cmFrom)
		...
		Query queryFrom = cmFrom.getAxes().get(0).getSeriesDefinitions().get(0).getDesignTimeSeries()
				.getDataDefinition().get(0);
		Query queryTo = cmNew.getAxes().get(0).getSeriesDefinitions().get(0).getDesignTimeSeries().getDataDefinition()
				.get(0);
	...
	public static void updateXTabDirection(CrosstabReportItemHandle xtab, boolean bTransposed)
	...
	public static List<ExtendedItemHandle> findChartInOtherMeasures(AggregationCellHandle cell, boolean bPlotChart) {
		if (cell.getCrosstab().getMeasureCount() > 1) {
			...
			for (int i = 0; i < cell.getCrosstab().getMeasureCount(); i++) {
				...
				if (bPlotChart && ChartCubeUtil.isPlotChart((DesignElementHandle) content)
						|| !bPlotChart && ChartCubeUtil.isAxisChart((DesignElementHandle) content)) {
	 ...
	 * @throws BirtException
	 */
	public static boolean removeAxisChartInXTab(AggregationCellHandle cell, boolean bTransposed, boolean bCleanSpan)
			throws BirtException {
		if (bCleanSpan) {
	...
	public static AggregationCellHandle getGrandTotalAggregationCell(AggregationCellHandle cell, boolean bTransposed) {
		...
		if (bTransposed) {
			return ((MeasureViewHandle) cell.getContainer()).getAggregationCell(null, null,
					cell.getDimensionName(ICrosstabConstants.COLUMN_AXIS_TYPE),
					cell.getLevelName(ICrosstabConstants.COLUMN_AXIS_TYPE));
		} else {
			return ((MeasureViewHandle) cell.getContainer()).getAggregationCell(
					cell.getDimensionName(ICrosstabConstants.ROW_AXIS_TYPE),
	...
	private static boolean isEmptyInAllGrandTotalCells(CrosstabReportItemHandle xtab, boolean bTransposed) {
		for (int i = 0; i < xtab.getMeasureCount(); i++) {
			...
			AggregationCellHandle grandTotalCell = getGrandTotalAggregationCell(mv.getCell(), bTransposed);
	...
	public static boolean addAxisChartInXTab(AggregationCellHandle cell, ChartWithAxes cwa,
		...
		if (bTransposed) {
			grandTotalAggCell = ((MeasureViewHandle) cell.getContainer()).getAggregationCell(null, null,
					cell.getDimensionName(ICrosstabConstants.COLUMN_AXIS_TYPE),
					cell.getLevelName(ICrosstabConstants.COLUMN_AXIS_TYPE));
		} else {
			grandTotalAggCell = ((MeasureViewHandle) cell.getContainer()).getAggregationCell(
					cell.getDimensionName(ICrosstabConstants.ROW_AXIS_TYPE),
					cell.getLevelName(ICrosstabConstants.ROW_AXIS_TYPE), null, null);
		}
		...
		if (grandTotalAggCell != null) {
			...
			ExtendedItemHandle axisChartHandle = createChartHandle(cell.getModelHandle(),
					ChartReportItemConstants.TYPE_AXIS_CHART, hostChartHandle);
	...
	public static boolean addAxisChartInXTab(AggregationCellHandle cell, ChartWithAxes cwa,
			ExtendedItemHandle hostChartHandle) throws BirtException {
		return addAxisChartInXTab(cell, cwa, hostChartHandle, !ChartInXTabStatusManager.hasGrandItem(hostChartHandle));
	}
	...
	public static boolean updateAxisChartCells(AggregationCellHandle cell, ChartWithAxes cwa) throws BirtException {
		...
		removeCellPadding(getGrandTotalAggregationCell(cell, !bTransposed), bTransposed);
	 ...
	 * @throws SemanticException
	 */
	private static void deleteGrandTotalItems(CrosstabReportItemHandle xtab, boolean bTransposed)
			throws SemanticException {
		for (int i = 0; i < xtab.getMeasureCount(); i++) {
			MeasureViewHandle mv = xtab.getMeasure(i);
			AggregationCellHandle aggCell = getGrandTotalAggregationCell(mv.getCell(), bTransposed);
	 ...
	 * @throws BirtException
	 */
	public static void updateAxisChart(AggregationCellHandle cell, ChartWithAxes cwa,
			ExtendedItemHandle hostChartHandle) throws BirtException {
		boolean bTransposed = cwa.isTransposed();
	...
	private static CrosstabCellHandle getGrandTotalCell(CrosstabCellHandle cell, boolean bTransposed) {
		...
		return cell.getCrosstab()
				.getGrandTotal(bTransposed ? ICrosstabConstants.ROW_AXIS_TYPE : ICrosstabConstants.COLUMN_AXIS_TYPE);
	}

	public static ExtendedItemHandle createChartHandle(DesignElementHandle anyHandle, String chartType,
			ExtendedItemHandle hostChartHandle) throws SemanticException {
		ExtendedItemHandle chartHandle = anyHandle.getElementFactory().newExtendedItem(null,
	...
	private static void removeCellPadding(CrosstabCellHandle cell, boolean bTransposed) throws SemanticException {
		if (cell != null) {
			if (bTransposed) {
				cell.getModelHandle().setProperty(StyleHandle.PADDING_TOP_PROP,
						new DimensionValue(0, DesignChoiceConstants.UNITS_PT));
				cell.getModelHandle().setProperty(StyleHandle.PADDING_BOTTOM_PROP,
						new DimensionValue(0, DesignChoiceConstants.UNITS_PT));
			} else {
				cell.getModelHandle().setProperty(StyleHandle.PADDING_LEFT_PROP,
						new DimensionValue(0, DesignChoiceConstants.UNITS_PT));
				cell.getModelHandle().setProperty(StyleHandle.PADDING_RIGHT_PROP,
						new DimensionValue(0, DesignChoiceConstants.UNITS_PT));
			}
		}
	}
