			...
			ControlListener
{
	...
	private transient Vector headings = null;
	...
	/**
	 * INT HOLDING THE VERTICAL SCROLL AMOUNT...ITS VALUE REPRESENTS HOW MANY
	 * ROWS TO THE BOTTOM THE USER HAS SCROLLED...I.E. HOW MANY ROWS ARE HIDDEN
	...
	/**
	 * INT HOLDING THE CURRENT SELECTED OR ACTIVE COLUMN...-1 INDICATES NO
	 * COLUMN IS SELECTED
	 */
	...
	private void placeComponents( )
	{
		...
		cnvCells = new TableCanvas( this,
				SWT.NONE,
				headings.size( ),
				new Color[]{},
				this );
		cnvCells.setLayoutData( new GridData( GridData.FILL_BOTH ) );
		cnvCells.setBackground( Display.getCurrent( )
	...
	public void addEntry( String sText, int iIndex )
			throws IllegalArgumentException
	{
		if ( headings == null || iIndex >= headings.size( ) )
		{
			...
			throws IllegalArgumentException
	{
		...
		return (String) headings.get( iIndex );
	}

	/**
	 * Returns the column heading of the column that the user right-clicked on
	 * last. null if user hasn't right-clicked in the table yet. This method is
	 * for use to determine the column for which the popup-menu is to be
	 * displayed...since the menu is handled externally.
	 * 
	 * @return the last column in which the user has right-clicked.
	 */
	public String getCurrentColumnHeading( )
	{
		return ( iColumnIndex != -1 ) ? (String) headings.get( iColumnIndex )
				: null;
	}
	 ...
	 * the menu is handled externally.
	 * 
	 * @return the last column in which the user has right-clicked.
	 */
	...
	public Color getColumnColor( int iIndex )
	{
		...
		{
			throw new IllegalArgumentException( Messages.getString( "CustomPreviewTable.Exception.InvalidColumnIndexSpecified" ) ); //$NON-NLS-1$
		}
...

	{
		...
		public TableCanvas( Composite parent, int style, int iColumns,
				Color[] colors, CustomPreviewTable container )
		{
			...
			for ( int i = 0; i < iColumns; i++ )
			{
				...
				{
					this.colors[i] = Display.getDefault( )
							.getSystemColor( SWT.COLOR_LIST_BACKGROUND );
				}
			}
			updateScrollbars( );
		...
		public void updateScrollbars( )
		{
			...
			if ( this.isVisible( ) )
			{
				...
				getHorizontalBar( ).setValues( iVisibleTableWidth
						+ iHiddenWidth,
						iVisibleTableWidth,
						iMaxTableWidth,
				...
				if ( iHeight == 0 )
				{
					iHeight = getParent( ).computeSize( SWT.DEFAULT,
							SWT.DEFAULT,
							true ).y;
				}
			...
			else
			{
				getHorizontalBar( ).setValues( iVisibleTableWidth,
						...
						SCROLL_HORIZONTAL_STEP,
						SCROLL_HORIZONTAL_STEP );
				getVerticalBar( ).setValues( iMaxRowIndex,
						iMaxRowIndex,
						iMaxRowIndex,
						1,
						1,
			...
			iLastProcessedHorizontalScrollPosition = iVisibleTableWidth
					+ iHiddenWidth;
		...
		private int getVisibleTableWidth( )
		{
			...
			{
				iVisibleTableWidth = getParent( ).computeSize( SWT.DEFAULT,
						SWT.DEFAULT ).x;
			}
		...
		private int calculateMaxColumnWidth( int columnIndex, Control control )
		{
			...
			for ( int rowIndex = 0; rowIndex < columnTexts.size( ); rowIndex++ )
			{
				...
				{
					maxWidth = Math.max( gc.textExtent( (String) obj ).x,
							maxWidth );
				}
			}
			gc.dispose( );
		...
		public void addEntry( String sText, int iColumn )
				throws IllegalArgumentException
		{
		...
		public void setText( String sText, int iColumn, int iRow )
				throws IllegalArgumentException
		{
			if ( iColumn > cells.length )
			{
			...
			if ( iRow < 0 )
			{
				throw new IllegalArgumentException( Messages.getString( "CustomPreviewTable.Exception.RowIndexGreaterThan0" ) ); //$NON-NLS-1$
			}
		...
		public void paintControl( PaintEvent pe )
		{
			...
			Color cDefaultBack = Display.getCurrent( )
					.getSystemColor( SWT.COLOR_LIST_BACKGROUND );
			// DEFAULT TEXT COLOR FOR COLUMN
			Color cDefaultFore = Display.getCurrent( )
					.getSystemColor( SWT.COLOR_LIST_FOREGROUND );
			Color cText = Display.getCurrent( )
					.getSystemColor( SWT.COLOR_INFO_FOREGROUND );
			Color cGrid = Display.getCurrent( )
			...
			for ( int iC = 0; iC < cells.length; iC++ )
			{
				...
				// THE VIEWABLE AREA!
				if ( ( iXEnd - columnWidthOffset ) < 0
						|| ( iXStart - columnWidthOffset ) > this.getSize( ).x )
				{
					continue;
				}
				...
				gc.fillRectangle( iXStart - columnWidthOffset,
						0,
						getColumnWidthFor( iC ),
						this.getSize( ).y );
				// RENDER VERTICAL LINES
				gc.setForeground( cGrid );
				gc.drawLine( iXStart - columnWidthOffset, 0, iXStart
						- columnWidthOffset, this.getSize( ).y );
				...
				for ( int iR = 0; iR <= iMaxRowIndex || iYEnd < getSize( ).y; iR++ )
				{
					...
					gc.setClipping( iXStart - columnWidthOffset,
							iYStart,
							getColumnWidthFor( iC ),
							( iYEnd - iYStart ) );
					// SET COLORS BASED ON SELECTION STATE
					if ( iC == iColumnIndex
							&& ( bColumnSelection || iR == this.iSelectedRow ) )
					{
						...
						gc.fillRectangle( iXStart - columnWidthOffset,
								...
								getColumnWidthFor( iC ),
								( iYEnd - iYStart ) );
						gc.drawRectangle( iXStart - columnWidthOffset,
								iYStart,
								getColumnWidthFor( iC ),
								( iYEnd - iYStart ) );
					}
