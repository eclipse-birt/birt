...
public final class Generator implements IGenerator
{
	...
	/**
	 * Returns expression keys and expressions.
	 * 
	 * @param cm
	 * @param iae
	 * @return
	 ...
	 * @since BIRT 2.3
	 */
	public List getRowExpressionsNKeys( Chart cm, IActionEvaluator iae )
		throws ChartException
	{
		if ( cm instanceof ChartWithAxes )
		{
			return getRowExpressionsNKeys( (ChartWithAxes) cm, iae );
		}
		else if ( cm instanceof ChartWithoutAxes )
		{
			return getRowExpressionsNKeys( (ChartWithoutAxes) cm, iae );
		}
		return null;	
	}
	...
	/**
	 * Returns expression keys and expressions of axis chart, it should returns
	 * a list, and each element is an string array, the 1th of array is
	 * expression key, the 2th of array is expression.
	 * 
	 * @param cwa
	 * @param iae
	 * @return
	 ...
	 * @since BIRT 2.3
	 */
	private static List getRowExpressionsNKeys( ChartWithAxes cwa,
			IActionEvaluator iae ) throws ChartException
	{
		final ArrayList alExpreKeys = new ArrayList( 4 );
		final ArrayList alExpressions = new ArrayList( 4 );
		...
		String baseAggExpr = getBaseSeriesAggExpr( sd );
		...
		for ( int j = 0; j < axaOrthogonal.length; j++ )
		{
			...
			for ( int k = 0; k < elSD.size( ); k++ )
			{
				...
				String orthAggExpr = getOrthSeriesAggExpr( sd, baseAggExpr );
				...
				{
					// FILTER OUT DUPLICATE ENTRIES
					if ( alExpreKeys.contains( sExpression ) )
					{
						int iRemovalIndex = alExpreKeys.indexOf( sExpression );
						if ( iRemovalIndex > iCount )
						{
							alExpreKeys.remove( iRemovalIndex );
							alExpreKeys.add( iCount, sExpression );
							alExpressions.remove( iRemovalIndex );
							alExpressions.add( iCount, sExpression );
				...
				for ( int i = 0; i < elOrthogonalSeries.size( ); i++ )
				{
					...
					if ( sExpression != null
							&& sExpression.trim( ).length( ) > 0 )
					{
						String exprKey = ChartUtil.generateExprKey( sExpression, orthAggExpr );
						
						bAnyQueries = true;
						if ( !alExpreKeys.contains( exprKey ) )
						{
							alExpreKeys.add( exprKey ); // APPEND AT END
							alExpressions.add( sExpression ); // APPEND AT END
						}
					}
				}
				...
				if ( triggerExprs != null )
				{
					...
					{
						String tgexp = triggerExprs[t];
						if ( !alExpreKeys.contains( tgexp ) )
						{
							alExpreKeys.add( tgexp ); // APPEND AT END
							alExpressions.add( tgexp ); // APPEND AT END
						}
					}
				}
		...
		List rowExprList = new ArrayList();
		for ( int i = 0; i < alExpreKeys.size( ); i++ )
		{
			String[] exprmap = new String[2];
			exprmap[0] = (String) alExpreKeys.get( i );
			exprmap[1] = (String) alExpressions.get( i );
			rowExprList.add( exprmap );
		}
		return rowExprList;
	}

	/**
	 * Returns aggregation expression on value series.
	 * 
	 * @param sd
	 * @param baseAggFunc
	 * @return
	 * @since BIRT 2.3
	 */
	private static String getOrthSeriesAggExpr( SeriesDefinition sd,
			String baseAggFunc )
	{
		String strOrthoAgg = null;
		if ( baseAggFunc != null )
		{
			if ( sd.getGrouping( ).isSetEnabled( ) && sd.getGrouping( ).isEnabled( ) )
			{
				// Set own group
				strOrthoAgg = sd.getGrouping( ).getAggregateExpression( );
			}
			
			// Set base group
			if ( strOrthoAgg == null || "".equals( strOrthoAgg ) ) //$NON-NLS-1$
			{
				strOrthoAgg = baseAggFunc;
			}
		}
		return strOrthoAgg;
	}
	...
	/**
	 * Returns aggregation expression on base series.
	 * 
	 * @param sd
	 * @return
	 * @since BIRT 2.3
	 */
	private static String getBaseSeriesAggExpr( SeriesDefinition sd )
	{
		String baseAggFunc = null;
		if ( sd.getGrouping( ) != null && sd.getGrouping( ).isEnabled( ) && sd.getGrouping( ).isSetEnabled( ) )
		{
			baseAggFunc = sd.getGrouping( ).getAggregateExpression( );
		}
		return baseAggFunc;
	}
	
	/**
	 * Returns expression keys and expressions of non-axis chart, it should returns
	 * a list, and each element is an string array, the 1th of array is
	 * expression key, the 2th of array is expression.
	 * 
	 * @param cwoa
	 ...
	 * @since BIRT 2.3
	 */
	private static List getRowExpressionsNKeys( ChartWithoutAxes cwoa,
			IActionEvaluator iae ) throws ChartException
	{
		final ArrayList alExpreKeys = new ArrayList(4);
		final ArrayList alExpressions = new ArrayList( 4 );
		...
		String baseAggFunc = getBaseSeriesAggExpr( sd );
		...
		if ( sExpression != null
				&& sExpression.trim( ).length( ) > 0
				&& !alExpreKeys.contains( sExpression ) )
		{
			alExpreKeys.add( sExpression );
			alExpressions.add( sExpression );
		}
		...
		for ( int k = 0; k < elSD.size( ); k++ )
		{
			...
			String orthAggFunc = getOrthSeriesAggExpr( sd, baseAggFunc );
			...
			for ( int i = 0; i < elOrthogonalSeries.size( ); i++ )
			{
				...
				if ( sExpression != null && sExpression.trim( ).length( ) > 0 )
				{
					String exprKey = ChartUtil.generateExprKey( sExpression, orthAggFunc );
					bAnyQueries = true;
					if ( !alExpreKeys.contains(  exprKey ) )
					{
						alExpreKeys.add( exprKey ); // APPEND AT END
		...
		{
			String[] exprmap = new String[2];
			exprmap[1] = (String) alExpreKeys.get( i );
			exprmap[0] = (String) alExpressions.get( i );
			rowExprList.add( exprmap );
		}
		return rowExprList;
	}
