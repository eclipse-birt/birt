...
public class ChartReportItemUIImpl extends ReportItemFigureProvider {
	...
	public final IFigure createFigure(final ExtendedItemHandle eih) {
		...
		try {
			...
			if (iri.isCopied() && ChartCubeUtil.isPlotChart(eih)) {
				...
				if (yAxis.getLineAttributes().isVisible() && ChartCubeUtil.findReferenceChart(eih) == null) {
					...
					if (ChartCubeUtil.isAxisChart((DesignElementHandle) content)) {
						...
						final DesignElementHandle oldPlotChart = axisChart
				...
				if (cm != null) {
					...
					if ((bo == null || bo.getWidth() == 0 || bo.getHeight() == 0)
							&& (nf.format(ChartReportItemConstants.DEFAULT_CHART_BLOCK_WIDTH) + "pt") //$NON-NLS-1$
									.equals(eih.getWidth().getStringValue())
							&& (nf.format(ChartReportItemConstants.DEFAULT_CHART_BLOCK_HEIGHT) + "pt") //$NON-NLS-1$
									.equals(eih.getHeight().getStringValue())) {
	...
	private void resizeToFitContainer(ExtendedItemHandle eih, IFigure ifg, int dpi) {
		...
		final Bounds parentBounds = BoundsImpl.create(0, 0, parentCA.width, parentCA.height).scaledInstance(72d / dpi);
		...
		parentBounds.setHeight(Math.max(ChartReportItemConstants.DEFAULT_CHART_BLOCK_HEIGHT,
				Math.min(parentBounds.getHeight(), parentBounds.getWidth() / 2)));
			...
			public void elementChanged(DesignElementHandle focus, NotificationEvent ev) {
				if (ev instanceof ContentEvent) {
					...
					if (cv.getAction() == ContentEvent.REMOVE) {
						DesignElementHandle handleSource = handleTarget
								.getElementProperty(ChartReportItemConstants.PROPERTY_HOST_CHART);
						// If the plot chart is null or it's the one to delete
						if (handleSource == null || cv.getContent() == handleSource.getElement()) {
							try {
