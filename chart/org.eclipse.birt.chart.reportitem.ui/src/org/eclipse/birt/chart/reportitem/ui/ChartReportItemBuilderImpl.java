...
public class ChartReportItemBuilderImpl extends ReportItemBuilderUI implements IUIServiceProvider {
	...
	public int open(final ExtendedItemHandle eih) {
		...
		try {
			...
			if (item == null) {
				logger.log(ILogger.ERROR, Messages.getString("ChartReportItemBuilderImpl.log.UnableToLocate")); //$NON-NLS-1$
				return Window.CANCEL;
			}

			final CommandStack commandStack = extendedHandle.getRoot().getCommandStack();
			final String TRANS_NAME = org.eclipse.birt.chart.reportitem.i18n.Messages
					.getString("ChartElementCommandImpl.editChart"); //$NON-NLS-1$
			...
			final ChartWizardContext context = uiFactory.createWizardContext(cmClone, this, imageProvider, dataProvider,
					dataSheet);
			...
			context.setInheritColumnsOnly(
					extendedHandle.getBooleanProperty(ChartReportItemConstants.PROPERTY_INHERIT_COLUMNS));
			...
			// Add instance of BIRTActionEValuator into chart wizard context for the
			// validation of expressions in chart UI.
			...
			if (contextResult != null && contextResult.getModel() != null) {
				...
				// Pressing Finish
				updateModel(extendedHandle, chartBuilder, crii, cm, contextResult.getModel(),
						contextResult.getOutputFormat(), contextResult.isInheritColumnsOnly());
				if (dataProvider.isPartChart()) {
					ChartXTabUIUtil.updateXTabForAxis(ChartCubeUtil.getXtabContainerCell(extendedHandle),
							extendedHandle, ChartXTabUIUtil.isTransposedChartWithAxes(cm),
	...
	protected void addCustomButtons(final ChartWizard chartBuilder, final ChartWizardContext context,
	...
	protected void maintainCompatibility(Chart cm) {
		...
		if (cm.getLegend().getSeparator() == null) {
			LineAttributes separator = LineAttributesImpl.create(ColorDefinitionImpl.BLACK(), LineStyle.SOLID_LITERAL,
					1);
		...
		if (cm.getSampleData() == null) {
			...
			if (cm instanceof ChartWithoutAxes) {
				...
				sdBase.setDataSetRepresentation(
						ChartUtil.getNewSampleData(ChartUIUtil.getAxisXForProcessing(chart).getType(), 0));
				sampleData.getBaseSampleData().add(sdBase);
	...
	protected void updateModel(ExtendedItemHandle eih, ChartWizard chartBuilder, ChartReportItemImpl crii, Chart cmOld,
			Chart cmNew, String outputFormat, boolean bInheritColumnsOnly) {
		try {
			// update the output format property information.
			eih.setProperty(ChartReportItemConstants.PROPERTY_OUTPUT, outputFormat);
			eih.setProperty(ChartReportItemConstants.PROPERTY_INHERIT_COLUMNS, bInheritColumnsOnly);
	...
	public final double getConvertedValue(double dOriginalValue, String sFromUnits, String sToUnits) {
		...
		// Pixels or Unknown
		dResult = (DimensionUtil.convertTo(dOriginalValue, getBIRTUnitsFor(sFromUnits), getBIRTUnitsFor(sToUnits)))
				.getMeasure();
	...
	protected ExpressionProvider createExpressionProvider(int command, final Object context) {
		return new ChartExpressionProvider((ExtendedItemHandle) context, wizardContext,
	...
	public String invoke(int command, String value, final Object context, String sTitle) throws ChartException {
		...
		case COMMAND_HYPERLINK_LEGEND:
			shell = new Shell(Display.getDefault(), SWT.DIALOG_TRIM | SWT.RESIZE | SWT.APPLICATION_MODAL);
			...
			try {
				...
				throw new ChartException(ChartReportItemUIActivator.ID, ChartException.UNDEFINED_VALUE, e);
			}
			break;
		...
		case COMMAND_CUBE_EXPRESSION_TOOLTIPS_DATAPOINTS:
			shell = new Shell(Display.getDefault(), SWT.DIALOG_TRIM | SWT.RESIZE | SWT.APPLICATION_MODAL);
			ChartUIUtil.bindHelp(shell, ChartHelpContextIds.DIALOG_EXPRESSION_BUILDER);
		...
		case COMMAND_RESOURCE_SELECTION_DIALOG:
			ResourceFileFolderSelectionDialog dialog = new ResourceFileFolderSelectionDialog(true, true,
					new String[] { "*.jpg;*.gif;*.png;" //$NON-NLS-1$
					});
	...
	public Object invoke(Command command, Object... inData) throws ChartException {
		...
		case EXPRESS_BUTTON_CREATE:
			if (inData.length > 3) {
				...
				ChartExpressionProvider ep = new ChartExpressionProvider(eih, wizardContext,
						ChartReportItemUIUtil.getExpressionBuilderStyle(iCode));
				if (inData.length > 6 && (inData[6].equals(ChartUIConstants.QUERY_CATEGORY)
						|| inData[6].equals(ChartUIConstants.QUERY_OPTIONAL))) {
				...
				IExpressionButton ceb = ChartExpressionButtonUtil.createExpressionButton(parent, control, eih, ep);
