...
public class MasterPageWriter extends org.eclipse.birt.report.engine.odf.writer.MasterPageWriter
{
	...
	{
		StyleEntry entry = StyleBuilder.createStyleEntry( ((IContent)child).getComputedStyle( ), StyleConstant.TYPE_TEXT );
		if ( blockStyle != null )
		{
			if ( entry == null )
			{
	...
	public void writeHeaderFooter( IContent headerFooter )
	{
		if ( headerFooter != null )
		{
			...
			while ( iter.hasNext( ) )
			{
				...
				{
					writer.openTag( "text:p" );
					StyleEntry blockEntry = StyleBuilder.createStyleEntry( headerFooter.getComputedStyle( ), StyleConstant.TYPE_TEXT );
