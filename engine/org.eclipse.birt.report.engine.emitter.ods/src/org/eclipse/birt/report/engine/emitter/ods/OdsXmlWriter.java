...
public class OdsXmlWriter extends AbstractOdfWriter implements IOdsWriter
{
	...
	protected static Logger logger = Logger.getLogger( OdsXmlWriter.class
			.getName( ) );
	 ...
	 * @author bidi_acgc
	 * @param isRTLSheet:
	 *            represents the direction of the sheet.
	 */
	 ...
	 *            represents the direction of the sheet.
	 */
	public OdsXmlWriter( OutputStream out, String encoding,
			OdsContext context, boolean isRTLSheet )
	{
		this.context = context;
		writer.open( out, encoding );
	}
	...
	private void startCell( int cellIndex, int colspan, int rowspan,
			StyleEntry cellStyle, HyperlinkInfo hyperLink ,BookmarkDef linkedBookmark)
	{
			...
			int colSpan )
	{
		...
		else
		{
			...
			outputData( type, value, style, column, colSpan, rowSpan,
	...
	private void outputImageData( ImageData imageData, StyleEntry style, int column, int colSpan )
	{
		...
		drawImage( imageData.getImageUrl( ),
				null,
				null,
				null,
				imageData.getImageHeight( ) / OdfUtil.INCH_PT,
				imageData.getImageWidth( ) / OdfUtil.INCH_PT, style, imageData.getDescription( ),
				null,
				getImageID( ) );
			...
			HyperlinkInfo hyperLink, BookmarkDef linkedBookmark )
	{
		...
		{
			if ( OdsUtil.isNaN( value ) || OdsUtil.isBigNumber( value )
					|| OdsUtil.isInfinity( value ) )
			{
				valueType = "string";
			}
			else
			{
				valueType = "number";
				//writer.attribute( "office:value", value.toString() );
			}
		}
		else if ( type == SheetData.DATE )
		{
			...
			{
				valueType = "time";
				//writer.attribute( "office:time-value", value.toString() );
			}
			else
			{
				valueType = "date";
				// TODO: date/datetime value format
				//writer.attribute( "office:date-value", value.toString() );
			}
		}
		else if ( value instanceof Boolean )
		{
			valueType = "boolean";
			//writer.attribute( "office:boolean-value", value.toString() );
		}
		else
		{
			valueType = "string";
		...
		if ( valueType != null )
		{
			writer.attribute( "office:value-type", valueType );
		}
		*/
		...
		if ( txt.length( ) > 0 )
		{
			if ( style != null )
			{
				String textTransform = (String) style
						.getStringProperty( StyleConstant.TEXT_TRANSFORM );
	 ...
	 */
	private void openHyperlink( HyperlinkInfo hyperLink,
