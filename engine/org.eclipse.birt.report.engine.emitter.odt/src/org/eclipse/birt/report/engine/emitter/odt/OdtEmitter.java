...
public class OdtEmitter extends AbstractOdfEmitter 
{
	...
	protected static Logger logger = Logger.getLogger( OdtEmitter.class
			.getName( ) );
	...
	/**
	 * Deferred bookmarks names that need to be output in the first paragraph
	 * met.
	...
	public void startForeign( IForeignContent foreign ) throws BirtException
	{
		...
		if ( IForeignContent.HTML_TYPE.equalsIgnoreCase( foreign.getRawType( ) ) )
		{
			...
			StyleEntry cellStyle = StyleBuilder.createStyleEntry( foreign.getComputedStyle(), StyleEntry.TYPE_TABLE_CELL );
	...
	protected void writeContent( int type, String txt, IContent content )
	{
		...
		writeText( type, txt, content, inlineFlag,
				computedStyleEntry, inlineStyleEntry );
	...
	private void outputPrePageProperties( IPageContent page ) throws IOException,
			BirtException
	{
		adjustInline( );
	...
	public void processPageLayout( IPageContent page )
	{
		...
		double footerHeight = OdfUtil.convertTo( page.getFooterHeight( ), 0.0,
				reportDpi );
		double headerHeight = OdfUtil.convertTo( page.getHeaderHeight( ), 0.0,
				reportDpi );
		double topMargin = OdfUtil.convertTo( page.getMarginTop( ), 0.0, reportDpi );
		double bottomMargin = OdfUtil.convertTo( page.getMarginBottom( ), 0.0,
				reportDpi );
		*/
	...
	public void startListBand( IListBandContent listBand )
	{
		...
		StyleEntry cellStyle = StyleBuilder.createStyleEntry( computeStyle( listBand.getComputedStyle( ) ), StyleEntry.TYPE_TABLE_CELL );
		cellStyle.setProperty( StyleConstant.WIDTH, context.getCurrentWidth( ) );
	 ...
	 * @return
	 */
	private SpanInfo processSpan( ICellContent cell,
			StyleEntry cellStyle )
	{
		int columnId = cell.getColumn( );
	...
	private void drawDiagonalLine( ICellContent cell, double cellWidth )
	{
		if ( cellWidth == 0 )
			return;
		int cellHeight = OdfUtil.convertTo( getCellHeight( cell ), 0,
				context.getReportDpi() ) / 20;
		if ( cellHeight == 0 )
			return;
		...
		{
			lineColor = OdfUtil.parseColor( cell.getComputedStyle( )
					.getColor( ) );
		}
	...
	public void startTable( ITableContent table )
	{
		...
		double width = OdfUtil.convertTo( table.getWidth( ), context
				.getCurrentWidth( ), context.getReportDpi() );
		width = Math.min( width, context.getCurrentWidth( ) );
	...
	public void startImage( IImageContent image )
	{
		...
		double height = OdfUtil.convertImageSize( image.getHeight( ), 0,
				context.getReportDpi() );
		int parentWidth = (int) ( OdfUtil
				.twipToPt( context.getCurrentWidth( ) )
				* context.getReportDpi() / OdfUtil.INCH_PT );
		double width = OdfUtil.convertImageSize( image.getWidth( ),
				parentWidth, context.getReportDpi() );
		context.addContainer( false );
		...
		if ( FlashFile.isFlash( mimeType, uri, extension ) )
		{
			...
			bodyWriter.drawImage( null, height, width, null, style, pStyle, inlineFlag,
					altText, bookmark, tocInfo );
			return;
		}
		...
		try
		{
			...
			{
				bodyWriter.drawImage( null, 0.0, 0.0, null, style, pStyle, inlineFlag,
						altText, bookmark, tocInfo );
				return;
			}
			...
			bodyWriter.drawImage( entry.getUri( ), height, width, hyper, style,
					pStyle, inlineFlag, altText, bookmark, tocInfo );
		}
		catch ( IOException e )
		{
			logger.log( Level.WARNING, e.getLocalizedMessage( ) );
	...
	protected void writeText( int type, String txt, IContent content,
			InlineFlag inlineFlag, StyleEntry computedStyle, StyleEntry inlineStyle )
	{
		...
		int paragraphWidth = (int) OdfUtil
				.twipToPt( context.getCurrentWidth( ) );
		...
		{
			bodyWriter.writeContent( type, txt, computedStyle, inlineStyle,
					computedStyle.getStyle().getFontFamily( ), hyper, inlineFlag,
					TextFlag.WHOLE, paragraphWidth, rtl, containerBookmarks, tableTocs );
		}
	...
	private void writeHeaderFooter( IPageContent currentPage, boolean isLastPage ) throws IOException, BirtException
	{
		...
		if ( previousPage.getPageHeader( ) != null
				&& ( previousPage.getPageNumber( ) > 1l || master
						.isShowHeaderOnFirst( ) ) )
		{
		...
		if ( previousPage.getPageFooter( ) != null && ( !isLastPage || master
				.isShowFooterOnLast( ) ))
		{
	...
	{
		double leftPadding = getPadding( style
				.getProperty( IStyle.STYLE_PADDING_LEFT ) );
		double rightPadding = getPadding( style
	...
	private double[] computeTblColumnWidths( ITableContent table, double tblWidth )
	{
		...
		for ( int i = 0; i < colCount; i++ )
		{
			...
			{
				double val = OdfUtil.convertTo( col.getWidth( ), tblWidth,
						context.getReportDpi());
				doubleCols[i] = val;
		...
		tblColumns = EmitterUtil.resizeTableColumn( (int)(tblWidth * coef), tblColumns, count,
				total );
