...
public class OdtEmitter extends AbstractOdfEmitter {
	...
	private void writeHeaderFooter(IPageContent currentPage, boolean isLastPage) throws IOException, BirtException {
		...
		StringBuilder displayName = new StringBuilder().append(master.getName());
		if (masterPageManager.getInstanceNumber() > 1) {
			displayName.append("-").append(masterPageManager.getInstanceNumber()); //$NON-NLS-1$
		}

		masterPageWriter.startMasterPage(pageLayout, masterPageManager.getCurrentMasterPage(), displayName.toString());
