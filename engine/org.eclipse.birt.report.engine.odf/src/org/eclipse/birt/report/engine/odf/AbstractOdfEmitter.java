...
{
	protected static Logger logger = Logger.getLogger( AbstractOdfEmitter.class
			.getName( ) );
	...
	protected StyleEntry[] getColStyles( double[] cols )
	{
		...
		for ( int i = 0; i < cols.length; i++ )
		{
			...
			if ( previousStyle != null
					&& previousStyle.getDoubleProperty( StyleConstant.WIDTH ) != null
					&& ( Math
							.abs( previousStyle
									.getDoubleProperty( StyleConstant.WIDTH )
									- cols[i] ) < 0.0001 ) )
			{
	...
	protected void processBackgroundImageStyle( StyleEntry style )
	{
		...
		String imageUri = EmitterUtil.getBackgroundImageUrl( style.getStyle( ),
				reportContent.getDesign( ).getReportDesign( ),
