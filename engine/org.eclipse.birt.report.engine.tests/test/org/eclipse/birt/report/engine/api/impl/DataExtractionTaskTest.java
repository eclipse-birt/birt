...
public class DataExtractionTaskTest extends EngineCase {
			...
			if (element instanceof TableHandle) {
				...
				if (iid.getComponentID() == 280) {
					...
					FilterExpression[0] = new FilterDefinition(
							new ConditionalExpression("row[\"CUSTOMERNUMBER_1\"]", OP_EQ, "\"SubQuery_Name: 128\""));
	...
	public void testDataExtractionFromRsetName() throws Exception {
		...
		// in this list, we have three resutl set items, one is ELEMENT_219,
		// ELEMENT_277, ELEMENT_289
	...
	public void testDataExtractionWithSorts() throws Exception {
		...
		if (result != null) {
			...
			if (iData != null) {
				while (iData.next()) {
					current = ((Integer) iData.getValue("CUSTOMERNUMBER")).intValue();
					if (previous != 0) {
	 ...
	 * assertEquals( 7, data.length( ) ); } catch ( Exception e ) { fail(
	 * e.getMessage( ) ); } }
	 */
	public void testFilters() throws BirtException {
		dataExTask.selectResultSet("ELEMENT_219");
		testFilter("OFFICECODE", OP_BETWEEN, "1", "4", "OFFICECODE", new String[] { "1", "2", "3", "4" });
		...
		testNoOperandFilter("row[\"OFFICECODE\"] != 7", OP_FALSE, "OFFICECODE", new String[] { "7" });
		...
		testFilter("OFFICECODE", OP_NOT_IN, new String[] { "2", "6" }, new String[] { "1", "3", "4", "5", "7" });
		testFilter("CITY", OP_NOT_LIKE, "\"S%\"", new String[] { "Boston", "NYC", "Paris", "Tokyo", "London" });
		testFilter("CITY", OP_NOT_MATCH, "\"S+\"", new String[] { "Boston", "NYC", "Paris", "Tokyo", "London" });

		testFilter("STATE", OP_NOT_NULL, null, new String[] { "CA", "MA", "NY", "Chiyoda-Ku" });
		testFilter("STATE", OP_NULL, null, "OFFICECODE", new String[] { "4", "6", "7" });
	...
	private void testNoOperandFilter(String expression, int operator, String expectedColumn, String[] expectedResult)
			throws EngineException, BirtException {
		IFilterDefinition[] simpleFilterExpression = createFilter(expression, operator);
		testFilterCondition(simpleFilterExpression, expectedColumn, expectedResult);
	}

	private void testFilter(String columnName, int operator, Object operand1, String expectedResult)
			throws EngineException, BirtException {
		testFilter(columnName, operator, operand1, new String[] { expectedResult });
	}
	...
	private void testFilter(String columnName, int operator, Object operand1, String expectedColumn,
			String[] expectedResult) throws EngineException, BirtException {
		testFilter(columnName, operator, operand1, null, expectedColumn, expectedResult);
	}

	private void testFilter(String columnName, int operator, Object operand1, Object operand2, String expectedColumn,
			String[] expectedResult) throws EngineException, BirtException {
		IFilterDefinition[] simpleFilterExpression = createFilter(columnName, operator, operand1, operand2);
		testFilterCondition(simpleFilterExpression, expectedColumn, expectedResult);
	}

	private void testFilterCondition(IFilterDefinition[] simpleFilterExpression, String expectedColumn,
			String[] expectedResult) throws EngineException, BirtException {
	...
	private void doTestExtractionTaskWithFilters(int resultSetCount) throws EngineException, BirtException {
		...
		FilterExpression[0] = new FilterDefinition(
				new ConditionalExpression("row[\"CUSTOMERNUMBER\"]", IConditionalExpression.OP_GE, "201", null));
		FilterExpression[1] = new FilterDefinition(
				new ConditionalExpression("row[\"CUSTOMERNUMBER\"]", IConditionalExpression.OP_LT, "300", null));
		// add filters to dataExtractionTask
		dataExTask.setFilters(FilterExpression);
		...
		if (result != null) {
			...
			if (iData != null) {
				iData.next();
				try {
					value = (String) DataTypeUtil.convert(iData.getValue("CUSTOMERNUMBER"), DataType.STRING_TYPE);
				} catch (Exception e) {
					value = null;
				}
	...
	private IFilterDefinition[] createFilter(String expression, int operator) {
		...
		return new IFilterDefinition[] { new FilterDefinition(conditionalExpression) };
	}
	...
	private IFilterDefinition[] createFilter(String columnName, int operator, Object operand1, Object operand2) {
		...
		if (!(operand1 instanceof String[])) {
			conditionalExpression = new ConditionalExpression("row[\"" + columnName + "\"]", operator,
