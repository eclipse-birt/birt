...
public class TOCBuilderTest extends TOCTestCase
{
	...
	public void testHiddenFormat1( )
	{
		...
		assertEquals( GOLDEN_TOC_TEST_1.replaceAll( "\\s", "" ), tocTree
				.replaceAll( "\\s", "" ) );
	}
	...
	void createTOCTest1( TOCBuilder builder )
	{
		...
		// the group
		{
			TOCEntry group = builder.startGroupEntry( null, null, null, null,
					-1 );
			{
				...
				{
					// detail one
					TOCEntry detail1 = builder.startEntry( list, "detail1",
							null, null, -1 );
					builder.closeEntry( detail1 );
					// detail two
					TOCEntry detail2 = builder.startEntry( list, "detail2",
							null, null, -1 );
	...
	public void testHiddenFormat2( )
	{
		...
		String tocTree = toString( builder.getTOCTree( ) );
		System.out.println( tocTree );
		assertEquals( GOLDEN_TOC_TEST_2.replaceAll( "\\s", "" ), tocTree
				.replaceAll( "\\s", "" ) );
	...
	protected void createTOCTest2( TOCBuilder builder )
	{
...

		{
			...
			// detail 1
			TOCEntry detail1 = builder.startEntry( group, "detail1", null,
					null, -1 );
			builder.closeEntry( detail1 ); // close detail

			// detail 2
			TOCEntry detail2 = builder.startEntry( group, "detail2", null,
					null, -1 );
	...
	public void testHiddenFormat3( )
	{
		...
		String tocTree = toString( builder.getTOCTree( ) );
		System.out.println( tocTree );
		assertEquals( GOLDEN_TOC_TEST_3.replaceAll( "\\s", "" ), tocTree
				.replaceAll( "\\s", "" ) );
	...
	protected void createTOCTest3( TOCBuilder builder )
	{
		...
		TOCEntry group = builder.startGroupEntry( null, null, null, "pdf", -1 );
		{
			...
			TOCEntry list = builder.startDummyEntry( group, "html" );
			{
				TOCEntry detail1 = builder.startEntry( list, "detail1", null,
						"html", -1 );
				builder.closeEntry( detail1 );

				TOCEntry detail2 = builder.startEntry( list, "detail2", null,
						null, -1 );
				builder.closeEntry( detail2 );
			}
			builder.closeEntry( list );
	...
	public void testHiddenFormat4( )
	{
		...
		String tocTree = toString( builder.getTOCTree( ) );
		System.out.println( tocTree );
		assertEquals( GOLDEN_TOC_TEST_4.replaceAll( "\\s", "" ), tocTree
				.replaceAll( "\\s", "" ) );
	...
	protected void createTOCTest4( TOCBuilder builder )
	{
		...
		TOCEntry group = builder.startGroupEntry( null, null, null, null, -1 );
		{
			...
			TOCEntry list = builder.startDummyEntry( group, "html" );
			{
				...
				{
					TOCEntry label = builder.startEntry( detail2, "label",
							null, -1 );
					builder.closeEntry( label );
				}
				builder.closeEntry( detail2 );
