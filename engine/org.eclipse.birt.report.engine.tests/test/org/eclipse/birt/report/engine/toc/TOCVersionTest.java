...
public class TOCVersionTest extends TOCTestCase
{
	...
	static final String GOLDEN_TOC = "<entry nodeId=\"null\">"
			...
			+ "            <entry nodeId=\"__TOC_1_1_2\" tocValue=\"group-footer\"/>"
			+ "        </group>"
			+ "        <entry nodeId=\"__TOC_1_2\" tocValue=\"list-footer\"/>"
			+ "    </entry>"
			+ "    <entry nodeId=\"__TOC_2\" tocValue=\"footer\"/>"
			+ "</entry>";
	...
	static final String GOLDEN_TOC_V3 = "<entry nodeId=\"/\">"
			...
			+ "            <entry nodeId=\"__TOC_6\" tocValue=\"group-footer\"/>"
			+ "        </group>"
			+ "        <entry nodeId=\"__TOC_7\" tocValue=\"list-footer\"/>"
			+ "    </entry>"
			+ "    <entry nodeId=\"__TOC_8\" tocValue=\"footer\"/>"
			+ "</entry>";
	...
	public void testReadV1( ) throws IOException
	{
		...
		assertEquals( GOLDEN_TOC.replaceAll( "\\s", "" ), tocTree.replaceAll(
				"\\s", "" ) );
		reader.close( );
		in.close( );
	}
	...
	public void testReadV3( ) throws IOException
	{
		...
		ITreeNode root = reader.readTree( );
		String tocTree = toString( root );
		assertEquals( GOLDEN_TOC_V3.replaceAll( "\\s", "" ), tocTree
				.replaceAll( "\\s", "" ) );
		reader.close( );
		in.close( );
	}
	...
	protected byte[] writeTOC( ) throws IOException
	{
		...
		{
			TOCEntry reportHeader = builder.startEntry( null, "report-header",
					null, -1 );
			...
			{
				TOCEntry listHeader = builder.startEntry( list, "list-header",
						null, -1 );
				builder.closeEntry( listHeader );

				TOCEntry group = builder.startGroupEntry( list, "group", null,
						null, -1 );
				{
					TOCEntry group21Header = builder.startEntry( group,
							"list-group-header", null, -1 );
					builder.closeEntry( group21Header );

					TOCEntry detail = builder.startEntry( group, "detail",
							null, null, -1 );
					builder.closeEntry( detail );

					TOCEntry group21Footer = builder.startEntry( group,
							"group-footer", null, null, -1 );
					builder.closeEntry( group21Footer );
				}
				builder.closeGroupEntry( group );

				TOCEntry listFooter = builder.startEntry( list, "list-footer",
						null, null, -1 );
