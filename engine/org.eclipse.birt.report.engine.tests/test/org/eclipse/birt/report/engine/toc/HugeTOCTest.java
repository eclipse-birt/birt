...
public class HugeTOCTest extends TOCTestCase
{
	...
	public void testHugeTOC( ) throws IOException
	{
		...
		System.out.println( "create TOC with " + TOC_SIZE * TOC_SIZE
				+ " entries" );
		try
		{
			...
			System.out
					.println( "file length:" + new File( TOC_FILE ).length( ) );

			System.out.println( "read TOC with " + TOC_SIZE * TOC_SIZE
					+ " entries" );

			TOCReader reader = new TOCReader( new ArchiveReader( archive ),
			...
			try
			{
				...
				for ( ITreeNode node1 : nodes1 )
				{
					...
					{
						assertEquals( "TOC_" + index1 + "_" + index2, node2
								.getTOCValue( ) );
