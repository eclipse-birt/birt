 ...
 * |		Output IID.
 * |-For row item:
 * |		Output type and row type for table header/footer row and group header/footer row.
 * |		Output group-id for all the rows.
 * |-For template item:
 * |		Output IID, bookmark and element type. Add the element into the iid list.
 * |-For text item and text data:
 * |		When the text item and text data is a top level element, or in Table/Group Header/Footer, 
 * |		output IID, bookmark and element type. Add the element into the iid list.
 * |-For label item:
 * |		Output IID, bookmark and element type. Add the element into the iid list.
 * |-For data item:
 * |		When the data item is a top level element or aggregatable( don't have a dataset, don't have a column binding, must in table/group header/footer row ), 
 * |		output IID, bookmark and element type. Add the element into the iid list.
 */
public class MetadataFilter implements IMetadataFilter
{
	...
	public HashMap needMetaData( ReportElementHandle elementHandle )
	{
		...
		if ( elementHandle instanceof RowHandle )
		{
			...
			else if ( rowParentHnadle instanceof TableGroupHandle )
			{
				...
				if ( TableGroupHandle.HEADER_SLOT == slotID )
				{
					...
					if ( groupLevel != -1 )
					{
						resultMap.put( IMetadataFilter.KEY_ATTR_TYPE, "wrgh"
								+ groupLevel );
					}
					resultMap.put( IMetadataFilter.KEY_ATTR_ROW_TYPE,
							"group-header" );
					resultMap.put( IMetadataFilter.KEY_OUTPUT_GOURP_ID, Boolean.TRUE );
				}
				else if ( TableGroupHandle.FOOTER_SLOT == slotID )
				{
					...
					if ( groupLevel != -1 )
					{
						resultMap.put( IMetadataFilter.KEY_ATTR_TYPE, "wrgf"
								+ groupLevel );
					}
					resultMap.put( IMetadataFilter.KEY_ATTR_ROW_TYPE,
							"group-footer" );
		...
		else if ( ( elementHandle instanceof TextItemHandle )
				|| ( elementHandle instanceof TextDataHandle ) )
		{
			if ( isTopLevelElement( elementHandle )
					|| isInHeaderFooter( elementHandle ) )
			{
				resultMap = new HashMap( );
				resultMap.put( IMetadataFilter.KEY_OUTPUT_IID, Boolean.TRUE );
				resultMap.put( IMetadataFilter.KEY_OUTPUT_BOOKMARK, Boolean.TRUE );
	...
	/**
	 * Checks if a element is in a table header/footer row, or group
	 * header/footer row.
	 * 
	 * @param text
	 * @return
		...
		while ( containerHandle != null )
		{
			...
			if ( containerHandle instanceof RowHandle )
			{
				...
				if ( rowParentHnadle instanceof TableHandle )
				{
					...
					if ( TableHandle.HEADER_SLOT == slotID
							|| TableHandle.FOOTER_SLOT == slotID )
					{
