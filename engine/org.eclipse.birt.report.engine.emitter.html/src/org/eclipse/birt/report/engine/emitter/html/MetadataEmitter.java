...
public class MetadataEmitter {
	 ...
	 * @param htmlOption
	 * @param idGenerator
	 * @param attrNamePrefix : the prefix of the attribute name.
	 */
	public MetadataEmitter(HTMLWriter writer, HTMLRenderOption htmlOption, String attrNamePrefix,
			IDGenerator idGenerator, HTMLReportEmitter htmlReportEmitter) {
		this.writer = writer;
...
//		}
	...
	public void startCell(ICellContent cell) {
		...
		if (needColumnFilter || needGroupIcon) {
			...
			// FIXME: code review: move the td outputting to "if ( needGroupIcon )". remove
			// useless style.
	...
	public void endCell(ICellContent cell) {
		...
		if (needColumnFilter) {
			...
			writer.attribute(HTMLTags.ATTR_STYLE, "cursor:pointer");
			writer.attribute(HTMLTags.ATTR_COLUMN, cell.getColumnInstance().getInstanceID().toString());
			String bookmark = idGenerator.generateUniqueID();
			htmlReportEmitter.outputBookmark(writer, null, htmlIDNamespace, bookmark);
		...
		while (ite.hasNext()) {
			...
			if (keyObj instanceof String) {
				...
				if (keyStr == IMetadataFilter.KEY_OUTPUT_IID) {
					...
					if (iid != null) {
						if ((genBy instanceof TableItemDesign) || (genBy instanceof ListItemDesign)
	...
	/**
	 * Checks if column filter icons need to be output. Column filter icons need to
	 * be output when:
	 * <ol>
	...
	private boolean needColumnFilter(ICellContent cell) {
		...
		DetailRowState state = (DetailRowState) detailRowStateStack.peek();
		return state.isStartOfDetail && columnInstance.hasDataItemsInDetail() && displayFilterIcon
	...
	public void startWrapTable(IContent table) {
		if (wrapTemplateTable && isTopLevelTemplateTable(table)) {
			...
			if (width != null) {
				...
				writer.attribute(HTMLTags.ATTR_STYLE, " border: medium none ; border-collapse: collapse; width: 100%;");
			}
			...
			writer.openTag(HTMLTags.TAG_TD);
			writer.openTag(HTMLTags.TAG_IMAGE);
			writer.attribute(HTMLTags.ATTR_SRC, imagePath + "./bizRD/images/sidetab_active.png");
			writer.attribute(HTMLTags.ATTR_STYLE, " width: 20px; height: 60px;");
			writer.closeTag(HTMLTags.TAG_IMAGE);
			writer.closeTag(HTMLTags.TAG_TD);
			writer.openTag(HTMLTags.TAG_TD);
...
class DetailRowState {
	...
	public DetailRowState(boolean isStartOfDetail, boolean hasOutput, boolean isTable) {
