...
public class PPTXRender extends PageDeviceRender {
	...
	public IPageDevice createPageDevice(String title, String author, String subject, String description,
		...
		try {
			...
			PPTXPageDevice pageDevice = new PPTXPageDevice(out, title, author, description, subject, tempFileDir,
					compressionMode);
	...
	public void visitText(ITextArea textArea) {
		PPTXPage page = (PPTXPage) pageGraphic;
		page.setLink(PPTUtil.getHyperlink(textArea, services, reportRunnable, context));
	...
	public void visitContainer(IContainerArea container) {
		if (container instanceof PageArea) {
		...
		} else if (needBufferOutput && editMode) {// other cases inside table buffer out
			...
			if (container instanceof ImageBlockContainer) {
				...
				if (bmk != null) {// addbookmarks
					Presentation presentation = ((PPTXPage) pageGraphic).getCanvas().getPresentation();
	...
	private void outputTable(TableArea table) {
		...
		if (!needBufferOutput) {
			...
			while (!bufferedOuptuts.isEmpty()) {
				ByteArrayOutputStream output = bufferedOuptuts.remove(bufferedOuptuts.size() - 1);
	...
	private void outputText(ContainerArea text) {
		if (editMode) {
			...
			tw.setLink(PPTUtil.getHyperlink(text, services, reportRunnable, context));
