...
public class TableWriter {
		...
		for (int rowidx = table.getChildrenCount() - 1; rowidx >= 0; rowidx--) {
			...
			if (child instanceof TableGroupArea) {
				...
				if (numOfCells == 0) {
					...
					if (numOfColumns != numOfCells) {// there is colspan, go one by one and then reduce grid if
	...
	protected void drawRow(RowArea row) {
		...
		while (!(parent instanceof TableArea) && style.getBackgroundColor() == null
		...
		while (currentCol != firstcellidx) {// first columns has rowspan
			int emptycol = currentCol;
			// create empty rowspan cell
			fillEmptyMergeCells(emptycol, 0, 0);
			if (emptycol == currentCol) {// no spanrow on column move to next one
				currentCol++;
			}
		}
		while (iter.hasNext()) {
	...
	private void startRow(RowArea row) {
		...
		if (height < MINIMUM_ROW_HEIGHT) {// if lower than minimum height set to zero:
			height = 0;
		} else {
	...
	private void endCell(CellArea cell) {
		...
		if (cellchild instanceof BlockTextArea && cell.getChildrenCount() == 1) {
			...
			if (textboxstyle != null) {
				...
				int textboxheight = canvas.getScaledValue(PPTXUtil.convertToEnums((cellchild.getHeight())));
				if (isRTL && marR <= 0) {
					marR = DEFAULT_MARGIN;
		...
		} else {// default behavior for empty cell
	...
	protected void visitChildren(CellArea container) {
		...
		while (iter.hasNext()) {
			IArea child = iter.next();
			if (child instanceof BlockTextArea && childrencount > 1) {// if the text is clipped
	...
	private boolean needStyleORClip(IArea blocktext) {
		...
		if (style != null && (style.getBackgroundColor() != null || style.getBackgroundImage() != null
				|| style.getBottomBorder() != null || style.getLeftBorder() != null || style.getRightBorder() != null
				|| style.getTopBorder() != null)) {
			return true;
		}
	...
	protected void drawCellBox(CellArea cell) {
		...
		if (imageRelationship != null) {
			...
			canvas.setBackgroundImg(imageRelationship, (int) offsetX, (int) offsetY, repeatmode);
	...
	protected void drawCellDiagonal(CellArea cell) {
		...
		if (diagonalInfo != null && diagonalInfo.getDiagonalNumber() == 1) {// only support single line : width should
			...
			writer.attribute("val", PPTXUtil.parseStyle(diagonalInfo.getDiagonalStyle()));
