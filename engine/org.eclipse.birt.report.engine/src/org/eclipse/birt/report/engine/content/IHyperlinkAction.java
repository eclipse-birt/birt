...
/**
 * Defines an interface that allows engine to pass hyperlink information to
 * an emitter, if the emitter determines to customize the hyperlinks calculated
 * in engine, or it wants to use a totally different hyperlink string   
 * <p>
...
public interface IHyperlinkAction {
	 ...
	 * link; null when the action type is not drillthrough, or no parameters are 
	 * defined for the drillthrough report to run. In the future, when the 
	 * drillthrough is against a report document, the parameter binding map is also
	 * null.  
	 */
	 ...
	 * link; null when the action type is not drillthrough, or no search criteria is
	 * used
	 */
