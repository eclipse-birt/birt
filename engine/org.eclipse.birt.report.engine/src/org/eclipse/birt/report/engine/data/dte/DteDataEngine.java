...
public class DteDataEngine extends AbstractDataEngine
{
	...
	protected void doPrepareQuery( Report report, Map appContext )
	{
		...
		for ( int i = 0; i < queries.size( ); i++ )
		{
			...
			try
			{
				IPreparedQuery preparedQuery = dteSession.prepare( query,
						appContext );
	...
	protected IBaseResultSet doExecuteQuery( IBaseResultSet resultSet, IQueryDefinition query )
	{
		...
		try
		{
			...
			if ( resultSet == null )
			{
				...
				resultSet = new QueryResultSet( this, context, query,
			...
			else
			{
				...
				resultSet = new QueryResultSet( this, context, resultSet, query,
						(IQueryResults) dteResults );
			}
	...
	protected IBaseResultSet doExecuteCube( IBaseResultSet resultSet, ICubeQueryDefinition query )
	{
		IPreparedQuery pQuery = (IPreparedQuery) queryMap.get( query );
		if ( pQuery == null )
		{
