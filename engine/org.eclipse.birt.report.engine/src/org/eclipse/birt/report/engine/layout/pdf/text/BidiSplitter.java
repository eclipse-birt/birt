...
/*				
				...
				for (int i = beginIndex; i < endIndex; i++)
				{
					...
					//does shapping for arabic characters.
					if ( -1 == ArabicStartIndex )
					{	
						if (chunkText[i] >= 0x0600 && chunkText[i] <= 0x06ff)
						{
							ArabicStartIndex = i;
						}
						else
						{
							sb.append(chunkText[i]);
						}
					}else
					{
						if (chunkText[i] < 0x0600 || chunkText[i] > 0x06ff)
						{
							String str = doArabicShapping(ArabicStartIndex, i);
							sb.append(str);
							ArabicStartIndex = -1;
							sb.append(chunkText[i]);
						}
...
*/	
	...
	private boolean needMirroring(char c) 
	{
...
//		ArabicLigaturizer.arabic_shape(
//			chunkText, start, end-start, dest, 0, end-start, 0);
