...
abstract public class GroupExecutor extends ReportItemExecutor {
	...
	void collectExecutableElements() {
		...
		try {
			ListingDesign listingDesign = (ListingDesign) listingExecutor.getDesign();
			IQueryResultSet rset = listingExecutor.getResultSet();
			GroupDesign groupDesign = (GroupDesign) getDesign();
	...
	/**
	 * handle the page-break-before of group. AUTO: page-break-always-excluding_fist
		...
		try {
			...
			if (groupDesign != null) {
				...
				if (needPageBreak) {
					...
					// remove soft page break which is caused by page-break-interval, or empty page
					// should be generated.
	...
	protected void handlePageBreakInsideOfGroup() {
		...
		if (groupDesign != null) {
			...
			if (DesignChoiceConstants.PAGE_BREAK_INSIDE_AVOID.equals(pageBreakInside)) {
				content.getStyle().setProperty(IStyle.STYLE_PAGE_BREAK_INSIDE, IStyle.AVOID_VALUE);
			}
		}
	}
