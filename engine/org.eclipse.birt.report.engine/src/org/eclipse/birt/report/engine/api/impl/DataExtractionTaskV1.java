			...
			IDataExtractionTask
{
	...
	IResultSet executeQuery( IResultSet prset, InstanceID iid )
			...
			throws BirtException
	{
		...
		dataEngine.prepare( query, appContext );
		IPreparedQuery pQuery = dataEngine.prepare( query );
	...
	private SubqueryDefinition cloneQuery( SubqueryDefinition query )
	{
		...
		newQuery.getResultSetExpressions( ).putAll(
				query.getResultSetExpressions( ) );
