...
public class ImageItemExecutor extends QueryItemExecutor {
	...
	protected void handleImage(ImageItemDesign imageDesign, IImageContent imageContent) throws BirtException {
		...
		case ImageItemDesign.IMAGE_EXPRESSION: // get image from
			...
			break;
		default:
			getLogger().log(Level.SEVERE, "[ImageItemExecutor] invalid image source"); //$NON-NLS-1$
	...
	protected void handleNamedImage(Expression nameExpr, IImageContent imageContent) throws BirtException {
		...
		EmbeddedImage embeddedImage = context.getReport().getReportDesign().findImage(imageName);
		if (embeddedImage != null) {
			imageContent.setData(embeddedImage.getData(context.getDesign().getModule()));
			String mimeType = embeddedImage.getType(context.getDesign().getModule());
			if (null != mimeType) {
	...
	protected void handleValueImage(Expression imgExpr, Expression fmtExpr, IImageContent imageContent)
	...
	protected void handleFileExpressionImage(Expression fileExpr, IImageContent imageContent) throws BirtException {
		...
		if (imageFile == null) {
			getLogger().log(Level.SEVERE, "[ImageItemExecutor] Source image file is missing"); //$NON-NLS-1$
