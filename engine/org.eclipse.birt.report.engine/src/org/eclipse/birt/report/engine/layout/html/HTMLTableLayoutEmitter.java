	...
	public void flush() throws BirtException {
		...
		while (iter.hasNext()) {
			...
			case LayoutEvent.START_BAND:
				ContentEmitterUtil.startContent((IContent) event.value, emitter);
				break;
			case LayoutEvent.END_GROUP:
			case LayoutEvent.END_BAND:
				ContentEmitterUtil.endContent((IContent) event.value, emitter);
				break;
			case LayoutEvent.ON_ROW:
				flushRow(((StartInfo) event.value).rowId, 0, true);
				break;
			case LayoutEvent.ON_FIRST_DROP_CELL:
				flushRow(((StartInfo) event.value).rowId, ((StartInfo) event.value).cellId, false);
				break;
			}
		}
		resetLayout();
		...
		if (cellEmitter != null) {
			...
			if (!isNestTable()) {
				...
				if (!isHidden) {
					if (hasDropCell()) {
						layoutEvents
								.push(new LayoutEvent(LayoutEvent.ON_ROW, new StartInfo(layout.getRowCount() - 1, 0)));
						return;
		...
		if (cellEmitter != null) {
			...
			if (!isNestTable()) {
				...
				if (hasDropCell()) {
					...
					if (emitter instanceof ContentEmitterAdapter
							&& !((ContentEmitterAdapter) emitter).isMultiplePagesEnabled()) {
						return;
					}
