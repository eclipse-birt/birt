...
public class InlineTextLayout extends Layout
{
	...
	{	
		...
		public boolean compose()
		{
			...
			{
				handleNext();
				if(InlineTextLayout.this.needPause())
				{
					return true;
				}
			}
			...
			if (cg.hasMore())
				...
				else
				{
					if (isInline)
					{
						...
						InlineTextLayout.this.addSpaceHolder(con);
					}
					return false;
				}
		}
		...
		{
			int freeSpace = InlineTextLayout.this.getFreeSpace();
			...
			if ( isNew || currentPos == chunk.getText().length() )
			{
				if (cg.hasMore())
				{
					...
					if (!pdfTextWrapping)
					{
						...
						InlineTextLayout.this.addTextLine(builtArea);
						InlineTextLayout.this.newLine();
						currentPos = chunk.getText().length();
						vestigeIndex = -1;
						return;
