...
abstract public class ContainerExecutor extends ReportItemExecutor
{
	 ...
	 */
	private boolean useNextSection;
	/**
		...
		if ( fragment != null )
		{
			...
			if ( useNextSection )
			{
				...
				else
				{
					...
					doSkipToExecutor( leftIndex.getInstanceID( ), nextOffset );
					uniqueId = leftIndex.getInstanceID( ).getUniqueID( );
				}
			}
		}
			...
			if ( childContent != null )
			{
				...
				if ( fragment != null )
				{
					...
					if ( rightEdge != Segment.RIGHT_MOST_EDGE )
					{
						...
						if ( isSameInstance( rightIndex.getInstanceID( ),
								childContent.getInstanceID( ) ) )
						{
							useNextSection = true;
						}
					}
