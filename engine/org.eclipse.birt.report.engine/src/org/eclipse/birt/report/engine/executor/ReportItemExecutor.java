...
public abstract class ReportItemExecutor implements IReportItemExecutor {
	...
	protected Boolean evaluateBoolean(Expression expr) {
		try {
			...
			getLogger().log(Level.WARNING, "Invalid boolean expression:" + (expr == null ? "null" : expr.toString()));
		}
		return null;
	}
	...
	protected void processAction(ReportItemDesign item, IContent itemContent) {
		...
		if (action != null) {
			...
			case ActionDesign.ACTION_DRILLTHROUGH:
				 ...
				 * ReportDesignHandle design = context.getDesign( ); if ( design != null ) { URL
				 * reportURL = design.findResource( reportName, IResourceLocator.LIBRARY ); if (
				 * reportURL != null ) { String reportFile = reportURL.getFile( ); if (
				 * reportFile != null ) { reportName = reportFile; } } }
				 */
				String format = drill.getFormat();
				// XXX Do not support Search criteria
				IHyperlinkAction obj = report.createActionContent();
				String targetFileType = drill.getTargetFileType();
				obj.setDrillThrough(bookmark, isBookmark, reportName, paramsVal, null, targetWindow, format,
						targetFileType);
				obj.setTooltip(tooltip);
				itemContent.setHyperlinkAction(obj);
				break;
					...
					if (hiddenFormats != null) {
						...
						if (design != null && design instanceof ReportItemDesign) {
							...
							if (expr != null) {
								...
								TOCEntry parentTOCEntry = getParentTOCEntry();
								long elementId = getElementId();
								tocEntry = tocBuilder.startEntry(parentTOCEntry, null, bookmark, hiddenFormats,
										elementId);
								if (bookmark == null) {
