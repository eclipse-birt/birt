...
public class ExecutionContext {
	...
	public void close() {
		...
		// Do this last since it destroys the appContext which may be used by status
		// handlers
		...
		if (expr != null) {
			...
			case Expression.SCRIPT:
				...
				if (compiledScript == null) {
					compiledScript = compile(script.getLanguage(), script.getFileName(), script.getLineNumber(),
	...
	/**
	 * The expression may be evaluated at onPrepare stage, at that time the reportIR
	 * is not initialized.
	 */
	...
	public Object evaluate(String language, String fileName, int lineNumber, String scriptText) throws BirtException {
		...
		ICompiledScript compiledScript = compile(language, fileName, lineNumber, scriptText);
		return evaluate(compiledScript);
	}

	private ICompiledScript compile(String language, String fileName, int lineNumber, String scriptText)
			throws BirtException {
		ICompiledScript compiledScript = runnable.getScript(language, scriptText);
		if (compiledScript == null) {
			compiledScript = getScriptContext().compile(language, fileName, lineNumber, scriptText);
	...
	public void setParameter(String name, Object value, Object displayText) {
		if (displayText == null || displayText instanceof String) {
			params.put(name, new ParameterAttribute(value, (String) displayText));
		} else if (displayText instanceof String[]) {
			params.put(name, new ParameterAttribute((Object[]) value, (String[]) displayText));
		}
	}
	...
	public ModuleHandle getDesign() {
		return runnable != null ? (ModuleHandle) runnable.getDesignHandle() : null;
	}
	...
	public void loadScript(String language, String fileName) {
		...
		try {
			...
			byte[] script = getResourceLocator().findResource(reportDesign, fileName, IResourceLocator.LIBRARY,
					appContext);
			ICompiledScript compiledScript = getScriptContext().compile(language, fileName, 1,
			...
			log.log(Level.SEVERE, "loading external script file " + fileName + " failed.", //$NON-NLS-1$ //$NON-NLS-2$
					ex);
			addException(new EngineException(MessageConstants.SCRIPT_FILE_LOAD_ERROR, fileName, ex)); // $NON-NLS-1$
			// TODO This is a fatal error. Should throw an exception.
		} catch (BirtException e) {
			log.log(Level.SEVERE, "Failed to execute script " + fileName + ".", //$NON-NLS-1$ //$NON-NLS-2$
					e);
			addException(new EngineException(MessageConstants.SCRIPT_EVALUATION_ERROR,
					new Object[] { fileName, e.toString() }, e)); // $NON-NLS-1$
		}
	}
		...
		public void addException(BirtException e) {
			for (int i = 0; i < exList.size(); i++) {
				...
				if (e.getErrorCode() != null && e.getErrorCode().equals(err.getErrorCode())
						...
						&& e.getLocalizedMessage().equals(err.getLocalizedMessage())) {
					countList.set(i, Integer.valueOf(((Integer) countList.get(i)).intValue() + 1));
					return;
				}
			}
			exList.add(e);
	...
	public void setPageNumber(long pageNo) {
		pageNumber = pageNo;
		if (scriptContext != null) {
			getRootContext().setAttribute("pageNumber", Long.valueOf(pageNumber));
		}
	...
	public void setTotalPage(long totalPage) {
		if (totalPage > this.totalPage) {
			...
			if (scriptContext != null) {
				getRootContext().setAttribute("totalPage", Long.valueOf(totalPage));
			}
	...
	public void cancel() {
		...
		if (task != null) {
			...
			if (handle != null) {
				EngineResourceHandle rc = new EngineResourceHandle(ULocale.forLocale(getLocale()));
				handle.showStatus(rc.getMessage(MessageConstants.TASK_CANCEL));
			}
		}
	}
	...
	public IBaseResultSet executeQuery(IBaseResultSet parent, IDataQueryDefinition query, Object queryOwner,
	...
	public void setResultSets(IBaseResultSet[] rsets) {
		...
		if (rsets != null) {
			...
			if (rsets[0] != null) {
				try {
					DataAdapterUtil.registerDataObject(scriptContext, new ResultIteratorTree(rsets[0]));
				} catch (AdapterException e) {
					log.log(Level.SEVERE, e.getLocalizedMessage(), e);
				}
			}
	...
	/**
	 * Since the data set in master page will be executed in each page and while the
	 * data set in report body will only be executed once, we need to remember the
	 * current report item is in master page or not. This will be used to help store
	 * the executed resultSetID and load it to distinguish them.
	 */
	 ...
	 */
	public void firePageBreakEvent(boolean isHorizontalPageBreak, boolean isSizeOverflowPageBreak) {
		if (pageBreakListeners != null) {
	...
	public void optimizeExecution() {
		if ((task != null) && (task.getTaskType() == IEngineTask.TASK_RUN) && !isFixedLayout) {
	...
	public URL getResource(String resourceName) {
		if (getDesign() != null) {
			return getDesign().findResource(resourceName, IResourceLocator.OTHERS, appContext);
		}
		return null;
	}
	...
	public void setPageVariable(String name, Object value) {
		...
		// lazy add special page variables
			...
			addPageVariable(new PageVariable(IReportContext.PAGE_VAR_PAGE_LABEL, PageVariable.SCOPE_PAGE, value));
		}
	}
	...
	public DataRequestSession newSession(DataSessionContext dteSessionContext) throws BirtException {
		...
		if (getEngine() instanceof IDataAdapterFactory) {
			return ((IDataAdapterFactory) getEngine()).createSession(dteSessionContext);
		}
