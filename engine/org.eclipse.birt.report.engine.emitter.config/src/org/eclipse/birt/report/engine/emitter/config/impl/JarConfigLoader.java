...
public class JarConfigLoader implements IDefaultConfigLoader {
	...
	protected void parseConfigFor(final String bundleName, final IEmitterDescriptor descriptor,
			final Map<String, RenderOptionDefn> options) throws Exception {
		// first, load .cfg file
		loadCfgFile(getResourceURL(bundleName, OPTIONS_CONFIG_FILE), descriptor, options);
		// then load .xml file, this overrides the .cfg file
		loadXMLFile(bundleName, RENDER_OPTIONS_FILE, descriptor, options);
	}

	private void loadCfgFile(final URL url, final IEmitterDescriptor descriptor,
			final Map<String, RenderOptionDefn> options) throws Exception {
		if (url != null) {
	...
	protected void loadXMLFile(final String bundleName, final String fileName, final IEmitterDescriptor descriptor,
			final Map<String, RenderOptionDefn> options) throws Exception {
	...
	private void parseConfigXML(final InputStream in, final IEmitterDescriptor descriptor,
			final Map<String, RenderOptionDefn> options) throws Exception {
	...
	private URL getResourceURL(final String bundleName, final String resourceName) {
		...
		public void startElement(String uri, String localName, String qName, Attributes attributes)
			...
			if (OPTION_QNAME.equalsIgnoreCase(qName)) {
				...
				if (!isEmpty(name)) {
					...
					options.put(name, new RenderOptionDefn(name, defualt, enabled));
				}
			}
