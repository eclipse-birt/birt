	...
	static Object fetchFieldDocument( Object fieldValue, byte fieldNativeDataType )
    {
        ...
        if( fieldNativeDataType == BSON.ARRAY )
        {
            ...
            if( listOfObjects.size() > 0 )
            {
            	...
            	Object childFieldValue = listOfObjects.get(0);
            	if (childFieldValue instanceof Document) {
            		return (Document)childFieldValue;
            	} else {
                    // log and ignore
            		getLogger().log( Level.INFO, "Ignoring fetching of unsupported BSON.OBJECT type " + childFieldValue ); //$NON-NLS-1$
            	}
            }
        }
        */
