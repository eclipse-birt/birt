...
import org.eclipse.birt.data.oda.mongodb3.impl.MDbResultSet;
import org.eclipse.birt.data.oda.mongodb3.impl.MDbResultSetMetaData;
...
public class MDbOperation
{
            ...
            boolean includeSortExpr )
    {
    	...
    	if (findIterable != null) {
	        ...
	        findIterable.maxTime(Bytes.QUERYOPTION_NOTIMEOUT, arg1)
            ...
            QueryProperties queryProps ) throws OdaException
    {
        ...
        if (!(command instanceof BasicDBObject) ) {
        	throw new OdaException( 
                    Messages.bind( "Unexpected data type ({0}) in Selected Fields property value in MapReduce command", command.getClass().getSimpleName()) );        	
        }
        ...
        if (object instanceof String) {
        	...
        	throw new OdaException( 
                    Messages.bind( "Unexpected data type ({0}) in {1} function", command.getClass().getSimpleName(), QueryModel.MAP_REDUCE_MAP_FUNCTION) );        	
        }
        ...
        if (object instanceof String) {
        	...
        	throw new OdaException( 
                    Messages.bind( Messages.driverUtil_invalidExpr, command.getClass().getSimpleName() + " in " + QueryModel.MAP_REDUCE_REDUCE_FUNCTION) );        	
        }
