...
public class SQBDataSetWizardPage extends DataSetWizardPage {
	...
	private static IConnectionProfile loadConnectionProfile(java.util.Properties connProps,
		...
		Map<String, Object> designSessionContext = DesignSessionUtil
				.createResourceIdentifiersContext(designResourceIdentifiers);
		try {
	...
	private ISQLBuilderEditorInput createSQBInput(Composite parent, IConnectionProfile connProfile) {
		...
		// if restored input has a different SQL statement, first ask user if ok to
		// replace
	...
	private Control createSQBControl(Composite parent, ISQLBuilderEditorInput sqbInput) {
		...
		if (!isInputLoaded && !(sqbInput instanceof DefaultSQBInput)) {
			...
			if (openInvalidInputMessageBox(parentShell, true)) {
				...
				isInputLoaded = m_sqbDialog.setInput(new DefaultSQBInput(sqbInput.getConnectionInfo()));
				m_updatedQueryInput = true;
			}
		}
	...
	private IStatus doConnect() {
		...
		// TODO - this connect task cannot be cancelled; should replace with a
		// cancellable one (BZ 228292)
	...
	protected DataSetDesign collectDataSetDesign(DataSetDesign design) {
		...
		if (m_sqbDialog.isDirty()) {
			SQLQueryUtility.updateDataSetDesign(design, m_sqbDialog.getSQLQueryStatement(),
					getConnectionProfile(false, false), getInitializationDesign(), m_initQuerySortSpec);
	...
	private static boolean openInvalidInputMessageBox(Shell parentShell, boolean askUser) {
		...
		if (askUser) {
			userMessage += NEWLINE_CHAR + Messages.sqbWizPage_inputFailOnOpenAskUserMessage;
			return MessageDialog.openQuestion(parentShell, Messages.sqbWizPage_invalidSqbStateTitle, userMessage);
		}

		// not an user option, raise warning and continue
		MessageDialog.openWarning(parentShell, Messages.sqbWizPage_invalidSqbStateTitle, userMessage);
		return true; // continue
	}

	private static boolean openReplaceSQLMessageBox(Shell parentShell) {
		return MessageDialog.openQuestion(parentShell, Messages.sqbWizPage_detectSqlTextChangedTitle,
				Messages.sqbWizPage_detectExternalSqlTextChangedMsg);
	}

	/**
	 * Internal class for a default SQB Input used when no existing input is
	 * available.
	 */
