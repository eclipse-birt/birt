...
public class MongoDBDataSetWizardPage extends DataSetWizardPage
{
	...
	private void createRightSelectedFieldsArea( Composite fieldsSelectionArea )
	{
		...
		selectedFieldsTable = new TableViewer( rightComposite,
				SWT.FULL_SELECTION | SWT.MULTI | SWT.BORDER | SWT.H_SCROLL
						| SWT.V_SCROLL );
		GridData gd = new GridData( GridData.FILL_BOTH );
		gd.heightHint = 300;
		gd.widthHint = 120;
		...
		downBtn.addSelectionListener( new SelectionListener( ) {
			...
			public void widgetSelected( SelectionEvent e )
			{
				if ( selectedFieldsTable.getTable( ).getSelectionCount( ) == 1 )
				{
					int index = selectedFieldsTable.getTable( )
							.getSelectionIndex( );
					if ( index >= 0 && index < ( selectedFieldsTable.getTable( )
							.getItemCount( ) - 1 ) )
					{
						FieldMetaData field = (FieldMetaData) selectedFields
								.get( index );
	...
	private void createCommandOpArea( Composite topArea )
	{
		...
		opTypeSelectionListener = new SelectionListener( ) {
			...
			public void widgetSelected( SelectionEvent e )
			{
				if ( opType != null && opType.displayName( ).trim( )
	...
	private void resetOpTypeComboItems( )
	{
		...
		{
			opTypeCombo.setItems( new String[]{EMPTY_STRING,
	...
	private void initializeControl( )
	{
		...
		{
			ExceptionHandler.showException( getShell( ), Messages.getString(
					"MongoDBDataSetWizardPage.MessageDialog.title.GeneralError" ), //$NON-NLS-1$
					e.getMessage( ), e );
		}
	...
	private void refreshAvailableFieldsViewer( )
	{
				...
				opType.name( ) ) || !UIHelper.isEmptyString( collectionName ) )
		{
			if ( treeEntry == null )
			{
				treeEntry = new FieldEntryWrapper( collectionName, metaData,
						searchLimit, queryProps );
				availableFieldsViewer
						.setInput( new FieldEntryWrapper[]{treeEntry} );
			}
				...
				try
				{
					if ( treeEntry == null )
					{
						treeEntry = new FieldEntryWrapper( collectionName,
								metaData, searchLimit, queryProps );
					}
					else
					{
						treeEntry.setCollectionName( collectionName );
		...
		for ( int i = 0; i < selectedFields.size( ); i++ )
		{
			if ( !existsField( selectedFields.get( i ) ) )
			{
				setMessage( Messages.getFormattedString(
						...
						new Object[]{selectedFields.get( i )
								.getFullDisplayName( )} ),
						ERROR );
				return false;
			}
