...
class SecondGroupCalculator extends DateGroupCalculator {
	...
	public Object calculate(Object value) {
		...
		if (intervalStart == null) {
			return new Double(
					Math.floor(this.dateTimeUtil.diffSecond(defaultStart, (Date) value) / getDateIntervalRange()));
		} else if (this.dateTimeUtil.diffSecond((Date) intervalStart, (Date) value) < 0) {
			return new Double(-1);
		} else {
			return new Double(Math
					.floor(this.dateTimeUtil.diffSecond((Date) intervalStart, (Date) value) / getDateIntervalRange()));
		}
	}
}
