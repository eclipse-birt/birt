...
/**
 * This class implement some utility methods that can be used by the consumer of
 * Data Engine.
 */
...
public class DataAdapterUtil {
	...
	private static void registerAggregationFunction() {
		aggrAdapterMap.put(DesignChoiceConstants.AGGREGATION_FUNCTION_SUM, IBuildInAggregation.TOTAL_SUM_FUNC);
		aggrAdapterMap.put(DesignChoiceConstants.AGGREGATION_FUNCTION_COUNT, IBuildInAggregation.TOTAL_COUNT_FUNC);
		aggrAdapterMap.put(DesignChoiceConstants.AGGREGATION_FUNCTION_MIN, IBuildInAggregation.TOTAL_MIN_FUNC);
		aggrAdapterMap.put(DesignChoiceConstants.AGGREGATION_FUNCTION_MAX, IBuildInAggregation.TOTAL_MAX_FUNC);
		aggrAdapterMap.put(DesignChoiceConstants.AGGREGATION_FUNCTION_AVERAGE, IBuildInAggregation.TOTAL_AVE_FUNC);
		aggrAdapterMap.put(DesignChoiceConstants.AGGREGATION_FUNCTION_WEIGHTEDAVG,
				IBuildInAggregation.TOTAL_WEIGHTEDAVE_FUNC);
		aggrAdapterMap.put(DesignChoiceConstants.AGGREGATION_FUNCTION_STDDEV, IBuildInAggregation.TOTAL_STDDEV_FUNC);
		aggrAdapterMap.put(DesignChoiceConstants.AGGREGATION_FUNCTION_FIRST, IBuildInAggregation.TOTAL_FIRST_FUNC);
		aggrAdapterMap.put(DesignChoiceConstants.AGGREGATION_FUNCTION_LAST, IBuildInAggregation.TOTAL_LAST_FUNC);
		aggrAdapterMap.put(DesignChoiceConstants.AGGREGATION_FUNCTION_MODE, IBuildInAggregation.TOTAL_MODE_FUNC);
		...
		aggrAdapterMap.put(DesignChoiceConstants.AGGREGATION_FUNCTION_IRR, IBuildInAggregation.TOTAL_IRR_FUNC);
		aggrAdapterMap.put(DesignChoiceConstants.AGGREGATION_FUNCTION_MIRR, IBuildInAggregation.TOTAL_MIRR_FUNC);
		aggrAdapterMap.put(DesignChoiceConstants.AGGREGATION_FUNCTION_NPV, IBuildInAggregation.TOTAL_NPV_FUNC);
	...
	/**
	 * This method is used to register the Java Script Objects which are defined in
	 * the scope of source ResultSet ( might be IResultSet or CubeCursor ) to target
	 * scope. One possible client of this method is Report Engine. A classic use
	 * case is that instead of register its own "row" object the Report Engine can
	...
	public static void registerDataObject(ScriptContext context, ILinkedResult source) throws AdapterException {
		try {
			Scriptable targetScope = ((IDataScriptEngine) context.getScriptEngine(IDataScriptEngine.ENGINE_NAME))
					.getJSScope(context);
			int type = ((ILinkedResult) source).getCurrentResultType();
			if (type == ILinkedResult.TYPE_TABLE) {
				targetScope.put("row", targetScope, new JSResultIteratorObject((ILinkedResult) source, targetScope));
				targetScope.put("data", targetScope, new JSResultIteratorObject((ILinkedResult) source, targetScope));
	...
	public static void unRegisterDataObject(ScriptContext context) throws AdapterException {
		...
		try {
			if (context != null) {
				targetScope = ((IDataScriptEngine) context.getScriptEngine(IDataScriptEngine.ENGINE_NAME))
						.getJSScope(context);
	 ...
	 * @return Data Engine data types compatible with <code>type</code>, including
	 *         <code>type</code> itself
	 * @throws exception if <code>DataType.ANY_TYPE</code>,
	 *                   <code>DataType.UNKNOWN_TYPE</code> or other unsupported int
	...
	public static int[] getCompatibleDataTypes(int type) throws AdapterException {
		...
		case DataType.STRING_TYPE:
			return new int[] { DataType.STRING_TYPE, DataType.BOOLEAN_TYPE, DataType.DECIMAL_TYPE, DataType.DOUBLE_TYPE,
					DataType.INTEGER_TYPE, DataType.DATE_TYPE, DataType.BLOB_TYPE, DataType.BINARY_TYPE,
		...
		case DataType.JAVA_OBJECT_TYPE:
			return new int[] { DataType.JAVA_OBJECT_TYPE, DataType.STRING_TYPE, DataType.BOOLEAN_TYPE,
					DataType.DECIMAL_TYPE, DataType.DOUBLE_TYPE, DataType.INTEGER_TYPE, DataType.DATE_TYPE,
					DataType.BLOB_TYPE, DataType.SQL_DATE_TYPE, DataType.SQL_TIME_TYPE };
		default:
	...
	public static int adaptModelSortDirection(String modelSortDir) {
		return DesignChoiceConstants.SORT_DIRECTION_DESC.equals(modelSortDir) ? ISortDefinition.SORT_DESC
	...
	public static String adaptArgumentName(String name) {
		return "Data Field".equals(name) ? org.eclipse.birt.data.aggregation.impl.Constants.EXPRESSION_NAME : name;
	}
			...
			if (rdHandle != null) {
				...
				if (values.size() > 0) {
					if (parameterHandle.getParamType().equals(DesignChoiceConstants.SCALAR_PARAM_TYPE_SIMPLE)) {
						try {
							return DataTypeUtil.convert(values.get(0),
					...
					try {
						...
						for (int i = 0; i < reValues.length; i++) {
							reValues[i] = DataTypeUtil.convert(values.get(i),
									DataAdapterUtil.modelDataTypeToCoreDataType(parameterHandle.getDataType()));
						}
						return reValues;
					} catch (BirtException e) {
