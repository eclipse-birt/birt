...
class MinuteGroupCalculator extends DateGroupCalculator {
	...
	public Object calculate(Object value) {
		...
		if (intervalStart == null) {
			return new Double(
					Math.floor(this.dateTimeUtil.diffMinute(defaultStart, (Date) value) / getDateIntervalRange()));
		} else if (this.dateTimeUtil.diffMinute((Date) intervalStart, (Date) value) < 0) {
			return new Double(-1);
		} else {
			return new Double(Math
					.floor(this.dateTimeUtil.diffMinute((Date) intervalStart, (Date) value) / getDateIntervalRange()));
		}
	}
}
