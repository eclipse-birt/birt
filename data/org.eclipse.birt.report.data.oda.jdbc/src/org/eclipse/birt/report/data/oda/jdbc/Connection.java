...
public class Connection implements IConnection {
	...
	public void open(Properties connProperties) throws OdaException {
		if (this.appContext != null) {
			...
			if (value != null && (value instanceof java.sql.Connection)) {
				...
				logger.logp(Level.FINER, Connection.class.getName(), "open", jdbcConn.toString());
				return;
			}
		}
		if (connProperties == null) {
			IllegalArgumentException e = new IllegalArgumentException("connProperties cannot be null");
			logger.logp(java.util.logging.Level.FINE, Connection.class.getName(), "open", e.getMessage(), e);
			throw e;
		}
		...
		if (logger.isLoggable(Level.FINE)) {
			...
			for (Enumeration enumeration = connProperties.propertyNames(); enumeration.hasMoreElements();) {
				...
				logMsg.append(propName).append("=").append(propVal).append(";"); //$NON-NLS-1$//$NON-NLS-2$
			}
			logger.logp(Level.FINE, Connection.class.getName(), "open", //$NON-NLS-1$
					logMsg.toString());
		}
		...
		if (dataSource != null) {
			...
			logger.logp(java.util.logging.Level.FINE, Connection.class.getName(), "open", //$NON-NLS-1$
					e.getMessage(), e);
			throw e;
		} else {
	...
	protected void connectByUrl(String url, Properties connProperties) throws OdaException {
		...
		try {
			if ((jndiNameUrl == null || jndiNameUrl.trim().length() == 0)
				...
				jdbcConn = ConnectionPoolFactory.getInstance().getConnection(driverClass, url, props,
						getDriverClassPath(), this.appContext);
			...
			if (e instanceof SQLException) {
				...
				logger.log(Level.SEVERE, "JDBC connection throws exception: Error Code " + e1.getErrorCode()
		...
		try {
			...
			if (jdbcConn == null) {
				jdbcConn = JDBCDriverManager.getInstance().getConnection(driverClass, url, jndiNameUrl, props,
						getDriverClassPath());
		...
		try {
			...
			if (jdbcConn.isClosed() == false) {
				...
				if (!jdbcConn.getAutoCommit() && DBConfig.getInstance()
						.qualifyPolicy(jdbcConn.getMetaData().getDriverName(), DBConfig.SET_COMMIT_TO_FALSE)) {
				...
				logger.log(Level.FINER,
						"JDBC connection: " + jdbcConn + " is already closed outside of JDBC ODA driver");
			}
		} catch (SQLException e) {
			try {
				if (DBConfig.getInstance().qualifyPolicy(jdbcConn.getMetaData().getDriverName(),
	...
	private Properties bidiTransform(Properties connectionProperties) {
		...
		for (Enumeration enumeration = connectionProperties.propertyNames(); enumeration.hasMoreElements();) {
			...
			if ((Constants.ODAUser.equals(propName) || Constants.ODAPassword.equals(propName)) && propValue != null) {
				p.put(propName, BidiTransform.transform(propValue, BidiConstants.DEFAULT_BIDI_FORMAT_STR,
						metadataBidiFormatStr));
			} else if (Constants.ODAURL.equals(propName)) {
				p.put(propName, BidiTransform.transformURL(propValue, BidiConstants.DEFAULT_BIDI_FORMAT_STR,
						metadataBidiFormatStr));
			} else {
