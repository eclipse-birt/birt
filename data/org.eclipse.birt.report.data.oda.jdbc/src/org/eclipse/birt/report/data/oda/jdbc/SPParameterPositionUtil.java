...
public final class SPParameterPositionUtil {
	...
	private void parseProcedureName(String sqlTxt, int[] point) throws JDBCException {
		...
		// Split name into period-separated elements. Note that quotes can be used to
		// enclose an element that contains periods.
		// E.g.,
		// schema.package.function ==> ["schema", "package", "function"]
		// schema."package.function" ==> ["schema", "package.function"]
		// "schema"."package"."function" ==> ["schema", "package", "function"]
		// Some drivers such as DataDirect for Oracle handles packages by giving us
		// composite name such as "package.proc"
		// as a stored proc name. In order to us to correctly parse the SQL, such
		// composite names must always be identity quoted.

		String idq = Pattern.quote(identifierQuoteString);
		// To explain the following pattern: the group matches either of these:
		// (a) a unquoted name (doesn't start with quote, and greedily matches all
		// characters until the next period)
		// (b) a quoted name (starts with a quote, then reluctantly matches all
		...
		while (matcher.find()) {
			...
			if (elem.startsWith(identifierQuoteString) && elem.endsWith(identifierQuoteString)) {
				...
				elem = elem.substring(identifierQuoteString.length(), elem.length() - identifierQuoteString.length());
