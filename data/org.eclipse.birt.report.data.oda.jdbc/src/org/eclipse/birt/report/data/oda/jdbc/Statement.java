...
public class Statement implements IQuery {
	...
	public void prepare(String command) throws OdaException {
		...
		try {
			if (command == null) {
				logger.logp(java.util.logging.Level.FINE, Statement.class.getName(), "prepare",
			...
			/*
			 * call the JDBC Connection.prepareStatement(String) method to get the
			 * preparedStatement
			 */
		...
		if (name.equals("queryTimeOut")) {
			...
			try {
				if (value != null && value.length() > 0) {
					...
					// Default Set to 10000. This will slowdown MsSql,MySql,Postgress performance
					// while enhance Oracle performance.
			...
			logger.logp(java.util.logging.Level.WARNING, Statement.class.getName(), "setProperty",
	...
	private void getMetaUsingPolicy0() {
		...
		try {
			/* redirect the call to JDBC preparedStatement.getMetaData() */
			resultmd = preStat.getMetaData();
			// Even in policy 1 some driver cannot guarantee to return the metadata. Say,
	...
	/*
	 * TODO: used by junit tests only; to be removed after update of tests
	 */
	public boolean execute() throws OdaException {
		logger.logp(java.util.logging.Level.FINE, Statement.class.getName(), "execute", "Statement.execute( )");
		assertNotNull(preStat);
		try {
			if (maxrows >= 0 && !maxRowsUpToDate) {
				preStat.setMaxRows(maxrows);
	...
	public void setBigDecimal(int parameterId, BigDecimal value) throws OdaException {
		...
		try {
			/*
			 * redirect the call to JDBC preparedStatement.setBigDecimal(int,BigDecimal)
			 */
	...
	public void setTimestamp(int parameterId, Timestamp value) throws OdaException {
		...
		try {
			/*
			 * redirect the call to JDBC preparedStatement.setTimestamp(int,Timestamp)
			 */
	...
	public SortSpec getSortSpec() throws OdaException {
		/* not supported */
		UnsupportedOperationException e = new UnsupportedOperationException("setSortSpec is not supported.");
		logger.logp(java.util.logging.Level.FINEST, Statement.class.getName(), "getSortSpec",
	...
	private static void rethrowRunTimeException(RuntimeException e, String msg) throws OdaException {
		...
		logger.logp(java.util.logging.Level.FINEST, Statement.class.getName(), "rethrowRunTimeException", msg,
				odaException);
		throw odaException;
	}
	...
	private void addLog(String methodName, Exception e) {
		logger.logp(java.util.logging.Level.FINE, Statement.class.getName(), methodName,
				"No named Parameter supported.", e);
	}
	...
	public int executeUpdate() throws OdaException {
		...
		logger.logp(java.util.logging.Level.FINER, Statement.class.getName(), "executeUpdate",
