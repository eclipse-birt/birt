...
public abstract class BidiSQLTransform implements ISQLSyntax
{
	...
	static class ImplementedRule
	{
		...
		ImplementedRule( String text, RuleData data )
		{
			...
			this.matcher = Pattern.compile( data.regex, data.flags ).matcher(
					text );
			getNextToken( 0 );
		}
	...
	private static final RuleData[] RULE_DATA = new RuleData[]{
			new RuleData( "'.+?'", ACTION.TRANSFORM_LITERAL, 0 ), //$NON-NLS-1$
	 ...
	 * 
	 * @param keywords
	 *            Variable-length arrays list to merge
	 */
	private static void mergeKeywords( String[]... keywords )
	{
		if ( keywords != null )
		{
			...
			for ( i = 0, size = sizes.length; i < size; i++ )
			{
				if ( sizes[i] > 0 )
				{
					System.arraycopy( keywords[i], 0, ALL_KEYWORDS, offset,
							sizes[i] );
					offset += sizes[i];
				}
			}
	...
	private static ImplementedRule getBestMatch( List<ImplementedRule> rules,
	 ...
	 * @param inContentFormat
	 *            Input content Bidi format
	 * @param outContentFormat
	 *            Input content Bidi format
	 * @param inMetadataFormat
	 *            Input metadata Bidi format
	 * @param outMetadataFormat
	 *            Input metadata Bidi format
	 * 
	 * @return Transformed query string
	 */

	public static String transform( String sql, String inContentFormat,
			String outContentFormat, String inMetadataFormat,
			String outMetadataFormat )
	{
		if ( !BidiFormat.isValidBidiFormat( inContentFormat )
				|| !BidiFormat.isValidBidiFormat( outContentFormat )
				|| !BidiFormat.isValidBidiFormat( inMetadataFormat )
				|| !BidiFormat.isValidBidiFormat( outMetadataFormat )
				|| inContentFormat.equals( outContentFormat )
				&& inMetadataFormat.equals( outMetadataFormat ) )
		{
			return sql;
		}
		int offset = 0;
		...
		while ( offset < length )
		{
			...
			if ( rule.action == ACTION.TRANSFORM_ID
					|| rule.action == ACTION.CHECK && !isKeyword( token ) )
			{
				buf.append( BidiTransform.transform( token, inMetadataFormat,
						outMetadataFormat ) );
			}
			else if ( rule.action == ACTION.TRANSFORM_LITERAL )
			{
				buf.append( BidiTransform.transform( token, inContentFormat,
						outContentFormat ) );
			}
			else
				buf.append( token );
