...
public class JDBCDriverManager {
	...
	/**
	 * Implementation of getConnection() methods. Gets connection from either
	 * java.sql.DriverManager, or from IConnectionFactory defined in the extension
	 */
	private Connection doConnect(String driverClass, String url, String jndiNameUrl, Properties connectionProperties,
			Collection<String> driverClassPath) throws SQLException, OdaException {
	...
	/**
	 * Obtain a JDBC connection from a Data Source connection factory via the
	 * specified JNDI name service. May return null if no JNDI Name URL is
	...
	/**
	 * Adds the specified user name and password to the specified collection as Jdbc
		...
		synchronized (driverExtensions) {
			...
			if (driverInfo != null) {
				...
				if (driverInfo instanceof IConfigurationElement) {
					...
					String factoryClass = ((IConfigurationElement) driverInfo)
							.getAttribute(OdaJdbcDriver.Constants.DRIVER_INFO_ATTR_CONNFACTORY);
					try {
						factory = (IConnectionFactory) ((IConfigurationElement) driverInfo)
								.createExecutableExtension(OdaJdbcDriver.Constants.DRIVER_INFO_ATTR_CONNFACTORY);

						logger.fine("Created connection factory class " //$NON-NLS-1$
								+ factoryClass + " for driverClass " //$NON-NLS-1$
								+ driverClass);
					} catch (CoreException e) {
						JDBCException ex = new JDBCException(ResourceConstants.CANNOT_INSTANTIATE_FACTORY, null,
								new Object[] { factoryClass, driverClass });
						logger.log(Level.WARNING, "Failed to instantiate connection factory for driverClass " //$NON-NLS-1$
								+ driverClass, ex);
						throw ex;
					}
					assert (factory != null);
	...
	private void loadDriverExtensions() {
		...
		synchronized (this) {
			...
			/*
			 * getConfigurationElementsFor is not working for server Platform. I have to
			 * work around this by walking the extension list IConfigurationElement[]
			 * configElems = extReg.getConfigurationElementsFor(
			...
			for (int e = 0; e < exts.length; e++) {
				...
				for (int i = 0; i < configElems.length; i++) {
					if (configElems[i].getName().equals(OdaJdbcDriver.Constants.DRIVER_INFO_ELEM_JDBCDRIVER)) {
						String driverClass = configElems[i]
								.getAttribute(OdaJdbcDriver.Constants.DRIVER_INFO_ATTR_DRIVERCLASS);
						String connectionFactory = configElems[i]
								.getAttribute(OdaJdbcDriver.Constants.DRIVER_INFO_ATTR_CONNFACTORY);
						...
						if (driverClass != null && driverClass.length() > 0 && connectionFactory != null
	...
	public boolean testConnection(String driverClassName, String connectionString, String jndiNameUrl, String userId,
		...
		try {
			// If connection is provided by driverInfo extension, use it to create
			// connection
			...
			if (jndiNameUrl != null) {
				Connection jndiDSConnection = getJndiDSConnection(driverClassName, jndiNameUrl,
	 ...
	 * @throws OdaException
	 */
	private void tryCreateConnection(String driverClassName, String connectionString, String userId, String password,
			Properties props) throws SQLException, OdaException {
		...
		try {
			...
			if (logger.isLoggable(Level.FINE)) {
				logger.info("Driver class not in class path: " //$NON-NLS-1$
						+ className + ". Trying to locate driver in drivers directory"); //$NON-NLS-1$
			}
	...
	public boolean deregisterDriver(String className) throws OdaException {
		...
		if (driver != null) {
			try {
				...
				logger.log(Level.WARNING, "Failed to deRegister wrapped driver instance.", e); //$NON-NLS-1$
			}
		}
		return true;
	}
	...
	/**
	 * If driver is found in the drivers directory, its class is not accessible in
	...
	private void registerDriver(Driver driver, String className) {
		...
		try {
			...
			// This shouldn't happen
			logger.log(Level.WARNING, "Failed to register wrapped driver instance from DriverManager.", e); //$NON-NLS-1$
		}
	}
	...
	private Class<?> loadExtraDriver(String className, boolean refreshUrlsWhenFail, boolean refreshClassLoader,
		...
		try {
			...
			// re-scan the driver directory. This re-scan is added for users would
			// potentially
