...
public class QueryModel
{
	...
	private static final String[] REQUIRED_MAPREDUCE_KEYS = new String[]{MAP_REDUCE_MAP_FUNCTION, MAP_REDUCE_REDUCE_FUNCTION, "out"}; //$NON-NLS-1$ //$NON-NLS-2$
																																		// //$NON-NLS-3$
		...
		for ( BasicDBObject groupOp : groupOps )
		{
			if ( !groupOp.containsField( DOC_ID_FIELD_NAME ) )
				throw new OdaException(
						Messages.bind( Messages.queryModel_missingGroupAggrKey, new Object[]{GROUP_AGGR_KEY, DOC_ID_FIELD_NAME, groupOp} ) );
		}
