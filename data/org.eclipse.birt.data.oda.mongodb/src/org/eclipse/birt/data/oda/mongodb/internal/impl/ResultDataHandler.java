...
public class ResultDataHandler {
	...
	private Map<String, FieldMetaData> m_intermediateFieldMDs; // quick map to optimize lookup of metadata for
																// intermediate level fields

	// cached values for each current top-level document
	private Map<String, ArrayFieldValues> m_nestedValues; // key is top-level row, and field name of each level of
															// nested collection being flattened
	private Map<String, Document> m_currentContainingDocs; // key is top-level row, and name of a result set field;
															// mapped to the current parent document that holds its
															// value
	...
	private FieldMetaData getFieldMetaData(String fieldName) {
		// result set metadata contains metadata for only the leaf fields defined in
		// result set
	...
	/*
	 * Fetches and returns the value of the named field from a top-level document in
	 * a db collection. This method flattens array(s) of sub-documents, if exists
	 * and designated in the metadata, and projects the sub-documents in an array
	 * into multiple ODA result set rows.
	 */
	public Object getFieldValue(String fieldName, Document currentRow) throws OdaException {
		...
		if (containerDoc == null) // no nested parent container doc
		{
			...
			// extract all values directly from current top-level document, if not already
			// cached
	...
	private Object getSubFieldValues(String fieldFullName, FieldMetaData fieldMD, Document documentObj) {
		...
		for (int i = fieldLevelNames.length - 2; i >= 0; i--) {
			// check the lowest ancestor level that is a flattenable nested collection
			// field, i.e.
			...
			if (isFlattenableLevelField(ancestorFullName)) {
				// strip the flattenable ancestor segments from the field full name to identify
				// the lower field level(s);
