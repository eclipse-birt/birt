...
public class ResultDataHandler
{
			...
			throws OdaException
	{
		...
		// documents
		Document containerDoc = getContainerDocument( fieldName,
				fieldMD,
				currentRow );
			...
			FieldMetaData fieldMD, Document documentObj )
	{
		...
		Document containingDoc = doGetContainerDocument( fieldFullName,
				fieldMD,
				documentObj,
				null );
			...
			FieldMetaData fieldMD, Document documentObj, String priorLevelName )
	{
		...
		if ( !m_currentContainingDocs.containsKey( levelFullName ) )
		{
			...
			if ( getLogger( ).isLoggable( Level.FINEST ) )
			{
				...
				getLogger( ).finest( Messages.bind(
						...
						new Object[]{
								firstLevelName, levelFullName, firstLevelMD
						} ) );
			}
			...
			if ( !firstLevelValues.hasContainerDocs( ) )
			{
				...
				Object firstLevelDocs = value != null
						? fetchFieldDocument( value ) : NULL_VALUE_FIELD;
		...
		String childFullName = MDbMetaData.stripParentName( fieldFullName,
				firstLevelName );
		return doGetContainerDocument( childFullName,
				null,
				currentContainerDoc,
				levelFullName );
	}
			...
			FieldMetaData fieldMD, Document documentObj )
	{
		...
		if ( fieldLevelNames.length == 1 )
		{
			String fieldSimpleName = fieldMD != null ? fieldMD.getSimpleName( )
					: fieldLevelNames[0];
