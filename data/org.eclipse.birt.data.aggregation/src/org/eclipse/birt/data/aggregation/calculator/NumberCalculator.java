...
/**
 * Calculator primarily for type Double. Note that all operands are expected to
 * be converted to Double before invoking any operation. Use getTypedObject()
 * method to convert operands to the desired datatype. Nulls are ignored in
 * calculations. NaN and Infinity are supported: if one of the operands is NaN
 * Then the result is NaN as well. If you need to override this behavior do so
 * in the respective aggregate function implementation.
 */
...
public class NumberCalculator implements ICalculator {
	...
	protected boolean isNaNorInfinity(Object a) {
		return (a instanceof Double && (((Double) a).isInfinite() || ((Double) a).isNaN())
				|| a instanceof Float && (((Float) a).isInfinite() || ((Float) a).isNaN()));
	}
}
