...
public class MongoDBDataSourcePageHelper
{
	...
	private Button URIElementsRadioBtn, URIRadioBtn, socketAliveCheckbox,
			useKerberosAuthenticationCheckBox, kerberosConfigurationRadioBtn, kerberosCredentialsRadioBtn;
	...
	private Text serverHostText, serverPortText, databaseNameText,
			...
			gssapiServiceNameText, kerberosConfigurationFileText, gssAPIConfigurationFileText, 
	...
	private String dbURI, serverHost, serverPort, dbName, userName, password, 
			gssapiServiceName, kerberosConfigurationFile, gssAPIConfigurationFile, 
			kerberosPrincipal, kerberosPassword;
	...
	private void createKerberosSettingsArea( Composite composite )
	{
		...
		kerberosConfigurationRadioBtn = new Button( kerberosSettingComposite3, SWT.RADIO );
		...
		Composite kerberosSettingComposite4 = new Composite( kerberosSettingsGroup, SWT.NONE );
		...
		/*
		kerberosCredentialsElementGroup = new Group( kerberosSettingComposite4, SWT.NONE );
		kerberosCredentialsElementGroup.setText(Messages.getString( "MongoDBDataSourceWizardPage.RadioButton.label.KerberosCredentials" )); //$NON-NLS-1$
		kerberosCredentialsElementGroup.setLayout( new GridLayout( 2, false ) );
		GridData credentialsGroupGridData = new GridData( GridData.FILL_HORIZONTAL );
		kerberosCredentialsElementGroup.setLayoutData( credentialsGroupGridData );

		Label kerberosPasswordLabel = new Label( kerberosCredentialsElementGroup, SWT.NONE );
		kerberosPasswordLabel.setText( Messages.getString( "MongoDBDataSourceWizardPage.text.label.KerberosPassword" ) ); //$NON-NLS-1$
		*/

		// Kerberos password
		kerberosPasswordText = new Text( kerberosSettingComposite4, SWT.BORDER );
		kerberosPasswordText.setLayoutData( new GridData( GridData.FILL_HORIZONTAL ) );
	...
	protected void refreshPageControls( )
	{
		...
		if (useKerberosAuthentication) {
			...
			kerberosPasswordText.setText( kerberosPassword == null ? EMPTY_STRING : kerberosPassword );
		}
		handleKerberosAuthenticationSelection();
	...
	private boolean validatePageProperties( )
	{
		...
		// Kerberos validation
		// Principal is required and if credentials is selected, password is required.
		if (isValid && useKerberosAuthentication) {
			if (kerberosPrincipalText.getText().trim().length() == 0) {
				isValid = false;
			} else if (isKerberosPasswordFieldFoucs && kerberosPasswordText.getText().trim().length() == 0) {
				isValid = false;
			}
		}
