...
public class RDLoad {
	...
	RDLoad(String tempDir, DataEngineContext context, QueryResultInfo queryResultInfo) throws DataException {
		...
		if (!isEmptyQueryResultID(this.queryResultInfo.getSelfQueryResultID())) {
			subQueryUtil = new RDSubQueryUtil(context, QueryResultIDUtil
					.getRealStreamID(queryResultInfo.getRootQueryResultID(), queryResultInfo.getSelfQueryResultID()),
					queryResultInfo.getSubQueryName());
		}

		streamManager = new StreamManager(context,
				new QueryResultInfo(queryResultInfo.getRootQueryResultID(), queryResultInfo.getParentQueryResultID(),
						queryResultInfo.getSelfQueryResultID(), queryResultInfo.getSubQueryName(),
						queryResultInfo.getSubQueryName() == null ? 0 : getSubQueryIndex(queryResultInfo.getIndex())));
	...
	public IExprDataResultSet loadExprDataResultSet(boolean isSummary) throws DataException {
		...
		if (streamManager.isBasedOnSecondRD() == false) {
			...
			exprDataResultSet = new ExprDataResultSet2(tempDir,
					streamManager.getInStream(DataEngineContext.EXPR_VALUE_STREAM, StreamManager.ROOT_STREAM,
							StreamManager.BASE_SCOPE),
					streamManager.getInStream(DataEngineContext.EXPR_ROWLEN_STREAM, StreamManager.ROOT_STREAM,
							StreamManager.BASE_SCOPE),
					streamManager.getInStream(DataEngineContext.ROW_INDEX_STREAM, StreamManager.ROOT_STREAM,
	...
	public IDataSetResultSet loadDataSetData(IOrderedIntSet preFilteredRowIds, Map<String, StringTable> stringTableMap,
			Map index) throws DataException {
	 ...
	 */
	public IDataSetResultSet loadDataSetData(IOrderedIntSet preFilteredRowIds, Map<String, StringTable> stringTableMap,
			Map index, Map appContext) throws DataException {
		return loadDataSetData(preFilteredRowIds, stringTableMap, index, true, appContext);
	}
	 ...
	 */
	public IDataSetResultSet loadDataSetData(IOrderedIntSet preFilteredRowIds, Map<String, StringTable> stringTableMap,
			Map index, boolean includeInnerID, Map appContext) throws DataException {
		return loadDataSetData(null, preFilteredRowIds, stringTableMap, index, includeInnerID, appContext);
	}

	public IDataSetResultSet loadDataSetData(IResultClass targetResultClass, IOrderedIntSet preFilteredRowIds,
			Map<String, StringTable> stringTableMap, Map index, boolean includeInnerID, Map appContext)
			throws DataException {
		...
		return new DataSetResultSet(stream, lensStream, targetResultClass, preFilteredRowIds, stringTableMap, index,
				adjustedVersion, includeInnerID, PLSUtil.isRowIdSaved(streamManager));
	}
	...
	public IBaseQueryDefinition loadOriginalQueryDefn(int streamPos, int streamScope) throws DataException {
		InputStream inputStream = streamManager.getInStream(DataEngineContext.ORIGINAL_QUERY_DEFN_STREAM, streamPos,
				streamScope);
	...
	public IBaseQueryDefinition loadQueryDefn(int streamPos, int streamScope) throws DataException {
		InputStream inputStream = streamManager.getInStream(DataEngineContext.QUERY_DEFN_STREAM, streamPos,
				streamScope);
		return loadQueryDefn(inputStream);
	}
	...
	public boolean isEmptyQueryResultID(String queryResultID) throws DataException {
		if (!this.context.hasInStream("DataEngine", null, DataEngineContext.EMPTY_NESTED_QUERY_ID)) {
			return false;
		}
		DataInputStream emptyQueryResultStream = new DataInputStream(
				context.getInputStream("DataEngine", null, DataEngineContext.EMPTY_NESTED_QUERY_ID));
	...
	public IBaseQueryDefinition loadQueryDefn(int streamType, int streamPos, int streamScope, String subname)
			throws DataException {
		InputStream inputStream = streamManager.getInStream(streamType, streamPos, streamScope, subname);
		return loadQueryDefn(inputStream);
	}
}
