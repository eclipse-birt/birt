...
/**
 * A report query's results opened and ready for data retrieval. A query results
...
public class QueryResults implements IQueryResults, IQueryService {
	...
	public IResultIterator getResultIterator() throws DataException {
			...
			if (iterator == null) {
				...
				if (logger.isLoggable(Level.FINE)) {
					String dataSetName = queryService.getDataSetRuntime() != null
							? queryService.getDataSetRuntime().getName()
							: null;
					logger.log(Level.FINE, "ODI query execution time: " + (endTime - startTime)
							+ " ms;\n   Executed data set: " + dataSetName);
				}

				if (queryService.getQueryDefn() instanceof IQueryDefinition
						&& !((QueryDefinition) queryService.getQueryDefn()).isTempQuery()) {
		...
		logger.logp(Level.FINER, QueryResults.class.getName(), "getResultIterator", "finished");
		return iterator;
	}
	...
	private boolean isDummyQuery(org.eclipse.birt.data.engine.odi.IResultIterator odiIterator) throws DataException {
		return queryService.getQueryDefn() instanceof IQueryDefinition
				...
				&& (odiIterator.getResultClass().getFieldCount() == 0
						|| (odiIterator.getResultClass().getFieldCount() == 1
								&& odiIterator.getResultClass().getFieldName(1).equals(ExprMetaUtil.POS_NAME)));
	}
	...
	private class DummyResultIterator extends ResultIterator {
		DummyResultIterator(IServiceForResultSet rService, org.eclipse.birt.data.engine.odi.IResultIterator odiResult,
		...
		public IResultIterator getSecondaryIterator(String subQueryName, Scriptable subScope) throws DataException {
			...
			PreparedDummyQuery preparedQuery = new PreparedDummyQuery(
					((ISubqueryDefinition) subQueryMap.get(subQueryName)), session);
	...
	private static class ResultService implements IServiceForResultSet {
		...
		/*
		 * @see
		 * org.eclipse.birt.data.engine.impl.IResultService#execSubquery(org.eclipse.
