...
/**
 *	A parameter meta-data interface that defines the metadata 
 *	of a IPreparedDSQuery's parameter, which can be of either
 *  input only, output only, or input and output modes.
 *	<p> 
 *  The metadata is defined based on a query's runtime metadata
 *  (as described by its underlying data source driver),
 *  merging with static input and output parameter hints 
 *  specified in a IDataSourceQuery.
 */
