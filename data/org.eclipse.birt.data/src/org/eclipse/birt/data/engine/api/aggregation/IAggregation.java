...
public interface IAggregation
{
    ...
    /**
     * Gets the type of the Aggregation. The aggregation can be SUMMARY_AGGR, meaning that only one value 
     * is calculated for the whole series of data rows (e.g., the Total.SUM function). Or it can be
     * RUNNING_AGGR, meaning that a value is calculated for each row in the series (e.g., the movingAve function).  
     ...
     * <p> 
     * The length of the returned array is the number of runtime parameters that this aggregate function
     * takes. Note that this number excludes the optional filter and group parameters common to all aggregate
     * functions. Those two parameters are handled by the DtE. <p>
     * If the n'th element in the array is true, it means that the n'th parameter is a dynamic parameter 
     * which needs to be evaluated at each row. Otherwise the parameter is static, and it only needs to be
     * evaluated once at the start of the accumulation <p>
