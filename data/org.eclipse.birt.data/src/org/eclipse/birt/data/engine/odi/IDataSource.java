 ...
 *	data source connection and corresponding connection properties.
 *  <br>
 *	Its instances are created by an IDataSourceFactory.
 *  <br>
 *	It is used to create associated IQuery instances that can be 
 *  executed to retrieve and transform result data.  
 *  <p>
 *  The knowledge of establishing an active connection to a data source
 *  is transparent to the ODI consumer.  An implementation might support
 *  using a pooled connection to a data source.  
 */
public interface IDataSource
{
     ...
     * <p>
     * A query text could be a native command text supported 
     * by the associated data source, 
     * such as a SQL select statement. <br>
     * Such direct query text specification could have
     * embedded parameters.  
     * @param queryType	The type of query supported by the
     * 					data source driver. 
     * @param queryText	The query text to be prepared
     * 					and executed by the data source driver.
	 ...
	 * Optional call.  Its call is implied when any associated query
	 * needs to access the underlying data source driver.
	 * If the data source is already opened, returns with no error.
