...
public class DataSourceQuery extends BaseQuery implements IDataSourceQuery, IPreparedDSQuery {
	...
	public IPreparedDSQuery prepare() throws DataException {
		...
		// for some jdbc driver need to carry out a query execution before the metadata
		// can be achieved
		// and only when the Parameters are successfully set the query execution can
		// succeed.
		addParameterDefns();

		// Here the "max rows" means the max number of rows that can fetch from data
		// source.
	...
	private Object getParameterInputValue(ParameterHint parameterHint) throws DataException {
		...
		if (inputValue != null) {
			if (inputValue.getClass().isArray() && !(inputValue instanceof byte[])) {
				...
				// only take the first provided value to pass it to data set, except for byte[]
				// Object
	...
	private IResultClass copyResultClass(IResultClass meta) throws DataException {
		...
		for (int i = 1; i <= meta.getFieldCount(); i++) {
			if (!meta.getFieldName(i).equals(ExprMetaUtil.POS_NAME)) {
				...
				ResultFieldMetadata metadata = new ResultFieldMetadata(m_driverPosition, m_name, m_label, m_dataType,
						m_nativeTypeName, m_isCustom, m_analysisType, m_analysisColumn, m_indexColumn,
						m_compressedColumn);
	...
	private static class OdaQueryCanceller implements ICancellable {
		...
		public boolean equals(Object object) {
			...
			OdaQueryCanceller that = (OdaQueryCanceller) object;
	...
	/**
	 * In ODA layer, it references the index positions of input parameters and
	 * output parameters in a single sequential list. However, in ODI layer, we need
	 * to clearly distinguish them since only retrieving output parameter is
	...
	private static Object convertToValue(Object inputValue, Class typeClass) throws DataException {
		try {
			...
			throw new DataException(ResourceConstants.CANNOT_CONVERT_PARAMETER_TYPE, ex,
