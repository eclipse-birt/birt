...
public class DimensionResultIterator implements IDimensionResultIterator {
	...
	public DimensionResultIterator(Dimension dimension, IDiskArray dimensionPosition, StopSign stopSign)
			throws IOException {
		Object[] params = { dimension, dimensionPosition };
...
//			if( dimension.length( ) < Constants.MAX_DIMENSION_LENGTH )
	...
	public Object getLevelAttribute(int levelIndex, int attributeIndex) throws IOException {
		...
		initDimensionRows();
		return ((DimensionRow) dimensionRows.get(currentPosition)).getMembers()[levelIndex]
				.getAttributes()[attributeIndex];
	}
	...
	public Object[] getLevelKeyValue(int levelIndex) throws IOException {
		...
		if (dimension.isTime()) {
			...
			return ((DimensionRow) dimensionRows.get(currentPosition)).getMembers()[levelIndex].getKeyValues();
		}
	}
	...
	private Date getCurrentTimeValue() throws IOException {
		Date timeValue = (Date) (((DimensionRow) dimensionRows.get(currentPosition)).getMembers()[0].getKeyValues()[0]);
		return timeValue;
	}
