...
/**
 * A type of IQuery that allows applications to obtain persistent instances 
 * from an underlying data source, and supports further data transforms. 
 * <p>
 * An IDataSourceQuery has 2 main elements: 
 * <br>
 * <ul>
 ...
 * </ul>
 * <p>
 * Other optional elements are supported.  
 * <br>
 * Interface methods are provided to bind the optional elements 
 * prior to execution.
 * Optional elements include: 
 * <br>
 * <ul>
 * <li>result specification to project the fields that
...
public interface IDataSourceQuery extends IQuery
{
	 ...
	 * when this query was instantiated by IDataSource.newQuery.
     ...
     * to provide hints for every expected result field.
     * Any field hints provided will be applied only 
     * if the underlying data provider is not able to 
     * provide runtime metadata.  
     * @param columnDefns	One or more field/column hints as a collection
     * 				of ColumnHint objects.
     */
	 ...
	 * should return.  The result specification is made up of one or more
	 * result field names.
	 * <p>
	 * If setResultProjection() is not called, or is called with
	 * a null result specification,
	 * all result data elements retrieved by the query text are bound to the
	 * corresponding fields in the result objects.  Otherwise, only those 
	 * fields specified in setResultProjection() would be bound.
	 * Any remaining fields defined in the result hints would not be
	 * included in the result objects returned by the execute method.
	...
	/**
	 * Returns the result specification to project the fields
	 * to return in a query result object.
	 ...
	 * Its processing is implementation-dependent on the 
	 * underlying data source.
	 * Note: This should be called before prepare().
     ...
     * in the result set's IResultObject definition
 	 * specifying its name and type. <br>
	 * Must be declared before preparing the query. <br>
     * A declared custom field is automatically included in the projected result 
     * of an IResultObject instance. 
     * It contains null value, until an ODI consumer directly assigns a value to it.
     * @param fieldName	The name of the custom field; 
     * 					must be unique in an result instance.
     * @param dataType	The data type of the custom field; must be one defined
     * 					in birt.data.engine.api.DataType.
     * 					Its value can be ANY_TYPE if it is not known 
     * 					at this time.
     * @throws DataException	if the field cannot be added.
     */
    public void declareCustomField( String fieldName, int dataType )
    	throws DataException;

    /**
     * Prepares the query instance for subsequent execution.
     * This method requires the Query instance to validate any bound elements, 
     * such as the Result Projection against the underlying result metadata, 
     * and report any incompatibilities by throwing a chained OdiException.
     * @throws DataException	if query validation error(s) occur.
     */
    public IPreparedDSQuery prepare() throws DataException;
    
    /**
     * Class to hold hints for a result set field. For use with setResultHints method.
     */
