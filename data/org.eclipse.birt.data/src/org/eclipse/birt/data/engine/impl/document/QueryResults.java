...
public class QueryResults implements IQueryResults, IQueryService {
	 ...
	 * @param outer
	 */
	public QueryResults(String tempDir, DataEngineContext context, String queryResultID, IBaseQueryResults outer,
	...
	QueryResults(String tempDir, DataEngineContext context, String baseResultID, String queryResultID,
			IResultMetaData resultMetaData, String subQueryName, int currParentIndex,
			IBaseQueryResults parentQueryResults, List<IGroupInstanceInfo> targetGroups) {
		assert tempDir != null;
		assert context != null;
	...
	private IPreparedQuery populateDummyPreparedQuery() {
		...
		try {
			...
			QueryResultInfo queryResultInfo = new QueryResultInfo(rootQueryResultID, parentQueryResultID, null, null,
					-1);
		...
		if (resultIterator == null) {
			if (subQueryName == null) // not a sub query
			{
				IBaseQueryDefinition queryDefn = this.getRDLoad(null, queryResultID)
						.loadQueryDefn(StreamManager.ROOT_STREAM, StreamManager.SELF_SCOPE);
