...
public class DiskCache implements ResultSetCache {
	...
	/**
	 * The MemoryCacheRowCount indicates the upper limitation of how many rows can
	 * be loaded into memory. Note this value is included as well. Look at the start
	 * three parameters of the parameter list, the first is the result object array
	 * which length is MemoryCacheRowCount, and the second is one result object
	 * which follows the object array according to the position sequence of data
	 * source. The last is the RowResultSet, and it might have or not have more
	 * result object.
	 *
	 * @param resultObjects
	 ...
	 */
	public DiskCache(IResultObject[] resultObjects, IResultObject resultObject, IRowResultSet rowResultSet,
			IResultClass rsMeta, Comparator comparator, int MemoryCacheRowCount, int maxRows, DataEngineSession session)
			throws DataException {
		// this.rsMeta = rsMeta;
		...
		try {
			...
			diskBasedResultSet.processStartResultObjects(resultObjects, comparator);

			logger.info("Start processRestResultObjects");
			diskBasedResultSet.processRestResultObjects(resultObject, rowResultSet,
					maxRows == -1 ? -1 : maxRows - resultObjects.length);
		} catch (IOException e) {
			throw new DataException(ResourceConstants.WRITE_TEMPFILE_ERROR, e);
		}
		countOfResult = diskBasedResultSet.getCount();

		logger.info("End of process, and the count of data is " + countOfResult);
	}
	...
	/**
	 * @return infoMap, including below information tempDir, to generated temp file
	 *         in DiskMergeSort goalFile, to generate the end result file
	...
	/**
	 * @return temp directory string, this folder name is unique and then different
	 *         session will not influence each other, which can support multi-thread
	 * @throws DataException
	 */
	...
	public void incrementalUpdate(OutputStream outputStream, OutputStream rowLensStream, int originalRowCount,
		...
		try {
			...
			for (int i = 0; i < rowCount - originalRowCount; i++) {
				...
				IResultObject ro = this.diskBasedResultSet.nextRow();
				offset += ResultSetUtil.writeResultObject(dos, ro, colCount, resultSetNameSet, stringTable, map,
						i + originalRowCount, version);
				if (auxiliaryIndexCreators != null) {
					for (IAuxiliaryIndexCreator creator : auxiliaryIndexCreators) {
