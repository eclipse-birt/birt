...
class GroupInstanceFilter
{
	...
	/**
	 * Populate the Group filter expression list and its corresponding group
	 * level list that being used in IExpressionProcessor.calculate() method.
	 * 
	 * @return
	 */
	private void populateGroupFilteringExpressions( List expressionList,
			List groupLevels )
	{
	...
	/**
	 * Populate the "accept" fields in GroupBoundaryInfos. If one
	 * GroupBoundaryInfo would be filtered out then the "accept" field is set to
	 * false else ture.
	 * 
	 * @param cx
			...
			throws DataException
	{
		if ( i > 1 )
		{
			...
			for ( int k = 0; k < groupBoundaryInfos[i - 2].size( ); k++ )
			{
				...
				makeAGroupFilteringMultiPass( cx,
						i,
						groupedFilters,
						currentGroupArray, passedGroups );
			...
			throws DataException
	{
		for ( int j = 0; j < groupFilters.size( ); j++ )
		{
			Object result = evaluteFilterExpression( cx,
					(IFilterDefinition) ( groupFilters.get( j ) ) );
