...
public class ExprResultSet implements IExprResultSet {
	 ...
	 */
	public ExprResultSet(String tempDir, StreamManager streamManager, int version, boolean isBasedOnSecondRD,
			IDataSetResultSet dataSetResultSet, int rowIdStartingIndex, IBaseQueryDefinition qd) throws DataException {
	...
	protected void prepare() throws DataException {
		...
		if (version >= VersionManager.VERSION_2_5_1_0) {
			aggrStreams = streamManager.getInStreams(DataEngineContext.AGGR_VALUE_STREAM, StreamManager.ROOT_STREAM,
					StreamManager.SELF_SCOPE);
			if (streamManager.hasInStream(DataEngineContext.AGGR_INDEX_STREAM, StreamManager.ROOT_STREAM,
					StreamManager.SELF_SCOPE)) {
				this.aggrUtil = new RDAggrUtil(streamManager, qd);
			} else if ((!aggrStreams.isEmpty())
					|| streamManager.hasInStream(DataEngineContext.COMBINED_AGGR_VALUE_STREAM,
				...
				if (streamManager.hasInStream(DataEngineContext.COMBINED_AGGR_INDEX_STREAM, StreamManager.ROOT_STREAM,
						StreamManager.SELF_SCOPE)) {
					combinedAggrIndex = streamManager.getInStream(DataEngineContext.COMBINED_AGGR_INDEX_STREAM,
							StreamManager.ROOT_STREAM, StreamManager.SELF_SCOPE);
				}

				if (streamManager.hasInStream(DataEngineContext.COMBINED_AGGR_VALUE_STREAM, StreamManager.ROOT_STREAM,
						StreamManager.SELF_SCOPE)) {
					combinedAggrValue = streamManager.getInStream(DataEngineContext.COMBINED_AGGR_VALUE_STREAM,
							StreamManager.ROOT_STREAM, StreamManager.SELF_SCOPE);
				}

				this.aggrUtil = new ProgressiveViewingRDAggrUtil(combinedAggrIndex, combinedAggrValue, aggrIndexStreams,
						aggrStreams);
			}
		}
		if (!this.isBasedOnSecondRD) {
			...
			if (version > VersionManager.VERSION_2_0) {
				rowLenRAIs = streamManager.getInStream(DataEngineContext.EXPR_ROWLEN_STREAM, StreamManager.ROOT_STREAM,
						StreamManager.SELF_SCOPE);
			}

			this.exprResultReader = new ExprDataReader1(this.rowExprsRAIs, this.rowLenRAIs, this.version,
					(this.qd instanceof IQueryDefinition && ((IQueryDefinition) qd).isSummaryQuery()) ? null
							: this.dataSetResultSet);
			...
			rowLenRAIs = streamManager.getInStream(DataEngineContext.EXPR_ROWLEN_STREAM, StreamManager.ROOT_STREAM,
					StreamManager.BASE_SCOPE);
			rowInfoRAIs = streamManager.getInStream(DataEngineContext.ROW_INDEX_STREAM, StreamManager.ROOT_STREAM,
					StreamManager.SELF_SCOPE);
			this.exprResultReader = new ExprDataReader2(tempDir, rowExprsRAIs, rowLenRAIs, rowInfoRAIs, version,
					(this.qd instanceof IQueryDefinition && ((IQueryDefinition) qd).isSummaryQuery()) ? null
							: this.dataSetResultSet);
