...
public class GroupInformationUtil
{
	 ...
	 * 
	 * @return The 1-based index of the outermost group in which the current row
	 *         is the last row; (N+1) if the current row is not at the end of
	 *         any group;
	 */
	public int getEndingGroupLevel( ) throws DataException
	{
		...
		if ( this.groupCalculationUtil.getResultSetCache( ).getCurrentIndex( ) == this.groupCalculationUtil.getResultSetCache( )
				.getCount( ) - 1 )
			return 0;
		...
		int childGroupIdx = this.groupCalculationUtil.getResultSetCache( )
				.getCurrentIndex( );
		...
		for ( level = this.groups.length - 1; level >= 0; level-- )
		{
			...
			GroupInfo nextGroup = GroupUtil.findGroup( level,
					currentGroupIdx + 1,
					groups );
			if ( nextGroup != null && childGroupIdx == nextGroup.firstChild - 1 )
			{
				...
				currentGroupIdx = GroupUtil.findGroup( level,
						currentGroupIdx,
						groups ).parent;
				continue;
			}
	 ...
	 * 
	 * @return The 1-based index of the outermost group in which the current row
	 *         is the first row; (N+1) if the current row is not at the start of
	 *         any group;
	 */
	public int getStartingGroupLevel( ) throws DataException
	{
		...
		for ( level = this.groups.length - 1; level >= 0; level-- )
		{
			...
			GroupInfo currentGroup = GroupUtil.findGroup( level,
					currentGroupIdx,
					this.groups );
	...
	private int findCurrentGroup( int groupLevel )
	{
		...
		for ( int i = this.groups.length - 1; i > groupLevel; i-- )
			currentGroupIdx = GroupUtil.findGroup( i,
					currentGroupIdx,
					this.groups ).parent;
	...
	public void first( int groupLevel ) throws DataException
	{
		...
		{
			currentGroupIdx = GroupUtil.findGroup( i - 1,
					currentGroupIdx,
					this.groups ).firstChild;
		}

		leafGroupIdx = currentGroupIdx;
		int currentRowID = GroupUtil.findGroup( this.groups.length - 1,
				leafGroupIdx,
				this.groups ).firstChild;
	...
	public void last( int groupLevel ) throws DataException
	{
				...
				currentGroupIdx >= this.groups[groupLevel].size( ) - 1 )
		{
			...
			int currentRowID = this.groupCalculationUtil.getResultSetCache( )
					.getCount( ) - 1;
		...
		int currentRowID = GroupUtil.findGroup( this.groups.length - 1,
				currentGroupIdx,
				this.groups ).firstChild - 1;
		this.groupCalculationUtil.getResultSetCache( ).moveTo( currentRowID );
		leafGroupIdx = currentGroupIdx - 1;
	}

	/**
	 * Gets the index of the current group at the specified group level. The
	 * index starts at 0
	 */
	...
	public void next( boolean hasNext ) throws DataException
	{
		...
		{
			GroupInfo nextLeafGroup = GroupUtil.findGroup( this.groups.length - 1,
					leafGroupIdx + 1,
					this.groups );
			if ( nextLeafGroup != null
					&& this.groupCalculationUtil.getResultSetCache( )
							.getCurrentIndex( ) >= nextLeafGroup.firstChild )
			{
	 ...
	 * GroupCalculationUtil.groups,except that all GroupInfo instances are
	 * replaced by GroupBoundaryInfo instances.
	 * 
	 * @return
	 * @throws DataException
	 */
	...
	/**
	 * Generate a GroupBoundaryInfo instance from certain groupLevel and
	 * groupIndex.
	 * 
	 * @param groupLevel
	 *            1-based group level
	 ...
	 * @throws DataException
	 */
	private GroupBoundaryInfo getGroupBoundaryInfo( int groupLevel,
	...
	/**
	 * Re-sort the GroupBoundaryInfo instances of a lower group level according
	 * the ordering of GroupBoundaryInfo instances of a higher group level .
	 * 
	 * @param higherGroup
	 * @param lowerGroup
	 * @return
	...
	/**
	 * This method return the OrderingInfo that will be finally used to
	 * re-generate the smartcache.
	 * 
	 * @param groups
	 * @return
	 */
