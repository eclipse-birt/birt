...
public class DataEngineContext {
	...
	/**
	 * flow mode is required to check if the request received is from regular report
	...
	/**
	 * this is a special mode, that a query which is running on a report document
	 * also needs to be stored into the same report document. This is a update
	 * operation, and it is a combination mode of presentation and generation. One
	 * note is in running in this mode, the report document must be same for reading
	 * and writing.
	 */
	...
	/**
	 * When mode is MODE_GENERATION, the writer stream of archive will be used. When
	 * mode is MODE_PRESENTATION, the reader stream of archive will be used. When
	...
	public static DataEngineContext newInstance(int mode, ScriptContext context, IDocArchiveReader reader,
			IDocArchiveWriter writer, ClassLoader classLoader) throws BirtException {
		IDataScriptEngine dse = (IDataScriptEngine) context.getScriptEngine(IDataScriptEngine.ENGINE_NAME);
		DataEngineContext result = new DataEngineContext(mode, dse.getJSScope(context), reader, writer, classLoader,
				context);
		return result;
	}
	 ...
	 * @throws BirtException
	 */
	public DataEngineContext(int mode, Scriptable scope, IDocArchiveReader reader, IDocArchiveWriter writer,
			ClassLoader classLoader, ScriptContext context) throws BirtException {
		Object[] params = { Integer.valueOf(mode), scope, reader, writer, classLoader };
		logger.entering(DataEngineContext.class.getName(), "DataEngineContext", //$NON-NLS-1$
				params);
	 ...
	 * These option values will override the values defined in individual data set
	 * and its application context. The option value has three posible values,
	 * CACHE_USE_DEFAULT, CACHE_USE_DISABLE, CACHE_USE_ALWAYS. The cacheCount values
	...
	/**
	 * According to the paramters of streamID, subStreamID and streamType, an output
	...
	public RAOutputStream getOutputStream(String streamID, String subStreamID, int streamType, String subName)
	...
	public void dropStream(String streamPath) {
		if (writer != null) {
			try {
				...
				// We should not try to drop stream here, because this may expose the real
				// stream in
	...
	/**
	 * According to the paramters of streamID, subStreamID and streamType, an input
	 * stream will be created for it. To make stream close simply, the stream needs
	 * to be closed by caller, and then caller requires to add buffer stream layer
	 * when needed.
	 *
	...
	public RAInputStream getInputStream(String streamID, String subStreamID, int streamType, String subname)
			throws DataException {
		String relativePath = getPath(streamID, subStreamID, streamType, subname);
	...
	/**
	 * @param streamType
	 * @return relative path, notice in reading data from file, directory can not be
	 *         created.
	 */
	public static String getPath(String streamID, String subStreamID, int streamType) {
		...
		StringBuilder streamRoot = new StringBuilder("/").append(streamID).append("/"); //$NON-NLS-1$ //$NON-NLS-2$
		if (subStreamID != null) {
			streamRoot.append(subStreamID).append("/"); //$NON-NLS-1$
		}
		return streamRoot.append(relativePath).toString();
	}

	public static String getPath(String streamID, String subStreamID, int streamType, String subName) {
		StringBuilder path = new StringBuilder().append(getPath(streamID, subStreamID, streamType));
		if (subName != null && subName.length() > 0) {
			path.append("/").append(subName); //$NON-NLS-1$
		}
