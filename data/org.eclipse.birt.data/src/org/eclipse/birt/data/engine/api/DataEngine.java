 ...
 * typically needs only one instance of this class, which can be used to prepare
 * and execute multiple data queries.
 * <p>
 * User of this class must always call the <code>shutdown</code> method when it
 * is done with an instance of this class to ensure release of all data source
...
abstract public class DataEngine {
	 ...
	 * non-negative integer as input, the unit of which would be MB. If this setting
	 * is 0, all temporary rows will be cached in memory during query processing.
	 */
	public static String MEMORY_BUFFER_SIZE = "org.eclipse.birt.data.query.ResultBufferSize";

	/**
	 * This setting allow the administrator to set a limit on the amount of data
	 * that a query against a data object is allowed to access. Queries should be
	 * aborted with error if it exceeds this limit.
	 */
	...
	/**
	 * Indicates whether enable sample data preview in current session. If the
	 * setting is set to true, we will match the cache based on session id and data
	 * source/data set name. We will also generate dummy data to avoid.
	 */
	...
	/**
	 * The size of cube to be saved in memory rather than in disk. The unit is MB.
	 * In case that the size of materialized cube exceed the assigned memory size,
	 ...
	 * provided definition object.
	 */
	abstract public void defineDataSource(IBaseDataSourceDesign dataSource) throws BirtException;
	 ...
	 * definition object.
	 */
	abstract public void defineDataSet(IBaseDataSetDesign dataSet) throws BirtException;
	...
	/**
	 * If user wants to use the latest data from data set rather than local cache,
	 ...
	 * @throws BirtException
	 */
	abstract public void clearCache(IBaseDataSourceDesign dataSource, IBaseDataSetDesign dataSet) throws BirtException;
	 ...
	 * <code>IQueryDefinition</code> (raw data transform) spec based on static
	 * definition found in a report design.
	 * <p>
	 * This report query spec could be further refined during engine execution after
	 * having resolved any related runtime condition. For example, a nested report
	 * item might not be rendered based on a runtime condition. Thus its associated
	 * data expression could be removed from the report query defn given to DtE to
	 * prepare.
	 * <p>
	 ...
	 */
	abstract public IPreparedQuery prepare(IQueryDefinition querySpec) throws BirtException;

	/**
	 * Verifies the elements of a report query spec, and provides a hint and
	 * application context object(s) to the query to prepare and optimize an
	 * execution plan. <br>
	 ...
	 *                   of the querySpec; could be null.
	 * @return The <code>IPreparedQuery</code> object that contains a prepared query
	 *         ready for execution.
	 * @throws BirtException if error occurs during the preparation of querySpec
	 * @since 2.0
	 */
	abstract public IPreparedQuery prepare(IQueryDefinition querySpec, Map appContext) throws BirtException;
	 ...
	 */
	abstract public void closeDataSource(String dataSourceName) throws BirtException;
	 ...
	 * @throws BirtException
	 */
	abstract public IPreparedCubeQuery prepare(ISubCubeQueryDefinition query, Map appContext) throws BirtException;

	/**
	 * Add a shut down listener. When the data engine shut down the listener will be
	 * notified.
	 */
	...
	/**
	 * Shuts down this instance of data engine, and releases all associated
	 * resources. This method should be called when the caller is done with an
	 * instance of the data engine.
	 */
