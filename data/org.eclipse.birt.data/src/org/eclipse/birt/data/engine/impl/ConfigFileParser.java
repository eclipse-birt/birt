 ...
 * Note that it is an internal tool that can only parse the firmly-formatted file
 * rather than a general parser tool.
 * 
 */
public class ConfigFileParser
{
	...
	/**
	 * This method is to check whether the current tree contains the specified data set node.
	 * 
	...
	/**
	 * This method is to get the timestamp column name by the specified data set name.
	 * 
	 *@param String id the data set name
	...
	/**
	 * This method is to get the timestamp format string by the specified data set name.
	 * 
	 *@param String id the data set name
				...
				if ( temp.startsWith( "</" ) )
				{
					...
					if ( obj instanceof String )
					{
						...
						if ( next.startsWith( "<" ) && next.endsWith( ">" ) )
						{
							...
							{
								node.setName( temp );
								next = next.substring( temp.length( ) + 1,
										next.length( ) - 1 ).trim( );
								if ( !next.equals( "" ) )
								{
	...
	/**
	 * An inner class serve as a data structure used to store 
