...
public class QueryExecutorUtil
{
	 ...
	 */
	public static IAggregationResultSet[] applyFilterOnOperation(
			BirtCubeView view, CubeQueryExecutorHelper cubeQueryExecutorHelper,
			CubeQueryExecutor executor,
			AggregationDefinition[] finalAggregation,
			IAggregationResultSet[] rs, IBindingValueFetcher fetcher,
			...
			BirtException
	{
		if ( !executor.getNestAggregationFilterEvalHelpers( ).isEmpty( ) )
		{
							...
							.canExecuteCubeOperation( ) )
			{
				...
				cubeQueryExecutorHelper.applyAggrFilters( finalAggregation,
						rs,
						stopSign );
			}
		}
	 ...
	 * @throws DataException 
	 */
	public static AggregationDefinition[] prepareCube( ICubeQueryDefinition query,
			CalculatedMember[] calculatedMember,  Scriptable scope, ScriptContext cx ) throws DataException
	{
		...
		AggregationDefinition[] fromCalculatedMembers
			= CubeQueryDefinitionUtil.createAggregationDefinitons( calculatedMember, query,  scope,  cx );
