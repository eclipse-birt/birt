...
public class GroupInfoUpdator
{
	...
	public GroupInfoUpdator( int level, String tempDir, List<GroupInfo> groups,
			int last, AggrValuesUpdator[] aggrValues )
	{
		this.level = level;
	...
	/**
	 * Try to mark the given group can be accepted and accept current processed group.
	 * 
	...
	protected void acceptPrevious( int groupIndex  ) throws DataException
	{
		...
		}else if ( currentGroup.groupId > groupIndex )
		{
			throw new DataException( ResourceConstants.GROUPUPDATE_ILLEGAL_GROUP_ORDER_STATE,
					new Object[]{
							currentGroup.groupId, groupIndex
					} );
		}
	...
	/**
	 * Try to mark the given group can be filtered out and accept current processed group.
	 * 
	 * @param groupIndex
	...
	protected RuntimeGroupInfo getRumtimeGroupInfo( int index )
	{
		...
		return new RuntimeGroupInfo( current,
				index,
				count,
				parentIdxAdj,
				childIdxAdj );
