...
{	
	 ...
	 * current algorithm implemenation, the data is read sequentially and then
	 * next begin will only have two possible values, 
	 * 1:in the scope of last [begin, end) 
	 * 2:(last end) +1.
	 * 
	 * @param begin
	 * @param end
	...
	IResultObject[] readData( int begin, int end ) throws IOException, DataException
	{
		...
		int cacheLength = cachedResultObjects[readerIndex] != null
				? cachedResultObjects[readerIndex].length : 0;
		if ( indexOfCachedRowData[readerIndex] == -1
		...
		else
		{
			...
			if ( offsetOfEnd > cacheLength )
			{
				...
				int fromBeginCachedLength = indexOfCachedRowData[readerIndex]
						+ cacheLength - begin;
				for ( int i = 0; i < fromBeginCachedLength; i++ )
					tempCachedData[i] = cachedResultObjects[readerIndex][offsetOfBegin
							+ i];
	 ...
	 * current algorithm implemenation, the data is written sequentially and
	 * then currPos value only has one possible values, that is the
	 * (currPos+length) of latest calling..
	 * 
	 * @param hint,
	 *            soring itself/merge sorting
	 * @param currPos,
	 *            current result object position
	 * @param resultObjects,
