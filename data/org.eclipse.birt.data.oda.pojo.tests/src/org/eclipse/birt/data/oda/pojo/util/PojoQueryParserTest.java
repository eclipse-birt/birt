...
public class PojoQueryParserTest {
	...
	public void testBasicParse() throws IOException, OdaException {
		...
		assertEqualMappingSource(ccm.getSource(), new MethodSource("getDean", null));
		...
		assertEqualColumnsMapping(internals[1], new OneColumnMapping(new MethodSource("getStudentName", null),
				new Column("Student Name", "String", 4)));
		...
		assertEqualColumnsMapping(ocm, new OneColumnMapping(new MethodSource("getTeacherName", null),
				new Column("Teacher Name", "String", 6)));
		ocm = (OneColumnMapping) mappings[4];
		assertEqualColumnsMapping(ocm, new OneColumnMapping(new FieldSource("age"), new Column("Age", "Integer", 7)));
