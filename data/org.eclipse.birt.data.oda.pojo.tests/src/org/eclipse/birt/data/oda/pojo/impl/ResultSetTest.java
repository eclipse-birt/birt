...
public class ResultSetTest {
	...
	public void testCheckInvalidMulti1TonMaps() throws OdaException {
		...
		ccm.addColumnsMapping(
				new OneColumnMapping(new MethodSource("getId", null), new Column("CourseId", "Integer", 7)));
	...
	private void verifyResultSetTeacherStudent(ResultSet rs, List<TeacherStudent> compounds) throws OdaException {
	...
	private void verifyResultSetTeacherStudentCourse(ResultSet rs, List<TeacherStudentCourse> compounds)
			throws OdaException {
		int expectedRowCount = compounds.size();
		...
		while (rs.next()) {
			...
			if (tsc.teacher.getDean() == null) {
				...
				Integer deanId = (Integer) rs.getColumnValue(4);
				String deanName = (String) rs.getColumnValue(5);
				assertEquals(tsc.teacher.getDean().getId(), deanId.intValue());
