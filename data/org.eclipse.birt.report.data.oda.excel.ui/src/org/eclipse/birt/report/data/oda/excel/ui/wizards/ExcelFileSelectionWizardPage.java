		...
		ISelectionChangedListener
{
	...
	private void initializeControl( )
	{
		...
		{
			currentSheetName = dataSetDesign.getPublicProperties( )
					.getProperty( ExcelODAConstants.CONN_WORKSHEETS_PROP );
		}
	...
	{
		FormData data = new FormData( );
		data.top = new FormAttachment( worksheetsCombo.getControl( ),
				10,
				SWT.BOTTOM );
		data.left = new FormAttachment( 0, 5 );
		data.right = new FormAttachment( 40, -5 );
				...
				.addSelectionListener( new SelectionAdapter( ) {
					...
					public void widgetSelected( SelectionEvent e )
					{
						...
						if ( count == 1 )
						{
							...
							if ( selectedColumnsViewer.getTable( )
									.getItemCount( ) == 1 )
							{
								btnMoveUp.setEnabled( false );
								btnMoveDown.setEnabled( false );
	 ...
	 * different excel file may have the some sheet name. This method is to
	 * check whether the data source(excel file) is changed.
	 * 
	 * @param queryText
	 * @param selectedFile
	 * @return
	...
	{
		java.util.List<String[]> propList = getQueryColumnsInfo( "select * from " + QueryTextUtil.getQuotedName( getFileName( file ) ), file, currentSheetName ); //$NON-NLS-1$
	...
	private IResultSetMetaData getResultSetMetaData( String queryText,
			Object file, IConnection conn, String sheetName )
			throws OdaException
	{
		...
		if ( file != null )
		{
			if ( file instanceof File )
			{
				...
				prop.put( ExcelODAConstants.CONN_FILE_URI_PROP,
	...
	private void savePage( DataSetDesign dataSetDesign )
	{
		...
		{
			String sheetName = dataSetDesign.getPublicProperties( )
					.getProperty( ExcelODAConstants.CONN_WORKSHEETS_PROP );
	...
	private void savePublicProperties( DataSetDesign dataSetDesign )
	{
		if ( dataSetDesign.getPublicProperties( ) == null )
		{
			try
			{
				...
				Properties dsProp = DesignSessionUtil.createDataSetPublicProperties( dsID,
						dstID,
						getPageProperties( ) );
		...
		{
			if ( dataSetDesign.getPublicProperties( )
					.findProperty( ExcelODAConstants.CONN_WORKSHEETS_PROP ) != null )
	...
	private void populateWorkSheetCombo( ) throws IOException
	{
		...
		if ( currentSheetName == null
				&& worksheetsCombo.getSelection( ).isEmpty( ) )
		{
