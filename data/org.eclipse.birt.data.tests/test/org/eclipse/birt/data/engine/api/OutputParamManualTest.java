...
public class OutputParamManualTest extends BaseTestCase {
	...
	private IBaseDataSetDesign getDataSet() throws Exception {
		...
		ParameterDefinition pd1 = new ParameterDefinition("param1", DataType.STRING_TYPE, true, false);
		pd1.setPosition(1);

		ParameterDefinition pd2 = new ParameterDefinition("@@param2", DataType.DECIMAL_TYPE, true, true);
		pd2.setPosition(2);

		InputParameterBinding pb1 = new InputParameterBinding("param1", new ScriptExpression("'%'"));

		InputParameterBinding pb2 = new InputParameterBinding("@@param2", new ScriptExpression("1"));
	...
	/**
	 * Test disk based feature. This test is using net.sourceforge.jtds.jdbc.Driver
	 * to access SQL server which is not included in default package. The JDBC
	 * driver has to installed in order for the test to pass.
	 */
