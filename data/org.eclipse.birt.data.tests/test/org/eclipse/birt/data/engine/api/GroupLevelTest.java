...
/**
 * 
 * Test results of group level are:
 * 1: when there is at least one row,
 * 		1: the starting group level is 0 if and only if it is the first row
 * 		2: the ending group level is 0 if and only it is the last row
 * 		3: before first row has the same result as the first row
 * 		4: the retrievation of the after last row is not available
 * 2: when there is no row
 * 		1: the retrievation of group level function is not available 
...
/**
	...
	private void prepare( int rowCount ) throws BirtException
	{
		...
		dataSet.setFetchScript( "if (count==0) "
				+ "{" + "return false; " + "} " + "else " + "{ "
