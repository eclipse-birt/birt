...
public class JointDataSetTest extends APITestCase
{
	...
	{
		OdaDataSetDesign dset1 = newDataSet( "dset1", "Select ID, CITY, STORE FROM "
				+ this.getTestTableName( ) + " where ID > 4 and ID <> 7 and ID <> 9 order by ID asc");

		OdaDataSetDesign dset2 = newDataSet( "dset2", "Select ID, SKU, CATEGORY, PRICE FROM "
				+ this.getTestTableName( ) + " where ID < 20 and ID <> 10 and ID <> 13 order by ID asc");
		...
		b.add( new JoinCondition(new ScriptExpression("dataSetRow[\"dset1::ID\"]"),new ScriptExpression("dataSetRow[\"ID\"]"),IJoinCondition.OP_EQ) );
		...
		{
			s += ri.getResultClass( ).getFieldName( i+1 ) + "\t\t\t";
		}
		s += "\n";
	...
	{
		OdaDataSetDesign dset = newDataSet( "dset", "Select ID, CITY, STORE FROM "
				+ this.getTestTableName( ) +" order by ID asc");
	...
	private String basicJoinTest( int joinType, int populateType ) throws Exception, BirtException, DataException
	{
		...
		if( populateType == BINARY_TREE_POPULATOR) 
			populator = JointDataSetPopulatorFactory.getBinaryTreeDataSetPopulator(it1, it2, meta, matcher, joinType, ((DataEngineImpl)this.dataEngine).getSession( ), fetchRowLimit);
		else 
			populator = JointDataSetPopulatorFactory.getCartesianJointDataSetPopulator(it1, it2, meta, matcher, joinType,  ((DataEngineImpl)this.dataEngine).getSession( ), fetchRowLimit);
		
		IResultObject ro;
		String s = "";
		for( int i = 0; i < resultClass.getFieldCount( ); i++ )
		{
			s += resultClass.getFieldName( i+1 ) + "\t\t\t";
		}
		s += "\n";
		long start = System.currentTimeMillis( );
		int count = 0;
    ...
    public void testAlias( ) throws Exception
	{
		...
		OdaDataSetDesign dset = newDataSet( "dset",
				"Select ID, CITY, STORE FROM "
						+ this.getTestTableName( ) + " order by ID asc" );

		List a = new ArrayList( );
		a.add( new JoinCondition( new ScriptExpression( "dataSetRow.ID" ),
				new ScriptExpression( "dataSetRow.ID" ),
				IJoinCondition.OP_EQ ) );

		JointDataSetDesign dset2 = new JointDataSetDesign( "dset2",
				dset.getName( ),
				dset.getName( ),
				IJointDataSetDesign.INNER_JOIN,
				a );
		...
		IPreparedQuery preparedQuery = this.dataEngine.prepare( query );
		IQueryResults qr = preparedQuery.execute( null );
		IResultClass resultClass = ( (ResultIterator) qr.getResultIterator( ) ).getOdiResult( )
				.getResultClass( );

		assertEquals( resultClass.getFieldAlias( resultClass.getFieldIndex( fieldName ) ),
				columnDefinition.getAlias( ) );
	}
