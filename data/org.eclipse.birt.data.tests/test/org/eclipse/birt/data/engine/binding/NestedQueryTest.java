...
public class NestedQueryTest extends APITestCase {
	...
	public void test4() throws Exception {
		...
		datasetCall = newDataSet("data set calls", " SELECT * FROM " + callsTableName);
		queryDefnCall = createCallQueryDefn();
		...
		FilterDefinition filterDefn = new FilterDefinition(new ConditionalExpression("row[\"ROW_CUSTOMERID\"]",
				ConditionalExpression.OP_EQ, "rows[0].CUSTOMERID"));
	...
	public void test5() throws Exception {
		...
		FilterDefinition filterDefn = new FilterDefinition(
				new ConditionalExpression("row._outer[\"ROW_CUSTOMERID\"]", ConditionalExpression.OP_EQ, "2"));
	...
	public void test7() throws Exception {
		...
		bindingExprFilter[0] = new ScriptExpression("dataSetRow.CU");
		FilterDefinition filterDefn = new FilterDefinition(
				new ConditionalExpression("Total.isTopN(row[\"ROW_DURATION\"],5)", ConditionalExpression.OP_TRUE));
	...
	private QueryDefinition createCustomerQueryDefn() throws Exception {
		...
		expressionsCustomer = new ScriptExpression[] { new ScriptExpression("dataSetRow.CUSTOMERID", 0),
		...
		return createQuery(bindingNameGroup, bindingExprGroup, groupDefn, bindingNameSort, bindingExprSort, sortDefn,
				null, null, null, bindingNameCustomer, expressionsCustomer);
	...
	private QueryDefinition createCallQueryDefn() throws Exception {
		...
		expressionsCall = new ScriptExpression[] { new ScriptExpression("dataSetRow.CUSTOMERID", 0),
				new ScriptExpression("dataSetRow.CALLTIME", 0), new ScriptExpression("dataSetRow.TONUMBER", 0),
				new ScriptExpression("dataSetRow.DURATION", 0), new ScriptExpression("dataSetRow.CHARGE", 0),
				new ScriptExpression("rows[0].CUSTOMERID", 0), new ScriptExpression("row._outer[\"ROW_CUSTOMERID\"]"),
		...
		return createQuery(null, null, null, null, null, null, null, null, null, bindingNameCall, expressionsCall);
	}
	...
	private void addParameterToQueryCall(ScriptExpression expression) {
		...
		InputParameterBinding inputParamBinding = new InputParameterBinding(1, expression);
	...
	private void runNestedQuery() throws Exception {
		...
		while (resultItCustomer.next()) {
			...
			IQueryResults queryResultsCalls = preparedQueryCall.execute(queryResultsCustomer, null);
