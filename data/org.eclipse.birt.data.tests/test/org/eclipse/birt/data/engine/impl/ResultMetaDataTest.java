...
public class ResultMetaDataTest extends APITestCase {
	...
	public void test1_SelectAll() throws Exception {
		OdaDataSetDesign dset = newDataSet("dset1", "Select * FROM " + this.getTestTableName());
	...
	public void test2_ResultHints() throws Exception {
		OdaDataSetDesign dset = newDataSet("dset2", "Select * FROM " + this.getTestTableName());
		QueryDefinition query = this.newReportQuery(dset, true);
	...
	/*
	 * public void test3_ComputedCol( ) throws Exception { // Use a new data set
	 * with SQL containing computed columns // The 2 computed columns are unnamed
	 * (at least with MS SQL Server) OdaDataSetDesign dset = newDataSet( "dset3",
	...
	public void test4_ProjectedCol() throws Exception {
		...
		OdaDataSetDesign dset = newDataSet("dset4", "Select * FROM " + this.getTestTableName());
		QueryDefinition query = this.newReportQuery(dset, true);

		// Add a BIRT computed column
		ComputedColumn compCol = new ComputedColumn("ComputedCol1", "row.intCol * 2", DataType.ANY_TYPE);
