...
public class TestTimeDimension {
	...
	public void testTimeDimension() throws Exception {
		...
		DocManagerMap.getDocManagerMap().set(String.valueOf(engine.hashCode()),
				engine.getSession().getTempDir() + engine.getSession().getEngine().hashCode(), documentManager);
		...
		IFilterDefinition filter = new FilterDefinition(
				new ConditionalExpression("dimension[\"dimension2\"][\"day\"]", IConditionalExpression.OP_LE, "13"));
	...
	private void createCube(IDocumentManager documentManager) throws IOException, BirtException {
		...
		cube.create(new String[][] { new String[] { "l1" }, new String[] { "l2" } }, dimensions, new Dataset2(),
				new String[] { "m1" }, new StopSign());
...
class Dataset2 implements IDatasetIterator {
	...
	static Date[] L2Col = { (new java.util.GregorianCalendar(1998, 10, 13)).getTime(),
