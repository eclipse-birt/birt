...
public class ExpressionCompilerTest {
	...
	public void expressionCompilerSetUp() throws Exception {
		...
		// Test cases can share one context because they are executed on the same test
		// thread
	...
	public void testDirectColRefByName() throws Exception {
		...
		expr = compiler.compile("r.col", null, cx);
		type = expr.getType();
	...
	public void testAggregateExpression() throws Exception {
		...
		aggrExprs.clear();
		expr = compiler.compile("Total.Sum( row.x )", aggrReg, cx);
		...
		aggrExprs.clear();
		expr = compiler.compile("Total.Sum( row[1], row.y > row.z, \"Group1\" )", aggrReg, cx);
		assertTrue(aggrExprs.size() == 1);
		assertTrue(aggrExprs.get(0) == expr);
		assertTrue(expr.getType() == CompiledExpression.TYPE_SINGLE_AGGREGATE);
		aggr = (AggregateExpression) expr;
		...
		aggrExprs.clear();
		expr = compiler.compile("Total.Sum( Total.Max(row.y), null, 1 )", aggrReg, cx);
		...
		aggrExprs.clear();
		expr = compiler.compile(" row[\"x\"] / Total.Sum( 1 )", aggrReg, cx);
		assertTrue(aggrExprs.size() == 1);
		assertTrue(expr.getType() == CompiledExpression.TYPE_COMPLEX_EXPR);

		aggrExprs.clear();
		expr = compiler.compile("Total.MovingAve( row.x, myfunc(), null, 1 )", aggrReg, cx);
		assertTrue(aggrExprs.size() == 1);
		assertTrue(aggrExprs.get(0) == expr);
		assertTrue(expr.getType() == CompiledExpression.TYPE_SINGLE_AGGREGATE);
		aggr = (AggregateExpression) expr;
	...
	public void testMultiExpression() throws Exception {
		...
		CompiledExpression expr = compiler.compile("a=1; b=a+1; b+1;", this.aggrReg, cx);
		...
		this.aggrExprs.clear();
		expr = compiler.compile("a=true; if ( ! a && Total.Count() > 0 ) b=Total.Sum(row.x); else b=1 ; b",
				this.aggrReg, cx);
		assertEquals(expr.getType(), CompiledExpression.TYPE_COMPLEX_EXPR);
		...
		assertTrue(((IAggrFunction) ((AggregateExpression) (aggrExprs.get(0))).getAggregation()).getName()
				.equalsIgnoreCase("COUNT"));
		assertTrue(((AggregateExpression) (aggrExprs.get(0))).getArguments().size() == 0);
		assertTrue(((IAggrFunction) ((AggregateExpression) (aggrExprs.get(1))).getAggregation()).getName()
				.equalsIgnoreCase("SUM"));
		assertTrue(((AggregateExpression) (aggrExprs.get(1))).getArguments().size() == 1);
	}

	/*
	 * Please refer to SCR #75905 Exception thrown out when choose "select value"
	 * without expression if set filter
	 */
	@Test
