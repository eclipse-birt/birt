...
{	
	...
	// column name
	private String[] scriptColumnNames = new String[]{
			"NUM", "SQUARE", "STR", "ANY"
	};
	private int[] scriptColumnTypes = new int[] {
			DataType.INTEGER_TYPE, DataType.DOUBLE_TYPE, DataType.STRING_TYPE, DataType.ANY_TYPE
	};
	private String[] scriptColumnTypeNames = new String[] {
			"INTEGER", "DOUBLE", "STRING", "ANY" 
	};
	// column alias name
	private String[] scriptColumnAliasNames = new String[]{
			"NUM2", "SQUARE2", "STR2", "ANY2"
	};
	
	private String[] computedColumnNames = new String[] {
			"CP1", "CP2" };
	...
	private void init( boolean dynamicMetadata ) throws Exception
	{
		...
		scriptExprs = new IScriptExpression[]{
				new ScriptExpression( "dataSetRow." + scriptColumnNames[0] ),
		...
		aliasScriptExprs = new IScriptExpression[]{
				new ScriptExpression( "dataSetRow." + scriptColumnAliasNames[0] ),
	...
	private ScriptDataSetDesign newDataSet( String name, boolean dynamicMetadata )
	{
		...
		// 1: add scripts
		dataSet.setOpenScript( 
			"testPrintln(\"@@EVENT:ScriptDataSet.open\");" +
			"count=11; dset_open=true; count--;");
		
		dataSet.setCloseScript( 
			"testPrintln(\"@@EVENT:ScriptDataSet.close\")" );
		
		dataSet.setFetchScript( 
			"if (count==0) {return false; } else " +
			"{ row.NUM=count; row.SQUARE=count*count; row.STR=\"row#\" + count; " +
			" row[4]=count<5; " +
			"--count; return true; }");
		...
		if ( dynamicMetadata )
		{
			...
			for ( int i = 0; i < scriptColumnNames.length; i++)
			{
				...
				String callText = "addDataSetColumn(\"" + 
					scriptColumnNames[i] + "\", \"" +
					scriptColumnTypeNames[i] + "\"); ";
	...
	private void fetch1_test_impl( boolean dynamicMetadata ) throws Exception
	{
		...
		while ( ri.next( ) )
		{
			...
			for ( int i = 0; i < scriptColumnNames.length; i++ )
			{
				...
				outputStr += scriptExprs[i].getText( ).replaceAll( "dataSetRow", "row" )
    ...
    public void testFetchWithAlias( ) throws Exception
	{
		...
		while ( ri.next( ) )
		{
			...
			for ( int i = 0; i < this.scriptColumnAliasNames.length; i++ )
			{
				...
				outputStr += aliasScriptExprs[i].getText( ).replaceAll( "dataSetRow", "row" )
						+ " value is:" + value.toString( ) + "    ";
			}
    ...
    public void testDateTimeValues() throws Exception
	{
		...
		dset.addComputedColumn( new ComputedColumn(
				"CP1", "row.d1.getUTCFullYear()",DataType.INTEGER_TYPE));
		dset.addComputedColumn( new ComputedColumn(
				"CP2", "row.d2.getUTCFullYear()",DataType.INTEGER_TYPE));
		dset.addComputedColumn( new ComputedColumn(
				"CP3", "new Date(0)",DataType.DATE_TYPE));

		dset.setOpenScript("count=0;");
		dset.setFetchScript(" if (++count > 1) return false;  " +
				"row.d1 = new Date(0);" + 
				"row.d2 = new Packages.java.util.Date(0); "+
				"return true;" );
