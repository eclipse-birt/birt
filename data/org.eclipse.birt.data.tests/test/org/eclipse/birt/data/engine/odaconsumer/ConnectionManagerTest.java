...
public class ConnectionManagerTest extends OdaconsumerTestCase
{
    ...
    public void connectionManagerTearDown() throws Exception
	{
		    ...
		    getClass().getName().endsWith( "ConnectionManagerTest" ) )
	    {
			...
			LogConfiguration offLogConfig = 
			    new LogConfiguration( Level.OFF, 
			            m_testerLogConfig.getLogDirectory(), "", "" );
			setOdaHelperLogConfig( JDBCOdaDataSource.DATA_SOURCE_TYPE, 
			        				offLogConfig );
	...
	public final void testPassThruContext() throws Exception
	{
		...
		Connection conn = getManager().openConnection( JDBCOdaDataSource.DATA_SOURCE_TYPE, 
											connProperties, dummyAppContext );
		assertNotNull( conn );
		...
		boolean hasExpectedLogs = TraceLogTesterUtil.getInstance().
			matchLogPatternsInFile( logFileName, expectedLogs );
		assertTrue( hasExpectedLogs );
	}

	private void sanityCheckProperties() throws Exception
	{
		if( System.getProperty( "odaJdbcUrl" ) == null ||
			System.getProperty( "odaJdbcUser" ) == null ||
			System.getProperty( "odaJdbcPassword" ) == null ||
			System.getProperty( "odaJdbcDriver" ) == null )
		{
	...
	private LogConfiguration getOdaHelperTestLogConfig( String dataSourceId ) throws Exception
	{
		...
		LogConfiguration testLogConfig = 
		    new LogConfiguration( Level.FINE, logDir, "", "" );
		return testLogConfig;
	}

	private String getOdaHelperLogDir( String dataSourceId )
	{
		return OdaConsumerPlugin.getDefault()
				.getStateLocation().append( "logs" )
				.append( "ConnectionManagerTest" ).toFile().getPath();
	}
	
	private void setOdaHelperLogConfig( String dataSourceId,
	        LogConfiguration testLogConfig ) throws Exception
	{
		IDriver driverHelper = 
			DriverManager.getInstance().getDriverHelper( dataSourceId );
