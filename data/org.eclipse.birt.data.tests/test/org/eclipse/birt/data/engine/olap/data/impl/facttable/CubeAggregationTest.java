...
public class CubeAggregationTest extends BaseTestCase
{
	...
	private DimLevel[] dimLevels = new DimLevel[]{
			new DimLevel( "geography", "province" ),
			new DimLevel( "geography", "city" ),
			new DimLevel( "customer", "name" ),
			new DimLevel( "product", "productID" )
	};
    ...
    public void testSumAggregation2(  ) throws IOException, DataException
	{
		...
		aggregationFunctions[0] = new AggregationFunctionDefinition( "saleWeightAvg", "totalSale", this.dimLevels[3], "productID", IBuildInAggregation.TOTAL_WEIGHTEDAVE_FUNC );
		DimLevel[] dimLevels = new DimLevel[]{this.dimLevels[0],this.dimLevels[2]};
		dimLevels = new DimLevel[]{this.dimLevels[0]};
		aggregations[0] = new AggregationDefinition( dimLevels, sortTypes, aggregationFunctions);
    ...
    public void testRunningAggregation1(  ) throws IOException, DataException
	{
		...
		int[] sortTypes = new int[]{0,0};
		AggregationFunctionDefinition[] aggregationFunctions = new AggregationFunctionDefinition[2];
		aggregationFunctions[0] = new AggregationFunctionDefinition( "pSale", "totalSale", IBuildInAggregation.TOTAL_RUNNINGSUM_FUNC );
		aggregationFunctions[1] = new AggregationFunctionDefinition( "pCount", "totalCost", IBuildInAggregation.TOTAL_RUNNINGCOUNT_FUNC );
		DimLevel[] dimLevels = new DimLevel[]{this.dimLevels[0],this.dimLevels[2]};
		aggregations[0] = new AggregationDefinition( dimLevels, sortTypes, aggregationFunctions);
		...
		Object[][] fieldValues = new Object[][]{
				{"AnHui",	"li",	new Double( 110 ),	new Integer( 1 )},
				...
				{"HeBei",	"li",	new Double( 100 ),	new Integer( 1 )},
				{"HeBei",	"li",	new Double( 250 ),	new Integer( 2 )},
		...
		{
			resultSets[0].seek( i );
			assertEquals( resultSets[0].getLevelKeyValue( 0 )[0],
					fieldValues[i][0] );
			assertEquals( resultSets[0].getLevelKeyValue( 2 )[0],
					fieldValues[i][1] );
			assertEquals( resultSets[0].getAggregationValue( 0 ),
					fieldValues[i][2] );
			assertEquals( resultSets[0].getAggregationValue( 1 ),
					fieldValues[i][3] );
		}
		...
		fieldValues = new Object[][]{
				{"AnHui",	"HeFei",		new Double( 110 ),	new Integer( 1 )},
		...
		{
			resultSets[1].seek( i );
			assertEquals( resultSets[1].getLevelKeyValue( 0 )[0],
					fieldValues[i][0] );
			assertEquals( resultSets[1].getLevelKeyValue( 1 )[0],
					fieldValues[i][1] );
			assertEquals( resultSets[1].getAggregationValue( 0 ),
					fieldValues[i][2] );
			assertEquals( resultSets[1].getAggregationValue( 1 ),
					fieldValues[i][3] );
		}
...
//		aggregationFunctions[0].setParaValue( "true" );
		...
		for ( int i = 0; i < fieldValues.length; i++ )
		{
			...
			for( int j=0;j<4;j++)
			{
				assertEquals( resultSets[0].getLevelKeyValue( j )[0],
						fieldValues[i][j] );
			}
			assertEquals( resultSets[0].getAggregationValue( 0 ),
					fieldValues[i][5] );
	...
	private void createAggregationResult( ) throws IOException, DataException
	{
		...
		aggregationResultSet = new AggregationResultSet( aggregation,
				getAggregationResultRow( ),
				keyNames,
				attrNames  );
