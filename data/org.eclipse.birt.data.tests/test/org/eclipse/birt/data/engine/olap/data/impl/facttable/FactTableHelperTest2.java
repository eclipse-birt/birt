...
public class FactTableHelperTest2 extends BaseTestCase {
	...
	private void testFactTableSaveAndLoad(IDocumentManager documentManager) throws IOException, BirtException {
		...
		FactTable factTable = factTableConstructor.saveFactTable(NamingUtil.getFactTableName("bigThreeDimensions"),
				CubeUtility.getKeyColNames(dimensions), CubeUtility.getKeyColNames(dimensions), factTable1, dimensions,
				measureColumnName, new StopSign());
		// assertEquals(factTable.getSegmentNumber( ), 1);
		factTable = factTableConstructor.load(NamingUtil.getFactTableName("bigThreeDimensions"), new StopSign());
//		assertEquals(factTable.getSegmentNumber( ), 1);
		...
		FactTableRowIterator facttableRowIterator = new FactTableRowIterator(factTable, dimensionNames,
				dimensionPosition, new StopSign());
...
//		 dimension2
		...
		FactTableAccessor factTableConstructor = new FactTableAccessor(documentManager);
		FactTable factTable = factTableConstructor.saveFactTable(NamingUtil.getFactTableName("bigThreeDimensions"),
				CubeUtility.getKeyColNames(dimensions), CubeUtility.getKeyColNames(dimensions), factTable2, dimensions,
				measureColumnName, new StopSign());
		// assertEquals(factTable.getSegmentNumber( ), 1);
		factTable = factTableConstructor.load(NamingUtil.getFactTableName("bigThreeDimensions"), new StopSign());
//		assertEquals(factTable.getSegmentNumber( ), 1);
	...
	private void testFactTableSaveAndLoad3(IDocumentManager documentManager)
		...
		filter[0][0] = SelectionFactory.createRangeSelection(new Object[] { new Integer(1) },
				new Object[] { new Integer(3) }, true, false);
		...
		FactTableRowIterator facttableRowIterator = new FactTableRowIterator(factTable, dimensionNamesForFilter,
				positionForFilter, new StopSign());
		assertTrue(facttableRowIterator != null);
		...
		IDataSet4Aggregation dataSet4Aggregation = new DataSetFromOriginalCube(facttableRowIterator, dimesionResultSets,
				null);
		AggregationExecutor aggregationCalculatorExecutor = new AggregationExecutor(null, dataSet4Aggregation,
...
//		 dimension2
	...
	private void testFactTableSaveAndLoad5(CubeMaterializer cubeMaterializer) throws IOException, BirtException {
		...
		IHierarchy hierarchy = cubeMaterializer.createHierarchy("dimension1", "hir1", iterator, levelDefs,
				new StopSign());
		...
		levelDefs = new ILevelDefn[1];
		levelDefs[0] = CubeElementFactory.createLevelDefinition("level21", new String[] { "level21" }, null);
		hierarchy = cubeMaterializer.createHierarchy("dimension2", "hir2", iterator, levelDefs, new StopSign());
		...
		levelDefs = new ILevelDefn[1];
		levelDefs[0] = new LevelDefinition("level31", new String[] { "level31" }, null);
		hierarchy = cubeMaterializer.createHierarchy("dimension3", "hir3", iterator, levelDefs, new StopSign());
		...
		measureColumnName[0] = "measure1";
		measureColumnName[1] = "measure2";
		cubeMaterializer.createCube("cube", CubeUtility.getKeyColNames(dimensions), dimensions, factTable2,
				measureColumnName, new StopSign());

		CubeQueryExecutorHelper cubeQueryExcutorHelper = new CubeQueryExecutorHelper(
				CubeQueryExecutorHelper.loadCube("cube", cubeMaterializer.getDocumentManager(), new StopSign()));
	...
	private void testMaxAggregation(IDocumentManager documentManager) throws IOException, BirtException, DataException {
		...
		FactTableRowIterator facttableRowIterator = new FactTableRowIterator(factTable, dimensionNamesForFilter,
				positionsForFilter, new StopSign());
		assertTrue(facttableRowIterator != null);
	...
	private void testNestAggregation(IDocumentManager documentManager)
			throws IOException, BirtException, DataException {
		Dimension[] dimensions = new Dimension[3];
		...
		IAggregationResultSet ars = new AggregationResultSetWithOneMoreDummyAggr(sourceAggrResultSet, "max_total",
				new ScriptExpression("data[\"total\"]"), scope, cx);
		...
		ars = new AggregationResultSetWithOneMoreDummyAggr(sourceAggrResultSet, "sum_total",
				new ScriptExpression("data[\"total\"]"), scope, cx);
		resultSet = AggregationHelper.execute(ars, aggregations, new StopSign());
