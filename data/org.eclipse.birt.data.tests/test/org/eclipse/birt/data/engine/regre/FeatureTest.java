...
public class FeatureTest extends APITestCase
{
    ...
    public void testAllFeatures( ) throws Exception
	{
		...
		datasetCall = newDataSet( "data set calls", " SELECT * FROM "
				+ callsTableName + " WHERE CustomerID = ?" );
		...
		ParameterDefinition inputParamDefn = new ParameterDefinition( "param1",
				DataType.INTEGER_TYPE, true, false );
		...
		InputParameterBinding inputParamBinding = new InputParameterBinding( 1,
				expressionsCstm[4] );//rows[0].CustomerID
	...
	private QueryDefinition createCustomerQueryDefn( )
	{
		...
		expressionsCstm = new ScriptExpression[]{
		...
		return createQueryDefnUsingGivenArgs( datasetCstm, exprNameCstm, expressionsCstm,
				groupDefn, sortDefn, null );
	}
	...
	private QueryDefinition createCallQueryDefn( )
	{
		...
		FilterDefinition[] filters = new FilterDefinition[]{new FilterDefinition(
				new ConditionalExpression( "dataSetRow.DURATION",
						ConditionalExpression.OP_GT, "0" ) )};
		return createQueryDefnUsingGivenArgs( datasetCall, exprNameCall,expressionsCall,
				null, null, filters );
	}
	 ...
	 */
	private QueryDefinition createQueryDefnUsingGivenArgs(
			IBaseDataSetDesign dataset, String[] exprNames, ScriptExpression[] expressions,
	...
	private void addComputedColumnsToDataSetCall( )
	{
		...
		ComputedColumn[] computedColumns = new ComputedColumn[]{new ComputedColumn(
				"Charge2", "row.DURATION*0.05",DataType.DOUBLE_TYPE )};
		for ( int i = 0; i < computedColumns.length; i++ )
		{
			( (BaseDataSetDesign) this.datasetCall )
					.addComputedColumn( computedColumns[i] );
		}
	}
