...
public class ProjectedColumnsTest extends ConnectionTest
{
	...
	private static String[] RESULTS = {
			"blah blah blah, 1.212312",
			"hahahahahahhahaha, 3.14",
			"niem, 1.23",
			"null, null",
			"seven zero six, 12.3636"
	};
	...
	public final void testProjectedColumnValidation( ) throws Exception
	{
		...
		{
			assertEquals( "Unrecognized projected column name: MadeUpColumn.",
					ex.getMessage( ) );
		}
	}
	...
	private void checkMetaData( IResultClass metadata ) throws DataException
	{
		...
		assertEquals( String.class,
				metadata.getFieldValueClass( "stringColumn" ) );
		assertEquals( Double.class,
				metadata.getFieldValueClass( "doubleColumn" ) );
	...
	private void testFields( IResultObject resultObject,
			IResultClass resultClass, int rowPosition, boolean useFieldName )
			throws DataException
	{
		...
		for ( int i = 1; i <= resultClass.getFieldCount( ); i++ )
		{
			...
			if ( !useFieldName )
				...
				value = ( i == 1 )
						? resultObject.getFieldValue( "stringColumn" )
			...
			throws Exception
	{
		...
		assertEquals( BigDecimal.class,
				metadata.getFieldValueClass( "MyColumn" ) );
    ...
    public void testCustomColumnValidation2( ) throws Exception
	{
		...
		catch ( DataException ex )
		{
			String msg = resourceHandle.getMessage( ResourceConstants.COLUMN_NAME_OR_ALIAS_ALREADY_USED,
					new Object[]{
							"My Decimal Column", new Integer( 5 )
					} );
			assertEquals( msg, ex.getMessage( ) );
		}
	}
    ...
    public void testCustomColumnWithProjection2( ) throws Exception
	{
		...
		String[] projectedColumns = {
				"stringColumn", "My Decimal Column", "doubleColumn"
		};
	...
	private void doTestWithClearParameters( Connection connection )
			throws Exception
	{
