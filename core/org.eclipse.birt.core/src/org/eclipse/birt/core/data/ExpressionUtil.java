...
public final class ExpressionUtil {
	...
	public static String createJSDimensionExpression(String dimensionName, String levelName) {
		return DIMENSION_INDICATOR + createJSExprComponent(dimensionName) + createJSExprComponent(levelName);
	}
	...
	public static String createJSParameterValueExpression(String parameterName) {
		return createJSParameterExpression(parameterName) + EXPRESSION_VALUE_SUFFIX;
	}
	 ...
	 * @return
	 * @throws BirtException
	 * @deprecated use <code>extractColumnExpressions( String, String )</code>
	 *             instead
	 */
	@Deprecated
	public static List<IColumnBinding> extractColumnExpressions(String oldExpression, boolean mode)
			throws BirtException {
	...
	public static boolean hasAggregation(String expression) {
		...
		try {
			return ExpressionParserUtility.hasAggregation(expression, ExpressionUtil.ROW_INDICATOR)
					|| ExpressionParserUtility.hasAggregation(expression, ExpressionUtil.DATASET_ROW_INDICATOR);
	...
	public static String toNewExpression(String oldExpression) {
		...
		for (int i = 0; i < chars.length; i++) {
			status = getParseIndicator(chars, i, status.omitNextQuote(), status.getCandidateKey1(),
					status.getCandidateKey2());

			i = status.getNewIndex();
			if (i >= status.getRetrieveSize() + 3) {
				if (status.isCandidateKey() && chars[i - status.getRetrieveSize() - 3] == 'r'
						&& chars[i - status.getRetrieveSize() - 2] == 'o'
						&& chars[i - status.getRetrieveSize() - 1] == 'w') {
					if (i - status.getRetrieveSize() - 4 <= 0
							|| isValidProceeding(chars[i - status.getRetrieveSize() - 4])) {
						if (chars[i] == ' ' || chars[i] == '.' || chars[i] == '[') {
							String firstPart = oldExpression.substring(0, i - status.getRetrieveSize() - 3);
							String secondPart = toNewExpression(oldExpression.substring(i - status.getRetrieveSize()));
							String newExpression = firstPart + "dataSetRow" + secondPart;
	...
	public static String updateParentQueryReferenceExpression(String oldExpression, boolean isParameterBinding) {
		...
		for (int i = 0; i < chars.length; i++) {
			...
			if (i >= status.getRetrieveSize() + 4) {
				if (status.isCandidateKey() && chars[i - status.getRetrieveSize() - 4] == 'r'
					...
					if (i - status.getRetrieveSize() - 5 <= 0
							|| isValidProceeding(chars[i - status.getRetrieveSize() - 5])) {
						if (chars[i] == ' ' || chars[i] == '.' || chars[i] == '[') {
							int start = i;
							int end = 1;
							...
							String secondPart = updateParentQueryReferenceExpression(
									oldExpression.substring(start - status.getRetrieveSize() + end),
									isParameterBinding);
							String newExpression = firstPart + (isParameterBinding ? "row" : "row._outer") + secondPart;
							return newExpression;
						}
					}
				}
	...
	public static String generateConstantExpr(String input, int dataType) throws BirtException {
		...
		if (dataType == DataType.STRING_TYPE || dataType == DataType.DATE_TYPE || dataType == DataType.SQL_DATE_TYPE
				|| dataType == DataType.SQL_TIME_TYPE || dataType == DataType.JAVA_OBJECT_TYPE) {
