...
public class Ext2FileSystem {
	 ...
	 *                 read write mode, if the file exist, open the exits file.
	 *                 "rwt": read write cache mode, if the file exist, create a
	 *                 empty one. the file is removed after the file is closed.
	 * @throws IOException
	 */
	...
	public Ext2FileSystem(String filePath, RandomAccessFile rf, String mode) throws IOException {
		...
		throw new IOException(CoreMessages.getFormattedString(ResourceConstants.UNSUPPORTED_FILE_MODE, mode));
	}
	...
	private void readHeader() throws IOException {
		...
		if (magicTag != EXT2_MAGIC_TAG) {
			throw new IOException(CoreMessages.getFormattedString(ResourceConstants.NOT_EXT2_ARCHIVE, magicTag));
		}
		int version = in.readInt();
		if (version != EXT2_VERSION_0) {
			throw new IOException(
					CoreMessages.getFormattedString(ResourceConstants.UNSUPPORTED_ARCHIVE_VERSION, version));
		}

		int blockSize = in.readInt();
		if (blockSize != BLOCK_SIZE) {
			throw new IOException(CoreMessages.getFormattedString(ResourceConstants.UNSUPPORTED_BLOCK_SIZE, blockSize));
		}
	}
	...
	synchronized protected FatBlock loadFatBlock(int blockId) throws IOException {
